...
#define XX_ROTATE_LEFT(inValue, inCount) ((inValue << inCount) | (inValue >> (32 - inCount)))
...
double getXYFractal(int inX, int inY, double inRoughness, double inScale)
{
    ...
    double sum = a * getXYRandomBN(inX / (32 * inScale), inY / (32 * inScale)) +
                 b * (a * getXYRandomBN(inX / (16 * inScale), inY / (16 * inScale)) +
                      b * (a * getXYRandomBN(inX / (8 * inScale), inY / (8 * inScale)) +
                           b * (a * getXYRandomBN(inX / (4 * inScale), inY / (4 * inScale)) +
                                b * (a * getXYRandomBN(inX / (2 * inScale), inY / (2 * inScale)) +
                                     b * (getXYRandomBN(inX / inScale, inY / inScale))))));
...
double getXYFractal2(int inX, int inY, double inRoughness, double inScale)
{
    ...
    double sum = a * getXYRandomBN(inX / (4 * inScale), inY / (4 * inScale)) +
                 b * (a * getXYRandomBN(inX / (2 * inScale), inY / (2 * inScale)) +
                      b * (a * getXYRandomBN(inX / (inScale), inY / (inScale))));
