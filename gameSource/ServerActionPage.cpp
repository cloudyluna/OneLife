...
ServerActionPage::ServerActionPage(const char *inServerURL, const char *inActionName, int inRequiredNumResponseParts,
                                   const char *inResponsePartNames[], char inAttachAccountHmac)
    : mServerURL(stringDuplicate(inServerURL)), mActionName(stringDuplicate(inActionName)),
      mAttachAccountHmac(inAttachAccountHmac), mNumResponseParts(inRequiredNumResponseParts), mWebRequest(-1),
      mResponseReady(false), mMinimumResponseSeconds(0), mRequestStartTime(0), mParameterHmacKey(NULL)
{

    for (int i = 0; i < mNumResponseParts; i++)
    {
        mResponsePartNames.push_back(stringDuplicate(inResponsePartNames[i]));
    }

    addServerErrorString("DENIED", "requestDenied");
}

ServerActionPage::ServerActionPage(const char *inServerURL, const char *inActionName, char inAttachAccountHmac)
    : mServerURL(stringDuplicate(inServerURL)), mActionName(stringDuplicate(inActionName)),
      mAttachAccountHmac(inAttachAccountHmac), mNumResponseParts(-1), mWebRequest(-1), mResponseReady(false),
      mMinimumResponseSeconds(0), mRequestStartTime(0), mParameterHmacKey(NULL)
{
...
void ServerActionPage::setResponsePartNames(int inRequiredNumResponseParts, const char *inResponsePartNames[])
{
...
void ServerActionPage::setActionParameter(const char *inParameterName, const char *inParameterValue)
{
...
void ServerActionPage::setActionParameter(const char *inParameterName, int inParameterValue)
{
...
void ServerActionPage::setActionParameter(const char *inParameterName, double inParameterValue)
{
...
void ServerActionPage::addServerErrorString(const char *inServerErrorString, const char *inUserMessageKey)
{
    mErrorStringList.push_back(stringDuplicate(inServerErrorString));
    mErrorStringUserMessageKeys.push_back(inUserMessageKey);
}

void ServerActionPage::addServerErrorStringSignal(const char *inServerErrorString, const char *inSignalToSet)
{

    mErrorStringListForSignals.push_back(stringDuplicate(inServerErrorString));
...
void ServerActionPage::startRequest()
{
    ...
    char *actionParameterListString = actionParameterListChars.getElementString();

    char *fullRequestURL = autoSprintf("%s?action=%s%s", mServerURL, mActionName, actionParameterListString);
...
void ServerActionPage::step()
{
    if (mWebRequest != -1)
    {
        ...
        if (mMinimumResponseSeconds > 0 && game_timeSec() - mRequestStartTime < mMinimumResponseSeconds)
        {
        ...
        case 1: {
            ...
            for (int i = 0; i < mErrorStringList.size(); i++)
            {
                if (strstr(result, *(mErrorStringList.getElement(i))) != NULL)
                {

                    mStatusError = true;
                    mStatusMessageKey = *(mErrorStringUserMessageKeys.getElement(i));
                    errorParsed = true;
                    break;
                }
            }
            for (int i = 0; i < mErrorStringListForSignals.size(); i++)
            {
                if (strstr(result, *(mErrorStringListForSignals.getElement(i))) != NULL)
                {

                    setSignal(*(mErrorStringSignals.getElement(i)));
                    errorParsed = true;
                    break;
                }
            }
            ...
            if (!errorParsed)
            {
                ...
                if (mNumResponseParts != -1
                    ...
                    && (lines->size() != mNumResponseParts + 1 ||
                        strcmp(*(lines->getElement(mNumResponseParts)), "OK") != 0))
                {
                    ...
                    for (int i = 0; i < lines->size(); i++)
                    {
                        delete[] *(lines->getElement(i));
                         ...
                         && strcmp(*(lines->getElement(lines->size() - 1)), "OK") != 0)
                {
                    mStatusError = true;
                    setStatus("err_badServerResponse", true);
                ...
                else
                {
                    ...
                    {
                        mResponseParts.push_back(*(lines->getElement(i)));
                    }
...
void ServerActionPage::setParametersFromField(const char *inParamName, TextField *inField)
{
...
void ServerActionPage::setParametersFromString(const char *inParamName, const char *inString)
{
