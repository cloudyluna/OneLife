...
ExistingAccountPage::ExistingAccountPage()
    : mBackground("background.tga", 0.75f), mGameLogo("logo.tga", 1.0f, {-360, 256}),

      // Left Pane Page 0
      mEmailField(mainFont, -360, 96, 10, false, translate("username"), NULL,
                  // forbid only spaces and backslash and
                  // single/double quotes
                  // also pipe and colon, reserved as separators for spawn code and family name target
                  "\"' \\|:#"),
      mEmailLockButton(mainFont, -108, 96, "!"), mPasteEmailButton(mainFont, 0, 68, translate("paste")),

      mKeyField(mainFont, -360, 0, 15, true, translate("accountKey"),
                // allow only ticket code characters
                "23456789ABCDEFGHJKLMNPQRSTUVWXYZ-"),
      mKeyLockButton(mainFont, -108, 0, "!"), mPasteButton(mainFont, 0, -112, translate("paste")),

      mNextToGameTabButton(mainFont, 0, 0, "NEXT"),

      // Left Pane Page 1
      mFriendsButton(mainFont, 0, 0, "YES"), mSoloButton(mainFont, 0, 0, "NO"),
      mTwinCodeField(mainFont, -360, 16, 10, false, translate("twinCode"), NULL, NULL),
      mGenerateButton(mainFont, 0, 0, translate("generate")), mTwinCodeCopyButton(mainFont, 0, 0, translate("copy")),
      mTwinCodePasteButton(mainFont, 0, 0, translate("paste")),

      mSpecificButton(mainFont, 0, 0, "SPECIFIC"), mRandomButton(mainFont, 0, 0, "RANDOM"),
      mSpawnSeed(mainFont, -360, -176, 10, false, "SPAWN CODE:", NULL,
                 // forbid spaces and hash
                 " #"),
      mSpawnSeedLockButton(mainFont, -108, -176, "!"),
      mTargetFamily(mainFont, -360, -176, 10, true, "TARGET FAMILY NAME:",
                    // allow only alphabets
                    "ABCDEFGHIJKLMNOPQRSTUVWXYZ"),

      mBackToAccountTabButton(mainFont, 0, 0, "BACK"), mLoginButton(mainFont, 0, 0, "PLAY"),
      ...
      mDisableCustomServerButton(mainFont, 0, 220, translate("disableCustomServer")),

      // Not in use
      mGenesButton(mainFont, 522, 300, translate("genesButton")), mClearAccountButton(mainFont, -360, -80, "CLEAR"),
      mReviewButton(mainFont, -400, -200, translate("postReviewButton")), mAtSignButton(mainFont, 252, 128, "@"),
      mViewAccountButton(mainFont, 0, 64, translate("view")),

      mPageActiveStartTime(0), mFramesCounted(0), mFPSMeasureDone(false), mHideAccount(false)
{
    ...
    mGenerateButton.setPosition(mTwinCodeField.getRightEdgeX() + 4 + mGenerateButton.getWidth() / 2,
                                mTwinCodeField.getPosition().y);
    mGenerateButton.setPadding(8, 4);

    mTwinCodeCopyButton.setPosition(mGenerateButton.getPosition().x,
                                    mTwinCodeField.getPosition().y - mainFont->getFontHeight() - 4 * 2 - 4 * 2 - 4);
    mTwinCodePasteButton.setPosition(mGenerateButton.getPosition().x, mTwinCodeCopyButton.getPosition().y -
                                                                          mainFont->getFontHeight() - 4 * 2 - 4 * 2 -
                                                                          4);
    ...
    const char *choiceList[3] = {translate("twins"), translate("triplets"), translate("quadruplets")};

    mPlayerCountRadioButtonSet = new RadioButtonSet(mainFont, 0, 0, 3, choiceList, true, 4);

    mPlayerCountRadioButtonSet->setPosition(mGenerateButton.getPosition().x + 4 + 175 / 2,
                                            mGenerateButton.getPosition().y);

    const char *specifySpawnChoiceList[2] = {"SPECIFY FAMILY NAME", "USE SPAWN CODE"};

    mSeedOrFamilyButtonSet = new RadioButtonSet(mainFont, 0, 0, 2, specifySpawnChoiceList, true, 4);

    mSeedOrFamilyButtonSet->setPosition(mSpawnSeedLockButton.getPosition().x + 4 + mSpawnSeedLockButton.getWidth() / 2,
    ...
    mSeedOrFamilyButtonSet->setCursorTip(
        "SPECIFY THE FAMILY YOU WANT TO JOIN, OR MAKE A FIXED SPAWN POINT WITH A SPAWN CODE");
    ...
    char *seed = SettingsManager::getSettingContents("spawnSeed", "");
    ...
    if (userEmail != NULL)
    {
        unsigned int timeSeed = (unsigned int)fmod(game_getCurrentTime(), UINT_MAX);
        unsigned int emailSeed = crc32((unsigned char *)userEmail, strlen(userEmail));
...
void ExistingAccountPage::updatefieldsAndLockButtons()
{
    ...
    mEmailLockButton.setPosition(mEmailField.getRightEdgeX() + mEmailLockButton.getWidth() / 2 + 4,
    ...
    mPasteEmailButton.setPosition(mEmailField.getRightEdgeX() + 4 +
                                      (mEmailLockButton.isVisible() ? mEmailLockButton.getWidth() : 0) + 12 +
                                      mPasteEmailButton.getWidth() / 2,
                                  mEmailField.getPosition().y);
    mPasteEmailButton.setPadding(8, 4);
    mPasteEmailButton.setVisible(mEmailField.isVisible() && isClipboardSupported() &&
                                 (emailFieldLockedMode == 2 || disableCredentialLocks));

    mAtSignButton.setPosition(mEmailField.getRightEdgeX() + 4 +
                                  (mEmailLockButton.isVisible() ? mEmailLockButton.getWidth() : 0) + 4 +
                                  mPasteEmailButton.getWidth() + 16 + mAtSignButton.getWidth() / 2,
                              mEmailField.getPosition().y);
    mAtSignButton.setPadding(8, 4);
    mAtSignButton.setVisible(mEmailField.isVisible() && isClipboardSupported() &&
                             (emailFieldLockedMode == 2 || disableCredentialLocks) && useSteamUpdate);
    ...
    mKeyLockButton.setPosition(mKeyField.getRightEdgeX() + mKeyLockButton.getWidth() / 2 + 4,
    ...
    mPasteButton.setPosition(mKeyField.getRightEdgeX() + 4 +
                                 (mKeyLockButton.isVisible() ? mKeyLockButton.getWidth() : 0) + 12 +
                                 mPasteButton.getWidth() / 2,
                             mKeyField.getPosition().y);
    mPasteButton.setPadding(8, 4);
    mPasteButton.setVisible(mKeyField.isVisible() && isClipboardSupported() &&
                            (keyFieldLockedMode == 2 || disableCredentialLocks));
    ...
    mSpawnSeedLockButton.setPosition(mSpawnSeed.getRightEdgeX() + mSpawnSeedLockButton.getWidth() / 2 + 4,
...
void ExistingAccountPage::makeActive(char inFresh)
{
    ...
    if (!pastSuccess || (strcmp(emailText, "") == 0 && strcmp(keyText, "") == 0))
    {
...
static bool insideTextField(float inX, float inY, DropdownList *targetField)
{
    ...
    return fabs(inX - pos.x) < targetField->getWidth() / 2 && fabs(inY - pos.y) < 48 / 2;
}
...
void ExistingAccountPage::actionPerformed(GUIComponent *inTarget)
{
    ...
    else if (inTarget == &mGenerateButton)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            pickedWord[i] = mWordList.getElementDirect(mRandSource.getRandomBoundedInt(0, mWordList.size() - 1));
        }
        char *code = autoSprintf("%s %s %s", pickedWord[0], pickedWord[1], pickedWord[2]);
    ...
    else if (inTarget == &mFamilyTreesButton)
    {
        ...
        if (strcmp(url, "") != 0)
        {
            ...
            char *string_to_hash = autoSprintf("%d", randSource.getRandomBoundedInt(0, 2000000000));
            ...
            char *fullURL = autoSprintf("%s?action=front_page&email_sha1=%s"
                                        "&ticket_hash=%s"
                                        "&string_to_hash=%s",
                                        url, emailHash, ticket_hash, string_to_hash);
            delete[] email;
            delete[] emailHash;
            delete[] ticket_hash;
...
void ExistingAccountPage::draw(doublePair inViewCenter, double inViewSize)
{
    ...
    if (!mFPSMeasureDone)
    {
        ...
        if (timePassed > 1 + settleTime)
        {
            ...
            int targetFPS = SettingsManager::getIntSetting("targetFrameRate", -1);
            char fpsFailed = true;
            ...
            else
            {
                ...
                char *message = autoSprintf(translate("fpsErrorLogin"), fps, targetFPS);
    ...
    if (useSteamUpdate && (!mTwinCodeField.isFocused() || !mTwinCodeField.isVisible()) && (keyFieldLockedMode != 2))
    {
    ...
    if (mFPSMeasureDone && !mRedetectButton.isVisible() && !mDisableCustomServerButton.isVisible())
    {
        ...
        pos.x = (mTutorialButton.getPosition().x + mLoginButton.getPosition().x) / 2;
        ...
        if (fitnessMessage != NULL)
            mGenesButton.setCursorTip(fitnessMessage);
