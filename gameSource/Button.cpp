...
Button::Button(double inX, double inY, double inWide, double inHigh, double inPixelSize)
    : PageComponent(inX, inY), mActive(true), mHover(false), mPressStartedHere(false), mDragOver(false), mWide(inWide),
      mHigh(inHigh), mPixWidth(inPixelSize), mMouseOverTip(NULL), mBracketCoverLength(-1.0), mDrawBackground(true)
{
...
void Button::draw()
{
    ...
    setViewCenterPosition(oldViewCenter.x - mContentsShift.x, oldViewCenter.y - mContentsShift.y);
...
void Button::drawBorder()
{
    if (mBracketCoverLength >= 0 && false)
    {
        ...
        drawRect(-mWide / 2, -mHigh / 2, -mWide / 2 + mBracketCoverLength, mHigh / 2);

        drawRect(mWide / 2 - mBracketCoverLength, -mHigh / 2, mWide / 2, mHigh / 2);
    }
    else
    {
        // one solid rect
