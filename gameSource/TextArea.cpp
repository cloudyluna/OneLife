...
TextArea::TextArea(Font *inLabelFont, Font *inDisplayFont, double inX, double inY, double inWide, double inHigh,
                   char inForceCaps, const char *inLabelText, const char *inAllowedChars, const char *inForbiddenChars)
    : TextField(inDisplayFont, inX, inY, 1, inForceCaps, inLabelText, inAllowedChars, inForbiddenChars), mWide(inWide),
      mHigh(inHigh), mLabelFont(inLabelFont), mSpellCheckOn(false), mCurrentLine(0), mRecomputeCursorPositions(false),
      mLastComputedCursorText(stringDuplicate("")), mVertSlideOffset(0), mSmoothSlidingUp(false),
      mSmoothSlidingDown(false), mTopShadingFade(0), mBottomShadingFade(0), mMaxLinesShown(0), mFirstVisibleLine(0),
      mLastVisibleLine(0), mPointerDownInside(false), mStepsSinceTextChanged(0), mEverDrawn(false)
{
...
void TextArea::step()
{
    ...
    if (mVertSlideOffset > 0)
    {
        ...
        mVertSlideOffset -= mFont->getFontHeight() / speedFactor * frameRateFactor;
...
void TextArea::draw()
{
    ...
    while (index < textLen)
    {
        ...
        while (index < textLen && mText[index] != ' ' && mText[index] != '\r')
        {
    ...
    while (index < words.size())
    {
        ...
        while (index < words.size() && strcmp(words.getElementDirect(index), "\n") != 0 &&
               lineLength + mFont->measureString(words.getElementDirect(index)) < mWide)
        {
            ...
            {
                thisLineCursorPos = oldNumChars + cursorInWord.getElementDirect(index);
            }
            if (selectionStartInWord.getElementDirect(index) != -1)
            {
                thisLineSelectionStartPos = oldNumChars + selectionStartInWord.getElementDirect(index);
            }
            if (selectionEndInWord.getElementDirect(index) != -1)
            {
                thisLineSelectionEndPos = oldNumChars + selectionEndInWord.getElementDirect(index);
            }
        ...
        if (index < words.size() && strcmp(words.getElementDirect(index), "\n") == 0)
        {
    ...
    if (!anyLineHasSelectionEnd && mSelectionEnd != -1)
    {
        ...
        *(selectionEndInLine.getElement(lines.size() - 1)) = strlen(lastLine);
    ...
    if (!textChange && !mSnapMove)
        for (int i = firstLine; i <= lastLine; i++)
        {
            if (cursorInLine.getElementDirect(i) != -1)
            {
                if (mCurrentLine != i && mVertSlideOffset == 0 && abs(mCurrentLine - i) < linesPossible)
                {
                    ...
                    if (i < mCurrentLine && (mSmoothSlidingUp || lastLine != mLastVisibleLine))
                    {
                        mVertSlideOffset += mFont->getFontHeight() * (mCurrentLine - i);
                        if (firstLine == 0)
                        {
                            mVertSlideOffset -= mFont->getFontHeight() * (linesBeforeCursor - i);
                        }
                        if (mLastVisibleLine == lines.size() - 1)
                        {
                            mVertSlideOffset -=
                                mFont->getFontHeight() * (linesAfterCursor - (lines.size() - 1 - mCurrentLine));
                        }
                    }
                    else if (i > mCurrentLine && (mSmoothSlidingDown || firstLine != mFirstVisibleLine))
                    {
                        mVertSlideOffset -= mFont->getFontHeight() * (i - mCurrentLine);

                        if (lastLine == lines.size() - 1)
                        {
                            mVertSlideOffset += mFont->getFontHeight() * (linesAfterCursor - (lastLine - i));
                        }
                        if (mFirstVisibleLine == 0)
                        {
                            mVertSlideOffset += mFont->getFontHeight() * (linesBeforeCursor - mCurrentLine);
                        }
                    }
                }
            }
    ...
    else if (firstLine == 0 && lineWithCursor == linesBeforeCursor && lastLine < lines.size() - 1)
    {
        mSmoothSlidingUp = false;
        mSmoothSlidingDown = true;
    }
    else if (lastLine == lines.size() - 1 && lineWithCursor == lines.size() - linesAfterCursor - 1 && firstLine > 0)
    {
        mSmoothSlidingUp = true;
    ...
    for (int i = drawFirstLine; i <= drawLastLine; i++)
    {
        ...
        if (mSpellCheckOn)
        {
            ...
            while (wordStartIndex < lineLen)
            {
                ...
                while (wordEndIndex < lineLen && lineString[wordEndIndex] != ' ')
                {
                    wordEndIndex++;
                }
                ...
                if (cursorPos != -1 && cursorPos >= wordStartIndex && cursorPos <= wordEndIndex &&
                ...
                if (!inDict)
                {
                    ...
                    double afterWordX = mFont->measureString(lineString, wordEndIndex);

                    setDrawColor(1, 0, 0, 1.0);
                    double y = pos.y - mFont->getFontHeight() / 4 - mFont->getFontHeight() / 16;

                    drawRect(pos.x + beforeWordX, y, pos.x + afterWordX, y + mFont->getFontHeight() / 16);
                }
                delete[] wordCopy;
                ...
                while (wordStartIndex < lineLen && lineString[wordStartIndex] == ' ')
                {
                    wordStartIndex++;
                }
                ...
                {
                    beforeWordX = mFont->measureString(lineString, wordStartIndex);
                }
            }
        }
        ...
        if (cursorInLine.getElementDirect(i) != -1)
        {
            ...
            if (mFocused && !isAnythingSelected() && !(mSelectionStart != -1 && mPointerDownInside))
            {
                drawRect(pos.x + cursorXOffset + extra, pos.y - mFont->getFontHeight() / 2,
                         pos.x + cursorXOffset + pixWidth + extra, pos.y + 0.55 * mFont->getFontHeight());
            }

            mCurrentLine = i;

            if (mRecomputeCursorPositions || strcmp(mLastComputedCursorText, mText) != 0)
            {
                ...
                for (int j = 0; j < lines.size(); j++)
                {
                    ...
                    totalLineLengthSoFar += strlen(lines.getElementDirect(j));

                    int cursorPos = totalLineLengthSoFar;

                    char *line = stringDuplicate(lines.getElementDirect(j));
                    ...
                    while (fabs(mFont->measureString(line) - cursorXOffset) < bestUpDiff)
                    {
                    ...
                    if (bestPos == totalLineLengthSoFar && bestPos != 0 && bestPos != totalTextLen)
                    {
        ...
        if (mSelectionStart != mSelectionEnd)
        {
            ...
            if (selStartLine < i && selEndLine > i)
            {
                ...
                selRectEndX = mFont->measureString(lines.getElementDirect(i));
            }
            if (selStartLine == i)
            {
                drawSelThisLine = true;
            ...
            if (drawSelThisLine)
            {
                ...
                if (mFocused)
                {
                    ...
                    SelRect r = {pos.x + selRectStartX + extraStart, pos.y - mFont->getFontHeight() / 2,
                                 pos.x + selRectEndX + extraEnd, pos.y + 0.5 * mFont->getFontHeight()};
        ...
        else if (mSelectionStart != -1 && mSelectionEnd != -1 && mPointerDownInside && selStartLine == i &&
                 selEndLine == i)
        {
            ...
            if (mFocused)
            {
                drawRect(pos.x + dummyCursorXOffset + extra, pos.y - mFont->getFontHeight() / 2,
                         pos.x + dummyCursorXOffset + pixWidth + extra, pos.y + 0.55 * mFont->getFontHeight());
            }
        }
    ...
    if (selRects.size() > 0)
    {
        ...
        for (int i = 0; i < selRects.size(); i++)
        {
            ...
            drawRect(r.startX - extra, r.startY - extra, r.endX + extra, r.endY + extra);
        }
        // clear center
    ...
    if (mTopShadingFade > 0)
    {
        ...
        float vertColors[] = {
            0.25, 0.25, 0.25, mTopShadingFade, 0.25, 0.25, 0.25, mTopShadingFade, 0.25, 0.25, 0.25, 0, 0.25,
            0.25, 0.25, 0};
    ...
    if (mBottomShadingFade > 0)
    {
        ...
        float vertColors[] = {
            0.25, 0.25, 0.25, mBottomShadingFade, 0.25, 0.25, 0.25, mBottomShadingFade, 0.25, 0.25, 0.25, 0, 0.25,
            0.25, 0.25, 0};
...
void TextArea::upHit()
{
    ...
    if (isCommandKeyDown())
    {
        ...
        while (mCursorPosition > 0 && mText[mCursorPosition - 1] != '\r')
        {
            mCursorPosition--;
        }

        // skip space and newline characters
        while (mCursorPosition > 0 && (mText[mCursorPosition - 1] == ' ' || mText[mCursorPosition - 1] == '\r'))
        {
            mCursorPosition--;
        }
        mSnapMove = true;
    ...
    else
    {
        ...
        if (mCurrentLine > 0)
        {
            ...
            if (mCursorPosition == mCursorTargetPositions.getElementDirect(mCurrentLine))
            {

                mCurrentLine--;
                mCursorPosition = mCursorTargetPositions.getElementDirect(mCurrentLine);
...
void TextArea::downHit()
{
    ...
    if (isCommandKeyDown())
    {
        ...
        // skip space and newline characters
        while (mCursorPosition < textLen && (mText[mCursorPosition] == ' ' || mText[mCursorPosition] == '\r'))
        {
            mCursorPosition++;
        }

        // skip non-newline characters
        while (mCursorPosition < textLen && mText[mCursorPosition] != '\r')
        {
            mCursorPosition++;
        }
...
void TextArea::specialKeyDown(int inKeyCode)
{
    ...
    case MG_KEY_PAGE_UP: {
        ...
        {
            mVertSlideOffset += (old - mCurrentLine) * mFont->getFontHeight();
        }
    ...
    case MG_KEY_PAGE_DOWN: {
        ...
        {
            mVertSlideOffset -= (mCurrentLine - old) * mFont->getFontHeight();
        }
...
void TextArea::specialKeyUp(int inKeyCode)
{
    ...
    if (inKeyCode == MG_KEY_RIGHT || inKeyCode == MG_KEY_LEFT)
    {
        mRecomputeCursorPositions = true;
    }
...
int TextArea::getClickHitCursorIndex(float inX, float inY)
{
    ...
    int lineHit = lrint(pixelHitY / mFont->getFontHeight() - 0.5);

    if (lineHit < 0 || lineHit >= mMaxLinesShown)
    {
        return -1;
    }

    if (mLastVisibleLine == 0 && strlen(mLineStrings.getElementDirect(0)) == 0)
    {
        // empty text area
        return -1;
    }
    ...
    if (lineHit <= mLastVisibleLine)
    {
        ...
        for (int i = 0; i <= limit; i++)
        {
            ...
            double thisGapX = mFont->measureString(textCopy) + mFont->getCharSpacing() / 2;
        ...
        int delta = bestCursorLinePos - mCursorTargetLinePositions.getElementDirect(lineHit);
...
void TextArea::pointerDown(float inX, float inY)
{
    ...
    if (inX > -mWide / 2 - 3 * pixWidth && inX < mWide / 2 + 3 * pixWidth && inY > -mHigh / 2 - 3 * pixWidth &&
