...
Picker::Picker(Pickable *inPickable, double inX, double inY)
    : PageComponent(inX, inY), mPressStartedHere(false), mPickable(inPickable), mSkip(0), mResults(NULL),
      mNumResults(0), mResultsUnclickable(NULL), mNextButton(smallFont, +60, -290, "Next"),
      mPrevButton(smallFont, -60, -290, "Prev"), mDelButton(smallFont, 15, -290, "x"),
      mDelConfirmButton(smallFont, -15, -290, "!"), mSearchField(mainFont, 0, 100, 4, true, "", NULL, ""),
      mSelectionIndex(-1), mSelectionRightClicked(false), mPastSearchCurrentIndex(-1)
{
...
void Picker::redoSearch(char inClearPageSkip)
{
    ...
    if (strstr(search, " ") != NULL || (strstr(search, ".") != NULL && strcmp(search, ".") != 0))
    {
        ...
        if (validTerms.size() > 0)
        {
            ...
            void **mainResults = mPickable->search(validTerms.getElementDirect(0), 0,
                                                   // limit of 1 million
            ...
            else
            {
                ...
                {
                    const char *mainResultName = mPickable->getText(mainResults[i]);
    ...
    if (!multiTermDone)
    {
        ...
        mResults = mPickable->search(search, mSkip, PER_PAGE, &mNumResults, &numRemain);
    }
    ...
    for (int i = 0; i < mNumResults; i++)
    {
        ...
        {
            char unclickable = mFilterFuncions.getElementDirect(u).unclickableFunc(mPickable->getID(mResults[i]));
...
void Picker::actionPerformed(GUIComponent *inTarget)
{
    ...
    {
        mPickable->deleteID(mPickable->getID(mResults[mSelectionIndex]));
...
void Picker::specialKeyDown(int inKeyCode)
{
    ...
    case MG_KEY_UP:
        if (mPastSearchCurrentIndex > 0)
        {
            ...
            mSearchField.setText(mPastSearches.getElementDirect(mPastSearchCurrentIndex));
            mSkip = mPastSearchSkips.getElementDirect(mPastSearchCurrentIndex);
    ...
    if (mResults != NULL)
    {
        ...
        for (int i = 0; i < mNumResults; i++)
        {
            ...
            for (int j = 0; j < parts.size(); j++)
            {
                ...
                smallFont->drawString(trimmed, textPos, alignLeft);
                textPos.y -= 12;
...
{
    if (inX > -80 && inX < 80 && inY < 75 && inY > -245)
    {
...
{
    if (index < mNumResults && index >= 0 && mResultsUnclickable[index])
    {
        return;
    }
...
{
    if (mPressStartedHere && inX > -80 && inX < 80 && inY < 75 && inY > -245)
    {
        ...
        if (mSelectionIndex < mNumResults && mSelectionIndex >= 0 && mResultsUnclickable[mSelectionIndex])
        {
            // unclickable, skip it
        ...
        if (oldSelection != mSelectionIndex || mSelectionIndex != -1)
        {
