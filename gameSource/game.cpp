...
static const char *customDataFormatWriteString = "version%d_mouseSpeed%f_musicOff%d_musicLoudness%f"
                                                 "_webRetrySeconds%d";

static const char *customDataFormatReadString = "version%d_mouseSpeed%f_musicOff%d_musicLoudness%f"
                                                "_webRetrySeconds%d";
...
char *getCustomRecordedGameData()
{
    ...
    float musicLoudnessSetting = SettingsManager::getFloatSetting("musicLoudness", 1.0f);

    int webRetrySecondsSetting = SettingsManager::getIntSetting("webRetrySeconds", 10);

    char *result = autoSprintf(customDataFormatWriteString, versionNumber, mouseSpeedSetting, musicOffSetting,
                     ...
                     char inPlayingBack)
{
    ...
    AppLog::infoF("OneLife client v%d (binV=%d, dataV=%d) starting up", versionNumber, binVersionNumber,
                  dataVersionNumber);
    ...
    pencilFont = new Font("font_pencil_32_32.tga", 3, 6, false, 16 * gui_fov_scale_hud);

    pencilFont->setMinimumPositionPrecision(1);

    pencilErasedFont = new Font("font_pencil_erased_32_32.tga", 3, 6, false, 16 * gui_fov_scale_hud);
    ...
    int numRead = sscanf(inCustomRecordedGameData, customDataFormatReadString, &readVersionNumber, &mouseSpeedSetting,
                         &musicOffSetting, &musicLoudnessSetting, &webRetrySecondsSetting);
    if (numRead != 6)
    {
        // no recorded game?
    }
    ...
    {
        reflectorURL = stringDuplicate("http://localhost/jcr13/oneLifeReflector/server.php");
    }

    setSoundLoudness(1.0);
    setSoundPlaying(true);

    const char *resultNamesA[4] = {"serverIP", "serverPort", "requiredVersionNumber", "autoUpdateURL"};

    getServerAddressPage = new ServerActionPage(reflectorURL, "reflect", 4, resultNamesA, false);
...
static void drawPauseScreen()
{
    ...
    if (isPaused() && (currentGamePage == existingAccountPage || currentGamePage == settingsPage))
    {
        pauseGame();
        return;
    }
    ...
    if (currentGamePage != livingLifePage)
    {
        ...
        oldMainFont->drawString(translate("pauseMessage1"), messagePos, alignCenter);

        messagePos.y -= 1.25 * (viewHeight / 15);
        oldMainFont->drawString(translate("pauseMessage2"), messagePos, alignCenter);
    }
            ...
            if (helpFileContents != NULL)
            {
                ...
                for (int i = 0; i < numLines; i++)
                {
                    ...
                    else if (strstr(lines[i], "#sheet") != NULL)
                    {
                        ...
                        writePos.y =
                            lastScreenViewCenter.y + columnStartY + columnHeight / 2; // reset lineHeight additions
                        // isSheet = true;
                        continue;
                    }
                    ...
                    if (totalNumOfHelpSheets == -1 || numOfSheetsPerPage == -1)
                        continue;
                    ...
                    if (currentHelpPage < 0)
                        currentHelpPage = 0;
                    if (currentHelpPage >= totalNumOfHelpPages)
                        currentHelpPage = totalNumOfHelpPages - 1;
                    ...
                    {
                        int current_columnX =
                            columnStartX + (abs(columnWidth) + columnOffset) * (columnNumberInPage - 1);
                    ...
                    else if (isTitle)
                    {
                        ...
                        // handwritingFont->drawString( lines[i], { writePos.x + (columnWidth - titleSize)/2, writePos.y
                        // - lineHeight }, alignLeft ); // Centered
                        handwritingFont->drawString(lines[i],
                                                    {writePos.x + 40 * gui_fov_scale, writePos.y - lineHeight},
                    ...
                    else if (isSub)
                    {
                        ...
                        handwritingFont->drawString(
                            lines[i], {writePos.x + 40 * gui_fov_scale, writePos.y - lineHeight * 0.75f}, alignLeft);
                        int subSize = handwritingFont->measureString(lines[i]);
                        setDrawColor(0.1f, 0.1f, 0.1f, 1 * pauseScreenFade);
                        pencilFont->drawString(
                            subString, {writePos.x + subSize + 60 * gui_fov_scale, writePos.y - lineHeight * 0.75f},
                            alignLeft);
                        writePos.y -= lineHeight;
                    }
                    else if (isPicture)
                    {
                        if (currentHelpPage != pictureOnPageNumber)
                            continue;
                        doublePair picturePos = {lastScreenViewCenter.x + picturePosX * gui_fov_scale,
    ...
    if (currentGamePage == livingLifePage)
    {
        ...
        if (1)
        { // Resume button
            doublePair buttonPos = {lastScreenViewCenter.x + resumeButtonPos.x,
        ...
        if (1)
        { // Settings button
            doublePair buttonPos = {lastScreenViewCenter.x + settingsButtonPos.x,
        ...
        if (1)
        { // Quit button
            doublePair buttonPos = {lastScreenViewCenter.x + quitButtonPos.x, lastScreenViewCenter.y + quitButtonPos.y};
        ...
        if (currentHelpPage < totalNumOfHelpPages - 1)
        { // Next Page button
            doublePair buttonPos = {lastScreenViewCenter.x + nextPageButtonPos.x,
        ...
        if (currentHelpPage > 0)
        { // Prev Page button
            doublePair buttonPos = {lastScreenViewCenter.x + prevPageButtonPos.x,
    ...
    if (currentGamePage != livingLifePage)
    {
        ...
        if (currentGamePage == livingLifePage)
        {
            mainFont->drawString(translate("pauseMessage5"), messagePos, alignCenter);
        }
        ...
        oldMainFont->drawString(translate(quitMessageKey), messagePos, alignCenter);
    }
}
...
#ifdef USE_DISCORD // <-- vscode doesnt know it's enabled at compile time... it can be overriden with
                   // .vscode/c_cpp_properties.json, content: {"configurations":[{"defines": ["USE_DISCORD"],}],}
void discordStep()
{
    ...
    else if (currentGamePage == extendedMessagePage)
    {
        ...
        {
            // TODO: sometimes we lose connection when we die, i think it's becuase server disconnects us before we get
            // our death message. just an age check would fix it, and show the died status instead of connection lost.
            if (livingLifePage != NULL && livingLifePage->getLastComputedAge() > 119.5)
            { // REVIEW: make 119.5 variable, not constant, create it from settings
    ...
    if (currentGamePage != NULL)
    {
        ...
        if (currentGamePage == loadingPage)
        {
            ...
            case 4: {
                ...
                if (progress == 1.0)
                {
                    ...
                    int numTrans = initTransBankStart(&rebuilding, true, true, true, true);
            ...
            if (getServerAddressPage->isResponseReady())
            {
                ...
                else
                {
                    ...
                    if (versionNumber < requiredVersion)
                    {
                        ...
                        {
                            char *autoUpdateURL = getServerAddressPage->getResponse("autoUpdateURL");
        ...
        else if (currentGamePage == livingLifePage)
        {
            ...
            else if (livingLifePage->checkSignal("versionMismatch"))
            {
                ...
                char *message =
                    autoSprintf(translate("versionMismatch"), versionNumber, livingLifePage->getRequiredVersion());
            ...
            else if (livingLifePage->checkSignal("loadFailure"))
            {
                ...
                if (failedFileName != NULL)
                {
                    ...
                    char *detailMessage = autoSprintf(translate("loadingMapFailedSubMessage"), failedFileName);
