...
int initTransBankStart(char *outRebuildingCache, char inAutoGenerateCategoryTransitions,
                       char inAutoGenerateUsedObjectTransitions, char inAutoGenerateGenericUseTransitions,
                       char inAutoGenerateVariableTransitions)
{
    ...
    cache = initFolderCache("transitions", outRebuildingCache, shouldFileBeCached);
    ...
    if (shouldFileBeCached(txtFileName))
    {
        ...
        if (target != -2)
        {
            ...
            if (contents != NULL)
            {
                ...
                sscanf(contents, "%d %d %lf %f %f %d %d %d %d %d %d", &newActor, &newTarget, &autoDecaySeconds,
                       &actorMinUseFraction, &targetMinUseFraction, &reverseUseActorFlag, &reverseUseTargetFlag, &move,
                       &desiredMoveDist, &noUseActorFlag, &noUseTargetFlag);
...
static void handleWhatProbSetProduces(int inPossibleSetID, TransRecord *inT)
{
...
static void regenUsesAndProducesMaps()
{
    ...
    for (int i = 0; i < numRecords; i++)
    {
        ...
        if (t->actorChangeChance < 1.0 && t->newActorNoChange != 0 && t->newActorNoChange != t->newActor)
        {
        ...
        if (t->targetChangeChance < 1.0 && t->newTargetNoChange != 0 && t->newTargetNoChange != t->newTarget &&
            t->newTargetNoChange != t->newActor && t->newTargetNoChange != t->newActorNoChange)
        {
    ...
    if (autoGenerateCategoryTransitions)
    {
        ...
        for (int i = 0; i < numObjects; i++)
        {
            ...
            for (int c = 0; c < numCats; c++)
            {
                ...
                SimpleVector<TransRecord *> *parentTransOrig = getAllUses(parentID);
                ...
                for (int t = 0; t < numParentTrans; t++)
                {
                    ...
                    if (tr->actor == parentID)
                    {
                        ...
                        TransRecord *oTR =
                            getTrans(oID, tr->target, tr->lastUseActor, tr->lastUseTarget, tr->contTransFlag);
                    ...
                    if (tr->target == parentID)
                    {
                        ...
                        TransRecord *oTR =
                            getTrans(tr->actor, oID, tr->lastUseActor, tr->lastUseTarget, tr->contTransFlag);
                    ...
                    if (actor == parentID)
                    {
                        ...
                        // don't write to disk
                        addTrans(actor, target, newActor, newTarget, tr->lastUseActor, tr->lastUseTarget,
                                 tr->contTransFlag, tr->reverseUseActor, tr->reverseUseTarget, tr->noUseActor,
                                 tr->noUseTarget, tr->autoDecaySeconds, tr->actorMinUseFraction,
                                 tr->targetMinUseFraction, tr->move, tr->desiredMoveDist, tr->actorChangeChance,
                                 tr->targetChangeChance, tr->newActorNoChange, tr->newTargetNoChange, true);
                    }
        ...
        for (int i = 0; i < numRecords; i++)
        {
            ...
            int transIDs[4] = {tr->actor, tr->target, tr->newActor, tr->newTarget};
            ...
            if (numPatternsInTrans > 0 && (transCats[0] != NULL || transCats[1] != NULL))
            {

                for (int p = 0; p < patternSize; p++)
                {
                    int newTransIDs[4] = {tr->actor, tr->target, tr->newActor, tr->newTarget};

                    for (int n = 0; n < 4; n++)
                    {
                        if (transCats[n] != NULL)
                        {
                            newTransIDs[n] = transCats[n]->objectIDSet.getElementDirect(p);
                        }
                    }
                    ...
                    TransRecord *existingTrans = getTrans(newTransIDs[0], newTransIDs[1], tr->lastUseActor,
                                                          tr->lastUseTarget, tr->contTransFlag);
                    if (existingTrans == NULL)
                    {
                        // no authored trans exists

                        addTrans(newTransIDs[0], newTransIDs[1], newTransIDs[2], newTransIDs[3], tr->lastUseActor,
                                 tr->lastUseTarget, tr->contTransFlag, tr->reverseUseActor, tr->reverseUseTarget,
                                 tr->noUseActor, tr->noUseTarget, tr->autoDecaySeconds, tr->actorMinUseFraction,
                                 tr->targetMinUseFraction, tr->move, tr->desiredMoveDist, tr->actorChangeChance,
                                 tr->targetChangeChance, tr->newActorNoChange, tr->newTargetNoChange, true);
                    }
                }
        ...
        if (autoGenerateGenericUseTransitions)
        {
            ...
            for (int i = 0; i < numObjects; i++)
            {
                ...
                if (o->foodValue == 0)
                {
                    ...
                    for (int g = 0; g < 2; g++)
                        if (genericTrans[g] != NULL && genericTrans[g]->newTarget == 0)
                        {
            ...
            for (int t = 0; t < transToAdd.size(); t++)
            {
                ...
                addTrans(tr.actor, tr.target, tr.newActor, tr.newTarget, tr.lastUseActor, tr.lastUseTarget,
                         tr.contTransFlag, tr.reverseUseActor, tr.reverseUseTarget, tr.noUseActor, tr.noUseTarget,
                         tr.autoDecaySeconds, tr.actorMinUseFraction, tr.targetMinUseFraction, tr.move,
                         tr.desiredMoveDist, tr.actorChangeChance, tr.targetChangeChance, tr.newActorNoChange,
                         tr.newTargetNoChange, true);
            }
    ...
    if (autoGenerateUsedObjectTransitions)
    {
        ...
        for (int i = 0; i < records.size(); i++)
        {
            ...
            if (!tr->lastUseTarget && !tr->lastUseActor)
            {
                ...
                if (actor != NULL && newActor != NULL && actor->numUses > 1 && actor->numUses == newActor->numUses)
                {
                ...
                else
                {
                    ...
                    if (actor != NULL && tr->reverseUseActor && newActor != NULL && newActor->numUses > 1)
                    {

                        TransIDPair tp = {actor->id, newActor->useDummyIDs[0]};
                        actorSteps.push_back(tp);
                    }
                    else
                    {
                        ...
                        if (actor != NULL && actor->numUses > 1)
                        {
                            ...
                            {
                                float useFraction = (float)(u + 1) / (float)(actor->numUses);
                ...
                if (target != NULL && newTarget != NULL && target->numUses > 1 && target->numUses == newTarget->numUses)
                {
                ...
                else
                {
                    ...
                    if (target != NULL && tr->reverseUseTarget && newTarget != NULL && newTarget->numUses > 1)
                    {

                        TransIDPair tp = {target->id, newTarget->useDummyIDs[0]};
                        targetSteps.push_back(tp);
                    }
                    else
                    {
                        // at least one
                        TransIDPair tp = {tr->target, tr->newTarget, tr->newTarget};
                        targetSteps.push_back(tp);
                        ...
                        if (target != NULL && target->numUses > 1)
                        {
                            ...
                            {
                                float useFraction = (float)(u + 1) / (float)(target->numUses);
                ...
                if (actorDecrement || targetDecrement)
                {
                    ...
                    if ((actorDecrement && tr->reverseUseActor) || (targetDecrement && tr->reverseUseTarget))
                    {
                        // transition replaced
                ...
                else
                {
                    ...
                    if (target != NULL && newActor != NULL && target->numUses > 1 &&
                        target->numUses == newActor->numUses && target->useChance == newActor->useChance)
                    {
                    ...
                    else if (actor != NULL && newTarget != NULL && actor->numUses > 1 &&
                             actor->numUses == newTarget->numUses && actor->useChance == newTarget->useChance)
                    {
                    ...
                    //  many carrot flowers)
                    else if (target != NULL && newTarget != NULL && target->numUses > 1 && newTarget->numUses > 1)
                    {
                        ...
                        for (int u = 0; u < target->numUses - 1; u++)
                        {
                            ...
                            float useFraction = (float)(u) / (float)(target->numUses);
                            // propagate used status to new target
                            int usesLeft = lrint(useFraction * newTarget->numUses);
                            ...
                            newTrans.newTarget = newTarget->useDummyIDs[usesLeft];
            ...
            if (!processed)
            {
                if (tr->lastUseActor || tr->lastUseTarget)
                {
                    ...
                    if (tr->lastUseActor && tr->lastUseTarget && actor != NULL && actor->numUses > 1 &&
                    ...
                    else if (tr->lastUseActor && actor != NULL && actor->numUses > 1)
                    {
                        ...
                        if (target != NULL && target->numUses > 1)
                        {
                            ...
                            if (newTarget != NULL && target->numUses == newTarget->numUses)
                            {
                                mapNewTarget = true;
                            }
                    ...
                    else if (tr->lastUseTarget && target != NULL && target->numUses > 1)
                    {
                        ...
                        if (actor != NULL && actor->numUses > 1)
                        {
                            ...
                            if (newActor != NULL && actor->numUses == newActor->numUses)
                            {
                                mapNewActor = true;
                            }
                ...
                else if (tr->reverseUseActor && newActor != NULL && newActor->numUses > 1)
                {
                ...
                else if (tr->reverseUseTarget && newTarget != NULL && newTarget->numUses > 1)
                {
                ...
                else
                {
                    ...
                    if (target != NULL && target->numUses > 1)
                    {
                        ...
                        for (int u = 0; u < target->numUses - 1; u++)
                        {
                            ...
                            if (useFraction >= tr->targetMinUseFraction)
                            {
                                targetDummies.push_back(target->useDummyIDs[u]);
                            }
                        }
                    }
                    else
                    ...
                    if (actorDummies.size() > 1 || targetDummies.size() > 1)
                    {
                        ...
                        {
                            newTrans.actor = actorDummies.getElementDirect(ad);
                            for (int td = 0; td < targetDummies.size(); td++)
                            {
                                newTrans.target = targetDummies.getElementDirect(td);
                                transToAdd.push_back(newTrans);
                            }
                        }
                    }
        ...
        for (int t = 0; t < transToDelete.size(); t++)
        {
            ...
            deleteTransFromBank(tr->actor, tr->target, tr->lastUseActor, tr->lastUseTarget, tr->contTransFlag, true);
            numRemoved++;
        }
        ...
        for (int t = 0; t < transToAdd.size(); t++)
        {
            ...
            addTrans(newTrans->actor, newTrans->target, newTrans->newActor, newTrans->newTarget, newTrans->lastUseActor,
                     newTrans->lastUseTarget, newTrans->contTransFlag, newTrans->reverseUseActor,
                     newTrans->reverseUseTarget, newTrans->noUseActor, newTrans->noUseTarget,
                     newTrans->autoDecaySeconds, newTrans->actorMinUseFraction, newTrans->targetMinUseFraction,
                     newTrans->move, newTrans->desiredMoveDist, newTrans->actorChangeChance,
                     newTrans->targetChangeChance, newTrans->newActorNoChange, newTrans->newTargetNoChange, true);
            numGenerated++;
        }
        ...
        for (int i = 0; i < records.size(); i++)
        {
            ...
            if (leadObject != NULL && leadObject != followObject)
            {
                ...
                for (int k = 0; k < num; k++)
                {
                    ...
                    if (newActor != NULL && newActor->numVariableDummyIDs == num)
                    {

                        newTrans.newActor = newActor->variableDummyIDs[k];
                    }

                    if (newTarget != NULL && newTarget->numVariableDummyIDs == num)
                    {
...
void regenerateHumanMadeMap()
{
    ...
    while (anyUnknown && anyAssigned)
    {
        ...
        for (int i = 0; i < numObjects; i++)
        {
            ...
            if (!humanMadeMap[oID] && !naturalMap[oID] && !unreachableMap[oID])
            {
...
void regenerateBecomeFoodMap()
{
    ...
    for (int j = 0; j < allFoods->size(); j++)
    {
        ...
        for (int i = 0; i < prodTrans->size(); i++)
        {
            ...
            if (!(actorDepth < foodDepth && targetDepth < foodDepth))
                continue;
    ...
    while (index < horizon.size())
    {
        ...
        for (int i = 0; i < prodTrans->size(); i++)
        {
            ...
            if (t->contTransFlag != 0)
                continue;

            // ignore raw last use transitions
            // the proper transitions are auto-generated and have these flags set to false
            if (t->lastUseActor || t->lastUseTarget)
                continue;

            int actor = t->actor;
            int target = t->target;
            if (actor > 0)
                actor = getObjectParent(actor);
            if (target > 0)
                target = getObjectParent(target);
            ...
            else if (t->newActor == nextID && t->target == t->newTarget && commonAncestors.getElementIndex(actor) == -1)
            {
                // target is not used in crafting, look at actor
                nextToCheck = actor;
            }
            else if (t->newTarget == nextID && t->actor == t->newActor && commonAncestors.getElementIndex(target) == -1)
            {
                // actor is not used in crafting, look at target
                nextToCheck = target;
            }
            else if (commonAncestors.getElementIndex(actor) != -1 && commonAncestors.getElementIndex(target) == -1)
            {
...
TransRecord *getTrans(int inActor, int inTarget, char inLastUseActor, char inLastUseTarget, int inContTransFlag)
{
    ...
    for (int i = 0; i < numRecords; i++)
    {
        ...
        if (r->actor == inActor && r->target == inTarget && r->lastUseActor == inLastUseActor &&
            r->lastUseTarget == inLastUseTarget && r->contTransFlag == inContTransFlag)
        {
            return r;
        }
    }
...
TransRecord *getPTrans(int inActor, int inTarget, char inLastUseActor, char inLastUseTarget, int inContTransFlag)
{
    ...
    TransRecord *r = getTrans(inActor, inTarget, inLastUseActor, inLastUseTarget, inContTransFlag);
    ...
    if (r->actorChangeChance == 1.0f && r->targetChangeChance == 1.0f && actorMeta == 0 && targetMeta == 0 &&
    ...
    *rStatic = *r;
    ...
    {
        if (randSource.getRandomBoundedDouble(0, 1.0) > r->actorChangeChance)
        {
            // no change to actor
    ...
    {
        if (randSource.getRandomBoundedDouble(0, 1.0) > r->targetChangeChance)
        {
            // no change to target
    ...
    {
        if (rStatic->newActor > 0 && getObject(rStatic->newActor)->mayHaveMetadata)
        {
            rStatic->newActor = packMetadataID(rStatic->newActor, passThroughMeta);
        }
        ...
        if (rStatic->newTarget > 0 && getObject(rStatic->newTarget)->mayHaveMetadata)
        {
            rStatic->newTarget = packMetadataID(rStatic->newTarget, passThroughMeta);
        }
    }
...
static TransRecord **search(SimpleVector<TransRecord *> inMapToSearch[], int inID, int inNumToSkip, int inNumToGet,
...
static char isTransPartOfPattern(int inCatID, TransRecord *inTrans)
{
    ...
    if (actorCat != NULL && actorCat->isPattern && actorCat->objectIDSet.size() == patternSize)
    {
        actorInPattern = true;
    }
    else if (targetCat != NULL && targetCat->isPattern && targetCat->objectIDSet.size() == patternSize)
    {
...
static TransRecord **searchWithCategories(SimpleVector<TransRecord *> inMapToSearch[], int inID, int inNumToSkip,
                                          int inNumToGet, int *outNumResults, int *outNumRemaining)
{
    ...
    if (catRec != NULL)
    {
        for (int i = 0; i < catRec->categoryIDSet.size(); i++)
        {
            ...
            if (catID < mapSize)
            {
                ...
                {
                    if (isTransPartOfPattern(catID, inMapToSearch[catID].getElementDirect(j)))
                    {
    ...
    TransRecord **initialResult = search(inMapToSearch, inID, inNumToSkip, inNumToGet, outNumResults, outNumRemaining);
    ...
    else if (extraRecords > 0)
    {
        ...
        while (i < catRec->categoryIDSet.size() && *outNumResults < inNumToGet)
        {
            ...
            TransRecord **catResult =
                search(inMapToSearch, catID, inNumToSkip, numLeftToGet, &catNumResults, &catNumRemaining);
            if (catResult != NULL)
            {
...
TransRecord **searchUses(int inUsesID, int inNumToSkip, int inNumToGet, int *outNumResults, int *outNumRemaining)
{
    ...
    if (autoGenerateCategoryTransitions)
    {
        return search(usesMap, inUsesID, inNumToSkip, inNumToGet, outNumResults, outNumRemaining);
    }
    else
    {
        return searchWithCategories(usesMap, inUsesID, inNumToSkip, inNumToGet, outNumResults, outNumRemaining);
    }
}
                             ...
                             int *outNumRemaining)
{
    ...
    if (autoGenerateCategoryTransitions)
    {
        return search(producesMap, inProducesID, inNumToSkip, inNumToGet, outNumResults, outNumRemaining);
    }
    else
    {
        return searchWithCategories(producesMap, inProducesID, inNumToSkip, inNumToGet, outNumResults, outNumRemaining);
    }
}
...
char isAncestor(int inTargetID, int inPossibleAncestorID, int inStepLimit)
{
    ...
    for (int i = 0; i < producesMap[inTargetID].size(); i++)
    {
        ...
        if (r->newTarget == inTargetID && r->target == inTargetID)
        {
            // inTargetID unchanged
            continue;
        }
        if (r->newActor == inTargetID && r->actor == inTargetID)
        {
            // unchanged as actor
            continue;
        }
        ...
        if (r->actor == inPossibleAncestorID || r->target == inPossibleAncestorID)
        {
            return true;
        }

        if (r->actor > 0 && isAncestor(r->actor, inPossibleAncestorID, inStepLimit - 1))
        {
            return true;
        }

        if (r->target > 0 && isAncestor(r->target, inPossibleAncestorID, inStepLimit - 1))
        {
            return true;
        }
    }
...
static char *getFileName(int inActor, int inTarget, char inLastUseActor, char inLastUseTarget, int inContTransFlag)
{
...
static char *getOldFileName(int inActor, int inTarget, char inLastUseActor, char inLastUseTarget, int inContTransFlag)
{
    const char *lastUseString = "";
...
void addTrans(int inActor, int inTarget, int inNewActor, int inNewTarget, char inLastUseActor, char inLastUseTarget,
              ...
              int inMove, int inDesiredMoveDist, float inActorChangeChance, float inTargetChangeChance,
              int inNewActorNoChange, int inNewTargetNoChange, char inNoWriteToFile)
{

    // exapand id-indexed maps if a bigger ID is being added
    if (inActor >= mapSize || inTarget >= mapSize || inNewActor >= mapSize || inNewTarget >= mapSize)
    {
        ...
        SimpleVector<TransRecord *> *newUsesMap = new SimpleVector<TransRecord *>[newMapSize];

        SimpleVector<TransRecord *> *newProducesMap = new SimpleVector<TransRecord *>[newMapSize];
    ...
    // one exists?
    TransRecord *t = getTrans(inActor, inTarget, inLastUseActor, inLastUseTarget, inContTransFlag);
    ...
    else
    {
        ...
        if (t->newTarget == inNewTarget && t->newActor == inNewActor && t->autoDecaySeconds == inAutoDecaySeconds &&
            t->actorMinUseFraction == inActorMinUseFraction && t->targetMinUseFraction == inTargetMinUseFraction &&
            t->reverseUseActor == inReverseUseActor && t->reverseUseTarget == inReverseUseTarget &&
            t->noUseActor == inNoUseActor && t->noUseTarget == inNoUseTarget && t->move == inMove &&
            t->desiredMoveDist == inDesiredMoveDist && t->actorChangeChance == inActorChangeChance &&
            t->targetChangeChance == inTargetChangeChance && t->newActorNoChange == inNewActorNoChange &&
            t->newTargetNoChange == inNewTargetNoChange)
        {
        ...
        else
        {
            ...
            if (t->newActor != 0 && t->newActor != inNewActor)
            {
                producesMap[t->newActor].deleteElementEqualTo(t);
            }
            if (t->newTarget != 0 && t->newTarget != inNewTarget)
            {
    ...
    if (writeToFile && !inNoWriteToFile)
    {
        ...
        if (transDir.exists() && transDir.isDirectory())
        {
            ...
            char *fileName = getFileName(inActor, inTarget, inLastUseActor, inLastUseTarget, inContTransFlag);

            File *transFile = transDir.getChildFile(fileName);

            // clear old-style file, which may be lingering
            char *oldFileName = getOldFileName(inActor, inTarget, inLastUseActor, inLastUseTarget, inContTransFlag);
            ...
            char *fileContents = autoSprintf("%d %d %.2f %f %f %d %d "
                                             "%d %d %d %d",
                                             inNewActor, inNewTarget, inAutoDecaySeconds, inActorMinUseFraction,
                                             inTargetMinUseFraction, reverseUseActorFlag, reverseUseTargetFlag, inMove,
                                             inDesiredMoveDist, noUseActorFlag, noUseTargetFlag);
...
char isObjectUsed(int inObjectID)
{
    ...
    TransRecord **results = searchUses(inObjectID, 0, 1, &numResults, &numRemaining);
    ...
    results = searchProduces(inObjectID, 0, 1, &numResults, &numRemaining);
...
char isGrave(int inObjectID)
{
    ...
    while (r != NULL && !r->deathMarker)
    {
        ...
        else
        {
            ...
            if (newID > 0 && seenIDs.getElementIndex(newID) == -1)
            {
                r = getObject(newID);
...
{
    printf("[%s](%.2f) + [%s](%.2f) => [%s] + [%s]", getObjName(inTrans->actor), inTrans->actorMinUseFraction,
           getObjName(inTrans->target), inTrans->targetMinUseFraction, getObjName(inTrans->newActor),
    ...
    {
        printf(" (pA=%0.2f,[%s])", inTrans->actorChangeChance, getObjName(inTrans->newActorNoChange));
    }

    if (inTrans->targetChangeChance < 1.0)
    {
        printf(" (pT=%0.2f,[%s])", inTrans->targetChangeChance, getObjName(inTrans->newTargetNoChange));
    }
