...
V::V(int inA, int inB) : a(inA), b(inB)
{
}
...
// has been defined
template <typename TKey, typename TValue, char UCustomHash = false> struct HashTable
{
...
void HashTable<TKey, TValue, UCustomHash>::insert(TKey inKey, TValue inValue)
{
    int h = hash(inKey);
...
template <typename TKey, typename TValue, char UCustomHash> int HashTable<TKey, TValue, UCustomHash>::hash(TKey inKey)
{
...
template <typename TKey, typename TValue, char UCustomHash>
int HashTable<TKey, TValue, UCustomHash>::hashBytes(unsigned char *inBytes, int inNumBytes)
{
    int byteSum = 0;
...
template <> int HashTable<char *, V, true>::customHash(char *inStringKey)
{
