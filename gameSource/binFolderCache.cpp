...
static char verifyCacheFile(FILE *inFile, int inNumFiles, int *outFailNumber)
{
...
BinFolderCache initBinFolderCache(const char *inFolderName, const char *inPattern, char *outRebuildingCache)
{
    ...
    *outRebuildingCache = false;
    ...
    for (int i = 0; i < numChildFiles; i++)
    {
        ...
        if (strcmp(name, curCacheName) != 0 && strstr(name, "bin_") != NULL && strstr(name, "cache.fcz") != NULL)
        {
...
char *getFileName(BinFolderCache inCache, int inFileNumber)
{
    ...
    {
        return inCache.dirFiles->getElementDirect(inFileNumber)->getFileName();
    }
...
unsigned char *getFileContents(BinFolderCache inCache, int inFileNumber, char *inFileName, int *outLen)
{
    if (inCache.dirFiles->size() > inFileNumber)
    {
        int rawLen;
        unsigned char *rawBuff = inCache.dirFiles->getElementDirect(inFileNumber)->readFileContents(&rawLen);

        int compLen;
        unsigned char *compBuff = zipCompress(rawBuff, rawLen, &compLen);
    ...
    else
    {
        ...
        int numRead = fscanf(inCache.cacheFile, "%d %d#", &compSize, &rawSize);
...
{
    int numChildFiles;
    File **childFiles = inFolder->getChildFiles(&numChildFiles);

    for (int i = 0; i < numChildFiles; i++)
    {
        char *name = childFiles[i]->getFileName();

        if (strstr(name, "bin_") != NULL && strstr(name, "cache.fcz") != NULL)
        {
