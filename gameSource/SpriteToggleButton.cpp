...
SpriteToggleButton::SpriteToggleButton(const char *inTGAFileName, const char *inTGAFileNameB, double inX, double inY,
                                       double inDrawScale)
    : SpriteButton(inTGAFileName, inX, inY, inDrawScale), mToggled(false), mShouldDestroySpriteB(false),
      mMouseOverTipB(NULL)
{
    ...
    {
        AppLog::errorF("Failed to read file for SpriteToggleButton: %s", inTGAFileNameB);
    }
}
