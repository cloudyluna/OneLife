...
EditorCategoryPage::EditorCategoryPage()
    : mObjectParentPicker(&objectPickableParent, -410, 90), mObjectChildPicker(&objectPickableChild, +410, 90),
      mTransEditorButton(mainFont, 0, 260, "Transitions"), mIsPatternCheckbox(220, 0, 2),
      mIsProbSetCheckbox(220, -20, 2), mMakeUniformButton(smallFont, 220, -80, "Make Uniform")
{
    ...
    else if (inTarget == &mObjectParentPicker)
    {
        ...
        else
        {
            ...
            if (mCurrentCategory != -1)
            {
                ...
                if (cr != NULL)
                {
                    int newMax = getCategory(mCurrentCategory)->objectIDSet.size() - 1;
...
static void drawObjectList(char inCategories, char inPattern, SimpleVector<int> *inList,
                           SimpleVector<float> *inWeights = NULL, int inSelectionIndex = -1)
{
    ...
    for (int i = 0; i < num; i++)
    {
        ...
        smallFont->drawString(getObject(objID)->description, textPos, alignLeft);
        ...
        if (inWeights != NULL)
        {
            ...
            smallFont->drawString(probString, weightTextPos, alignLeft);
            delete[] probString;
        }
        ...
        if (inCategories && getCategory(objID)->isPattern)
        {
            ...
            smallFont->drawString("Pat", textPos, alignRight);
        }
        else if (inCategories && getCategory(objID)->isProbabilitySet)
        {
            ...
            smallFont->drawString("Prob", textPos, alignRight);
        }
        else if (inPattern)
        {
            textPos.x -= 20;
            ...
            smallFont->drawString(iString, textPos, alignRight);
...
void EditorCategoryPage::draw(doublePair inViewCenter, double inViewSize)
{
    ...
    if (mCurrentObject != -1)
    {
        ...
        drawObject(r, 2, pos, 0, false, false, 20, 0, false, false, getEmptyClothingSet());
    ...
    else if (mCurrentCategory != -1)
    {
        ...
        if (cat != NULL)
        {
            ...
            drawObjectList(false, cat->isPattern, &(cat->objectIDSet), w, mSelectionIndex);
        }
        else
        {
            mIsPatternCheckbox.setToggled(false);
...
void EditorCategoryPage::keyDown(unsigned char inASCII)
{
    ...
    if (inASCII == 8)
    {
        ...
        if (mCurrentObject != -1)
        {
            ...
            if (catID != -1)
            {
                ...
                int newMax = getNumCategoriesForObject(mCurrentObject) - 1;
        ...
        {
            int objID = getCategory(mCurrentCategory)->objectIDSet.getElementDirect(mSelectionIndex);

            removeObjectFromCategory(mCurrentCategory, objID, mSelectionIndex);
    ...
    if (inASCII >= '0' && inASCII <= '9' && mCurrentCategory != -1)
    {
        ...
        if (r->isProbabilitySet)
        {
            ...
            float oldWeight = r->objectWeights.getElementDirect(mSelectionIndex);
            ...
            setMemberWeight(mCurrentCategory, r->objectIDSet.getElementDirect(mSelectionIndex), newWeight);
        }
    }
    ...
    if (inASCII == 'd' || inASCII == 'D')
    {
        ...
        if (r != NULL && r->objectIDSet.size() > mSelectionIndex)
        {
            ...
            if (inASCII == 'd')
            {
                ...
                {
                    moveCategoryMemberUp(mCurrentCategory, objID, mSelectionIndex);
                    mSelectionIndex--;
                }
            }
        ...
        */
        if (mCurrentCategory != -1)
        {
            ...
            if (r != NULL && r->objectIDSet.size() > mSelectionIndex)
            {
                ...
                for (int i = 0; i < offset; i++)
                {
                    moveCategoryMemberDown(mCurrentCategory, objID, mSelectionIndex);
                    mSelectionIndex++;
                }
