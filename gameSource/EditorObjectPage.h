...
class EditorObjectPage : public GamePage, public ActionListener
{
  ...
  protected:
    ...
    double getClosestSpriteOrSlot(float inX, float inY, int *outSprite, int *outSlot);
    ...
    void drawSpriteLayers(doublePair inDrawOffset, char inBehindSlots);

    void recursiveRotate(ObjectRecord *inObject, int inRotatingParentIndex, doublePair inRotationCenter,
                         double inRotationDelta);

    void recursiveMove(ObjectRecord *inObject, int inMovingParentIndex, doublePair inMoveDelta);
