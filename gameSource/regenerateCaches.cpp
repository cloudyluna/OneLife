...
void runRebuild(const char *inBankName, int inNumExpectedSteps, float (*inStepFunction)())
{
...
int main()
{
    ...
    if (groundTileCacheFolder.exists() && groundTileCacheFolder.isDirectory())
    {

        int numChildFiles;
        File **childFiles = groundTileCacheFolder.getChildFiles(&numChildFiles);
...
static Image *readTGAFile(File *inFile)
{
    ...
    if (!inFile->exists())
    {
        ...
        printf("CRITICAL ERROR:  TGA file %s does not exist", fileName);
        delete[] fileName;
    ...
    if (result == NULL)
    {
        ...
        printf("CRITICAL ERROR:  could not read TGA file %s, wrong format?", fileName);
        delete[] fileName;
    }
