...
SoundWidget::SoundWidget(Font *inDisplayFont, int inX, int inY)
    : PageComponent(inX, inY), mSoundUsage(blankSoundUsage), mCurSoundIndex(0),
      mPrevSubSoundButton(smallFont, -90, 0, "<"), mNextSubSoundButton(smallFont, -46, 0, ">"),
      mRemoveSubSoundButton(smallFont, -68, 0, "x"),

      mRecordButton("recordButton.tga", -85, -30), mStopButton("stopButton.tga", -85, -30),
      mPlayButton("playButton.tga", -51, -30),

      mPlayRandButton("playButton.tga", -17, 0), mClearButton("clearButton.tga", 17, 0),
      mCopyButton("copyButton.tga", 51, 0), mPasteButton("pasteButton.tga", 85, 0),
      mVolumeSlider(inDisplayFont, -70, -30, 2, 124, 20, 0, 1.0, "V"),
...
void SoundWidget::actionPerformed(GUIComponent *inTarget)
{
    ...
    {
        if (mSoundUsage.numSubSounds > 0 && !equal(mSoundUsage, sClipboardSoundUsage))
        {
    ...
    else if (inTarget == &mPasteButton)
    {
        if (sClipboardSoundUsage.numSubSounds > 0)
        {
            ...
            if (sClipboardSoundUsage.numSubSounds == 1 && mCurSoundIndex == mSoundUsage.numSubSounds)
            {
                // case:
                ...
                addSound(&mSoundUsage, sClipboardSoundUsage.ids[0], sClipboardSoundUsage.volumes[0]);
    ...
    else if (inTarget == &mRemoveSubSoundButton)
    {
        ...
        if (mCurSoundIndex == oldUsage.numSubSounds - 1 && mCurSoundIndex > 0)
        {
            mCurSoundIndex--;
        }
...
{
    mPasteButton.setVisible(sClipboardSoundUsage.numSubSounds > 0 && mRecordButton.isVisible());
}
...
void SoundWidget::draw()
{
    ...
    if (!mRecordButton.isVisible() && !mStopButton.isVisible())
    {
