...
static const char *animTypeNames[11] = {"ground", "held",    "moving",      "ground2", "eating", "doing",
                                        "biking", "sitting", "endAnimType", "extra",   "extraB"};
...
int initAnimationBankStart(char *outRebuildingCache)
{
    ...
    if (drawMouthShapes)
    {
        ...
        if (mouthShapeFolder.exists() && mouthShapeFolder.isDirectory())
        {
            ...
            File **mouthShapeFiles = mouthShapeFolder.getChildFilesSorted(&numMouthShapes);
    ...
    NudeToggle = SettingsManager::getIntSetting("nudeEnabled", 1);
        ...
        if (animText != NULL)
        {
            ...
            if (numLines > 4)
            {
                ...
                if (strchr(lines[next], ':') != NULL)
                {
                    ...
                    sscanf(lines[next], "type=%d:%d,randStartPhase=%d", &(typeRead), &extraIndexRead,
                           &randomStartPhaseRead);
                }
                else
                {
                    sscanf(lines[next], "type=%d,randStartPhase=%d", &(typeRead), &randomStartPhaseRead);
                }
                ...
                if (r->numSounds > 0)
                {
                    ...
                    for (int j = 0; j < r->numSounds && next < numLines; j++)
                    {
                        ...
                        if (end != NULL)
                        {
                            ...
                            int footstepValue = 0;
                            sscanf(&(end[1]), "%lf %lf %lf %lf %d", &(r->soundAnim[j].repeatPerSec),
                                   &(r->soundAnim[j].repeatPhase), &(r->soundAnim[j].ageStart),
                                   &(r->soundAnim[j].ageEnd), &footstepValue);
                ...
                for (int j = 0; j < r->numSprites && next < numLines; j++)
                {
                    ...
                    {
                        sscanf(lines[next], "offset=(%lf,%lf)", &(r->spriteAnim[j].offset.x),
                    ...
                    {
                        sscanf(lines[next], "startPause=%lf", &(r->spriteAnim[j].startPauseSec));
                        next++;
                        if (next >= numLines)
                        {
                            break;
                    ...
                    sscanf(lines[next],
                           ...
                           "%lf %lf %lf %lf %lf "
                           "%lf %lf %lf %lf %lf",
                           &(r->spriteAnim[j].xOscPerSec), &(r->spriteAnim[j].xAmp), &(r->spriteAnim[j].xPhase),

                           &(r->spriteAnim[j].yOscPerSec), &(r->spriteAnim[j].yAmp), &(r->spriteAnim[j].yPhase),

                           &(r->spriteAnim[j].rotationCenterOffset.x), &(r->spriteAnim[j].rotationCenterOffset.y),
                           ...
                           &(r->spriteAnim[j].fadeOscPerSec), &(r->spriteAnim[j].fadeHardness),
                           &(r->spriteAnim[j].fadeMin), &(r->spriteAnim[j].fadeMax), &(r->spriteAnim[j].fadePhase));
                ...
                for (int j = 0; j < r->numSlots && next < numLines; j++)
                {
                    ...
                    if (strstr(lines[next], "offset") != NULL)
                    {
                        sscanf(lines[next], "offset=(%lf,%lf)", &(r->slotAnim[j].offset.x), &(r->slotAnim[j].offset.y));
                        next++;
                    ...
                    if (strstr(lines[next], "startPause") != NULL)
                    {
                        sscanf(lines[next], "startPause=%lf", &(r->slotAnim[j].startPauseSec));
                        next++;
                        if (next >= numLines)
                        {
                            break;
                    ...
                    sscanf(lines[next],
                           ...
                           &(r->slotAnim[j].xOscPerSec), &(r->slotAnim[j].xAmp), &(r->slotAnim[j].xPhase),

                           &(r->slotAnim[j].yOscPerSec), &(r->slotAnim[j].yAmp), &(r->slotAnim[j].yPhase),
...
void addAnimation(AnimationRecord *inRecord, char inNoWriteToFile)
{
    ...
    if (newID >= mapSize)
    {
        ...
        SimpleVector<AnimationRecord *> *newExtraMap = new SimpleVector<AnimationRecord *>[newMapSize];
    ...
    if (!inNoWriteToFile)
    {
        ...
        if (animationFile != NULL)
        {
            ...
            if (inRecord->type < endAnimType)
            {
                lines.push_back(autoSprintf("type=%d,randStartPhase=%d", inRecord->type, inRecord->randomStartPhase));
            }
            else
            {
                lines.push_back(
                    autoSprintf("type=%d:%d,randStartPhase=%d", extra, extraIndex, inRecord->randomStartPhase));
            }

            lines.push_back(autoSprintf("forceZeroStart=%d", inRecord->forceZeroStart));
            ...
            for (int j = 0; j < inRecord->numSounds; j++)
            {
                lines.push_back(autoSprintf(
                    "soundParam=%s %lf %lf %lf %lf %d", printSoundUsage(inRecord->soundAnim[j].sound),
                    inRecord->soundAnim[j].repeatPerSec, inRecord->soundAnim[j].repeatPhase,
                    inRecord->soundAnim[j].ageStart, inRecord->soundAnim[j].ageEnd, inRecord->soundAnim[j].footstep));
            }
            ...
            {
                lines.push_back(autoSprintf("offset=(%lf,%lf)", inRecord->spriteAnim[j].offset.x,
                                            inRecord->spriteAnim[j].offset.y));

                lines.push_back(autoSprintf("startPause=%lf", inRecord->spriteAnim[j].startPauseSec));
                ...
                lines.push_back(autoSprintf(
                    ...
                    "%lf %lf %lf %lf %lf "
                    "%lf %lf %lf %lf %lf",
                    inRecord->spriteAnim[j].xOscPerSec, inRecord->spriteAnim[j].xAmp, inRecord->spriteAnim[j].xPhase,

                    inRecord->spriteAnim[j].yOscPerSec, inRecord->spriteAnim[j].yAmp, inRecord->spriteAnim[j].yPhase,

                    inRecord->spriteAnim[j].rotationCenterOffset.x, inRecord->spriteAnim[j].rotationCenterOffset.y,

                    inRecord->spriteAnim[j].rotPerSec, inRecord->spriteAnim[j].rotPhase,

                    inRecord->spriteAnim[j].rockOscPerSec, inRecord->spriteAnim[j].rockAmp,
                    inRecord->spriteAnim[j].rockPhase,

                    inRecord->spriteAnim[j].durationSec, inRecord->spriteAnim[j].pauseSec,

                    inRecord->spriteAnim[j].fadeOscPerSec, inRecord->spriteAnim[j].fadeHardness,
            ...
            {
                lines.push_back(
                    autoSprintf("offset=(%lf,%lf)", inRecord->slotAnim[j].offset.x, inRecord->slotAnim[j].offset.y));

                lines.push_back(autoSprintf("startPause=%lf", inRecord->slotAnim[j].startPauseSec));

                lines.push_back(autoSprintf(
                    "animParam="
                    "%lf %lf %lf %lf %lf %lf %lf %lf",
                    inRecord->slotAnim[j].xOscPerSec, inRecord->slotAnim[j].xAmp, inRecord->slotAnim[j].xPhase,

                    inRecord->slotAnim[j].yOscPerSec, inRecord->slotAnim[j].yAmp, inRecord->slotAnim[j].yPhase,
...
void clearAnimation(int inObjectID, AnimType inType)
{
    ...
    if (r != NULL)
    {
        ...
        else
        {
            ...
            {
                if (idExtraMap[inObjectID].getElementDirect(i)->extraIndex == extraIndex)
                {
...
static double getOscOffset(double inFrameTime, double inOffset, double inOscPerSec, double inAmp, double inPhase)
{
    return inOffset + inAmp * sin((inFrameTime * inOscPerSec + inPhase) * 2 * M_PI);
}

char isAnimFadeNeeded(int inObjectID, AnimType inCurType, AnimType inTargetType)
{

    if ((inCurType != ground2 && inTargetType == ground2) || (inCurType == ground2 && inTargetType != ground2))
    {
...
char isAnimFadeNeeded(int inObjectID, AnimationRecord *inCurR, AnimationRecord *inTargetR)
{
    ...
    for (int i = 0; i < obj->numSprites; i++)
    {
        ...
        if (inCurR->spriteAnim[i].xAmp != 0 && inCurR->spriteAnim[i].xPhase != 0 && inCurR->spriteAnim[i].xPhase != 0.5)
            return true;

        if (inCurR->spriteAnim[i].yOscPerSec > 0)
            return true;

        if (inCurR->spriteAnim[i].yAmp != 0 && inCurR->spriteAnim[i].yPhase != 0 && inCurR->spriteAnim[i].yPhase != 0.5)
            return true;

        if (inCurR->spriteAnim[i].rockOscPerSec > 0)
            return true;

        if (inCurR->spriteAnim[i].rockAmp != 0 && inCurR->spriteAnim[i].rockPhase != 0 &&
        ...
        if (inCurR->spriteAnim[i].fadeMax != 1 || inCurR->spriteAnim[i].fadePhase != 0)
            return true;

        // if target starts out of phase, must fade

        if (inTargetR->spriteAnim[i].xAmp != 0 && inTargetR->spriteAnim[i].xPhase != 0 &&
            inTargetR->spriteAnim[i].xPhase != 0.5)
            return true;

        if (inTargetR->spriteAnim[i].yAmp != 0 && inTargetR->spriteAnim[i].yPhase != 0 &&
            inTargetR->spriteAnim[i].yPhase != 0.5)
            return true;

        if (inTargetR->spriteAnim[i].rockAmp != 0 && inTargetR->spriteAnim[i].rockPhase != 0 &&
            inTargetR->spriteAnim[i].rockPhase != 0.5)
            return true;

        if (inTargetR->spriteAnim[i].rotPerSec != 0 || inTargetR->spriteAnim[i].rotPhase != 0)
            return true;

        if (inTargetR->spriteAnim[i].fadeMax != 1 || inTargetR->spriteAnim[i].fadePhase != 0)
            return true;
    }
    ...
    for (int i = 0; i < obj->numSlots; i++)
    {
        ...
        if (inCurR->slotAnim[i].xAmp != 0 && inCurR->slotAnim[i].xPhase != 0 && inCurR->slotAnim[i].xPhase != 0.5)
            return true;

        if (inCurR->slotAnim[i].yOscPerSec > 0)
            return true;

        if (inCurR->slotAnim[i].yAmp != 0 && inCurR->slotAnim[i].yPhase != 0 && inCurR->slotAnim[i].yPhase != 0.5)
            return true;

        // if target starts out of phase, must fade

        if (inTargetR->slotAnim[i].xAmp != 0 && inTargetR->slotAnim[i].xPhase != 0 &&
            inTargetR->slotAnim[i].xPhase != 0.5)
            return true;

        if (inTargetR->slotAnim[i].yAmp != 0 && inTargetR->slotAnim[i].yPhase != 0 &&
...
char isAnimEmpty(int inObjectID, AnimType inType)
{
    ...
    for (int i = 0; i < obj->numSprites; i++)
    {
        ...
        if (r->spriteAnim[i].xAmp != 0 && r->spriteAnim[i].xPhase != 0 && r->spriteAnim[i].xPhase != 0.5)
            return false;

        if (r->spriteAnim[i].yOscPerSec > 0)
            return false;

        if (r->spriteAnim[i].yAmp != 0 && r->spriteAnim[i].yPhase != 0 && r->spriteAnim[i].yPhase != 0.5)
            return false;

        if (r->spriteAnim[i].rockOscPerSec > 0)
            return false;

        if (r->spriteAnim[i].rockAmp != 0 && r->spriteAnim[i].rockPhase != 0 && r->spriteAnim[i].rockPhase != 0.5)
            return false;
    ...
    for (int i = 0; i < obj->numSlots; i++)
    {
        ...
        if (r->slotAnim[i].xAmp != 0 && r->slotAnim[i].xPhase != 0 && r->slotAnim[i].xPhase != 0.5)
            return false;

        if (r->slotAnim[i].yOscPerSec > 0)
            return false;

        if (r->slotAnim[i].yAmp != 0 && r->slotAnim[i].yPhase != 0 && r->slotAnim[i].yPhase != 0.5)
            return false;
    }
...
ObjectAnimPack drawObjectAnimPacked(int inObjectID, AnimType inType, double inFrameTime, double inAnimFade,
                                    ...
                                    AnimType inFrozenArmType, AnimType inFrozenArmFadeTargetType, doublePair inPos,
                                    double inRot, char inWorn, char inFlipH, double inAge, int inHideClosestArm,
                                    char inHideAllLimbs, char inHeldNotInPlaceYet, ClothingSet inClothing,
                                    SimpleVector<int> *inClothingContained, int inNumContained, int *inContainedIDs,
...
void drawObjectAnim(ObjectAnimPack inPack)
{
    ...
    {
        p = drawObjectAnim(inPack.inObjectID, 2, inPack.inType, inPack.inFrameTime, inPack.inAnimFade,
                           inPack.inFadeTargetType, inPack.inFadeTargetFrameTime, inPack.inFrozenRotFrameTime,
                           inPack.outFrozenRotFrameTimeUsed, inPack.inFrozenArmType, inPack.inFrozenArmFadeTargetType,
                           inPack.inPos, inPack.inRot, inPack.inWorn, inPack.inFlipH, inPack.inAge,
                           inPack.inHideClosestArm, inPack.inHideAllLimbs, inPack.inHeldNotInPlaceYet,
                           inPack.inClothing, inPack.inClothingContained);
    }
    else
    {
        drawObjectAnim(inPack.inObjectID, inPack.inType, inPack.inFrameTime, inPack.inAnimFade, inPack.inFadeTargetType,
                       inPack.inFadeTargetFrameTime, inPack.inFrozenRotFrameTime, inPack.outFrozenRotFrameTimeUsed,
                       inPack.inFrozenArmType, inPack.inFrozenArmFadeTargetType, inPack.inPos, inPack.inRot,
                       inPack.inWorn, inPack.inFlipH, inPack.inAge, inPack.inHideClosestArm, inPack.inHideAllLimbs,
                       inPack.inHeldNotInPlaceYet, inPack.inClothing, inPack.inClothingContained, inPack.inNumContained,
    ...
    if (inPack.additionalHeldID > 0 && p.valid)
    {
        ...
        computeHeldDrawPos(p, inPack.inPos, heldObject, inPack.inFlipH, &holdPos, &holdRot);

        drawObjectAnim(inPack.additionalHeldID, 2, inPack.inType, inPack.inFrameTime, inPack.inAnimFade,
                       inPack.inFadeTargetType, inPack.inFadeTargetFrameTime, inPack.inFrozenRotFrameTime,
                       inPack.outFrozenRotFrameTimeUsed, endAnimType, endAnimType, holdPos, holdRot, false,
                       inPack.inFlipH, -1, 0, false, false, emptyClothing, NULL);
    }
}

HoldingPos drawObjectAnim(int inObjectID, int inDrawBehindSlots, AnimType inType, double inFrameTime, double inAnimFade,
                          AnimType inFadeTargetType, double inFadeTargetFrameTime, double inFrozenRotFrameTime,
                          char *outFrozenRotFrameTimeUsed, AnimType inFrozenArmType, AnimType inFrozenArmFadeTargetType,
                          doublePair inPos, double inRot, char inWorn, char inFlipH, double inAge, int inHideClosestArm,
                          char inHideAllLimbs, char inHeldNotInPlaceYet, ClothingSet inClothing,
                          SimpleVector<int> *inClothingContained, double *outSlotRots, doublePair *outSlotOffsets)
{
    ...
    if (r == NULL)
    {
        ...
        animLayerCutoff = -1;
        return drawObject(getObject(inObjectID), inDrawBehindSlots, inPos, inRot, inWorn, inFlipH, inAge,
    ...
    else
    {
        ...
        AnimationRecord *rArmFade = getAnimation(inObjectID, inFrozenArmFadeTargetType);
        ...
        return drawObjectAnim(inObjectID, inDrawBehindSlots, r, inFrameTime, inAnimFade, rB, inFadeTargetFrameTime,
                              inFrozenRotFrameTime, outFrozenRotFrameTimeUsed, rF, rArm, rArmFade, inPos, inRot, inWorn,
                              inFlipH, inAge, inHideClosestArm, inHideAllLimbs, inHeldNotInPlaceYet, inClothing,
                              inClothingContained, outSlotRots, outSlotOffsets);
    }
}

void drawObjectAnimHighlighted(float inHighlightFade, int inObjectID, AnimType inType, double inFrameTime,
                               double inAnimFade, AnimType inFadeTargetType, double inFadeTargetFrameTime,
                               double inFrozenRotFrameTime, char *outFrozenRotFrameTimeUsed, AnimType inFrozenArmType,
                               AnimType inFrozenArmFadeTargetType, doublePair inPos, double inRot, char inWorn,
                               char inFlipH, double inAge, int inHideClosestArm, char inHideAllLimbs,
                               char inHeldNotInPlaceYet, ClothingSet inClothing, SimpleVector<int> *inClothingContained,
                               int inNumContained, int *inContainedIDs, SimpleVector<int> *inSubContained)
{

    // draw object normally
    drawObjectAnim(inObjectID, inType, inFrameTime, inAnimFade, inFadeTargetType, inFadeTargetFrameTime,
                   inFrozenRotFrameTime, outFrozenRotFrameTimeUsed, inFrozenArmType, inFrozenArmFadeTargetType, inPos,
                   inRot, inWorn, inFlipH, inAge, inHideClosestArm, inHideAllLimbs, inHeldNotInPlaceYet, inClothing,
    ...
    if (inHighlightFade > 0)
    {
        ...
        for (int i = startPass; i < numPasses; i++)
        {
            ...
            drawObjectAnim(inObjectID, inType, inFrameTime, inAnimFade, inFadeTargetType, inFadeTargetFrameTime,
                           inFrozenRotFrameTime, outFrozenRotFrameTimeUsed, inFrozenArmType, inFrozenArmFadeTargetType,
                           inPos, inRot, inWorn, inFlipH, inAge, inHideClosestArm, inHideAllLimbs, inHeldNotInPlaceYet,
                           inClothing, inClothingContained, inNumContained, inContainedIDs, inSubContained);
...
static double processFrameTimeWithPauses(AnimationRecord *inAnim, int inLayerIndex,
                                         // true if sprite, false if slot
                                         char inSpriteOrSlot, double inFrameTime)
{
...
HoldingPos drawObjectAnim(int inObjectID, int inDrawBehindSlots, AnimationRecord *inAnim, double inFrameTime,
                          double inAnimFade, AnimationRecord *inFadeTargetAnim, double inFadeTargetFrameTime,
                          double inFrozenRotFrameTime, char *outFrozenRotFrameTimeUsed,
                          AnimationRecord *inFrozenRotAnim, AnimationRecord *inFrozenArmAnim,
                          AnimationRecord *inFrozenArmFadeTargetAnim, doublePair inPos, double inRot, char inWorn,
                          char inFlipH, double inAge, int inHideClosestArm, char inHideAllLimbs,
                          char inHeldNotInPlaceYet, ClothingSet inClothing, SimpleVector<int> *inClothingContained,
                          double *outSlotRots, doublePair *outSlotOffsets)
{
    ...
    *outFrozenRotFrameTimeUsed = false;
    ...
    if (obj->numSprites > MAX_WORKING_SPRITES)
    {
        ...
        drawObject(obj, inDrawBehindSlots, inPos, inRot, inWorn, inFlipH, inAge, inHideClosestArm, inHideAllLimbs,
    ...
    for (int i = 0; i < obj->numSprites; i++)
    {
        ...
        spriteFrameTime = processFrameTimeWithPauses(spriteAnim, i, true, spriteFrameTime);
        if (inAnimFade < 1)
        {
            targetSpriteFrameTime = processFrameTimeWithPauses(spriteFadeTargetAnim, i, true, targetSpriteFrameTime);
        }
        ...
        if (i < spriteAnim->numSprites)
        {
            ...
            double sinVal = getOscOffset(spriteFrameTime, 0, spriteAnim->spriteAnim[i].fadeOscPerSec, 1.0,
            ...
            else
            {
                ...
                if (absSinVal != 0)
                {
                    hardVersion = (sinVal / absSinVal) * pow(absSinVal, 1.0 / (hardness * 10 + 1));
                }
                else
                {
                    hardVersion = 0;
            ...
            double fade =
                (spriteAnim->spriteAnim[i].fadeMax - spriteAnim->spriteAnim[i].fadeMin) * (0.5 * hardVersion + 0.5) +
            ...
            spritePos.x += inAnimFade * getOscOffset(spriteFrameTime, spriteAnim->spriteAnim[i].offset.x,
                                                     spriteAnim->spriteAnim[i].xOscPerSec,
                                                     spriteAnim->spriteAnim[i].xAmp, spriteAnim->spriteAnim[i].xPhase);

            spritePos.y += inAnimFade * getOscOffset(spriteFrameTime, spriteAnim->spriteAnim[i].offset.y,
                                                     spriteAnim->spriteAnim[i].yOscPerSec,
                                                     spriteAnim->spriteAnim[i].yAmp, spriteAnim->spriteAnim[i].yPhase);

            double rock =
                inAnimFade * getOscOffset(spriteFrameTime, 0, spriteAnim->spriteAnim[i].rockOscPerSec,
                                          spriteAnim->spriteAnim[i].rockAmp, spriteAnim->spriteAnim[i].rockPhase);

            doublePair rotCenterOffset = mult(spriteAnim->spriteAnim[i].rotationCenterOffset, inAnimFade);
            ...
            if (inAnimFade < 1 && i < spriteFadeTargetAnim->numSprites)
            {
                ...
                double sinValB =
                    getOscOffset(targetSpriteFrameTime, 0, spriteFadeTargetAnim->spriteAnim[i].fadeOscPerSec, 1.0,
                ...
                else
                {
                    ...
                    if (absSinValB != 0)
                    {
                        hardVersionB = (sinValB / absSinValB) * pow(absSinValB, 1.0 / (hardnessB * 10 + 1));
                    }
                    else
                    {
                        hardVersionB = 0;
                ...
                double fadeB =
                    (spriteFadeTargetAnim->spriteAnim[i].fadeMax - spriteFadeTargetAnim->spriteAnim[i].fadeMin) *
                ...
                spritePos.x +=
                    targetWeight * getOscOffset(targetSpriteFrameTime, spriteFadeTargetAnim->spriteAnim[i].offset.x,
                ...
                spritePos.y +=
                    targetWeight * getOscOffset(targetSpriteFrameTime, spriteFadeTargetAnim->spriteAnim[i].offset.y,
                ...
                rock += targetWeight * getOscOffset(targetSpriteFrameTime, 0,
                ...
                rotCenterOffset = add(rotCenterOffset, mult(spriteFadeTargetAnim->spriteAnim[i].rotationCenterOffset,

                                                            targetWeight));
            }

            double totalRotOffset =
                spriteAnim->spriteAnim[i].rotPerSec * spriteFrameTime + spriteAnim->spriteAnim[i].rotPhase;

            // use frozen rot instead if either current or
            // target satisfies
            if (inAnim->type != moving && spriteAnim->spriteAnim[i].rotPerSec == 0 &&
                spriteAnim->spriteAnim[i].rotPhase == 0 && spriteAnim->spriteAnim[i].rockOscPerSec == 0 &&
                spriteAnim->spriteAnim[i].rockPhase == 0 && inFrozenRotAnim->spriteAnim[i].rotPerSec != 0)
            {

                // use frozen instead
                totalRotOffset = inFrozenRotAnim->spriteAnim[i].rotPerSec * inFrozenRotFrameTime +
                                 inFrozenRotAnim->spriteAnim[i].rotPhase;

                *outFrozenRotFrameTimeUsed = *outFrozenRotFrameTimeUsed || true;
            }
            else if (inAnimFade < 1 && i < spriteFadeTargetAnim->numSprites && spriteAnim->type == moving &&
                     spriteFadeTargetAnim->type != moving && spriteFadeTargetAnim->spriteAnim[i].rotPerSec == 0 &&
            ...
            if (inAnimFade < 1 && i < spriteFadeTargetAnim->numSprites)
            {
                double totalTargetRotOffset = spriteFadeTargetAnim->spriteAnim[i].rotPerSec * targetSpriteFrameTime +
                                              spriteFadeTargetAnim->spriteAnim[i].rotPhase;

                if (spriteFadeTargetAnim->type != moving && spriteFadeTargetAnim->spriteAnim[i].rotPerSec == 0 &&
                    spriteFadeTargetAnim->spriteAnim[i].rotPhase == 0 &&
                    spriteFadeTargetAnim->spriteAnim[i].rockOscPerSec == 0 &&
                    spriteFadeTargetAnim->spriteAnim[i].rockPhase == 0 && inFrozenRotAnim->spriteAnim[i].rotPerSec != 0)
                {

                    // use frozen instead
                    totalTargetRotOffset = inFrozenRotAnim->spriteAnim[i].rotPerSec * inFrozenRotFrameTime +
                                           inFrozenRotAnim->spriteAnim[i].rotPhase;
                ...
                // relative to 0 on circle
                double relativeTargetRotOffset = totalTargetRotOffset - floor(totalTargetRotOffset);
                ...
                double aveCenteredOffset = inAnimFade * centeredOffset + targetWeight * centeredTargetOffset;
    ...
    for (int i = 0; i < limit; i++)
    {
        ...
        while (nextParent != -1)
        {
            ...
            if (workingDeltaRot[nextParent] != 0)
            {
                ...
                spritePos = add(spritePos, rotate(workingDeltaSpritePos[nextParent], -angle));
        ...
        else if (i == topBackArmIndex)
        {
            ...
            for (int e = 0; e < drawWithEmots.size(); e++)
                if (drawWithEmots.getElementDirect(e)->bodyEmot != 0 && obj->person)
                {
                    ...
                    char used;
                    drawObjectAnim(drawWithEmots.getElementDirect(e)->bodyEmot, clothingAnimType, inFrameTime,
                                   inAnimFade, clothingFadeTargetAnimType, inFadeTargetFrameTime, inFrozenRotFrameTime,
                                   &used, endAnimType, endAnimType, add(animBodyPos, inPos), animBodyRotDelta, true,
            ...
            if (inClothing.bottom != NULL)
            {
                ...
                char used;
                drawObjectAnimHighlighted(clothingHighlightFades[4], inClothing.bottom->id, clothingAnimType,
                                          inFrameTime, inAnimFade, clothingFadeTargetAnimType, inFadeTargetFrameTime,
                                          inFrozenRotFrameTime, &used, endAnimType, endAnimType, bottomPos, bottomRot,
                                          true, inFlipH, -1, 0, false, false, emptyClothing, NULL, numCont, cont, NULL);
            ...
            if (inClothing.tunic != NULL)
            {
                ...
                char used;
                drawObjectAnimHighlighted(clothingHighlightFades[1], inClothing.tunic->id, clothingAnimType,
                                          inFrameTime, inAnimFade, clothingFadeTargetAnimType, inFadeTargetFrameTime,
                                          inFrozenRotFrameTime, &used, endAnimType, endAnimType, tunicPos, tunicRot,
                                          true, inFlipH, -1, 0, false, false, emptyClothing, NULL, numCont, cont, NULL);
            ...
            if (inClothing.backpack != NULL)
            {
                ...
                char used;
                drawObjectAnimHighlighted(clothingHighlightFades[5], inClothing.backpack->id, clothingAnimType,
                                          inFrameTime, inAnimFade, clothingFadeTargetAnimType, inFadeTargetFrameTime,
                                          inFrozenRotFrameTime, &used, endAnimType, endAnimType, backpackPos,
                                          backpackRot, true, inFlipH, -1, 0, false, false, emptyClothing, NULL, numCont,
                                          cont, NULL);
        ...
        if (!skipSprite)
        {
            ...
            if (drawMouthShapes && spriteID == mouthAnchorID && mouthShapeFrame < numMouthShapeFrames)
            {
            ...
            else
            {
                ...
                if (sh != NULL)
                {
                    drawSprite(sh, pos, drawObjectScale, rot, logicalXOR(inFlipH, obj->spriteHFlip[i]));
                }
            }
        ...
        if (i == eyesIndex)
            for (int e = 0; e < drawWithEmots.size(); e++)
                if (drawWithEmots.getElementDirect(e)->eyeEmot != 0)
                {
                    ...
                    char used;
                    drawObjectAnim(drawWithEmots.getElementDirect(e)->eyeEmot, clothingAnimType, inFrameTime,
                                   inAnimFade, clothingFadeTargetAnimType, inFadeTargetFrameTime, inFrozenRotFrameTime,
                                   &used, endAnimType, endAnimType, cPos, animHeadRotDelta, true, inFlipH, -1, 0, false,
                                   false, emptyClothing, NULL, 0, NULL, NULL);
                }

        // face emot on top of eyes, if they exist, or head
        // if not
        if (((eyesIndex != -1 && i == eyesIndex) || (eyesIndex == -1 && i == headIndex)) && obj->person)
            for (int e = 0; e < drawWithEmots.size(); e++)
                if (drawWithEmots.getElementDirect(e)->faceEmot != 0)
                {

                    char used;
                    drawObjectAnim(drawWithEmots.getElementDirect(e)->faceEmot, clothingAnimType, inFrameTime,
                                   inAnimFade, clothingFadeTargetAnimType, inFadeTargetFrameTime, inFrozenRotFrameTime,
                                   &used, endAnimType, endAnimType, add(animHeadPos, inPos), animHeadRotDelta, true,
                                   inFlipH, -1, 0, false, false, emptyClothing, NULL, 0, NULL, NULL);
                }
        ...
        if (i == headIndex && mouthIndex != -1)
            for (int e = 0; e < drawWithEmots.size(); e++)
                if (drawWithEmots.getElementDirect(e)->mouthEmot != 0)
                {
                    ...
                    char used;
                    drawObjectAnim(drawWithEmots.getElementDirect(e)->mouthEmot, clothingAnimType, inFrameTime,
                                   inAnimFade, clothingFadeTargetAnimType, inFadeTargetFrameTime, inFrozenRotFrameTime,
                                   &used, endAnimType, endAnimType, add(animHeadPos, inPos), animHeadRotDelta, true,
                                   inFlipH, -1, 0, false, false, emptyClothing, NULL, 0, NULL, NULL);
                }
        ...
        if (i == headIndex && obj->person)
            for (int e = 0; e < drawWithEmots.size(); e++)
                if (drawWithEmots.getElementDirect(e)->otherEmot != 0)
                {
                    ...
                    char used;
                    drawObjectAnim(drawWithEmots.getElementDirect(e)->otherEmot, clothingAnimType, inFrameTime,
                                   inAnimFade, clothingFadeTargetAnimType, inFadeTargetFrameTime, inFrozenRotFrameTime,
                                   &used, endAnimType, endAnimType, add(animHeadPos, inPos), animHeadRotDelta, true,
                                   inFlipH, -1, 0, false, false, emptyClothing, NULL, 0, NULL, NULL);
                }
        ...
        if (!skipSprite && inClothing.backShoe != NULL && i == backFootIndex)
        {
            ...
            char used;
            drawObjectAnimHighlighted(clothingHighlightFades[3], inClothing.backShoe->id, clothingAnimType, inFrameTime,
                                      inAnimFade, clothingFadeTargetAnimType, inFadeTargetFrameTime,
                                      inFrozenRotFrameTime, &used, endAnimType, endAnimType, backShoePos, backShoeRot,
                                      true, inFlipH, -1, 0, false, false, emptyClothing, NULL, numCont, cont, NULL);
        ...
        else if (!skipSprite && inClothing.frontShoe != NULL && i == frontFootIndex)
        {
            ...
            char used;
            drawObjectAnimHighlighted(clothingHighlightFades[2], inClothing.frontShoe->id, clothingAnimType,
                                      inFrameTime, inAnimFade, clothingFadeTargetAnimType, inFadeTargetFrameTime,
                                      inFrozenRotFrameTime, &used, endAnimType, endAnimType, frontShoePos, frontShoeRot,
                                      true, inFlipH, -1, 0, false, false, emptyClothing, NULL, numCont, cont, NULL);
    ...
    if (inClothing.hat != NULL)
    {
        ...
        char used;
        drawObjectAnimHighlighted(clothingHighlightFades[0], inClothing.hat->id, clothingAnimType, inFrameTime,
                                  inAnimFade, clothingFadeTargetAnimType, inFadeTargetFrameTime, inFrozenRotFrameTime,
                                  &used, endAnimType, endAnimType, cPos, animHeadRotDelta, true, inFlipH, -1, 0, false,
    ...
    if (obj->person)
        for (int e = 0; e < drawWithEmots.size(); e++)
            if (drawWithEmots.getElementDirect(e)->headEmot != 0)
            {
                ...
                char used;
                drawObjectAnim(drawWithEmots.getElementDirect(e)->headEmot, clothingAnimType, inFrameTime, inAnimFade,
                               clothingFadeTargetAnimType, inFadeTargetFrameTime, inFrozenRotFrameTime, &used,
                               endAnimType, endAnimType, add(animHeadPos, inPos), animHeadRotDelta, true, inFlipH, -1,
                               0, false, false, emptyClothing, NULL, 0, NULL, NULL);
            }
        ...
        for (int i = 0; i < obj->numSlots; i++)
        {
            ...
            while (nextParent != -1)
            {
                ...
                if (workingDeltaRot[nextParent] != 0)
                {
                    ...
                    slotPos = add(slotPos, rotate(workingDeltaSpritePos[nextParent], -angle));
...
void drawObjectAnim(int inObjectID, AnimType inType, double inFrameTime, double inAnimFade, AnimType inFadeTargetType,
                    double inFadeTargetFrameTime, double inFrozenRotFrameTime, char *outFrozenRotFrameTimeUsed,
                    AnimType inFrozenArmType, AnimType inFrozenArmFadeTargetType, doublePair inPos, double inRot,
                    char inWorn, char inFlipH, double inAge, int inHideClosestArm, char inHideAllLimbs,
                    char inHeldNotInPlaceYet, ClothingSet inClothing, SimpleVector<int> *inClothingContained,
                    int inNumContained, int *inContainedIDs, SimpleVector<int> *inSubContained)
{
    ...
    if (r == NULL)
    {
        ...
        drawObject(getObject(inObjectID), inPos, inRot, inWorn, inFlipH, inAge, inHideClosestArm, inHideAllLimbs,
                   inHeldNotInPlaceYet, inClothing, inNumContained, inContainedIDs, inSubContained);
    }
    else
    {
        if (inFadeTargetType == ground2)
        ...
        drawObjectAnim(inObjectID, r, inFrameTime, inAnimFade, rB, inFadeTargetFrameTime, inFrozenRotFrameTime,
                       outFrozenRotFrameTimeUsed, rF, rArm, rArmFade, inPos, inRot, inWorn, inFlipH, inAge,
                       inHideClosestArm, inHideAllLimbs, inHeldNotInPlaceYet, inClothing, inClothingContained,
                       inNumContained, inContainedIDs, inSubContained);
    }
}

void drawObjectAnim(int inObjectID, AnimationRecord *inAnim, double inFrameTime, double inAnimFade,
                    AnimationRecord *inFadeTargetAnim, double inFadeTargetFrameTime, double inFrozenRotFrameTime,
                    char *outFrozenRotFrameTimeUsed, AnimationRecord *inFrozenRotAnim, AnimationRecord *inFrozenArmAnim,
                    AnimationRecord *inFrozenArmFadeTargetAnim, doublePair inPos, double inRot, char inWorn,
                    char inFlipH, double inAge, int inHideClosestArm, char inHideAllLimbs, char inHeldNotInPlaceYet,
                    ClothingSet inClothing, SimpleVector<int> *inClothingContained, int inNumContained,
                    int *inContainedIDs, SimpleVector<int> *inSubContained)
{
    ...
    drawObjectAnim(inObjectID, 0, inAnim, inFrameTime, inAnimFade, inFadeTargetAnim, inFadeTargetFrameTime,
                   inFrozenRotFrameTime, outFrozenRotFrameTimeUsed, inFrozenRotAnim, inFrozenArmAnim,
                   inFrozenArmFadeTargetAnim, inPos, inRot, inWorn, inFlipH, inAge, inHideClosestArm, inHideAllLimbs,
                   inHeldNotInPlaceYet, inClothing, inClothingContained, slotRots, slotOffsets);
    ...
    for (int i = 0; i < obj->numSlots; i++)
    {
        if (i < inNumContained)
        {
            ...
            slotFrameTime = processFrameTimeWithPauses(inAnim, i, false, slotFrameTime);

            if (inAnimFade < 1)
            {
                targetSlotFrameTime = processFrameTimeWithPauses(inFadeTargetAnim, i, false, targetSlotFrameTime);
            }
            ...
            if (i < inAnim->numSlots)
            {
                ...
                posWiggle.x += inAnimFade * getOscOffset(slotFrameTime, inAnim->slotAnim[i].offset.x,
                                                         inAnim->slotAnim[i].xOscPerSec, inAnim->slotAnim[i].xAmp,
                                                         inAnim->slotAnim[i].xPhase);

                posWiggle.y += inAnimFade * getOscOffset(slotFrameTime, inAnim->slotAnim[i].offset.y,
                                                         inAnim->slotAnim[i].yOscPerSec, inAnim->slotAnim[i].yAmp,
                ...
                if (inAnimFade < 1)
                {
                    ...
                    posWiggle.x +=
                        targetWeight * getOscOffset(targetSlotFrameTime, inFadeTargetAnim->slotAnim[i].offset.x,
                                                    inFadeTargetAnim->slotAnim[i].xOscPerSec,
                                                    inFadeTargetAnim->slotAnim[i].xAmp,
                                                    inFadeTargetAnim->slotAnim[i].xPhase);

                    posWiggle.y +=
                        targetWeight * getOscOffset(targetSlotFrameTime, inFadeTargetAnim->slotAnim[i].offset.y,
            ...
            if (inSubContained != NULL && inSubContained[i].size() > 0)
            {
                ...
                AnimationRecord *subFadeTargetAnim = getAnimation(contained->id, inFadeTargetAnim->type);
                ...
                for (int s = 0; s < contained->numSlots; s++)
                {
                    if (s < inSubContained[i].size())
                    {
                        ...
                        if (subAnim != NULL && s < subAnim->numSlots)
                        {
                            ...
                            subPos.x +=
                                inAnimFade * getOscOffset(slotFrameTime, subAnim->slotAnim[s].offset.x,
                                                          subAnim->slotAnim[s].xOscPerSec, subAnim->slotAnim[s].xAmp,
                                                          subAnim->slotAnim[s].xPhase);

                            subPos.y +=
                                inAnimFade * getOscOffset(slotFrameTime, subAnim->slotAnim[s].offset.y,
                                                          subAnim->slotAnim[s].yOscPerSec, subAnim->slotAnim[s].yAmp,
                            ...
                            if (inAnimFade < 1)
                            {
                                ...
                                subPos.x += targetWeight * getOscOffset(targetSlotFrameTime,
                                ...
                                subPos.y += targetWeight * getOscOffset(targetSlotFrameTime,
                        ...
                        drawObject(subContained, 2, subPos, subRot, false, inFlipH, inAge, 0, false, false,
                                   emptyClothing);
                    }
                }
    ...
    drawObjectAnim(inObjectID, 1, inAnim, inFrameTime, inAnimFade, inFadeTargetAnim, inFadeTargetFrameTime,
                   inFrozenRotFrameTime, outFrozenRotFrameTimeUsed, inFrozenRotAnim, inFrozenArmAnim,
                   inFrozenArmFadeTargetAnim, inPos, inRot, inWorn, inFlipH, inAge, inHideClosestArm, inHideAllLimbs,
                   inHeldNotInPlaceYet, inClothing, inClothingContained);
}

AnimationRecord *copyRecord(AnimationRecord *inRecord, char inCountLiveSoundUses)
{
    ...
    memcpy(newRecord->soundAnim, inRecord->soundAnim, sizeof(SoundAnimationRecord) * newRecord->numSounds);

    for (int i = 0; i < newRecord->numSounds; i++)
    {
        newRecord->soundAnim[i] = copyRecord(inRecord->soundAnim[i], inCountLiveSoundUses);
    }
    ...
    memcpy(newRecord->spriteAnim, inRecord->spriteAnim, sizeof(SpriteAnimationRecord) * newRecord->numSprites);

    memcpy(newRecord->slotAnim, inRecord->slotAnim, sizeof(SpriteAnimationRecord) * newRecord->numSlots);
...
SoundAnimationRecord copyRecord(SoundAnimationRecord inRecord, char inCountLiveSoundUses)
{
...
void performLayerSwaps(int inObjectID, SimpleVector<LayerSwapRecord> *inSwapList, int inNewNumSprites)
{
    ...
    for (int i = 0; i < allAnims.size(); i++)
    {
        ...
        SpriteAnimationRecord *tempSpriteAnim = new SpriteAnimationRecord[numTempSprites];
