...
{
    if (webRequest == -1 && SettingsManager::getIntSetting("useLifeTokenServer", 1) && userEmail != NULL &&
        strcmp(userEmail, "") != 0)
    {
        ...
        char *serverURL = SettingsManager::getStringSetting("lifeTokenServerURL",
                                                            "http://onehouronelife.com/lifeTokenServer/server.php");

        char *url = autoSprintf("%s?action=get_token_count&email=%s", serverURL, emailEncoded);
        ...
        if (result != 0)
        {
            ...
            if (result == 1)
            {
                ...
                if (strstr(text, "DENIED") == NULL)
                {
                    ...
                    sscanf(text, "%d\n%d\n%d\n%d\nOK", &numTokens, &secondsPerToken, &cap, &secondsLeft);
...
char *getLifeTokenString()
{
    ...
    return autoSprintf(translate("tokenMessage"), numTokens, cap, translate(firstLifeKey), minutes,
                       translate(minuteKey), cap, translate(secondLifeKey));
}
    ...
    if (numTokens != -1)
    {
        ...
        else if (numTokens == 0)
        {
            ...
            if (!returnMessage)
            {
                ...
                mainFont->drawString(translate("tokenTimeMessage"), inPos, alignRight);

                char *timeString = autoSprintf("%d:%02d:%02d", h, m, s);

                mainFont->drawString(timeString, inPos, alignLeft);
