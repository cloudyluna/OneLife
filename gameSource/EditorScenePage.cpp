...
EditorScenePage::EditorScenePage()
    : mPlayingTime(false), mRecordingFrames(false), mAnimEditorButton(mainFont, 210, 260, "Anim"),
      mSaveNewButton(smallFont, -300, 260, "Save New"), mReplaceButton(smallFont, -500, 260, "Replace"),
      mDeleteButton(smallFont, 500, 260, "Delete"), mSaveTestMapButton(smallFont, -300, 200, "Export Test Map"),
      mLoadTestMapButton(smallFont, -500, 200, "Import Test Map"), mNextSceneButton(smallFont, -420, 260, ">"),
      mPrevSceneButton(smallFont, -580, 260, "<"), mClearSceneButton(smallFont, 350, 260, "Clear"),
      mGroundPicker(&groundPickable, -410, 90), mObjectPicker(&objectPickable, 410, 90),
      ...
      mSceneW(400), mSceneH(90), mShiftX(0), mShiftY(0), mCurX(6), mCurY(3), mZeroX(6), mZeroY(3), mFrameCount(0),
      mLittleDheld(false), mBigDheld(false), mScenesFolder(NULL, "scenes"), mNextFile(NULL)
{
...
void EditorScenePage::floodFill(int inX, int inY, int inOldBiome, int inNewBiome)
{
    ...
    if (inX < mCurX - 6 || inX > mCurX + 6 || inY < mCurY - 4 || inY > mCurY + 4)
    {
        return;
    }
...
void EditorScenePage::actionPerformed(GUIComponent *inTarget)
{
    ...
    else if (inTarget == &mGroundPicker)
    {
        ...
        if (biome >= 0)
        {
            ...
            if (wasRightClick)
            {
                ...
                floodFill(mCurX, mCurY, c->biome, biome);
            }
            else
            {
                // single cell
    ...
    else if (inTarget == &mNextSceneButton)
    {
        ...
        mSceneID += jump;
        while (mSceneID < mNextSceneNumber && !tryLoadScene(mSceneID))
        {
            mSceneID++;
        }
        mReplaceButton.setVisible(true);
    ...
    else if (inTarget == &mPrevSceneButton)
    {
        ...
        mSceneID -= jump;
        while (mSceneID >= 0 && !tryLoadScene(mSceneID))
        {
            mSceneID--;
        }
        mReplaceButton.setVisible(true);
...
void EditorScenePage::drawUnderComponents(doublePair inViewCenter, double inViewSize)
{
    ...
    for (int y = 0; y < mSceneH; y++)
    {
        for (int x = 0; x < mSceneW; x++)
        {
            ...
            if (y > mCurY + 4 || y < mCurY - 4 || x > mCurX + 7 || x < mCurX - 7)
            {
    ...
    for (int y = 0; y < mSceneH; y++)
    {
        for (int x = 0; x < mSceneW; x++)
        {
            ...
            if (y > mCurY + 4 || y < mCurY - 4 || x > mCurX + 7 || x < mCurX - 6)
            {
        ...
        for (int b = 0; b < 5; b++)
        {
            ...
            for (int x = 0; x < mSceneW; x++)
            {
                ...
                if (c->oID > 0)
                {
                    ...
                    drawObjectAnim(c->oID, c->anim, thisFrameTime, 0, c->anim, thisFrameTime, frozenRotFrameTime, &used,
                                   ground, ground, cellPos, 0, false, c->flipH, -1, 0, false, false, c->clothing, NULL,
    ...
    if (c->oID > 0)
    {
        ...
        char *s = autoSprintf("oID=%d  %s", c->oID, getObject(c->oID)->description);
