...
static const char *personAnimNames[NUM_PERSON_ANIM] = {"Ground", "Moving", "Eating", "Doing"};
static const AnimType personAnimTypes[NUM_PERSON_ANIM] = {ground, moving, eating, doing};
...
EditorScenePage::EditorScenePage()
    : mPlayingTime(false), mRecordingFrames(false),
      ...
      mPersonAgeSlider(smallFont, -55, -220, 2, 100, 20, 0, 100, "Age"),
      mCellAnimRadioButtons(smallFont, -200, -250, NUM_CELL_ANIM, cellAnimNames, true, 2),
      mPersonAnimRadioButtons(smallFont, -55, -250, NUM_PERSON_ANIM, personAnimNames, true, 2),
      mCellAnimFreezeSlider(smallFont, -450, -340, 2, 300, 20, -2, 2, "Cell Time"),
      mPersonAnimFreezeSlider(smallFont, 50, -340, 2, 300, 20, -2, 2, "Person Time"),
      mCellSpriteVanishSlider(smallFont, -450, -310, 2, 100, 20, 0, 1, "Use"),
      mCellSpriteVarSlider(smallFont, -250, -310, 2, 100, 20, 0, 1, "Var"),
      mCellXOffsetSlider(smallFont, -450, -230, 2, 175, 20, -MAX_OFFSET, MAX_OFFSET, "X Offset"),
      mCellYOffsetSlider(smallFont, -450, -260, 2, 175, 20, -MAX_OFFSET, MAX_OFFSET, "Y Offset"),
      mPersonXOffsetSlider(smallFont, 200, -230, 2, 175, 20, -MAX_OFFSET, MAX_OFFSET, "X Offset"),
      mPersonYOffsetSlider(smallFont, 200, -260, 2, 175, 20, -MAX_OFFSET, MAX_OFFSET, "Y Offset"),
      mCellMoveDelayField(smallFont, -450, -290, 4, false, "Move Delay Sec", "0123456789."),
      mPersonMoveDelayField(smallFont, 200, -290, 4, false, "Move Delay Sec", "0123456789."),
      mPersonEmotField(smallFont, 360, -290, 7, true, "Emot", "*/ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789,"),
      mCellDestSprite(loadSprite("centerMark.tga")), mPersonDestSprite(loadSprite("internalPaperMark.tga")),
      mFloorSplitSprite(loadSprite("floorSplit.tga", false)), mShowUI(true), mShowWhite(false), mCursorFade(1.0),
      mSceneW(mapSizeX), mSceneH(mapSizeY), mShiftX(-initCenterX), mShiftY(initCenterY), mCurX(initCenterX),
      mCurY(initCenterY), mZeroX(initCenterX), mZeroY(initCenterY), mFrameCount(0), mLittleDheld(false),
      mBigDheld(false), mScenesFolder(NULL, "scenes"), mNextFile(NULL)
{
    ...
    if (mScenesFolder.isDirectory())
    {
        ...
        if (!mNextFile->exists())
        { // It might change the scene order, and will change the count, when it's created.
...
void EditorScenePage::floodFill(int inX, int inY, int inOldBiome, int inNewBiome)
{

    if (inX < 0 || inX >= mSceneW || inY < 0 || inY >= mSceneH)
    {
        return;
    }
    ...
    if (inX < cursorGridX - 10 || inX > cursorGridX + 10 || inY < cursorGridY - 10 || inY > cursorGridY + 10)
    {
        return;
    }

    if (mCells[inY][inX].biome == inOldBiome && mCells[inY][inX].biome != inNewBiome)
    {
...
void EditorScenePage::actionPerformed(GUIComponent *inTarget)
{
        ...
        if (f != NULL)
        {
            ...
            for (int y = 0; y < mSceneH; y++)
            {
                for (int x = 0; x < mSceneW; x++)
                {
                    ...
                    fprintf(f, "%d %d %d %d %d", x - mZeroX, -(y - mZeroY), c->biome, floorID, oID);

                    for (int i = 0; i < c->contained.size(); i++)
                    {
                        fprintf(f, ",%d", c->contained.getElementDirect(i));
                        SimpleVector<int> *subVec = c->subContained.getElement(i);
    ...
    else if (inTarget == &mNextSceneButton)
    {
        ...
        if (mSceneID >= mNextSceneNumber)
            mSceneID = mNextSceneNumber - 1;
    ...
    {
        c->anim = cellAnimTypes[mCellAnimRadioButtons.getSelectedItem()];
    }
    else if (inTarget == &mPersonAnimRadioButtons)
    {
        p->anim = personAnimTypes[mPersonAnimRadioButtons.getSelectedItem()];
    }
    ...
    else if (inTarget == &mPersonEmotField)
    {
        ...
        if (strstr(text, "/") == text || strstr(text, "*") == text)
        {
            // starts with / or *
...
void EditorScenePage::checkVisible()
{
    ...
    mCellMoveDelayField.setVisible(c->destCellXOffset != 0 || c->destCellYOffset != 0);
    mPersonMoveDelayField.setVisible(p->destCellXOffset != 0 || p->destCellYOffset != 0);
    ...
    if (p->oID > 0)
    {
        ...
        if (p->currentEmot != NULL)
        {
            ...
            {
                char *s = autoSprintf(",%s", p->extraEmot.getElementDirect(i)->triggerWord);
...
static void stepMovingCell(SceneCell *inC)
{
    ...
    double totalCellDist = sqrt(c->destCellXOffset * c->destCellXOffset + c->destCellYOffset * c->destCellYOffset);
...
static void restartCell(SceneCell *inC)
{
    ...
    if (c->oID > 0 && (c->destCellXOffset != 0 || c->destCellYOffset != 0))
    {
...
static void drawOutlineString(const char *inString, doublePair inPos, TextAlignment inAlign)
{

    setDrawColor(0, 0, 0, 1);

    doublePair offsets[8] = {{-2, 0}, {2, 0}, {0, -2}, {0, 2}, {-2, -2}, {-2, 2}, {2, -2}, {2, 2}};
...
void EditorScenePage::drawUnderComponents(doublePair inViewCenter, double inViewSize)
{
    ...
    for (int y = 0; y < mSceneH; y++)
    {
        for (int x = 0; x < mSceneW; x++)
        {
            ...
            if (y > mCurY + yLimit || y < mCurY - yLimit || x > mCurX + xLimit || x < mCurX - xLimit)
            {
    ...
    for (int y = 0; y < mSceneH; y++)
    {
        for (int x = 0; x < mSceneW; x++)
        {
            ...
            if (f->oID <= 0)
            {
                ...
                if (cellOID > 0 && getObject(cellOID)->floorHugging)
                {
                    ...
                    if (x < mSceneW - 1 && mFloorCells[y][x + 1].oID > 0 &&
                        !getObject(mFloorCells[y][x + 1].oID)->noCover)
                    {
                        // floor to our right
            ...
            for (int p = 0; p < 3; p++)
            {
                ...
                drawObjectAnim(oID, 2, ground, frameTime, 0, ground, frameTime, frameTime, &used, ground, ground, pos,
                               0, false, false, -1, false, false, false, getEmptyClothingSet(), NULL);
                if (p > 0)
                {
                    stopStencil();
                }
    ...
    for (int y = 0; y < mSceneH; y++)
    {
        ...
        if (y > mCurY + yLimit || y < mCurY - yLimit)
        {
        ...
        for (int b = 0; b < 5; b++)
        {
            ...
            if (b == 1)
            {
                ...
                for (int x = 0; x < mSceneW; x++)
                {
                    ...
                    if (x > mCurX + xLimit || x < mCurX - xLimit)
                    {
                    ...
                    if (p->oID > 0)
                    {
                        ...
                        if (p->age >= 60 && p->graveID != -1)
                        {

                            setDrawnObjectScale(scale);
                            holdingPos = drawObjectAnim(p->graveID, 2, ground, thisFrameTime, 0, ground, thisFrameTime,
                                                        frozenRotFrameTime, &used, frozenArmAnimType, frozenArmAnimType,
                                                        personPos, 0, false, p->flipH, p->age, hideClosestArm,
                                                        hideAllLimbs, false, p->clothing, NULL);
                        }
                        else
                        {
                            ...
                            if (splitHeld)
                            {
                                ...
                                // held later
                                prepareToSkipSprites(getObject(p->oID), false, true);
                                clothingToDraw = getEmptyClothingSet();
                            }

                            setDrawnObjectScale(scale);
                            holdingPos = drawObjectAnim(p->oID, 2, p->anim, thisFrameTime, 0, p->anim, thisFrameTime,
                                                        frozenRotFrameTime, &used, frozenArmAnimType, frozenArmAnimType,
                                                        personPos, 0, false, p->flipH, p->age, hideClosestArm,
                                                        hideAllLimbs, false, clothingToDraw, NULL);
                            if (splitHeld)
                            {
                        ...
                        if (heldObject != NULL)
                        {
                            ...
                            computeHeldDrawPos(holdingPos, personPos, heldObject, p->flipH, &holdPos, &holdRot);
                            ...
                            SimpleVector<int> *subContained = p->subContained.getElementArray();

                            setAnimationEmotion(p->heldEmotion);
                            addExtraAnimationEmotions(&(p->heldExtraEmotion));

                            if (splitHeld)
                            {
                                setDrawnObjectScale(scale);
                                // draw behind part
                                prepareToSkipSprites(getObject(p->heldID), true);
                                drawObjectAnim(p->heldID, heldAnimType, thisFrameTime, 0, heldFadeTargetType,
                                               thisFrameTime, heldFrozenRotFrameTime, &used, moving, moving, holdPos,
                                               holdRot, false, p->flipH, heldAge, false, false, false, heldClothing,
                                               NULL, p->contained.size(), contained, subContained);
                                restoreSkipDrawing(getObject(p->heldID));

                                // now draw player in between
                                drawObjectAnim(p->oID, 2, p->anim, thisFrameTime, 0, p->anim, thisFrameTime,
                                               frozenRotFrameTime, &used, frozenArmAnimType, frozenArmAnimType,
                                               personPos, 0, false, p->flipH, p->age, hideClosestArm, hideAllLimbs,
                                               false, p->clothing, NULL);

                                // draw front part of held
                                prepareToSkipSprites(getObject(p->heldID), false);
                            }

                            drawObjectAnim(p->heldID, heldAnimType, thisFrameTime, 0, heldFadeTargetType, thisFrameTime,
                                           heldFrozenRotFrameTime, &used, moving, moving, holdPos, holdRot, false,
                                           p->flipH, heldAge, false, false, false, heldClothing, NULL,
                                           p->contained.size(), contained, subContained);
            ...
            for (int x = 0; x < mSceneW; x++)
            {
                ...
                if (c->oID > 0)
                {
                    ...
                    if ((b == 0 && !(o->drawBehindPlayer || o->anySpritesBehindPlayer)) ||
                        (b != 0 && o->drawBehindPlayer))
                    {
                        continue;
                    }
                    if ((b == 3 && !(o->wallLayer && !o->frontWall)) ||
                    ...
                    if (b == 4 && !(o->wallLayer && o->frontWall))
                    {
                        continue;
                    }

                    if ((b == 1 && !o->permanent) || (b == 2 && o->permanent))
                    {
                        continue;
                    }
                    ...
                    SimpleVector<int> *subContained = c->subContained.getElementArray();
                    ...
                    if (cellO->numUses > 1)
                    {
                        setupSpriteUseVis(cellO, c->numUsesRemaining, cellO->spriteSkipDrawing);
                    }
                    ...
                    drawObject(getObject(c->oID), cellPos, 0, false, c->flipH, -1, 0, false, false, c->clothing,
                               c->contained.size(), contained, subContained);
                    ...
                    if (cellO->numUses > 1)
                    {
                        setupSpriteUseVis(cellO, cellO->numUses, cellO->spriteSkipDrawing);
                    }
                }
            }
        }
    ...
    if (c->oID > 0 && getObject(c->oID)->numSlots > c->contained.size())
    {
    ...
    double w = smallFont->measureString(posStringX) + smallFont->measureString(posStringY);
    ...
    if (pickedOID > 0)
    {
        ...
        char *s = autoSprintf("Picked: #%d  %s", pickedOID, getObject(pickedOID)->description);
    ...
    if (c->oID > 0)
    {
        ...
        char *s = autoSprintf("Drawn: #%d  %s", c->oID, getObject(c->oID)->description);
    ...
    {
        if (mapChanged)
        { // Output the current scene filename to the screen.
            File *f = getSceneFile(mSceneID);
...
void EditorScenePage::keyDown(unsigned char inASCII)
{
    ...
    if (tolower(inASCII) == 'c')
    {
        if (mShowUI)
        {
            ...
            {
                if (cursorGridY + copyAreaSize <= mSceneH && cursorGridX + copyAreaSize <= mSceneW &&
                    cursorGridY >= 0 && cursorGridX >= 0)
                {
    ...
    if (tolower(inASCII) == 'v')
    {
        if (mShowUI)
        {
            ...
            if (!isShiftKeyDown())
            {
                if (mCopyBuffer.oID > 0 && getObject(mCopyBuffer.oID)->person)
                {
                    // *p = mCopyBuffer;
                }
                else
    ...
    else if (inASCII == 'v')
    {
        ...
        else if (mCopyBuffer.oID > 0 && getObject(mCopyBuffer.oID)->person)
        {
            *p = mCopyBuffer;
        }
        else
    ...
    else if (inASCII == 'i')
    {
        ...
        if (mCopyBuffer.oID > 0 && c->oID > 0 && getObject(c->oID)->numSlots > c->contained.size())
        {
            // room
...
{
    if (mUndoButton.isMouseOver())
        return true;
    if (mRedoButton.isMouseOver())
        return true;
    if (mSaveNewButton.isMouseOver())
        return true;
    if (mReplaceButton.isMouseOver())
        return true;
    if (mConfirmReplaceButton.isMouseOver())
        return true;
    if (mDeleteButton.isMouseOver())
        return true;
    if (mConfirmDeleteButton.isMouseOver())
        return true;
    if (mSaveTestMapButton.isMouseOver())
        return true;
    if (mNextSceneButton.isMouseOver())
        return true;
    if (mPrevSceneButton.isMouseOver())
        return true;

    doublePair gPos = mGroundPicker.getPosition();
    if (abs(inX - gPos.x) < 100 && abs(inY - gPos.y) < 350)
        return true;
    doublePair oPos = mObjectPicker.getPosition();
    if (abs(inX - oPos.x) < 100 && abs(inY - oPos.y) < 350)
        return true;
...
void EditorScenePage::pointerDown(float inX, float inY)
{
    ...
    if (mShowUI && hoverAnyUI(inX, inY))
        return;

    int x = round(inX / (CELL_D * scale)) + mCurX;
    int y = round(-inY / (CELL_D * scale)) + mCurY;

    if ((!mShowUI && !isLastMouseButtonRight()) || (mShowUI && !isLastMouseButtonRight() && isCommandKeyDown()))
    {
        mCurX = x;
        mCurY = y;
...
File *EditorScenePage::getSceneFile(int inSceneID)
{
    ...
    if (inSceneID >= numFiles)
    {
        ...
        do
        { // If we're not in the bounds of that list, we want a NEW file.
...
{
    inLines->push_back(autoSprintf("hat=%d", inSet.hat == NULL ? 0 : inSet.hat->id));
    inLines->push_back(autoSprintf("tunic=%d", inSet.tunic == NULL ? 0 : inSet.tunic->id));

    inLines->push_back(autoSprintf("frontShoe=%d", inSet.frontShoe == NULL ? 0 : inSet.frontShoe->id));

    inLines->push_back(autoSprintf("backShoe=%d", inSet.backShoe == NULL ? 0 : inSet.backShoe->id));

    inLines->push_back(autoSprintf("bottom=%d", inSet.bottom == NULL ? 0 : inSet.bottom->id));

    inLines->push_back(autoSprintf("backpack=%d", inSet.backpack == NULL ? 0 : inSet.backpack->id));
}
...
void scanClothingLine(char *inLine, ObjectRecord **inSpot, const char *inFormat)
{
    int id = -1;
...
int scanCell(char **inLines, int inNextLine, SceneCell *inCell)
{
    ...
    sscanf(inLines[next], "numUsesRemaining=%d", &(inCell->numUsesRemaining));
    next++;
...
char EditorScenePage::tryLoadScene(int inSceneID)
{
    ...
    if (f->exists() && !f->isDirectory())
    {
        ...
        if (fileText != NULL)
        {
            ...
            if (numLines > 1)
            { // One line files obviously aren't real.
                ...
                if (next < numLines)
                {                                    // if we ever look at lines[next] after running out of lines
                ...
                while (numRead == 2)
                {
                    ...
                    next = scanCell(lines, next,
                                    c); // TODO: Keep scanCell from segfaulting when the file was interrupted midstream.
...
void EditorScenePage::checkNextPrevVisible()
{
    ...
    if (mSceneID == -1)
    { // We're on an unsaved scene. We can always look for the latest from here.
        mNextSceneButton.setVisible(false);

        mPrevSceneButton.setVisible(num >= 1);
    }
    else
    { // Bounds are from 0 to one less than the number of files in the directory.
        if (mSceneID < num - 1)
        {
