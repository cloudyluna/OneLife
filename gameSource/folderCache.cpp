...
FolderCache initFolderCache(const char *inFolderName, char *outRebuildingCache,
                            char (*inInclusionTest)(char *inFileName), char inForceRebuild)
{
    *outRebuildingCache = false;
    ...
    if (cacheFile->exists())
    {
        ...
        int rawLength;
        unsigned char *rawCacheContents = cacheFile->readFileContents(&rawLength);
        ...
        if (rawSize > 0 && compSize > 0)
        {
            ...
            if (rawData != NULL)
            {
                ...
                for (int i = 0; i < c.numFiles; i++)
                {
                    ...
                    c.fileRecords[i].fileName = stringDuplicate(nextScanPointer);

                    nextScanPointer = &(spacePos[1]);

                    c.fileRecords[i].dataBlockOffset = scanIntAndSkip(&nextScanPointer);

                    c.fileRecords[i].length = scanIntAndSkip(&nextScanPointer);
                }

                char *dataBlockStart = strstr(nextScanPointer, "#");
    ...
    if (cacheGood)
    {
        ...
        int numChildFiles;
        File **childFiles = folderDir->getChildFilesSorted(&numChildFiles);
        ...
        for (int i = 0; i < numChildFiles; i++)
        {
            ...
            if (!childFiles[i]->isDirectory() &&
                ...
                inInclusionTest(fileName) && strcmp(fileName, "cache.fcz") != 0)
            {
                ...
                if (j < c.numFiles)
                {
                    ...
                    {
                        cacheStale = true;
                        printf("Cache file %s does not match folder file %s\n", cacheFileNames[j], fileName);
                        delete[] fileName;
                        break;
                    }
                }
                ...
                {
                    cacheStale = true;
                    printf("Folder file %s not present in cache.\n", fileName);
                    delete[] fileName;
                    break;
                }
                j++;
    ...
    if (!cacheGood)
    {
        ...
        *outRebuildingCache = true;
        ...
        File **childFiles = folderDir->getChildFiles(&numChildFiles);
...
char *getFileName(FolderCache inCache, int inFileNumber)
{
    ...
    else
    {
        ...
        inCache.fileRecords[inFileNumber].fileName = inCache.fileRecords[inFileNumber].file->getFileName();
...
char *getFileContents(FolderCache inCache, int inFileNumber)
{
    if (inCache.dataBlock != NULL)
    {
        ...
        char *fileContents = new char[inCache.fileRecords[inFileNumber].length + 1];

        memcpy(fileContents, &(inCache.dataBlock[inCache.fileRecords[inFileNumber].dataBlockOffset]),
    ...
    {
        char *fileContents = inCache.fileRecords[inFileNumber].file->readFileContents();
        ...
        if (inCache.fileRecords[inFileNumber].dataBlockOffset == -1)
        {
            ...
            inCache.fileRecords[inFileNumber].dataBlockOffset = inCache.newDataBlock->size();
...
{
    if (inCache.dataBlock == NULL && inCache.folderDir != NULL && inCache.folderDir->exists() &&
        inCache.folderDir->isDirectory())
    {
        ...
        {
            if (inCache.fileRecords[i].fileName != NULL && inCache.fileRecords[i].dataBlockOffset != -1)
            {
        ...
        int compSize;
        unsigned char *compData = zipCompress((unsigned char *)data, rawLength, &compSize);
