...
TextField::TextField(Font *inDisplayFont, double inX, double inY, int inCharsWide, char inForceCaps,
    ...
    : PageComponent(inX, inY), mActive(true), mContentsHidden(false),
      ...
      mAllowedChars(NULL), mForbiddenChars(NULL), mHover(false), mFocused(false), mText(new char[1]), mTextLen(0),
      mCursorPosition(0), mIgnoreArrowKeys(false), mIgnoreMouse(false), mDrawnText(NULL), mCursorDrawPosition(0),
      mHoldDeleteSteps(-1), mFirstDeleteRepeatDone(false), mLabelOnRight(false), mLabelOnTop(false),
      mSelectionStart(-1), mSelectionEnd(-1), mShiftPlusArrowsCanSelect(false), mCursorFlashSteps(0),
      mUsePasteShortcut(false), mDrawLabelWithShadow(inDrawLabelWithShadow)
{
...
void TextField::setDeleteRepeatDelays(int inFirstDelaySteps, int inNextDelaySteps)
{
...
void TextField::setFloat(float inF, int inDigitsAfterDecimal, char inTrimZeros)
{
