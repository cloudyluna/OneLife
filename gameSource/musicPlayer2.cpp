    ...
    if (oggData == NULL && asyncLoadHandle != -1)
    {
        ...
        if (checkAsyncFileReadDone(asyncLoadHandle))
        {
            ...
            if (oggData != NULL)
            {
                ...
                if (musicOGG != NULL)
                {
                    musicLengthSeconds = (double)getOGGTotalSamples(musicOGG) / (double)getSampleRate();
...
void getSoundSamples(Uint8 *inBuffer, int inLengthToFillInBytes)
{
    ...
    double sampleComputedAge = (samplesSeenSinceAgeSet / (double)getSampleRate()) * ageRate + age;
    ...
    if (!musicOGGPlaying)
    {
        ...
        else if (!soundEffectsFaded && startAge - fadeSeconds * ageRate < sampleComputedAge)
        {
