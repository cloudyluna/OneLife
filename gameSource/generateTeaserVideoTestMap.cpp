...
void runSteps(const char *inBankName, int inNumExpectedSteps, float (*inStepFunction)())
{
...
static void usage()
{
    printf("\nUsage:\n\n generateTeaserVideoTestMap "
           "outMap.txt [baseMap.txt]\n\n");
...
char *getLatestString(int inID)
{
    ...
    if (inID != 0)
    {
        ...
        latestString = autoSprintf("(latest: %s)", des);
...
int main(int inNumArgs, char **inArgs)
{
    ...
    for (int i = 0; i < numObjects; i++)
    {
        ...
        if (o->mapChance == 0 && !o->person)
        {
            ...
            TransRecord **trans = searchProduces(o->id, 0, 1000, &numTrans, &numRemain);
            ...
            if (trans != NULL)
            {
                ...
                {
                    if (trans[t]->actor == -1 && trans[t]->autoDecaySeconds != 0)
                    {
                        humanMade = false;
                        break;
                    }
            ...
            if (humanMade && o->permanent)
            {
                ...
                TransRecord **trans = searchUses(o->id, 0, 1000, &numTrans, &numRemain);

                if (trans != NULL)
                {
                    for (int t = 0; t < numTrans; t++)
                    {
                        if (trans[t]->actor == -1 && trans[t]->autoDecaySeconds != 0 && trans[t]->newTarget > 0)
                        {

                            if (getObjectDepth(trans[t]->newTarget) == 0)
                            {
        ...
        else if (o->mapChance > 0 && getObjectDepth(o->id) == 0)
        {
            if (strstr(o->description, "Tree") != NULL)
            {
                ...
                if (strstr(o->description, "Branch") != NULL)
                {
                    ...
                    if (t != NULL && t->newTarget != o->id && t->newTarget > 0)
                    {
    ...
    while (queue.checkMinPriority() > 0)
    {
        ...
        while (queue.checkMinPriority() == lastP && queue.checkMinPriority() > 0)
        {
        ...
        while (strataQueue.checkMinPriority() > 0)
        {
            ...
            while (exploreList.size() > 0)
            {
                ...
                if ((r->actor == 0 || r->actor == -1 || r->actor == -2 || objectIncluded[r->actor]) &&
                    (r->target == 0 || r->target == -1 || objectIncluded[r->target]))
                {

                    if (r->newActor > 0 && !objectIncluded[r->newActor])
                    {
                        ...
                        if (objectAllowed[r->newActor])
                        {
                            ...
                            printf("  Adding %s\n", getObject(r->newActor)->description);
                        }
                        // explore further into tree
                        exploreList.push_back_other(getAllUses(r->newActor));
                    }
                    if (r->newTarget > 0 && !objectIncluded[r->newTarget])
                    {
                        ...
                        exploreList.push_back_other(getAllUses(r->newTarget));
                    }
                }
            }
        }
    ...
    {
        printf("%d. %s\n", i, getObject(orderedObjects.getElementDirect(i))->description);
    }
    ...
    for (int y = -5; y <= 5; y++)
    {
        ...
        for (int x = -20; x <= xMax + 20; x++)
        {
            ...
            */
            ...
            else if (rowBiome == 0 && y <= -5 && y != -11 && x != -10 && (x >= 0 || x < -2))
            {
                // grassy
            ...
            if (addTree)
            {
                id = treeList.getElementDirect(randSource.getRandomBoundedInt(0, treeList.size() - 1));
            }
            else if (y == -12 && x == -10)
            {
                id = berryBush;
