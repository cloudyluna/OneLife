...
PageComponent::PageComponent(double inX, double inY)
    : mX(inX), mY(inY), mParent(NULL), mVisible(true), mIgnoreEvents(false), mMouseEventHog(NULL), mCursorTip(NULL)
{
}
...
void PageComponent::base_draw(doublePair inViewCenter, double inViewSize)
{
    ...
    if (mCursorTip != NULL && isMouseOver())
    {
        ...
        drawRect(pos.x - padding, pos.y - 8 / 2 - padding, pos.x + textWidth + padding, pos.y + 8 / 2 + padding);
...
void PageComponent::base_pointerMove(float inX, float inY)
{
    ...
    if (mMouseEventHog != NULL)
    {
        ...
        if (mMouseEventHog->isVisible())
        { //&& mMouseEventHog->isActive() ) {
...
void PageComponent::setWaiting(char inWaiting, char inWarningOnly)
{
