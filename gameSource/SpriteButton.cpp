...
SpriteButton::SpriteButton(SpriteHandle inSprite, int inWide, int inHigh, double inX, double inY, double inDrawScale)
    : Button(inX, inY, inWide * 2 * inDrawScale, inHigh * 2 * inDrawScale, inDrawScale), mShouldDestroySprite(false),
      mSprite(inSprite), mOverrideHighlightColor(false), mDrawScale(inDrawScale)
{
}
...
SpriteButton::SpriteButton(const char *inTGAFileName, double inX, double inY,
    ...
    : // placeholder until we load file below
      Button(inX, inY, 1, 1, inDrawScale), mShouldDestroySprite(false), mSprite(NULL), mOverrideHighlightColor(false),
      mDrawScale(inDrawScale)
{
    ...
    {
        AppLog::errorF("Failed to read file for SpriteButton: %s", inTGAFileName);
    }
}
