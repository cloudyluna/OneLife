...
void newbieTips::init(LivingLifePage *inLivingLifePage, SimpleVector<LiveObject> *inGameObjects, int inmMapD,
                      int inPathFindingD)
{
...
void newbieTips::livingLifeStep(int mTutorialNumber, int mLiveTutorialTriggerNumber)
{
    ...
    {
        if (mLiveTutorialTriggerNumber == 2)
        { // Berry bush
            startTipsArrow({14, 0}, true);
        }
        else if (mLiveTutorialTriggerNumber == 3)
        { // Food meter
            startTipsArrow({-viewWidth / minitech::guiScale + 70, 40}, false);
        }
        else if (mLiveTutorialTriggerNumber == 5001)
        { // Crafting guide - make/use mode
        ...
        else if (mLiveTutorialTriggerNumber == 7)
        { // Crafting guide - how to make sharp stone
            if (!minitech::minitechMinimized)
            {
        ...
        else if (mLiveTutorialTriggerNumber == 10 || mLiveTutorialTriggerNumber == 1010)
        { // Yum slip
            if (yumSlipShowing)
            {
        ...
        else if (mLiveTutorialTriggerNumber == 11)
        { // Tule reeds
            if (getObjId(118, 1) == 121)
            { // Tule Reeds
                startTipsArrow({118, 1}, true);
            }
            else if (getObjId(120, 1) == 121)
            { // Tule Reeds
                startTipsArrow({120, 1}, true);
            }
        }
        else if (mLiveTutorialTriggerNumber == 1101)
        { // Vertical door
            startTipsArrow({125, 0}, true);
        }
        else if (mLiveTutorialTriggerNumber == 13)
        { // Temp meter
            startTipsArrow({-145, 25}, false);
        }
        else if (mLiveTutorialTriggerNumber == 15 || mLiveTutorialTriggerNumber == 1501)
        { // Box of clothes
            startTipsArrow({162, 0}, true);
        }
        else if (mLiveTutorialTriggerNumber == 1901)
        { // Crafting guide - search
            if (!minitech::minitechMinimized)
            {
        ...
        else if (mLiveTutorialTriggerNumber == 1902)
        { // Crafting guide - how to make hatchet
            if (!minitech::minitechMinimized)
            {
    ...
    else if (mTutorialNumber == 0 && newbieTipsEnabled)
    {
        ...
        if (ourLiveObject->age > 3 && hungerSlipShowing >= 1)
        {
            if (tryToStartSession(1))
            {
                ...
                messageToDisplay = "YOU ARE GETTING HUNGRY, WALK AROUND AND LOOK FOR FOOD !##REMEMBER TO CHECK FOR YUM "
        ...
        if (ourLiveObject->age < 3 && ourLiveObject->heldByAdultID == -1)
        {
            ...
            if (mother != NULL)
            {
                ...
                if (dist > 3.5)
                {
                    ...
                    {
                        shouldDisplayMessage = true;
                        messageToDisplay = "STAY CLOSE TO MOM, SAY 'F' WHEN YOU'RE HUNGRY.##IF YOU WANT TO BE BORN "
                                           "ELSEWHERE, USE THE   /DIE   COMMAND.";
                    }
                }
                else
                {
        ...
        {
            shouldDisplayMessage = true;
            messageToDisplay = "YOU HAVE A BABY! CLICK TO HOLD AND FEED THEM.##WHEN YOU ARE HOLDING THEM, SAY   YOU "
                               "ARE [NAME]    TO NAME THEM.";
            kidLessonDone = true;
        }
        else
        {
        ...
        if (amFemale)
        {
            ...
            if (count > lastNeverHeldKidsDeathCount && count > 1)
            {
                if (tryToStartSession(4))
                {
                    shouldDisplayMessage = true;
                    messageToDisplay =
                        "PLEASE DO NOT ABANDON YOUR BABIES.##IF YOU DON'T WANT BABIES, SAY   NO BB   TO BE INFERTILE.";
        ...
        if (ourLiveObject->holdingID > 0)
        {
            ...
            bool holdingWeapon = heldObjectParent == 560 ||   // Knife
                ...
                && justUsedOnObjectID > 0)
            {
                ...
                repeatedlyKillingFarmAnimal = targetParent == 4757 || // Hen
            ...
            if ((repeatedlyKillingFarmAnimal || (justTriedToKill && holdingWeapon)) && tryToStartSession(5))
            {
                shouldDisplayMessage = true;
                messageToDisplay =
                    "PLEASE BE REMINDED THAT THE SERVER IS MODERATED.##GRIEFING MAY GET YOU BANNED. PLEASE PLAY NICE.";
            }
            else
            {
                tryToEndSession(5);
        ...
        if (game_getCurrentTime() - lastSeedLessonTime > 120 && dist > 500 && tryToStartSession(6))
        {
            shouldDisplayMessage = true;
            messageToDisplay =
                "IF YOU WANT TO START A CAMP IN THE WILD,##YOU CAN ENTER YOUR OWN  'SPAWN CODE'  IN THE LOGIN PAGE.";
        ...
        {
            shouldDisplayMessage = true;
            messageToDisplay = "IF YOU FIND SETTLEMENTS SEEMINGLY ABANDONED##PLEASE DO NOT TAKE ANYTHING, OTHERS MAY "
...
bool newbieTips::isEasyFood(int id)
{
    ...
    if (food_id == 837)
        return false; // We dont eat mushroom...
    ObjectRecord *food = getObject(food_id, true);
    if (food == NULL)
        return false;
    if (food->foodValue > 0 || food->bonusValue > 0)
        return true;
    return false;
}
...
LiveObject *newbieTips::getMother()
{
    ...
    for (int i = 0; i < players->size(); i++)
    {
        ...
        if (player == NULL)
            continue;
        if (player->id == ourLin.getElementDirect(0))
            return player;
    }
    return NULL;
}
...
bool newbieTips::haveKids()
{
    ...
    for (int i = 0; i < players->size(); i++)
    {
        ...
        if (player == NULL)
            continue;
        if (player->lineage.getElementDirect(0) == me->id)
            return true;
    }
    return false;
}
