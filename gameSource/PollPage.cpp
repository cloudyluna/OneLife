...
PollPage::PollPage(const char *inReviewServerURL)
    : mReviewServerURL(stringDuplicate(inReviewServerURL)), mSubmitButton(mainFont, 526, -300, translate("submit")),
      mPollID(0), mQuestion(NULL), mAnswerButtons(NULL), mWebRequestIsSubmit(false), mWebRequest(-1)
{
...
void PollPage::actionPerformed(GUIComponent *inTarget)
{
    ...
    if (inTarget == &mSubmitButton)
    {
        ...
        char *stringToHash = autoSprintf("%dv%d", mPollID, pick);
        ...
        char *url = autoSprintf("%s?action=poll_vote"
                                ...
                                "&vote_number=%d"
                                "&hash_value=%s",
                                mReviewServerURL, userEmail, mPollID, pick, hash);
...
void PollPage::draw(doublePair inViewCenter, double inViewSize)
{
...
void PollPage::makeActive(char inFresh)
{
    ...
    if (strcmp(userEmail, "") == 0 || strcmp(accountKey, "") == 0)
    {
        setSignal("done");
        return;
    }
...
int PollPage::measureSplitQuestion()
{
    ...
    while (charsLeft.size() > 0)
    {
        ...
        while (charsLeft.size() > 0 && (workingLen < 900 || charsLeft.getElementDirect(0) != ' '))
        {
...
void PollPage::step()
{
    ...
    if (!mWebRequestIsSubmit)
    {
        ...
        mAnswerButtons =
            new RadioButtonSet(mainFont, -400, 100 - 30 * questionLines, answers.size(), (const char **)array, true, 4);
        delete[] array;
