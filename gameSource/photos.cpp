...
int getNextPhotoSequenceNumber()
{
    ...
    else if (sequenceNumberWebRequest == -1)
    {
        ...
        char *getURL = autoSprintf("%s?action=get_sequence_number&email=%s", url, encodedEmail);
...
// about this.
void takePhoto(doublePair inCameraLocation, int inCameraFacing, int inSequenceNumber, char *inServerSig, int inAuthorID,
               char *inAuthorName, SimpleVector<int> *inSubjectIDs, SimpleVector<char *> *inSubjectNames)
{
    ...
    Image *im = getScreenRegionRaw(rectStartX, rectStartY, screenScale * 400, screenScale * 400);
    ...
    if (screenScale != 1.0)
    {
        ...
        int result = stbir_resize_uint8(bytes, im->getWidth(), im->getHeight(), 0, outBytes, 400, 400, 0, 4);
        if (result == 1)
        {
            delete im;
    ...
    {
        gray[p] = r[p] * rWeight + g[p] * gWeight + b[p] * bWeight;
    }
    ...
    */
    ...
    int result = stbi_write_jpg_to_func(jpegWriteFunc, NULL, w, h, 1, data, 90);
    ...
    else
    {
        ...
        */
        ...
        jpegBytes.appendArray((unsigned char *)keyHash, strlen(keyHash));
        ...
        char *jpegBase64 = base64Encode(jpegData, jpegBytes.size(), false);
        ...
        char *postBody = autoSprintf("action=submit_photo"
                                     "&email=%s"
                                     "&sequence_number=%d"
                                     "&hash_value=%s"
                                     "&server_sig=%s"
                                     ...
                                     "&jpg_base64=%s",
                                     encodedEmail, inSequenceNumber, keyHash, serverSig, serverIP, inAuthorID,
                                     subjectIDs, inAuthorName, subjectNames, jpegURL);
        delete[] encodedEmail;
        delete[] jpegURL;
        delete[] keyHash;
