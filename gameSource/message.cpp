...
void drawMessage(const char *inTranslationKey, doublePair inCenter, char inRed, double inFade, bool oldFont)
{
    ...
    if (strstr(inMessage, "##") != NULL)
    {
        ...
        for (int i = 0; i < numSubMessages; i++)
        {
            ...
            else
            {
                if (oldFont)
                {
                    oldMainFont->drawString(subMessages[i], thisMessagePos, messageAlign);
                }
                else
                {
                    mainFont->drawString(subMessages[i], thisMessagePos, messageAlign);
                }
            }
