...
static double *zeroPad(double *inSource, int inSourceLength, int inNewLength)
{
...
static void fftConvolve(int inWindowSize, double *inA, int inLengthA, double **inBPaddedFFTWindows, int inNumBWindows,
                        int inLengthB, double *inDest)
{
    int windowsA = lrint(ceil(inLengthA / (double)inWindowSize));

    double *paddedA = zeroPad(inA, inLengthA, windowsA * inWindowSize);

    double *paddedDest = zeroPad(inDest, inLengthA + inLengthB, windowsA * inWindowSize + inNumBWindows * inWindowSize);
    ...
    for (int a = 0; a < windowsA; a++)
    {
        ...
        double *paddedAWindow = zeroPad(&(paddedA[offsetA]), inWindowSize, inWindowSize * 2);
...
MultiConvolution startMultiConvolution(double *inB, int inLengthB)
{
    ...
    double *paddedB = zeroPad(inB, inLengthB, m.savedNumWindowsB * m.savedWindowSize);
    ...
    for (int i = 0; i < m.savedNumWindowsB; i++)
    {
        ...
        double *paddedBWindow = zeroPad(&(paddedB[offsetB]), m.savedWindowSize, m.savedWindowSize * 2);
...
void multiConvolve(MultiConvolution inMulti, double *inA, int inLengthA, double *inDest)
{
    // double start = Time::getCurrentTime();

    fftConvolve(inMulti.savedWindowSize, inA, inLengthA, inMulti.savedFFTBufferB, inMulti.savedNumWindowsB,
                inMulti.savedNumSamplesB, inDest);

    // printf( "Convolution of %dx%d took %.3f seconds\n",
    //         inLengthA, savedNumSamplesB, Time::getCurrentTime() - start );
}

static void fftConvolve(int inWindowSize, double *inA, int inLengthA, double *inB, int inLengthB, double *inDest)
{
...
void convolve(double *inA, int inLengthA, double *inB, int inLengthB, double *inDest)
{
