...
{
    blurRadius = SettingsManager::getIntSetting("groundTileEdgeBlurRadius", 12);
...
float initGroundSpritesStep()
{
    ...
    if (nextStep < allBiomes.size())
    {
        ...
        if (rawImage != NULL)
        {
            ...
            {
                printf("Ground texture %s with w=%d and h=%d does not "
            ...
            {
                printf("Ground texture %s has %d channels instead of 4", fileName, rawImage->mNumChannels);
            }
            else
            {
                ...
                for (int ty = 0; ty < tH; ty++)
                {
                    ...
                    for (int tx = 0; tx < tW; tx++)
                    {
                        ...
                        char *cacheFileName =
                            autoSprintf("groundTileCache/biome_%d_x%d_y%d.tga", cacheFileNumber, tx, ty);

                        groundSprites[b]->tiles[ty][tx] = loadSpriteBase(cacheFileName, false);

                        char *squareCacheFileName =
                            autoSprintf("groundTileCache/biome_%d_x%d_y%d_square.tga", cacheFileNumber, tx, ty);

                        groundSprites[b]->squareTiles[ty][tx] = loadSpriteBase(squareCacheFileName, false);
                    ...
                    for (int ty = 0; ty < tH; ty++)
                    {
                        ...
                        for (int tx = 0; tx < tW; tx++)
                        {
                            ...
                            if (groundSprites[b]->squareTiles[ty][tx] == NULL)
                            {
                                ...
                                Image *tileImage = image->getSubImage(tx * CELL_D, ty * CELL_D, CELL_D, CELL_D);

                                writeTGAFile(cacheFileName, tileImage);

                                delete[] cacheFileName;

                                groundSprites[b]->squareTiles[ty][tx] = fillSprite(tileImage, false);
                            ...
                            if (groundSprites[b]->tiles[ty][tx] == NULL)
                            {
                                ...
                                for (int c = 0; c < 3; c++)
                                {
                                    ...
                                    for (int y = imStartY; y < imEndY; y++)
                                    {
                                        ...
                                        for (int x = imStartX; x < imEndX; x++)
                                        {
                                            ...
                                            chanDest[dY * tileD + dX] = chanSrc[wrapY * w + wrapX];
                                            dX++;
                                        }
                                        dY++;
                                    }
                                ...
                                for (int y = 0; y < tileD; y++)
                                {
                                    ...
                                    for (int x = 0; x < tileD; x++)
                                    {
                                        ...
                                        double r = sqrt(deltY * deltY + deltX * deltX);
                                ...
                                int edgeStartA = CELL_D - (CELL_D / 2 + blurRadius);

                                int edgeStartB = CELL_D + (CELL_D / 2 + blurRadius + 1);

                                for (int y = edgeStartA; y <= edgeStartB; y++)
                                {
                                    for (int x = edgeStartA; x <= edgeStartB; x++)
                                    {
                                ...
                                groundSprites[b]->tiles[ty][tx] = fillSprite(&tileImage, false);
                            }
                        }
                    }
                    delete image;
