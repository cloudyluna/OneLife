        ...
        for (int y = 1; y < gridD - 1; y++)
        {
            for (int x = 1; x < gridD - 1; x++)
            {
                ...
                for (int n = 0; n < numNeighbors; n++)
                {
                    ...
                    heatDelta += nWeights[n] * centerLeak * nLeak * (tempHeatGrid[ni] - centerOldHeat);
                }
...
void drawFrameNoUpdate(char inUpdate)
{
    ...
    if (mouseOverSqaure != -1)
    {
        ...
        char *string = autoSprintf("Heat = %.1f", heatGrid[mouseOverSqaure]);
...
void pointerMove(float inX, float inY)
{
    ...
    if (gridX >= 0 && gridX < gridD && gridY >= 0 && gridY < gridD)
    {
