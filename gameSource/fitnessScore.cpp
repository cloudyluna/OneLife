...
{
    fitnessServerURL =
        SettingsManager::getStringSetting("fitnessServerURL", "http://localhost/jcr13/fitnessServer/server.php");
...
void triggerFitnessScoreUpdate()
{
    ...
    if (userEmail != NULL && strcmp(userEmail, "") != 0)
    {
...
static void stepActiveRequest()
{
    ...
    if (result == 1 && game_getCurrentTime() - triggerTime < 1)
    {
    ...
    if (result == 1)
    {
        ...
        if (nextSequenceNumber == -1)
        {
            ...
            char *url = autoSprintf("%s?action=%s"
                                    ...
                                    "&sequence_number=%d"
                                    "&hash_value=%s",
                                    fitnessServerURL, nextAction, encodedEmail, nextSequenceNumber, hash);
            delete[] encodedEmail;
            delete[] hash;
        ...
        if (strcmp(nextAction, "get_client_score_details") == 0)
        {
            ...
            for (int i = 3; i < lines->size() - 1; i++)
            {
                ...
                if (numParts == 7)
                {
                    ...
                    r.relationName = replaceAll(relationWorking, "_", " ", &found);
...
char *drawFitnessScore(doublePair inPos, char inMoreDigits, bool returnMessage)
{
    ...
    if (score != -1)
    {
        ...
        if (rank != 0)
        {
            message = autoSprintf(translate("scoreMessage"), scoreString, rank, rankSuffix);
        }
        else
        {
            // no rank
            message = autoSprintf(translate("scoreMessageNoRank"), scoreString);
        }
...
static void drawFadeRect(doublePair inBottomLeft, doublePair inTopRight, FloatColor inBottomColor,
                         FloatColor inTopColor)
{

    double vert[8] = {inBottomLeft.x, inBottomLeft.y, inBottomLeft.x, inTopRight.y,
                      inTopRight.x,   inTopRight.y,   inTopRight.x,   inBottomLeft.y};

    float vertColor[16] = {inBottomColor.r, inBottomColor.g, inBottomColor.b, inBottomColor.a,
...
static void drawFace(doublePair inPos, int inDisplayID, double inAge, int inDrawIndex)
{
    ...
    if (0 < groundSpritesArraySize && groundSprites[0] != NULL)
    {
    ...
    doublePair frontFootPos = faceO->spritePos[frontFootIndex];

    int bodyIndex = getBodyIndex(faceO, inAge);

    doublePair bodyPos = faceO->spritePos[bodyIndex];

    doublePair framePos = add(add(faceO->spritePos[headIndex], getAgeHeadOffset(inAge, headPos, bodyPos, frontFootPos)),
    ...
    drawObject(faceO, 2, inPos, 0, false, false, inAge, 0, false, false, getEmptyClothingSet());
...
void drawFitnessScoreDetails(doublePair inPos, int inSkip)
{
    ...
    if (score != -1)
    {
        ...
        if (rank > 0)
        {
            ...
            char *leaderboardString = autoSprintf(translate("leaderboardMessage"), rank, rankSuffix, leaderboardName);
        ...
        mainFont->drawString(translate("geneticHistoryTitle"), titlePos, alignLeft);
        ...
        mainFont->drawString(translate("fitnessTitle"), titlePos, alignRight);
        ...
        for (int i = inSkip; i < indLimit; i++)
        {
            ...
            if (yearsAgo == 1)
            {
                diedAgoString = autoSprintf("%s %d %s", translate("died"), yearsAgo, translate("yearAgo"));
            }
            else if (yearsAgo < 1000)
            {
                diedAgoString = autoSprintf("%s %d %s", translate("died"), yearsAgo, translate("yearsAgo"));
            }
            else if (yearsAgo < 10000)
            {
                diedAgoString = autoSprintf("%s %d %s", translate("died"), yearsAgo / 100, translate("centuriesAgo"));
            }
            else
            {
                diedAgoString = autoSprintf("%s %d %s", translate("died"), yearsAgo / 1000, translate("millenniaAgo"));
            }
            ...
            char *ageString = autoSprintf("%s %0.1f", translate("age"), r.age);
        ...
        if (bottomShadingFade)
        {
            ...
            drawFadeRect(bottom, top, bottomColor, topColor);
        }
