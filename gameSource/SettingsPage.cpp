...
SettingsPage::SettingsPage()
    : mBackground("background.tga", 0.75f),
      ...
      mGameplayButton(mainFont, -452.5, 288, "GAMEPLAY"), mControlButton(mainFont, -452.5, 192, "CONTROL"),
      mScreenButton(mainFont, -452.5, 96, "SCREEN"), mSoundButton(mainFont, -452.5, 0, "SOUND"),
#ifdef USE_DISCORD
      ...
      // Gameplay
      mEnableFOVBox(561, 128, 4), mEnableCenterCameraBox(561, 52, 4), mEnableNudeBox(-335, 148, 4),
      mUISizeSlider(mainFont, -335, 148, 4, 200, 30, 0.5, 1.0, ""),

      mUseCustomServerBox(-168, -148, 4), mCustomServerAddressField(mainFont, 306, -150, 14, false, "", NULL,
                                                                    // forbid spaces
                                                                    " "),
      mCustomServerPortField(mainFont, 84, -208, 4, false, "", "0123456789", NULL),
      mCopyButton(mainFont, 381, -216, translate("copy")), mPasteButton(mainFont, 518, -216, translate("paste")),

      // Control
      mEnableKActionsBox(561, 90, 4), mCursorScaleSlider(mainFont, 297, 155, 4, 200, 30, 1.0, 10.0, translate("scale")),

      // Screen
      mRedetectButton(mainFont, 153, 249, translate("redetectButton")), mVsyncBox(0, 208, 4), mFullscreenBox(0, 128, 4),
      mBorderlessBox(0, 168, 4), mTargetFrameRateField(mainFont, -16, 232, 3, false,
                                                       "", // no label
                                                       "0123456789", NULL),
      mTrippingEffectDisabledBox(0, 168, 4),

      // Sound
      mMusicLoudnessSlider(mainFont, 0, 40, 4, 200, 30, 0.0, 1.0, translate("musicLoudness"), true),
      mSoundEffectsLoudnessSlider(mainFont, 0, -48, 4, 200, 30, 0.0, 1.0, translate("soundLoudness"), true)
#ifdef USE_DISCORD
      ,
      mEnableDiscordRichPresence(0, 168, 4), mEnableDiscordRichPresenceStatus(0, 128, 4),
      mEnableDiscordShowAgeInStatus(0, 88, 4), mEnableDiscordRichPresenceDetails(0, 48, 4),
      mDiscordHideFirstNameInDetails(0, 8, 4)
#endif // USE_DISCORD
      ,
      mCommandShortcuts(mainFont, -360, -176, 10, true, "", "ABCDEFGHIJKLMNOPQRSTUVWXYZ.-,'?!/ 0123456789", NULL, 4),
      mEnableAdvancedShowUseOnObjectHoverKeybind(0, 168, 4), mEnableCoordinatesBox(0, 128, 4),
      mEnablePersistentEmoteBox(0, 88, 4), mEnableYumFinderBox(0, 48, 4), mEnableObjectSearchBox(0, 8, 4),
      mEnableFamilyDisplayBox(0, -32, 4), mEnableDangerousTileBox(0, -72, 4), mGenerateTownPlannerMapsBox(561, 52, 4),
      mEnableShowingHeldFoodPips(561, 52, 4), mEnableAlwaysShowPlayerLabelsBox(561, 52, 4)
{
    ...
    const char *choiceList[3] = {translate("system"), translate("drawn"), translate("both")};

    mCursorModeSet = new RadioButtonSet(mainFont, 561, 275, 3, choiceList, false, 4, true);
    ...
    mEnableAdvancedShowUseOnObjectHoverKeybind.setCursorTip("SHOW OBJECT REMAINING USE ON CURSOR HOVER.");
    ...
    mOldBorderlessSetting = SettingsManager::getIntSetting("borderless", 0);

    mBorderlessBox.setToggled(mOldBorderlessSetting);

    mTrippingEffectDisabledSetting = SettingsManager::getIntSetting("trippingEffectDisabled", 1);

    mTrippingEffectDisabledBox.setToggled(mTrippingEffectDisabledSetting);

    mEnableNudeSetting = SettingsManager::getIntSetting("nudeEnabled", 1);

    mEnableNudeBox.setToggled(mEnableNudeSetting);

    mEnableFOVSetting = SettingsManager::getIntSetting("fovEnabled", 0);

    mEnableFOVBox.setToggled(mEnableFOVSetting);

    mEnableKActionsSetting = SettingsManager::getIntSetting("keyboardActions", 0);

    mEnableKActionsBox.setToggled(mEnableKActionsSetting);

    mEnableCenterCameraSetting = SettingsManager::getIntSetting("centerCamera", 0);

    mEnableCenterCameraBox.setToggled(mEnableCenterCameraSetting);

#ifdef USE_DISCORD
    mDiscordRichPresenceSetting = SettingsManager::getIntSetting("discordRichPresence", 1);

    mEnableDiscordRichPresence.setToggled(mDiscordRichPresenceSetting);

    mDiscordRichPresenceStatusSetting = SettingsManager::getIntSetting("discordRichPresenceStatus", 1);

    mDiscordShowAgeInStatusSetting = SettingsManager::getIntSetting("discordRichPresenceShowAge", 1);
    ...
    mDiscordRichPresenceDetailsSetting = SettingsManager::getIntSetting("discordRichPresenceDetails", 0);

    mEnableDiscordRichPresenceDetails.setToggled(mDiscordRichPresenceDetailsSetting);

    mDiscordHideFirstNameInDetailsSetting = SettingsManager::getIntSetting("discordRichPresenceHideFirstName", 1);

    mDiscordHideFirstNameInDetails.setToggled(mDiscordHideFirstNameInDetailsSetting);
#endif // USE_DISCORD

    commandShortcutsRawText = SettingsManager::getSettingContents("commandShortcuts", "");
    ...
    showUseOnHoverEnabled = SettingsManager::getIntSetting("showUseOnObjectHoverKeybind", 0);

    mEnableAdvancedShowUseOnObjectHoverKeybind.setToggled(showUseOnHoverEnabled);

    coordinatesEnabled = SettingsManager::getIntSetting("coordinatesEnabled", 0);

    mEnableCoordinatesBox.setToggled(coordinatesEnabled);

    persistentEmoteEnabled = SettingsManager::getIntSetting("persistentEmoteEnabled", 0);

    mEnablePersistentEmoteBox.setToggled(persistentEmoteEnabled);
...
{
    int newVsyncSetting = SettingsManager::getIntSetting("countingOnVsync", -1);

    int newFullscreenSetting = SettingsManager::getIntSetting("fullscreen", 1);

    int newBorderlessSetting = SettingsManager::getIntSetting("borderless", 0);

    if (getCountingOnVsync() != newVsyncSetting || mOldFullscreenSetting != newFullscreenSetting ||
...
void SettingsPage::actionPerformed(GUIComponent *inTarget)
{
    ...
    if (inTarget == &mBackButton)
    {
        ...
        SettingsManager::setSetting("customServerPort", mCustomServerPortField.getInt());
    ...
    else if (inTarget == &mVsyncBox)
    {
        ...
        else
        {
            ...
            // any tweaked halfFrameRate setting should be cleared
            SettingsManager::setSetting("halfFrameRate", 0);
            SettingsManager::setSetting("targetFramesPerSecond", targetFramesPerSecond);
        }
    }
    ...
    else if (inTarget == &mRestartButton)
    {
        ...
        if ((newVsyncSetting == 1 && !getCountingOnVsync()) ||
            (newVsyncSetting == 1 && newFullscreenSetting != mOldFullscreenSetting) ||
    ...
    else if (inTarget == &mSoundEffectsLoudnessSlider)
    {
        ...
        if (!mSoundEffectsLoudnessSlider.isPointerDown())
        {
            ...
            setSoundEffectsLoudness(mSoundEffectsLoudnessSlider.getValue());
    ...
    else if (inTarget == &mCopyButton)
    {
        ...
        char *fullAddress = autoSprintf("%s:%d", address, mCustomServerPortField.getInt());
        delete[] address;
    ...
    else if (inTarget == &mEnableAdvancedShowUseOnObjectHoverKeybind)
    {
        ...
        SettingsManager::setSetting("showUseOnObjectHoverKeybind", newSetting);
    ...
    else if (inTarget == &mEnableCoordinatesBox)
    {
        ...
        SettingsManager::setSetting("coordinatesEnabled", newSetting);
    }
    else if (inTarget == &mEnablePersistentEmoteBox)
    {
        ...
        SettingsManager::setSetting("persistentEmoteEnabled", newSetting);
    }
    else if (inTarget == &mEnableYumFinderBox)
    {
        ...
        SettingsManager::setSetting("yumFinderEnabled", newSetting);
    }
    else if (inTarget == &mEnableObjectSearchBox)
    {
        ...
        SettingsManager::setSetting("objectSearchEnabled", newSetting);
    }
    else if (inTarget == &mEnableFamilyDisplayBox)
    {
        ...
        SettingsManager::setSetting("familyDisplayEnabled", newSetting);
    }
    else if (inTarget == &mEnableDangerousTileBox)
    {
        ...
        if (newSetting)
            dangerousTileEnabled = true;
        SettingsManager::setSetting("dangerousTileEnabled", newSetting);
    }
    ...
    else if (inTarget == &mEnableAlwaysShowPlayerLabelsBox)
    {
        ...
        if (newSetting)
            alwaysShowPlayerLabelEnabled = true;
        SettingsManager::setSetting("alwaysShowPlayerLabelEnabled", newSetting);
    }
...
void SettingsPage::draw(doublePair inViewCenter, double inViewSize)
{
    setDrawColor(1, 1, 1, 1);
    ...
    // prevent someone from making spamming requests to update their status, discord may block our key, due to spamming
    // requests! allow for at least 2 seconds to pass until the user is allowed to change the discord setting again.
...
void SettingsPage::makeActive(char inFresh)
{
    ...
    if (inFresh)
    {
        ...
        int useCustomServer = SettingsManager::getIntSetting("useCustomServer", 0);

        mUseCustomServerBox.setToggled(useCustomServer);

        char *address = SettingsManager::getStringSetting("customServerAddress", "localhost");
...
void SettingsPage::updatePage()
{
    ...
    mCopyButton.setPosition(mCustomServerAddressField.getRightEdgeX() + mCopyButton.getWidth() / 2 + 12,
                            mCustomServerAddressField.getPosition().y);
    mPasteButton.setPadding(8, 4);
    mPasteButton.setPosition(mCustomServerAddressField.getRightEdgeX() + 12 + mCopyButton.getWidth() + 12 +
                                 mPasteButton.getWidth() / 2,
                             mCustomServerAddressField.getPosition().y);
    ...
    mEnableDiscordShowAgeInStatus.setVisible(mPage == 4 && mEnableDiscordRichPresence.getToggled() &&
                                             mEnableDiscordRichPresenceStatus.getToggled());
    mEnableDiscordRichPresenceDetails.setVisible(mPage == 4 && mEnableDiscordRichPresence.getToggled() &&
                                                 mEnableDiscordRichPresenceStatus.getToggled());
    mDiscordHideFirstNameInDetails.setVisible(mPage == 4 && mEnableDiscordRichPresenceDetails.getToggled() &&
...
{
    if (mOldFullscreenSetting != mFullscreenBox.getToggled() || mOldBorderlessSetting != mBorderlessBox.getToggled() ||
