        ...
        for (int t = 0; t < numTGAFiles; t++)
        {
            ...
            if (!tgaFiles[t]->isDirectory())
            {
                ...
                if (strstr(tgaFileName, ".tga") != NULL)
                {
                    ...
                    SpriteHandle thumbnailSprite = loadSpriteBase(fullName, false);
                    ...
                    if (thumbnailSprite != NULL)
                    {
                        ...
                        r->image = readTGAFileBase(fullName);
...
OverlayRecord **searchOverlays(const char *inSearch, int inNumToSkip, int inNumToGet, int *outNumResults,
                               int *outNumRemaining)
{
...
int addOverlay(const char *inTag, Image *inSourceImage)
{
    ...
    if (overlaysDir.exists() && overlaysDir.isDirectory())
    {
        ...
        File *nextNumberFile = overlaysDir.getChildFile("nextOverlayNumber.txt");
