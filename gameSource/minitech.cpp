...
const char *biomeNames[] = {
    "GRASSLANDS", "SWAMP",  "YELLOW PRAIRIES", "BADLANDS",      "TUNDRA",
    "DESERT",     "JUNGLE", "DEEP WATER",      "FLOWER FIELDS", "SHALLOW WATER",
};
const std::vector<std::string> biomeNamesVec = {
    "GRASSLANDS", "SWAMP",  "YELLOW PRAIRIES", "BADLANDS",      "TUNDRA",
    "DESERT",     "JUNGLE", "DEEP WATER",      "FLOWER FIELDS", "SHALLOW WATER",
};
...
void minitech::setLivingLifePage(LivingLifePage *inLivingLifePage, SimpleVector<LiveObject> *inGameObjects, int inmMapD,
                                 int inPathFindingD, SimpleVector<int> *inmMapContainedStacks,
...
void minitech::initOnBirth()
{
    ...
    if (mainFont != NULL)
        delete mainFont;
    if (tinyHandwritingFont != NULL)
        delete tinyHandwritingFont;
    if (tinyMainFont != NULL)
        delete tinyMainFont;
...
void minitech::changeScale(float newScale)
{
    guiScale = newScale;
    if (handwritingFont != NULL)
        handwritingFont->setScaleFactor(16 * guiScale);
    if (mainFont != NULL)
        mainFont->setScaleFactor(16 * guiScale);
    if (tinyHandwritingFont != NULL)
        tinyHandwritingFont->setScaleFactor(16 / 2 * guiScale);
    if (tinyMainFont != NULL)
        tinyMainFont->setScaleFactor(16 / 2 * guiScale);
}
...
bool minitech::isCategory(int objId)
{
    ...
    if (c->isPattern)
    {
        ...
        {
            if (strstr(parent->description, "@") != NULL || strstr(parent->description, "Perhaps") != NULL)
            {
                return true;
            }
        }
...
minitech::mouseListener *minitech::getMouseListenerByArea(vector<mouseListener *> *listeners, doublePair posTL,
                                                          doublePair posBR)
{
    for (int i = 0; i < (int)listeners->size(); i++)
    {
        if (posEqual((*listeners)[i]->posTL, posTL) && posEqual((*listeners)[i]->posBR, posBR))
        {
            return (*listeners)[i];
        }
    }
...
int minitech::compareObjUse(int idA, int idB)
{
    ...
    if (b->numUses > 1)
        bParentId = b->id;
    if (a->isUseDummy)
        aParentId = a->useDummyParent;
    if (b->isUseDummy)
        bParentId = b->useDummyParent;
    ...
    if (bParentId == b->id)
        bUse = b->numUses;
    if (a->isUseDummy)
        aUse = a->thisUseDummyIndex + 1;
    if (b->isUseDummy)
        bUse = b->thisUseDummyIndex + 1;
...
float minitech::getTransProbability(TransRecord *trans)
{
    ...
    int cOrD = -1;
    if (isProbabilitySet(origIdC))
        cOrD = 0;
    if (isProbabilitySet(origIdD))
        cOrD = 1;
    if (cOrD != -1)
    {
        CategoryRecord *c;
        if (cOrD == 0)
            c = getCategory(origIdC);
        if (cOrD == 1)
            c = getCategory(origIdD);
        ...
        for (int i = 0; i < idSet.size(); i++)
        {
            ...
            *staticTrans = *t;
            ...
            if (cOrD == 0)
                staticTrans->newActor = newId;
            if (cOrD == 1)
                staticTrans->newTarget = newId;

            if (staticTrans->newActor == idC && staticTrans->newTarget == idD)
                return wSet.getElementDirect(i);
        }
    }
    return -1.0;
}
...
void minitech::drawObj(doublePair posCen, int objId, string strDescFirstLine, string strDescSecondLine)
{
    ...
    char used;
    drawObjectAnim(objId, 2, ground, 0.0, 0, ground, 0.0, 0.0, &used, ground, ground, posAfterOffset, 0, false, false,
                   -1, false, false, false, getEmptyClothingSet(), NULL);

    setLivingLifeBouncingYOffsetToggle(true);
    setDrawnObjectScale(1.0);
}

void minitech::drawStr(string str, doublePair posCen, string font, bool withBackground, bool avoidOffScreen)
{
...
void minitech::drawTileRect(int x, int y, string color, bool flashing)
{
    ...
    if (color == "green")
        setDrawColor(0, 1, 0, alpha);
    if (color == "blue")
        setDrawColor(0, 0, 1, alpha);
...
vector<TransRecord *> minitech::getUsesTrans(int objId)
{
    ...
    for (int t = 0; t < numTrans; t++)
    {
        ...
        // Skip useDummy when not holding them (e.g. holding bowl, skip bucket of water useDummies, only keep first and
        // last use)
        if (isUseDummyAndNotLastUse(idA) && isUseDummyAndNotLastUse(idC) && idA != objId)
            continue;
        if (isUseDummyAndNotLastUse(idB) && isUseDummyAndNotLastUse(idD) && idB != objId)
            continue;
        // Skip categories
        if (isCategory(idA) || isCategory(idB) || isCategory(idC) || isCategory(idD))
            continue;
        // Skip raw lastUse transitions, the proper ones are auto-generated and not tagged as lastUse
        if (trans->lastUseActor || trans->lastUseTarget)
            continue;
        // Skip generic use transitions when they are not food
        if (idB == -1 && idD == 0 && getObject(idA) != NULL && getObject(idA)->foodValue == 0 &&
            trans->contTransFlag == 0)
            continue;
        ...
        if (trans->contTransFlag != 0)
        {
            ...
            if (getObject(idA) != NULL && getObject(idB) != NULL && getObject(idA)->permanent &&
                getObject(idB)->permanent)
                continue;
        }

        // Skip transitions that involve uncraftable objects
        if (!showUncraftables && (isUncraftable(idA) || isUncraftable(idB) || isUncraftable(idC) || isUncraftable(idD)))
            continue;
...
vector<TransRecord *> minitech::getProdTrans(int objId)
{
    ...
    for (int t = 0; t < numTrans; t++)
    {
        ...
        // parse probabilitySet transitions (e.g. the only transitin making Pond with Dead Goose has a probSet as
        // newActor)
        int cOrD = -1;
        if (isProbabilitySet(idC))
            cOrD = 0;
        ...
        if (idA == objId || idB == objId)
            continue;
        // Skip useDummy when not holding them (e.g. holding bowl, skip bucket of water useDummies, only keep first and
        // last use)
        if (isUseDummyAndNotLastUse(idA) && isUseDummyAndNotLastUse(idC) && idC != objId)
            continue;
        if (isUseDummyAndNotLastUse(idB) && isUseDummyAndNotLastUse(idD) && idD != objId)
            continue;
        // Skip categories
        if (isCategory(idA) || isCategory(idB) || isCategory(idC) || isCategory(idD))
            continue;
        ...
        if (c->isProbabilitySet)
        {
            ...
            for (int t = 0; t < numPTrans; t++)
            {
                ...
                *staticTrans = *(prodPTrans->getElementDirect(t));
                ...
                if (staticTrans->actor == cId)
                    staticTrans->actor = objId;
                if (staticTrans->target == cId)
                    staticTrans->target = objId;
                if (staticTrans->newActor == cId)
                    staticTrans->newActor = objId;
                if (staticTrans->newTarget == cId)
                    staticTrans->newTarget = objId;
...
vector<TransRecord *> minitech::sortProdTrans(vector<TransRecord *> unsortedTrans)
{
    ...
    for (int i = 0; i < (int)unsortedTrans.size(); i++)
    {
        ...
        // For craft-from-scratch transition, object depth of both ingredience would be shallower than the product
        // object
        int currDepth = getObjectDepth(currentHintObjId);
        if ((idA <= 0 || getObjectDepth(idA) < currDepth) && (idB <= 0 || getObjectDepth(idB) < currDepth))
        {
            rankScores[i] = 0;
        }
    }
...
void minitech::updateDrawTwoTech()
{
    ...
    if (minitechMinimized)
    {
        ...
        minitechListener = getMouseListenerByArea(&twotechMouseListeners, sub(posTL, screenPos), sub(posBR, screenPos));
        ...
        mouseListener *maxListener =
            getMouseListenerByArea(&twotechMouseListeners, sub(maxTL, screenPos), sub(maxBR, screenPos));
    ...
    else
    {
        ...
        if (currentTwoTechPage < 0)
            currentTwoTechPage = maxPage - 1;
        if (currentTwoTechPage >= maxPage)
            currentTwoTechPage = 0;
        ...
        doublePair posLineLCen = {posTL.x + paddingX, posTL.y - paddingY - contentOffsetY - iconSize / 2};
        ...
        {
            if (i > 0)
                posLineLCen.y -= iconSize + lineSpacing;
            ...
            if (trans == NULL)
            {
                ...
                if (currentHintObj->numBiomes > 0)
                {
                    ...
                    for (int b = 0; b < currentHintObj->numBiomes; b++)
                    {
                        ...
                        pair<mouseListener *, int> biomeIconListenerId(biomeIconListener,
                                                                       100000 + currentHintObj->biomes[b]);
            ...
            if (trans->contTransFlag != 0)
            {
                ...
                if (newTarget != NULL && newTarget->numSlots > 0 && (newActor == NULL || newActor->numSlots == 0))
                {
                    // in
                    inOrOutContainmentTrans = 0;
                }
                else if (newActor != NULL && newActor->numSlots > 0 && (newTarget == NULL || newTarget->numSlots == 0))
                {
                    // out
            ...
            doublePair posLineTL = {posLineLCen.x - paddingX, posLineLCen.y + iconSize / 2};
            doublePair posLineBR = {posLineTL.x + recWidth, posLineTL.y - iconSize};

            mouseListener *lineListener =
                getMouseListenerByArea(&twotechMouseListeners, sub(posLineTL, screenPos), sub(posLineBR, screenPos));
            ...
            mouseListener *iconAListener =
                getMouseListenerByArea(&twotechMouseListeners, sub(iconLT, screenPos), sub(iconBR, screenPos));
            ...
            if (iconAListener->mouseClick && trans->actor > 0)
            {
                ...
                if (compareObjUse(trans->actor, trans->newActor) == -1)
                    currentHintObjId = getDummyParent(trans->actor);
                if (compareObjUse(trans->actor, trans->newActor) == 1)
                    currentHintObjId = getDummyLastUse(trans->actor);
            }
            ...
            else
            {
                ...
                if (thirdLineWords != "")
                    drawStr(thirdLineWords, thirdLine, "tinyHandwritten", false);
            }

            pos.x += iconSize;
            iconLT = {pos.x - iconSize / 2, pos.y + iconSize / 2};
            iconBR = {pos.x + iconSize / 2, pos.y - iconSize / 2};
            mouseListener *iconBListener =
                getMouseListenerByArea(&twotechMouseListeners, sub(iconLT, screenPos), sub(iconBR, screenPos));
            ...
            if (iconBListener->mouseClick && trans->target > 0)
            {
                ...
                if (compareObjUse(trans->target, trans->newTarget) == -1)
                    currentHintObjId = getDummyParent(trans->target);
                if (compareObjUse(trans->target, trans->newTarget) == 1)
                    currentHintObjId = getDummyLastUse(trans->target);
            }

            // pos for newbieTips use
            if (trans->actor == 34 && trans->target == 70)
                hatchetRecipePos = pos;
            ...
            if (iconCListener->mouseClick && trans->newActor > 0)
            {
                ...
                if (compareObjUse(trans->newActor, trans->actor) == -1)
                    currentHintObjId = getDummyParent(trans->newActor);
                if (compareObjUse(trans->newActor, trans->actor) == 1)
                    currentHintObjId = getDummyLastUse(trans->newActor);
            }

            // pos for newbieTips use
            if (trans->actor == 33 && trans->target == 32)
                sharpyRecipePos = pos;
            ...
            {
                // Eating transitions
                // Other generic use transitions in which target has no food value should be filtered out in
                // getUsesTrans and getProdTrans
            }
            else
            {
            ...
            if (iconDListener->mouseClick && trans->newTarget > 0)
            {
                ...
                if (compareObjUse(trans->newTarget, trans->target) == -1)
                    currentHintObjId = getDummyParent(trans->newTarget);
                if (compareObjUse(trans->newTarget, trans->target) == 1)
                    currentHintObjId = getDummyLastUse(trans->newTarget);
            }
        }
        ...
        if (showPreviousPageButton)
        {
            ...
            prevListener = getMouseListenerByArea(&twotechMouseListeners, sub(prevPageButtonTLPos, screenPos),
        ...
        if (showNextPageButton)
        {
            ...
            nextListener = getMouseListenerByArea(&twotechMouseListeners, sub(nextPageButtonTLPos, screenPos),
    ...
    doublePair firstLineLT = {firstLine.x - textWidth / 2 - paddingX / 2,
                              firstLine.y + tinyLineHeight / 2 + paddingY / 2};
    doublePair firstLineBR = {firstLine.x + textWidth / 2 + paddingX / 2,
                              firstLine.y - tinyLineHeight / 2 - paddingY / 2};
    doublePair secondLineLT = {secondLine.x - textWidth / 2 - paddingX / 2,
                               secondLine.y + tinyLineHeight / 2 + paddingY / 2};
    doublePair secondLineBR = {secondLine.x + textWidth / 2 + paddingX / 2,
                               secondLine.y - tinyLineHeight / 2 - paddingY / 2};
    ...
    mouseListener *useListener =
        getMouseListenerByArea(&twotechMouseListeners, sub(firstLineLT, screenPos), sub(firstLineBR, screenPos));
    if (useListener->mouseClick)
        useOrMake = 0;
    mouseListener *makeListener =
        getMouseListenerByArea(&twotechMouseListeners, sub(secondLineLT, screenPos), sub(secondLineBR, screenPos));
    if (makeListener->mouseClick)
        useOrMake = 1;
    ...
    mouseListener *headerIconListener =
        getMouseListenerByArea(&twotechMouseListeners, sub(iconTL, screenPos), sub(iconBR, screenPos));
    ...
    mouseListener *minListener =
        getMouseListenerByArea(&twotechMouseListeners, sub(minLT, screenPos), sub(minBR, screenPos));
    ...
    if (currentHintObjIndex > 0)
    {
        ...
        mouseListener *prevObjListener =
            getMouseListenerByArea(&twotechMouseListeners, sub(prevObjLT, screenPos), sub(prevObjBR, screenPos));
    ...
    if (currentHintObjIndex < pageRecords.size() - 1)
    {
        ...
        mouseListener *nextObjListener =
            getMouseListenerByArea(&twotechMouseListeners, sub(nextObjLT, screenPos), sub(nextObjBR, screenPos));
    ...
    doublePair posBR = {posTL.x + recWidth, posTL.y - recHeight};
    minitechListener = getMouseListenerByArea(&twotechMouseListeners, sub(headerTL, screenPos), sub(posBR, screenPos));
    if (minitechListener->mouseHover)
    {
        isMinitechHovered = true;
    }
    ...
    for (int i = 0; i < (int)iconListenerIds.size(); i++)
    {
        ...
        if (listener->mouseHover && id > 0)
        {
            ...
            if (id >= 100000)
            {
                ...
                if (biomeId < numBiomes)
                    biomeName = biomeNames[biomeId];
            ...
            if (unsortedHits.size() > 0)
            {
                ...
                sort(index.begin(), index.end(), [&](size_t a, size_t b) {
                    for (int pass = 0; pass <= 2; pass++)
                    {
                        ...
                        if (!showCommentsAndTagsInObjectDescription && pass > 0)
                            break;
                        ...
                        if (descWords[a].size() == 0)
                            descWords[a] = Tokenize(aDesc, "[\\s\\#\\,]+");
                        if (descWords[b].size() == 0)
                            descWords[b] = Tokenize(bDesc, "[\\s\\#\\,]+");
                        ...
                        for (int i = 0; i < (int)hintWords.size(); i++)
                        {
                            ...
                            {
                                if ((hintWord.compare(aDescWords[j]) == 0 && pass < 2) ||
                                    (aDescWords[j].find(hintWord) != string::npos && pass == 2))
                                    aScore++;
                            }
                            for (int k = 0; k < (int)bDescWords.size(); k++)
                            {
                                if ((hintWord.compare(bDescWords[k]) == 0 && pass < 2) ||
...
void minitech::livingLifeDraw(float mX, float mY)
{
    ...
    if ((lastHintObjId != currentHintObjId || lastUseOrMake != useOrMake) && !minitechMinimized)
    {
        ...
        unsortedTrans.erase(unique(unsortedTrans.begin(), unsortedTrans.end()), unsortedTrans.end());
...
bool minitech::livingLifeKeyDown(unsigned char inASCII)
{
    ...
    if (commandKey && !shiftKey && inASCII == 24)
    { // ctrl + X
        nextHintObj();
        return true;
    }
    if (commandKey && !shiftKey && inASCII == 26)
    { // ctrl + Z
        prevHintObj();
        return true;
    }
    ...
    if (!shiftKey && commandKey && inASCII + 64 == toupper(minimizeKey))
    { // Ctrl + V
