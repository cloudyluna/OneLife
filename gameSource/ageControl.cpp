...
static doublePair defaultAgeScaling[] = {{20, 20}, {104, 40}, {117, 57}, {120, 60}};

#include "minorGems/util/SettingsManager.h"

void initAgeControl()
{
    babyHeadDownFactor = SettingsManager::getFloatSetting("babyHeadDownFactor", 0.6);

    babyBodyDownFactor = SettingsManager::getFloatSetting("babyBodyDownFactor", 0.75);

    oldHeadDownFactor = SettingsManager::getFloatSetting("oldHeadDownFactor", 0.35);

    oldHeadForwardFactor = SettingsManager::getFloatSetting("oldHeadForwardFactor", 2);
                            ...
                            doublePair inFrontFootSpritePos)
{
    ...
    if (inAge >= 40)
    {
        ...
        double vertOffset = ((inAge - 40) / 20) * oldHeadDownFactor * maxHead;

        double footOffset = inFrontFootSpritePos.x - inHeadSpritePos.x;

        double forwardOffset = ((inAge - 40) / 20) * oldHeadForwardFactor * footOffset;
...
void initAgeScaling()
{
    ...
    for (int i = 0; i < numParts; i++)
    {
        if (strcmp(parts[i], "") != 0)
        {
            ...
            float serverAge;
            float displayAge;
            int numRead = sscanf(parts[i], "%f %f", &serverAge, &displayAge);
            if (numRead == 2)
            {
