...
void initMusicPlayer()
{
    ...
    if (musicOGG != NULL)
    {
        ...
        for (int i = 0; i < numChunks; i++)
        {
            ...
            int chunkStartSample = musicNumSamples - (i + 1) * chunkLengthSeconds * sampleRate;
            ...
            readNextSamplesOGG(musicOGG, numToRead, &(chunkPreFadeSamplesL[i][preOffset]),
...
void restartMusic(double inAge)
{
    ...
    int numExtraSamples = (int)(numChunksAlreadyPassed * chunkLengthSeconds * sampleRate);

    currentSamplePos =
        musicNumSamples + numExtraSamples - numChunksToPlayThisTime * (int)(chunkLengthSeconds * sampleRate);
...
void getSoundSamples(Uint8 *inBuffer, int inLengthToFillInBytes)
{
    ...
    int numRead = readNextSamplesOGG(musicOGG, numSamples, samplesL, samplesR);
    ...
    if (numTimesReachedEnd < numChunks - 1 && musicNumSamples - endSamplePos < crossFadeSamples)
    {
        ...
        float *samplesLNext = chunkPreFadeSamplesL[numChunksToPlayNextTime - 1];
        float *samplesRNext = chunkPreFadeSamplesR[numChunksToPlayNextTime - 1];
        ...
        for (int i = 0; i < numRead; i++)
        {
            ...
            if (musicNumSamples - absIndex <= crossFadeSamples)
            {
                ...
                int crossFadeI = crossFadeSamples - (musicNumSamples - absIndex);

                // t in [-1, 1]
                double t = (2 * crossFadeI / (double)crossFadeSamples) - 1;

                double thisWeight = sqrt(0.5 * (1 - t));
                double nextWeight = sqrt(0.5 * (1 + t));

                samplesL[i] = thisWeight * samplesL[i] + nextWeight * samplesLNext[crossFadeI];

                samplesR[i] = thisWeight * samplesR[i] + nextWeight * samplesRNext[crossFadeI];
            }
        }
    }
    ...
    if (numRead < numSamples)
    {
        ...
        else
        {
            ...
            currentSamplePos = musicNumSamples - numChunksToPlayThisTime * (int)(chunkLengthSeconds * sampleRate);

            seekOGG(musicOGG, currentSamplePos);

            int numLeft = numSamples - numRead;

            int numReadB = readNextSamplesOGG(musicOGG, numLeft, &(samplesL[numRead]), &(samplesR[numRead]));
