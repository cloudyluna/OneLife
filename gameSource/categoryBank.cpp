...
int initCategoryBankStart(char *outRebuildingCache)
{
    ...
    cache = initFolderCache("categories", outRebuildingCache, shouldFileBeCached);
    ...
    if (shouldFileBeCached(txtFileName))
    {
        ...
        if (categoryText != NULL)
        {
            ...
            if (numLines >= 2)
            {
                ...
                r->parentID = 0;
                sscanf(lines[next], "parentID=%d", &(r->parentID));
                ...
                int numObjects = 0;
                sscanf(lines[next], "numObjects=%d", &(numObjects));
    ...
    if (inParentID < mapSize)
    {
        if (idMap[inParentID] != NULL)
        {
            ...
            for (int i = 0; i < r->objectIDSet.size(); i++)
            {
                ...
                reverseMap[r->objectIDSet.getElementDirect(i)]->categoryIDSet.deleteElementEqualTo(inParentID);
            }
...
void saveCategoryToDisk(int inParentID)
{
    ...
    else
    {
        ...
        for (int i = 0; i < r->objectIDSet.size(); i++)
        {
            ...
            {
                lines.push_back(
                    autoSprintf("%d %f", r->objectIDSet.getElementDirect(i), r->objectWeights.getElementDirect(i)));
            }
            else
            {
                lines.push_back(autoSprintf("%d", r->objectIDSet.getElementDirect(i)));
            }
        }
...
void addCategoryToObject(int inObjectID, int inParentID)
{
    ...
    if (inObjectID >= reverseMapSize)
    {
        ...
        memcpy(newMap, reverseMap, sizeof(ReverseCategoryRecord *) * reverseMapSize);
...
void removeObjectFromCategory(int inParentID, int inObjectID, int inListIndex)
{
...
void moveCategoryUp(int inObjectID, int inParentID)
{
    ...
    if (rr != NULL)
    {
        ...
        if (index > 0)
        {
            ...
            *(rr->categoryIDSet.getElement(newIndex)) = rr->categoryIDSet.getElementDirect(index);
...
void moveCategoryDown(int inObjectID, int inParentID)
{
    ...
    if (rr != NULL)
    {
        ...
        if (index != -1 && index < rr->categoryIDSet.size() - 1)
        {
...
void moveCategoryMemberDown(int inParentID, int inObjectID, int inListIndex)
{
    ...
    if (r != NULL)
    {
        ...
        if (index != -1 && index != r->objectIDSet.size() - 1)
        {
