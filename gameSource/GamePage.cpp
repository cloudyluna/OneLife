...
GamePage::GamePage()
    : PageComponent(0, 0), mStatusError(false), mStatusMessageKey(NULL), mStatusMessage(NULL),
      mSkipDrawingSubComponents(false), mTip(NULL), mLastTip(NULL), mLastTipFade(1), mTipAtTopOfScreen(false),
      mStatusAtTopOfScreen(false), mSignal(NULL), mResponseWarningTipShowing(false)
{
...
void GamePage::base_draw(doublePair inViewCenter, double inViewSize)
{
    ...
    if (sWaitingFade > 0)
    {
        ...
        if (sWaiting && currentActiveSerialWebRequest != -1)
        {
            ...
            switch (getWebRequestRetryStatus(currentActiveSerialWebRequest))
            {
            case 0:
                break;
            case 1:
        ...
        setDrawColor(r, g, b, sWaitingFade * sCurrentWaitingSpriteFade);

        drawSprite(sWaitingSprites[sCurrentWaitingSprite], spritePos);

        if (sLastWaitingSprite != -1)
        {

            setDrawColor(r, g, b, sWaitingFade * (1 - sCurrentWaitingSpriteFade));

            drawSprite(sWaitingSprites[sLastWaitingSprite], spritePos);
        }
...
void GamePage::base_step()
{
    ...
    if (sWaiting && canShowWaitingIcon() &&
        (!sShowWaitingWarningOnly || getWebRequestRetryStatus(currentActiveSerialWebRequest) > 0))
    {
...
void GamePage::pointerMove(float inX, float inY)
{
    if (sResponseWarningShowing && currentActiveSerialWebRequest != -1)
    {
        ...
        if (fabs(inX - sResponseWarningPosition.x) < 20 && fabs(inY - sResponseWarningPosition.y) < 20)
        {

            int status = getWebRequestRetryStatus(currentActiveSerialWebRequest);

            if (status >= 2)
            {

                char *tipString = autoSprintf(translate("responseWarning"), status - 1);
