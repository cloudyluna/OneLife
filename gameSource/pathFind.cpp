...
inline static char isRecordBetter(pathSearchRecord *inA, pathSearchRecord *inB)
{
...
// sorted insertion
static void insertSearchRecord(pathSearchQueue *inQueue, pathSearchRecord *inRecordToInsert)
{
...
static pathSearchRecord *pullSearchRecord(pathSearchQueue *inQueue, int inSquareIndex)
{
    ...
    while (currentRecord != NULL && currentRecord->squareIndex != inSquareIndex)
    {
...
char pathFind(int inMapH, int inMapW, char *inBlockedMap, GridPos inStart, GridPos inGoal, int *outFullPathLength,
              GridPos **outFullPath, GridPos *outClosest)
{
    ...
    pathSearchRecord startRecord = {inStart,
    ...
    while (recordsToSearch.head != NULL && !done)
    {
        ...
        if (false)
            printf("Best record found:  "
                   "(%d,%d), cost %d, total %f, "
                   "pred %d, this index %d\n",
                   bestRecord.pos.x, bestRecord.pos.y, bestRecord.cost, bestRecord.total, bestRecord.predIndex,
                   doneQueue.size());
        ...
        else
        {
            ...
            for (int n = 0; n < 8; n++)
            {
                ...
                if (x < 0 || x >= inMapW || y < 0 || y >= inMapH)
                {

                    continue;
                }

                if (currentBlocked && y == bestPos.y - 1)
                {
                ...
                if (!inBlockedMap[neighborSquareIndex])
                {
                    ...
                    if (!alreadyOpen && !alreadyDone)
                    {
                        ...
                        // add this neighbor
                        double dist = getGridDistance(neighbors[n], inGoal);
                        ...
                        */
                        ...
                        pathSearchRecord nRecord = {neighbors[n], neighborSquareIndex, cost, dist,
                                                    dist + cost,  predIndex,           NULL};
                        pathSearchRecord *heapRecord = new pathSearchRecord(nRecord);
    ...
    pathSearchRecord *currentRecord = doneQueue.getElement(currentIndex);

    pathSearchRecord *predRecord = doneQueue.getElement(currentRecord->predIndex);
...
char pathFind(int inMapH, int inMapW, char *inBlockedMap, GridPos inStart, GridPos inWaypoint, GridPos inGoal,
              int *outFullPathLength, GridPos **outFullPath,
              ...
              GridPos *outClosest)
{
    ...
    char firstFound =
        pathFind(inMapH, inMapW, inBlockedMap, inStart, inWaypoint, outFullPathLength, outFullPath, outClosest);
    ...
    char secondFound =
        pathFind(inMapH, inMapW, inBlockedMap, inWaypoint, inGoal, &secondLength, &secondPath, &secondClosest);
