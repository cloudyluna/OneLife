...
char *getPureAccountKey()
{
    ...
    char *pureCode = join(codeParts, numParts, "");
...
char *getAccountHmac()
{
    ...
    char *toHash = autoSprintf("%d%d", serverSequenceNumber, accountHmacVersionNumber);
    ...
    char *result = autoSprintf("&sequence_number=%d&account_hmac=%s", serverSequenceNumber, hash);
    delete[] hash;
...
char *replaceAccountHmac(char *inString)
{
    ...
    else if (strstr(inString, keyA) != NULL && strstr(inString, keyB) != NULL)
    {
        ...
        int i = 0;
        while (hmacStart[i] != '\0' &&
               ((hmacStart[i] >= '0' && hmacStart[i] <= '9') || (hmacStart[i] >= 'A' && hmacStart[i] <= 'F')))
        {
            i++;
        }
        ...
        char *newHashInPlace = replaceOnce(inString, startPointerA, newHash, &found);
