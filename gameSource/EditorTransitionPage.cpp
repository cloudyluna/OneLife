...
static const char *moveButtonNames[NUM_MOVE_BUTTONS] = {"None",  "Chase", "Flee", "Random", "North",
                                                        "South", "East",  "West", "Find"};

#define NUM_CONTTRANS_MODES 5
static const char *contTransModeNames[NUM_CONTTRANS_MODES] = {"None", "First", "Last", "Not Swap", "Any"};

EditorTransitionPage::EditorTransitionPage()
    : mAutoDecayTimeField(smallFont, 0, -170, 6, false, "AutoDecay Seconds", "-0123456789.", NULL),
      mContTransModesButtons(smallFont, -330, -170, NUM_CONTTRANS_MODES, contTransModeNames, true, 2),
      mLastUseActorCheckbox(-330, 75, 2), mLastUseTargetCheckbox(130, 75, 2), mReverseUseActorCheckbox(-330, -75, 2),
      mReverseUseTargetCheckbox(130, -75, 2), mNoUseActorCheckbox(-330, -95, 2), mNoUseTargetCheckbox(130, -95, 2),
      mMovementButtons(smallFont, 240, 40, NUM_MOVE_BUTTONS, moveButtonNames, true, 2),
      mDesiredMoveDistField(smallFont, 240, 40 - NUM_MOVE_BUTTONS * 20 - 30, 4, false, "Dist", "0123456789", NULL),
      mActorMinUseFractionField(smallFont, -290, 115, 4, false, "Min Use Fraction", "0123456789.", NULL),
      mTargetMinUseFractionField(smallFont, 90, 115, 4, false, "Min Use Fraction", "0123456789.", NULL),
      mSaveTransitionButton(mainFont, -320, 0, "Save"), mObjectPicker(&objectPickable, +410, 90),
      mObjectPickerAlt(&objectPickableAlt, -540, 0), mObjectEditorButton(mainFont, 410, 260, "Objects"),
      mCategoryEditorButton(mainFont, -410, 260, "Categories"), mProducedByNext(smallFont, 180, 260, "Next"),
      mProducedByPrev(smallFont, -180, 260, "Prev"), mProducesNext(smallFont, 180, -260, "Next"),
      mProducesPrev(smallFont, -180, -260, "Prev"), mDelButton(smallFont, +150, 0, "Delete"),
      mDelConfirmButton(smallFont, +150, 40, "Delete?"), mSwapActorButton("swapButton.tga", -200, 0),
      mSwapTargetButton("swapButton.tga", 0, 0), mSwapTopButton("swapButtonH.tga", -100, 120),
      mSwapBottomButton("swapButtonH.tga", -100, -120)
{
    ...
    for (int y = 0; y < 2; y++)
    {
        ...
        for (int x = 0; x < 2; x++)
        {
            ...
            mClearButtons[i] = new TextButton(smallFont, xP + offset, yP, "X");
...
void EditorTransitionPage::checkIfSaveVisible()
{
    char saveVis = (mCurrentTransition.target != 0 ||
                    ...
                    (mCurrentTransition.actor > 0 && getObject(mCurrentTransition.actor)->foodValue > 0 &&
                     mCurrentTransition.newActor != 0 && mCurrentTransition.newTarget == 0) ||
    ...
    char delVis = saveVis || (mCurrentTransition.actor > 0 && mCurrentTransition.newActor != 0);
    ...
    mActorMinUseFractionField.setFloat(mCurrentTransition.actorMinUseFraction, -1, true);

    mTargetMinUseFractionField.setFloat(mCurrentTransition.targetMinUseFraction, -1, true);

    if (saveVis && getObjectByIndex(&mCurrentTransition, 0) <= 0 && getObjectByIndex(&mCurrentTransition, 2) == 0)
    {
        ...
        {
            char *decayString = autoSprintf("%.2f", mCurrentTransition.autoDecaySeconds);
...
void EditorTransitionPage::redoTransSearches(int inObjectID, char inClearSkip)
{
    ...
    printf("Searching for %d, skips = %d,%d\n", inObjectID, mProducedBySkip, mProducesSkip);

    int numResults, numLeft;
    TransRecord **resultsA = searchProduces(inObjectID, mProducedBySkip, NUM_TREE_TRANS_TO_SHOW, &numResults, &numLeft);
    ...
    if (resultsA != NULL)
    {
        ...
        for (int i = 0; i < numResults; i++)
        {
            ...
            if (mProducedBy[i].newActor != inObjectID && mProducedBy[i].newTarget != inObjectID)
            {
                // transition does not produce inObjectID directly
                // must be a category transition

                mProducedByType[i] = isObjectInCategory(mProducedBy[i].newActor, inObjectID);

                if (mProducedByType[i] == 0)
                {
                    mProducedByType[i] = isObjectInCategory(mProducedBy[i].newTarget, inObjectID);
                }
            }
    ...
    TransRecord **resultsB = searchUses(inObjectID, mProducesSkip, NUM_TREE_TRANS_TO_SHOW, &numResults, &numLeft);
    ...
    if (resultsB != NULL)
    {
        ...
        for (int i = 0; i < numResults; i++)
        {
            ...
            if (mProduces[i].actor != inObjectID && mProduces[i].target != inObjectID)
            {
                // transition does not use inObjectID directly
                // must be a category transition

                mProducesType[i] = isObjectInCategory(mProduces[i].actor, inObjectID);

                if (mProducesType[i] == 0)
                {
                    mProducesType[i] = isObjectInCategory(mProduces[i].target, inObjectID);
                }
            }
...
void EditorTransitionPage::actionPerformed(GUIComponent *inTarget)
{
    ...
    if (inTarget == &mSaveTransitionButton)
    {
        ...
        if (target != 0 && getObject(mCurrentTransition.target)->person)
        {
        ...
        else if (target == 0 && mCurrentTransition.actor > 0 && mCurrentTransition.newActor != 0 &&
                 mCurrentTransition.newTarget == 0 && getObject(mCurrentTransition.actor)->foodValue > 0)
        {
        ...
        else if (target == 0 && mCurrentTransition.actor > 0 && mCurrentTransition.newTarget != 0 &&
                 getObject(mCurrentTransition.actor)->foodValue == 0)
        {

            target = -1;
        }
        else if (target != 0 && mCurrentTransition.newActor == 0 && mCurrentTransition.actor > 0 &&
        ...
        addTrans(actor, target, mCurrentTransition.newActor, mCurrentTransition.newTarget,
                 mCurrentTransition.lastUseActor, mCurrentTransition.lastUseTarget,
                 mContTransModesButtons.getSelectedItem(), mCurrentTransition.reverseUseActor,
                 mCurrentTransition.reverseUseTarget, mCurrentTransition.noUseActor, mCurrentTransition.noUseTarget,
                 decayTime, mActorMinUseFractionField.getFloat(), mTargetMinUseFractionField.getFloat(),
                 mMovementButtons.getSelectedItem(), mDesiredMoveDistField.getInt());
    ...
    else if (inTarget == &mDelConfirmButton)
    {
        ...
        else if (target == 0 && mCurrentTransition.actor > 0)
        {
        ...
        deleteTransFromBank(actor, target, mLastUseActorCheckbox.getToggled(), mLastUseTargetCheckbox.getToggled(),
    ...
    else if (inTarget == &mObjectPicker || inTarget == &mObjectPickerAlt)
    {
        ...
        if (mCurrentlyReplacing != -1)
        {
            ...
            if (objectID != -1)
            {
                setObjectByIndex(&mCurrentTransition, mCurrentlyReplacing, objectID);
    ...
    {
        mCurrentTransition.reverseUseActor = mReverseUseActorCheckbox.getToggled();
    }
    else if (inTarget == &mReverseUseTargetCheckbox)
    {
        mCurrentTransition.reverseUseTarget = mReverseUseTargetCheckbox.getToggled();
    }
    else if (inTarget == &mNoUseActorCheckbox)
    {
        mCurrentTransition.noUseActor = mNoUseActorCheckbox.getToggled();
    }
    else if (inTarget == &mNoUseTargetCheckbox)
    {
        mCurrentTransition.noUseTarget = mNoUseTargetCheckbox.getToggled();
    }
    ...
    else
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            if (inTarget == mPickButtons[i])
            {
                ...
                int replacingID = getObjectByIndex(&mCurrentTransition, mCurrentlyReplacing);
        ...
        for (int i = 0; i < NUM_TREE_TRANS_TO_SHOW; i++)
        {
            if (inTarget == mProducedByButtons[i])
            {
                ...
                for (int j = 0; j < 4; j++)
                {
                    if (getObjectByIndex(&mCurrentTransition, j) == mLastSearchID)
                    {
...
static void drawTransObject(int inID, doublePair inPos)
{
    if (inID > 0)
    {
        ...
        drawObject(getObject(inID), 2, inPos, 0, false, false, 20, 0, false, false, getEmptyClothingSet());

        setDrawnObjectScale(1.0);
    }
}

void EditorTransitionPage::draw(doublePair inViewCenter, double inViewSize)
{
    ...
    doublePair centerA = mult(add(mPickButtons[0]->getCenter(), mPickButtons[1]->getCenter()), 0.5);

    setDrawColor(1, 1, 1, 1);
    mainFont->drawString("+", centerA, alignCenter);

    doublePair centerB = mult(add(mPickButtons[2]->getCenter(), mPickButtons[3]->getCenter()), 0.5);
    ...
    for (int i = 0; i < NUM_TREE_TRANS_TO_SHOW; i++)
    {
        ...
        if (mProducedByButtons[i]->isVisible())
        {
            ...
            notePos.x -= 48;
            notePos.y += 60;
            smallFont->drawString(noteString, notePos, alignLeft);
    ...
    if (mCurrentTransition.target != 0 && getObject(mCurrentTransition.target)->person)
    {
        ...
        smallFont->drawString("Generic on-person Transition", pos, alignCenter);
    }
    else if (mCurrentTransition.target != 0 && mCurrentTransition.newActor == 0 && mCurrentTransition.actor > 0 &&
             getObject(mCurrentTransition.actor)->person)
    {
        ...
        smallFont->drawString("Default Transition", pos, alignCenter);
    }
    else if (mCurrentTransition.target == 0 && mCurrentTransition.actor > 0 &&
             getObject(mCurrentTransition.actor)->foodValue > 0 && mCurrentTransition.newActor != 0 &&
             mCurrentTransition.newTarget == 0)
    {
        ...
        smallFont->drawString("Eating Transition", pos, alignCenter);
    }
    else if (mCurrentTransition.target == 0 && mCurrentTransition.actor > 0 &&
             getObject(mCurrentTransition.actor)->foodValue == 0 && mCurrentTransition.newActor != 0 &&
             mCurrentTransition.newTarget == 0)
    {
        ...
        smallFont->drawString("Generic Use Transition", pos, alignCenter);
    }
    else if (mCurrentTransition.target == 0 && mCurrentTransition.actor > 0 &&
             getObject(mCurrentTransition.actor)->foodValue == 0 && mCurrentTransition.newTarget != 0)
    {
        ...
        smallFont->drawString("Use on Bare Ground", pos, alignCenter);
    }
    ...
    if (mAutoDecayTimeField.isVisible() && mAutoDecayTimeField.getFloat() < 0)
    {
        ...
        char *displayString = autoSprintf("(%d epoc%s)", numEpocs, plural);
