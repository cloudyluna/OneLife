...
RadioButtonSet::RadioButtonSet(Font *inDisplayFont, double inX, double inY, int inNumItems, const char **inItemNames,
                               char inRightLabels, double inDrawScale, char inDrawNamesWithShadow)
    : PageComponent(inX, inY), mHover(false), mDisplayFont(inDisplayFont), mNumItems(inNumItems),
      mItemNames(new char *[inNumItems]), mCheckboxes(new CheckboxButton *[inNumItems]), mRightLabels(inRightLabels),
      mCheckboxSep(inDrawScale * baseCheckboxSep), mSelectedItem(0), mDrawNamesWithShadow(inDrawNamesWithShadow)
{
    ...
    for (int i = 0; i < mNumItems; i++)
    {
        ...
        mCheckboxes[i] = new CheckboxButton(0, 0 - inDrawScale * 10 * i, inDrawScale);
