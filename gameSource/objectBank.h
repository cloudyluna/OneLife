...
int initObjectBankStart(char *outRebuildingCache, char inAutoGenerateUsedObjects = false,
...
int reAddObject(ObjectRecord *inObject, char *inNewDescription = NULL, char inNoWriteToFile = false,
                int inReplaceID = -1);
...
int addObject(const char *inDescription, char inContainable, float inContainSize, double inVertContainRotationOffset,
              ...
              char *inSpriteUseVanish, char *inSpriteUseAppear, char inNoWriteToFile = false, int inReplaceID = -1,
              int inExistingObjectHeight = -1, char inConsiderIDOffset = false);
...
// spectrum
int getRandomFamilyMember(int inRace, int inMotherID, int inFamilySpan, char inForceGirl = false);
...
char checkSpriteAncestor(ObjectRecord *inRecord, int inChildIndex, int inPossibleAncestorIndex);
...
double getClosestObjectPart(ObjectRecord *inObject,
                            ...
                            // in clothing objects
                            char inWorn, double inAge, int inPickedLayer, char inFlip, float inXCenterOffset,
                            float inYCenterOffset, int *outSprite,
                            // 0, 1, 2, 3 if clothing hit
                            int *outClothing, int *outSlot,
                            // whether sprites marked as multiplicative
                            // blend-mode should be considered clickable
                            char inConsiderTransparent = true, char inConsiderEmptySlots = false);
...
char isSpriteSubset(int inSuperObjectID, int inSubObjectID, SimpleVector<SubsetSpriteIndexMap> *outMapping = NULL);
...
void setupSpriteUseVis(ObjectRecord *inObject, int inUsesRemaining, char *inSpriteVis);
...
void prepareToSkipSprites(ObjectRecord *inObject, char inDrawBehind, char inSkipAll = false);
