...
EditorSpriteTrimPage::EditorSpriteTrimPage()
    : mImportEditorButton(mainFont, -310, 260, "Sprites"), mSaveButton(mainFont, 400, 64, "Save"),
      mClearRectButton(mainFont, 400, -65, "X Rect"), mFreehandSplitButton(mainFont, 400, 192, "Freehand Split"),
      mBrushSizeSlider(smallFont, 400, 292, 2, 100, 20, 1, 50, "Brush Size"), mFreehandSplitMode(false),
      mSpritePicker(&spritePickable, -410, 90), mPickedSprite(-1), mPickingRect(false), mFreehandSelection(NULL),
      mFreehandSelectionSprite(NULL), mCursorOffsetX(0), mCursorOffsetY(0)
{
...
static Image *trimTrans(Image *inImage)
{
    ...
    {
        return inImage->getSubImage(firstX, firstY, 1 + lastX - firstX, 1 + lastY - firstY);
    }
    else
    {
        return NULL;
    ...
    else if (inTarget == &mSaveButton && mFreehandSplitMode)
    {
        ...
        if (im != NULL)
        {
            ...
            if (imInTrim != NULL)
            {
                ...
                addSprite(tag, finalSprite, final, r->multiplicativeBlend);
    ...
    else if (inTarget == &mSaveButton)
    {
        ...
        if (im != NULL)
        {
            ...
            for (int i = 0; i < numSprites; i++)
            {
                ...
                Image *subIm = im->getSubImage(oldSprite->w / 2 + rE.xStart, oldSprite->h / 2 - rE.yStart,
                                               rE.xEnd - rE.xStart, rE.yStart - rE.yEnd);
                ...
                if (r.intersectSides[0])
                {
                    for (int y = 0; y < over; y++)
                    {
                        ...
                        for (int x = 0; x < subW; x++)
                        {
                            ...
                            if (subA[p] < 1)
                            {
                                subA[p] = (subA[p] - F * subA[p]) / (1 - F * subA[p]);
                            }
                            else
                            {
                                // opaque
                ...
                spriteIDs[i] =
                    addSprite(newTag, fillSprite(subExpanded, false), subExpanded, oldSprite->multiplicativeBlend);
            ...
            addObject(objName, false, 1, 0, false, false, false, 3, 9999999, false, false, -1, false, 0, 0, false,
                      spriteBehindPlayer, spriteAdditiveBlend, (char *)"0", 0, 0, 0, false, false, false, 0, false,
                      false, false, NULL, false, false, false, false, false, 0, 0, 1, 0, 0, zeroOffset, 'n', zeroOffset,
                      0, // deadly distance
                      1, // use distance
                      blankSoundUsage, blankSoundUsage, blankSoundUsage, blankSoundUsage, 0, 0, 0, 0, slotPos, 0,
                      slotVert, slotParent, 1, false, false, numSprites, spriteIDs, spritePos, spriteRot, spriteHFlip,
                      spriteColor, spriteAgeStart, spriteAgeEnd, spriteParent, spriteInvisibleWhenHolding,
                      spriteInvisibleWhenWorn, spriteBehindSlots, spriteInvisibleWhenContained,
                      spriteIgnoredWhenCalculatingCenterOffset, spriteIsHead, spriteIsBody, spriteIsBackFoot,
                      spriteIsFrontFoot, 1, 1.0f, spriteUseVanish, spriteUseAppear, false, -1, -1, true);
...
char EditorSpriteTrimPage::trimRectByExisting(PickedRect *inRect, char inUpdateCovered)
{
    ...
    for (int i = 0; i < mRects.size(); i++)
    {
        ...
        if (otherR.xStart >= r.xStart && otherR.xEnd <= r.xEnd && otherR.yStart <= r.yStart && otherR.yEnd >= r.yEnd)
        {
        ...
        else
        {
            ...
            if (r.xStart > otherR.xStart && r.xStart < otherR.xEnd && r.xEnd > otherR.xStart && r.xEnd < otherR.xEnd &&
                r.yStart < otherR.yStart && r.yStart > otherR.yEnd)
            {
            ...
            if (otherR.xStart > r.xStart && otherR.xStart < r.xEnd && otherR.xEnd > r.xStart && otherR.xEnd < r.xEnd &&
                r.yStart < otherR.yStart && r.yStart > otherR.yEnd)
            {
            ...
            if (r.xStart > otherR.xStart && r.xStart < otherR.xEnd && r.xEnd > otherR.xStart && r.xEnd < otherR.xEnd &&
                r.yEnd < otherR.yStart && r.yEnd > otherR.yEnd)
            {
            ...
            if (otherR.yStart < r.yStart && otherR.yStart > r.yEnd && otherR.yEnd < r.yStart && otherR.yEnd > r.yEnd &&
                r.xStart > otherR.xStart && r.xStart < otherR.xEnd)
            {
            ...
            if (otherR.yStart < r.yStart && otherR.yStart > r.yEnd && otherR.yEnd < r.yStart && otherR.yEnd > r.yEnd &&
                r.xEnd > otherR.xStart && r.xEnd < otherR.xEnd)
            {
            ...
            if (r.xStart > otherR.xStart && r.xStart < otherR.xEnd && r.yStart < otherR.yStart &&
                r.yStart > otherR.yEnd)
            {
            ...
            if (r.xEnd > otherR.xStart && r.xEnd < otherR.xEnd && r.yStart < otherR.yStart && r.yStart > otherR.yEnd)
            {
            ...
            if (r.xStart > otherR.xStart && r.xStart < otherR.xEnd && r.yEnd < otherR.yStart && r.yEnd > otherR.yEnd)
            {
            ...
            if (r.xEnd > otherR.xStart && r.xEnd < otherR.xEnd && r.yEnd < otherR.yStart && r.yEnd > otherR.yEnd)
            {
...
void EditorSpriteTrimPage::drawUnderComponents(doublePair inViewCenter, double inViewSize)
{
    if (mPickedSprite != -1)
    {
        ...
        if (h != NULL)
        {
            ...
            for (int i = 0; i < mRects.size(); i++)
            {
                ...
                drawRect(r.xStart, r.yStart, r.xEnd, r.yEnd);
            }
            ...
            if (mPickingRect)
            {
                ...
                drawRect(rect.xStart, rect.yStart, rect.xEnd, rect.yEnd);
            }
            else
            {
                ...
                if (mFreehandSplitMode)
                {
                    ...
                    drawRect(lastMouseX + mCursorOffsetX - rA, lastMouseY + mCursorOffsetY + rA,
                             lastMouseX + mCursorOffsetX + rB, lastMouseY + mCursorOffsetY - rB);
                }
            ...
            doublePair zoomPos = {lastMouseX + mCursorOffsetX, lastMouseY + mCursorOffsetY};
...
char EditorSpriteTrimPage::isPointInSprite(int inX, int inY)
{
    if (mPickedSprite != -1)
    {
        ...
        if (h != NULL)
        {
            ...
            if (inX >= -r->w / 2 && inX <= r->w / 2 && inY >= -r->h / 2 && inY <= r->h / 2)
            {
...
void EditorSpriteTrimPage::addPointToSelection(int inX, int inY, double inVal)
{
...
void EditorSpriteTrimPage::pointerUp(float inX, float inY)
{
    ...
    if (mPickingRect)
    {
        ...
        PickedRect r = {
            mPickStartX, mPickStartY, mPickEndX, mPickEndY, {false, false, false, false}, {false, false, false, false}};
...
void EditorSpriteTrimPage::keyDown(unsigned char inASCII)
{
    ...
    if (mFreehandSplitMode)
    {
        ...
        if (inASCII == 13)
        {
            ...
            addPointToSelection(lastMouseX + mCursorOffsetX, lastMouseY + mCursorOffsetY, 1.0);
        }
        else if (inASCII == 8)
        {
            // backspace
            addPointToSelection(lastMouseX + mCursorOffsetX, lastMouseY + mCursorOffsetY, 0);
        }
    }
}
