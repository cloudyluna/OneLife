...
EditorImportPage::EditorImportPage()
    : mImportButton(smallFont, +170, 280, "Sprite Import"), mImportLinesButton(smallFont, +170, 240, "Lines Import"),
      mNextSpriteImportButton(smallFont, +240, 280, ">"), mPrevSpriteImportButton(smallFont, +100, 280, "<"),
      mNextLinesImportButton(smallFont, +240, 240, ">"), mPrevLinesImportButton(smallFont, +100, 240, "<"),
      mCurrentSpriteImportCacheIndex(0), mCurrentLinesImportCacheIndex(0), mImportPathOverride(NULL),
      mXTopLinesButton(smallFont, +280, 240, "X"), mImportOverlayButton(smallFont, +370, 260, "Overlay Import"),
      mSelect(false), mImportedSheet(NULL), mImportedSheetSprite(NULL), mWhiteOutSheet(NULL),
      mWhiteOutSheetSprite(NULL), mProcessedSelection(NULL), mProcessedSelectionSprite(NULL), mProcessedShadow(NULL),
      mProcessedShadowSprite(NULL), mSelectionMultiplicative(false),
      mShadowSlider(smallFont, 90, 200, 2, 100, 20, 0, 1, "Shadow"), mSolidCheckbox(305, 200, 2),
      mBlackLineThresholdSlider(smallFont, 90, 170, 2, 100, 20, 0, 1, "Black Threshold"),
      mBlackLineThresholdDefaultButton(smallFont, 305, 170, "D"),
      mPaperThresholdSlider(smallFont, 90, 140, 2, 100, 20, 0, 1, "Paper Threshold"),
      mPaperThresholdDefaultButton(smallFont, 305, 140, "D"),
      mSaturationSlider(smallFont, 90, 110, 2, 100, 20, -1, 2, "Saturation"),
      mSaturationDefaultButton(smallFont, 305, 110, "D"),
      mSpriteTagField(mainFont, 0, -260, 6, false, "Tag", NULL, " "), mSaveSpriteButton(mainFont, 210, -260, "Save"),
      mSaveOverlayButton(smallFont, 310, -260, "Save Overlay"), mInvertButton(smallFont, 210, -320, "Invert"),
      mInvertColors(false), mSpritePicker(&spritePickable, -410, 90), mOverlayPicker(&overlayPickable, 410, 90),
      mSpriteTrimEditorButton(mainFont, -460, 260, "Trim"), mObjectEditorButton(mainFont, 0, 260, "Objects"),
      mCenterMarkSprite(loadSprite("centerMark.tga")), mInternalPaperMarkSprite(loadSprite("internalPaperMark.tga")),
      mCenterSet(true), mClearRotButton(smallFont, -400, -280, "0 Rot"),
      mClearScaleButton(smallFont, -400, -240, "1 Scale"), mFlipOverlayButton(smallFont, -330, -280, "Flip H"),
      mClearOverlayButton(smallFont, -330, -240, "X Ovly"), mShowTagMessage(false), mAKeyDown(false)
{
...
{
    if (inTarget == &mImportButton || inTarget == &mImportLinesButton || inTarget == &mImportOverlayButton)
    {
        ...
        if (inTarget == &mImportButton || inTarget == &mImportLinesButton)
        {
            ...
            {
                importPath = SettingsManager::getStringSetting("editorImportPath");
        ...
        if (importFile != NULL)
        {
            ...
            if (importFile->exists())
            {
                ...
                if (image != NULL)
                {
                    ...
                    if (inTarget == &mImportButton || inTarget == &mImportLinesButton)
                    {
                        ...
                        if (!loadedFromCache)
                        {
                            ...
                            if (cacheDir.exists() && cacheDir.isDirectory())
                            {
                                ...
                                char *cacheName = autoSprintf("%d.png", nextIndex);

                                File *cacheFile = cacheDir.getChildFile(cacheName);
                                ...
                                if (nextIndex != 0)
                                {
                                    char *lastCacheName = autoSprintf("%d.png", nextIndex - 1);

                                    File *lastCacheFile = cacheDir.getChildFile(lastCacheName);

                                    delete[] lastCacheName;

                                    if (importFile->contentsMatches(lastCacheFile))
                                    {
                                ...
                                if (!alreadyThere)
                                {
                                    ...
                                    if (inTarget == &mImportButton)
                                    {
                                        mCurrentSpriteImportCacheIndex = nextIndex;
                                    }
                                    else
                                    {
                                        mCurrentLinesImportCacheIndex = nextIndex;
                                    }
                                ...
                                else
                                {
                                    if (inTarget == &mImportButton)
                                    {
                                        mCurrentSpriteImportCacheIndex = nextIndex - 1;
                                    }
                                    else
                                    {
                                        mCurrentLinesImportCacheIndex = nextIndex - 1;
                                    }
                                }
                    ...
                    if (inTarget == &mImportButton || inTarget == &mImportOverlayButton)
                    {
                    ...
                    else
                    {
                        ...
                        int numPixels = expanded->getWidth() * expanded->getHeight();
    ...
    else if (inTarget == &mPrevSpriteImportButton || inTarget == &mNextSpriteImportButton ||
             inTarget == &mPrevLinesImportButton || inTarget == &mNextLinesImportButton)
    {

        int increment = -1;

        if (inTarget == &mNextSpriteImportButton || inTarget == &mNextLinesImportButton)
        {
            increment = +1;
        }
        ...
        if (inTarget == &mPrevLinesImportButton || inTarget == &mNextLinesImportButton)
        {
            cacheName = "lineImportCache";
        ...
        if (cacheDir.exists() || cacheDir.isDirectory())
        {
            ...
            *currentIndex += increment;
            ...
            if (mImportPathOverride != NULL)
            {
                ...
                if (inTarget == &mNextSpriteImportButton || inTarget == &mPrevSpriteImportButton)
                {
    ...
    else if (inTarget == &mSaveSpriteButton)
    {
        ...
        if (strcmp(tag, "") != 0)
        {
            ...
            if (mShadowSlider.isVisible() && mShadowSlider.getValue() > 0)
            {

                addShadow(mProcessedSelection, mProcessedShadow, mShadowSlider.getValue());
            ...
            int newID = addSprite(tag, mProcessedSelectionSprite, mProcessedSelection, mSelectionMultiplicative,
                                  mProcessedCenterOffset.x, mProcessedCenterOffset.y);
    ...
    {
        *(mOverlayFlip.getLastElement()) = !(mOverlayFlip.getLastElementDirect());
    }
    else if (inTarget == &mClearOverlayButton)
    {
        ...
        {
            mClearScaleButton.setVisible(mOverlayScale.getLastElementDirect() != 1);
            mClearRotButton.setVisible(mOverlayRotation.getLastElementDirect() != 0);
        }
    }
...
void EditorImportPage::drawUnderComponents(doublePair inViewCenter, double inViewSize)
{
    ...
    if (mImportedSheetSprite != NULL)
    {
        ...
        for (int i = 0; i < mLinesOffset.size(); i++)
        {
            doublePair pos = add(mSheetOffset, mLinesOffset.getElementDirect(i));
        ...
        if (mSelect)
        {
            ...
            drawRect(mSelectStart.x, mSelectStart.y, mSelectEnd.x, mSelectEnd.y);
        }
        ...
        {
            drawSprite(mInternalPaperMarkSprite, add(mSheetOffset, mInternalPaperPoints.getElementDirect(i)));
        }
    }
    ...
    for (int i = 0; i < mCurrentOverlay.size(); i++)
    {
        ...
        drawSprite(mCurrentOverlay.getElementDirect(i)->thumbnailSprite, mOverlayOffset.getElementDirect(i),
                   mOverlayScale.getElementDirect(i), mOverlayRotation.getElementDirect(i),
                   mOverlayFlip.getElementDirect(i));
    }
    toggleMultiplicativeBlend(false);
}

void EditorImportPage::draw(doublePair inViewCenter, double inViewSize)
{
    ...
    if (mOverlayScale.getLastElementDirect() != 1.0)
    {
        ...
        char *string = autoSprintf("Scale: %.3f", mOverlayScale.getLastElementDirect());
    ...
    if (mOverlayRotation.getLastElementDirect() != 0)
    {
        ...
        char *string = autoSprintf("Rot: %.3f", mOverlayRotation.getLastElementDirect());
...
void EditorImportPage::pointerMove(float inX, float inY)
{
    ...
    if (mCurrentOverlay.size() > 0)
    {
        ...
        if (mMovingOverlay)
        {
            ...
            *(mOverlayOffset.getLastElement()) = sub(pos, mMovingOverlayPointerStart);
        }
        if (mScalingOverlay)
        {
            doublePair pos = {inX, inY};
            *(mOverlayScale.getLastElement()) = mMovingOverlayScaleStart + 0.75 * mMovingOverlayScaleStart *
                                                                               (pos.y - mMovingOverlayPointerStart.y) /
                                                                               100;
        ...
        if (mRotatingOverlay)
        {
            ...
            *(mOverlayRotation.getLastElement()) =
                mMovingOverlayRotationStart + (pos.x - mMovingOverlayPointerStart.x) / 400;
        }
    }
}
...
void EditorImportPage::pointerDown(float inX, float inY)
{
    ...
    if (mPlacingInternalPaper)
    {
        ...
        doublePair pos = {inX - mSheetOffset.x, inY - mSheetOffset.y};
    ...
    if (mWhitingOut && mWhiteOutSheet != NULL)
    {
        ...
        if (imY > r && imY < mSheetH - r && imX > r && imX < mSheetW - r)
        {
    ...
    // middle of screen?
    if ((inX > -310 && inX < 310 && inY > -210 && inY < 90) ||
    ...
    if (inX - mSheetOffset.x > -mSheetW / 2 && inX - mSheetOffset.x < mSheetW / 2 &&
        inY - mSheetOffset.y > -mSheetH / 2 && inY - mSheetOffset.y < mSheetH / 2)
    {
...
void EditorImportPage::keyDown(unsigned char inASCII)
{
    ...
    {
        mMovingOverlay = true;
        mMovingOverlayPointerStart.x = lastMouseX - mOverlayOffset.getLastElementDirect().x;
        mMovingOverlayPointerStart.y = lastMouseY - mOverlayOffset.getLastElementDirect().y;
    }
    else if (inASCII == 's')
    {
    ...
    else if (inASCII == 'r')
    {
        ...
        if (mCurrentOverlay.size() > 0)
        {
            mMovingOverlayRotationStart = mOverlayRotation.getLastElementDirect();
        }
    }
    else if (inASCII == 'c')
    {
    ...
    else if (inASCII == 'x')
    {
        if (mImportedSheet != NULL)
        {
            ...
            if (startImX >= 0 && startImX < mSheetW && startImY >= 0 && startImY < mSheetH)
            {

                Color c = mImportedSheet->getColor(startImY * mSheetW + startImX);
...
void EditorImportPage::processSelection()
{
    ...
    if (mCenterPoint.x >= mSelectStart.x && mCenterPoint.x < mSelectEnd.x && mCenterPoint.y <= mSelectStart.y &&
    ...
    for (int i = 0; i < mInternalPaperPoints.size(); i++)
    {
        ...
        if (point.x >= 0 && point.x < imW && point.y >= 0 && point.y < imH)
        {
    ...
    Image *cutImage = mImportedSheet->getSubImage(startImX, startImY, imW, imH);
    ...
    if (mAnyWhiteOutSet)
    {
        ...
        cutWhiteout = mWhiteOutSheet->getSubImage(startImX, startImY, imW, imH);
    ...
    for (int i = 0; i < trimmedPaperPoints.size(); i++)
    {
        ...
        whitePixelsWithUnexploredNeighbors.push_back(lrint(p.y) * imW + lrint(p.x));
    }
    ...
    while (!done)
    {
        ...
        if (numFrontier > 0)
        {
            ...
            foundI = whitePixelsWithUnexploredNeighbors.getElementDirect(numFrontier - 1);
            whitePixelsWithUnexploredNeighbors.deleteElement(numFrontier - 1);
        }
            ...
            for (int n = 0; n < 4; n++)
            {
                ...
                if (nX >= 0 && nX < w && nY >= 0 && nY < h)
                {
                    ...
                    if (whiteMap[nI] == -1)
                    {
                        ...
                        // is it white?
                        if (r[nI] > threshold && g[nI] > threshold && b[nI] > threshold)
                        {
    ...
    if (!mSelectionMultiplicative)
        for (int i = 0; i < numPixels; i++)
        {
            if (whiteMap[i] == 1)
            {
                ...
                if (r[i] <= paperThreshold && g[i] <= paperThreshold && b[i] <= paperThreshold)
                {
                    ...
                    for (int n = 0; n < 4; n++)
                    {
                        ...
                        if (nX >= 0 && nX < w && nY >= 0 && nY < h)
                        {
                            ...
                            if (whiteMap[nI] != 1 || (rCopy[nI] <= paperThreshold && gCopy[nI] <= paperThreshold &&
    ...
    for (int i = 0; i < mCurrentOverlay.size(); i++)
    {
        ...
        double cosAngle = cos(-2 * M_PI * mOverlayRotation.getElementDirect(i));
        double sinAngle = sin(-2 * M_PI * mOverlayRotation.getElementDirect(i));
        ...
        int offsetW = (overW - sheetW) / 2 - (int)mOverlayOffset.getElementDirect(i).x + (int)mSheetOffset.x;
        int offsetH = (overH - sheetH) / 2 + (int)mOverlayOffset.getElementDirect(i).y - (int)mSheetOffset.y;
        ...
        for (int c = 0; c < 3; c++)
        {
            double *overC = mCurrentOverlay.getElementDirect(i)->image->getChannel(c);
            ...
            for (int y = 0; y < cutH; y++)
            {
                ...
                double overScaledY = (overY - overH / 2) / mOverlayScale.getElementDirect(i);
                ...
                for (int x = 0; x < cutW; x++)
                {
                    ...
                    // scale and rotate relative to center
                    double overScaledX = (overX - overW / 2) / mOverlayScale.getElementDirect(i);
                    ...
                    if (mOverlayRotation.getElementDirect(i) != 0)
                    {
                        ...
                        double rotX = overScaledX * cosAngle - overScaledY * sinAngle;
                        double rotY = overScaledX * sinAngle + overScaledY * cosAngle;
                    ...
                    if (overScaledFinalY >= 0 && overScaledFinalY < overH - 1)
                    {

                        if (overScaledFinalX >= 0 && overScaledFinalX < overW - 1)
                        {
                            ...
                            if (mOverlayScale.getElementDirect(i) == 1 && mOverlayRotation.getElementDirect(i) == 0)
                            {
                                // no interp needed
                            ...
                            else
                            {
                                ...
                                double cornerA1 = overC[floorX + floorY * overW];
                                double cornerA2 = overC[ceilX + floorY * overW];

                                double cornerB1 = overC[floorX + ceilY * overW];

                                double cornerB2 = overC[ceilX + ceilY * overW];
                                ...
                                double blend = bottomBlend * yOffset + (1 - yOffset) * topBlend;
    ...
    if (lastX > firstX && lastY > firstY)
    {
        ...
        trimmedImage = cutImage->getSubImage(firstX, firstY, 1 + lastX - firstX, 1 + lastY - firstY);

        delete cutImage;

        if (cutLinesImage != NULL)
        {
            trimmedLinesImage = cutLinesImage->getSubImage(firstX, firstY, 1 + lastX - firstX, 1 + lastY - firstY);
    ...
    if (!mSelectionMultiplicative)
    {
        ...
        if (trimmedLinesImage != NULL)
        {
            ...
            shadowLinesImage = trimmedLinesImage->expandImage(w + 6 + 6, h + 6 + 6);
    ...
    */
    ...
    if (expandedLines != NULL)
    {
        ...
        int numPixels = expandedLines->getWidth() * expandedLines->getHeight();
    ...
    if (mSelectionMultiplicative)
    {
        ...
        int numPixels = mProcessedSelection->getWidth() * mProcessedSelection->getHeight();
        ...
        for (int i = 0; i < numPixels; i++)
        {
            ...
            else if (r[i] > paperThreshold && g[i] > paperThreshold && b[i] > paperThreshold)
            {
