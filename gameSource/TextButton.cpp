...
TextButton::TextButton(Font *inDisplayFont, double inX, double inY, const char *inLabelText)
    : Button(inX, inY,
             // leave w and h unset for now
             0, 0,
             // pix width
             inDisplayFont->getFontHeight() / 8),
      mFont(inDisplayFont), mLabelText(NULL)
{
...
void TextButton::setPadding(double inHorizontalPadding, double inVerticalPadding)
{
    mWide = mFont->measureString(mLabelText) + 2 * (inHorizontalPadding + mPixWidth);
