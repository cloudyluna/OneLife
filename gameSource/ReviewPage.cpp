...
ReviewPage::ReviewPage(const char *inReviewServerURL)
    : ServerActionPage(inReviewServerURL, "submit_review", false),
      mReviewNameField(mainFont, -242, 250, 10, false, translate("reviewName"),
                       "abcdefghijklmnopqrstuvwxyz"
                       "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
                       "1234567890"
                       " ._-"),
      mReviewTextArea(mainFont, mainFontReview, 0, 0, 800, 320, false, translate("reviewText"),
                      "abcdefghijklmnopqrstuvwxyz"
                      "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
                      "1234567890"
                      " !?$%*&()+-='\":;,.\r",
                      NULL),
      mSpellcheckButton(396, -200, 4), mBackButton(mainFont, -526, -140, translate("backButton")),
      mPostButton(mainFont, 526, -140, translate("postButton")),
      mRemoveButton(mainFont, 526, -40, translate("removeButton")), mCopyButton(mainFont, -526, 140, translate("copy")),
      mPasteButton(mainFont, -526, 40, translate("paste")), mClearButton(mainFont, 526, 140, translate("clear")),
      mGettingSequenceNumber(false), mRemoving(false)
{

    const char *choiceList[2] = {translate("recommendYes"), translate("recommendNo")};

    mRecommendChoice = new RadioButtonSet(mainFont, 396, 258, 2, choiceList, false, 4);
...
void ReviewPage::actionPerformed(GUIComponent *inTarget)
{
    ...
    else if (inTarget == &mPostButton)
    {
        ...
        char *stringToHash = autoSprintf("%d%s%s", reviewScore, nameSHA1, textSHA1);
...
void ReviewPage::draw(doublePair inViewCenter, double inViewSize)
{
...
void ReviewPage::makeActive(char inFresh)
{
    ...
    int reviewRecommend = SettingsManager::getIntSetting("reviewRecommend", 1);
