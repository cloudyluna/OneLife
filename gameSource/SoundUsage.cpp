...
SoundUsage scanSoundUsage(char *inString)
{
    ...
    if (idVector.size() > 0)
    {
        ...
        if (idVector.size() == 1 && idVector.getElementDirect(0) == -1)
        {
            return blankSoundUsage;
        }

        SoundUsage u = {idVector.size(), idVector.getElementArray(), volVector.getElementArray()};
        return u;
    }
    else
    {
...
const char *printSoundUsage(SoundUsage inUsage)
{
    ...
    for (int i = 0; i < inUsage.numSubSounds; i++)
    {
        ...
        numPrinted += sprintf(&(printBuffer[numPrinted]), "%s%d:%f", sep, inUsage.ids[i], inUsage.volumes[i]);
    }
...
SoundUsage copyUsage(SoundUsage inUsage)
{
    ...
    memcpy(out.volumes, inUsage.volumes, out.numSubSounds * sizeof(double));
...
static int shufflePlayOrdering[MAX_SHUFFLE_PLAY_SOUNDS][MAX_SHUFFLE_PLAY_SOUNDS];
...
SoundUsagePlay playRandom(SoundUsage inUsage)
{
    ...
    if (!shufflePlayReady[inUsage.numSubSounds] || nextShufflePlaySlot[inUsage.numSubSounds] >= inUsage.numSubSounds)
    {
    ...
    if (regenOrder)
    {
        ...
        for (int j = inUsage.numSubSounds - 1; j >= 1; j--)
        {
            ...
            shufflePlayOrdering[inUsage.numSubSounds][k] = shufflePlayOrdering[inUsage.numSubSounds][j];
    ...
    int pick = shufflePlayOrdering[inUsage.numSubSounds][nextShufflePlaySlot[inUsage.numSubSounds]];
...
void addSound(SoundUsage *inUsage, int inID, double inVolume)
{
    ...
    if (inUsage->numSubSounds > 0)
    {
        ...
        memcpy(newVols, inUsage->volumes, inUsage->numSubSounds * sizeof(double));
    }
...
void removeSound(SoundUsage *inUsage, int inIndex)
{
    if (inUsage->numSubSounds > inIndex)
    {
        ...
        {
            // after removed
            memcpy(&(newIDs[inIndex]), &(inUsage->ids[inIndex + 1]),
                   (inUsage->numSubSounds - inIndex - 1) * sizeof(int));

            memcpy(&(newVols[inIndex]), &(inUsage->volumes[inIndex + 1]),
                   (inUsage->numSubSounds - inIndex - 1) * sizeof(double));
        }
        clearSoundUsage(inUsage);
...
char equal(SoundUsage inA, SoundUsage inB)
{
    if (inA.numSubSounds == inB.numSubSounds)
    {
        ...
        {
            if (inA.ids[i] != inB.ids[i] || inA.volumes[i] != inB.volumes[i])
            {
