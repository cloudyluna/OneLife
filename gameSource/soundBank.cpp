...
void setVolumeScaling(int inMaxSimultaneousSoundEffects, double inMusicHeadroom)
{
...
void setSoundEffectsLoudness(double inLoudness)
{
    ...
    SettingsManager::setSetting("soundEffectsLoudness", (float)soundEffectsLoudness);
}
...
{
    int numBytes;
    unsigned char *data = inFile->readFileContents(&numBytes);
...
static void writeAiffFile(File *inFile, int16_t *inSamples, int inNumSamples)
{
    int headerLength;
    unsigned char *header = getAIFFHeader(1, 16, sampleRate, inNumSamples, &headerLength);
...
static int16_t *generateWetConvolve(MultiConvolution inMulti, int inNumSamples, int16_t *inSamples,
                                    int *outNumWetSamples)
{
    ...
    multiConvolve(inMulti, sampleFloats, inNumSamples, wetSampleFloats);
    ...
    for (int i = 0; i < numWetSamples; i++)
    {
        wetSamples[i] = (int16_t)(lrint(32767 * normalizeFactor * wetSampleFloats[i]));
    }
...
static void generateReverb(SoundRecord *inRecord, File *inReverbFolder)
{
    ...
    if (!cacheFile->exists())
    {
        ...
        if (samples != NULL)
        {
            ...
            int16_t *wetSamples = generateWetConvolve(reverbConvolution, numSamples, samples, &numWetSamples);
            delete[] samples;
...
int initSoundBankStart(char *outRebuildingCache)
{
    ...
    soundEffectsLoudness = SettingsManager::getFloatSetting("soundEffectsLoudness", 1.0);

    soundEffectsOff = SettingsManager::getIntSetting("soundEffectsOff", 0);

    SettingsManager::setSetting("soundEffectsOff", soundEffectsOff);

    sampleRate = SettingsManager::getIntSetting("soundSampleRate", 44100);
    maxID = 0;
    ...
    soundCache = initBinFolderCache("sounds", ".aiff|.ogg", &rebuildingSounds);

    reverbCache = initBinFolderCache("reverbCache", ".aiff", &rebuildingReverbs);
    ...
    if (reverbFile.exists())
    {
        ...
        if (reverbFolder->exists() && reverbFolder->isDirectory())
        {
            ...
            int16_t *reverbSamples = readAIFFFile(&reverbFile, &numReverbSamples);
            ...
            if (reverbSamples != NULL)
            {
                ...
                reverbConvolution = startMultiConvolution(reverbFloats, numReverbSamples);
    ...
    *outRebuildingCache = rebuildingSounds || rebuildingReverbs || (reverbsToRegenerate.size() > 0);

    return reverbsToRegenerate.size() + soundCache.numFiles + reverbCache.numFiles;
}

float initSoundBankStep()
{

    if (currentSoundFile == soundCache.numFiles && nextReverbToRegenerate == reverbsToRegenerate.size() &&
    ...
    if (currentSoundFile < soundCache.numFiles)
    {
        ...
        if (strstr(fileName, ".aiff") != NULL || strstr(fileName, ".ogg") != NULL)
        {
            char added = false;
            ...
            int soundDataLength;
            unsigned char *soundData = getFileContents(soundCache, i, fileName, &soundDataLength);
            ...
            if (soundData != NULL)
            {
                ...
                if (strstr(fileName, ".aiff") != NULL)
                {
                    samples = readMono16AIFFData(soundData, soundDataLength, &numSamples);
                }
    ...
    else if (nextReverbToRegenerate < reverbsToRegenerate.size())
    {
        ...
        if (nextReverbToRegenerate == reverbsToRegenerate.size())
        {
            ...
            reverbCache = initBinFolderCache("reverbCache", ".aiff", &rebuilding);
        }
    }
    else if (currentReverbFile < reverbCache.numFiles)
    {
        ...
        if (strstr(fileName, ".aiff") != NULL)
        {
            ...
            int aiffDataLength;
            unsigned char *aiffData = getFileContents(reverbCache, i, fileName, &aiffDataLength);
            ...
            if (aiffData != NULL)
            {
                ...
                if (r != NULL)
                {
                    ...
                    int numSamples;
                    int16_t *samples = readMono16AIFFData(aiffData, aiffDataLength, &numSamples);
    ...
    return (float)(currentSoundFile + currentReverbFile + nextReverbToRegenerate) /
           (float)(soundCache.numFiles + reverbCache.numFiles + reverbsToRegenerate.size());
}
...
static void loadSound(int inID)
{
    ...
    if (r != NULL)
    {
        ...
        if (r->sound == NULL && !r->loading)
        {
            ...
            loadingR.asyncReverbLoadHandle = startAsyncFileRead(fullReverbName);
...
static void freeSoundRecord(int inID)
{
    if (inID < mapSize)
    {
        if (idMap[inID] != NULL)
        {
            ...
            if (idMap[inID]->sound != NULL || idMap[inID]->reverbSound != NULL)
            {
...
void stepSoundBank()
{
    ...
    for (int i = 0; i < loadingSounds.size(); i++)
    {
            ...
            checkAsyncFileReadDone(loadingR->asyncReverbLoadHandle))
        {
            ...
            int lengthSound;
            unsigned char *dataSound = getAsyncFileData(loadingR->asyncSoundLoadHandle, &lengthSound);

            int lengthReverb;
            unsigned char *dataReverb = getAsyncFileData(loadingR->asyncReverbLoadHandle, &lengthReverb);
            ...
            if (dataSound == NULL)
            {
                ...
                setLoadingFailureFileName(autoSprintf("sounds/%d.aiff", loadingR->soundID));
            }
            else if (dataReverb == NULL)
            {
                printf("Reading reverb data from cache failed, sound ID %d\n", loadingR->soundID);

                setLoadingFailureFileName(autoSprintf("reverbCache/%d.aiff", loadingR->soundID));
            }
            else
            {

                int numSamples;
                int16_t *samples = readMono16AIFFData(dataSound, lengthSound, &numSamples);
                ...
                samples = readMono16AIFFData(dataReverb, lengthReverb, &numSamples);
...
void playSound(int inID, double inVolumeTweak, double inStereoPosition, double inReverbMix)
{
    if (soundEffectsOff)
    {
        return;
    ...
    if (inID < mapSize)
    {
        if (idMap[inID] != NULL)
        {
            ...
            if (reverbDisabled || idMap[inID]->reverbSound == NULL)
            {
                ...
                playSoundSprite(idMap[inID]->sound, soundEffectsLoudness * inVolumeTweak * playedSoundVolumeScale,
                                inStereoPosition);
            }
            else
            {
                //  play both simultaneously
                double volume = soundEffectsLoudness * inVolumeTweak * playedSoundVolumeScale;

                SoundSpriteHandle handles[] = {idMap[inID]->sound, idMap[inID]->reverbSound};
                double volumes[] = {volume * (1 - inReverbMix), volume};
...
void playSound(SoundUsage inUsage, double inStereoPosition, double inReverbMix)
{
...
{
    return 0.5 + 0.5 * (-(inDstance - (2 + minFadeStartDistance)) / (1 + fabs(inDstance - (2 + minFadeStartDistance))));
}

static double sigmoidF(double inDstance)
{
    return (1 / (sigmoidH(0) - sigmoidH(maxAudibleDistance))) * (sigmoidH(inDstance) - sigmoidH(maxAudibleDistance));
}

// returns true if sound should play
static char getVolumeAndPan(doublePair inVectorFromCameraToSoundSource, double *outVolume, double *outPan)
{
...
void playSound(SoundUsage inUsage, doublePair inVectorFromCameraToSoundSource)
{

    double volume, pan;

    char shouldPlay = getVolumeAndPan(inVectorFromCameraToSoundSource, &volume, &pan);
    ...
    double reverbMix = (1.0 - reverbContstant) * (1.0 - volume) + reverbContstant;

    SoundUsagePlay p = playRandom(inUsage);

    playSound(p.id, volume * p.volume, pan, reverbMix);
}

void playSound(SoundSpriteHandle inSoundSprite, double inVolumeTweak, doublePair inVectorFromCameraToSoundSource)
{
    double volume, pan;
    ...
    playSoundSprite(inSoundSprite, inVolumeTweak * soundEffectsLoudness * volume * playedSoundVolumeScale, pan);
}
...
int stopRecordingSound()
{
    ...
    if (soundsDir.exists() && soundsDir.isDirectory() && soundsRawDir.exists() && soundsRawDir.isDirectory())
    {

        int nextSoundNumber = 1;

        File *nextNumberFile = soundsDir.getChildFile("nextSoundNumber.txt");
        ...
        writeAiffFile(rawSoundFile, &(samples[finalStartPoint]), finalNumSamples);
        ...
        int16_t *wetSamples = generateWetConvolve(eqConvolution, finalNumSamples, &(samples[finalStartPoint]), &numWet);
    ...
    r->id = newID;
    r->sound = setSoundSprite(&(samples[finalStartPoint]), finalNumSamples);
    r->reverbSound = NULL;
...
void printOrphanedSoundReport()
{
    ...
    for (int i = 0; i < mapSize; i++)
    {
        if (i < mapSize)
        {
            if (idMap[i] != NULL)
            {
                if (!isSoundUsedByObject(i) && !isSoundUsedByAnim(i))
                {
