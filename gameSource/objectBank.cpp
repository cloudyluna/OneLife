    ...
    for (int i = 0; i <= MAX_RACE; i++)
    {
        if (racePersonObjectIDs[i].size() > 0)
        {
            ...
            for (int j = 0; j < num; j++)
            {
                ...
                if ((flip && boysLeft > 0) || girlsLeft == 0)
                {

                    racePersonObjectIDs[i].push_back(boys.getElementDirect(boyIndex));
                ...
                {
                    racePersonObjectIDs[i].push_back(girls.getElementDirect(girlIndex));
...
{
    setDrawColor(inColor.r, inColor.g, inColor.b, globalAlpha);
}

static char shouldFileBeCached(char *inFileName)
{
    if (strstr(inFileName, ".txt") != NULL && strstr(inFileName, "groundHeat_") == NULL &&
        strcmp(inFileName, "nextObjectNumber.txt") != 0 && strcmp(inFileName, "nextObjectNumberOffset.txt") != 0)
    {
        return true;
    }
    return false;
...
int initObjectBankStart(char *outRebuildingCache, char inAutoGenerateUsedObjects, char inAutoGenerateVariableObjects)
{
    maxID = 0;

    currentFile = 0;

    cache = initFolderCache("objects", outRebuildingCache, shouldFileBeCached);
...
char *boolArrayToSparseCommaString(const char *inLineName, char *inArray, int inLength)
{
...
void sparseCommaLineToBoolArray(const char *inExpectedLineName, char *inLine, char *inBoolArray, int inBoolArrayLength)
{
    ...
    for (int i = 0; i < numParts; i++)
    {
        ...
        if (scannedInt >= 0 && scannedInt < inBoolArrayLength)
        {
...
static void fillObjectBiomeFromString(ObjectRecord *inRecord, char *inBiomes)
{
...
static void setupEyesAndMouth(ObjectRecord *inR)
{
    ...
    if (r->person && isSpriteBankLoaded())
    {
        ...
        for (int i = 0; i < r->numSprites; i++)
        {
            ...
            if (r->spriteIsEyes[i] && r->spriteAgeStart[i] < 30 && r->spriteAgeEnd[i] > 30)
            {

                r->mainEyesOffset = sub(r->spritePos[i], r->spritePos[headIndex]);
            }
        }
    }
}
...
static void setupTapout(ObjectRecord *inR)
{
    ...
    if (triggerPos != NULL)
    {
        ...
        int numRead = sscanf(triggerPos, "+tapoutTrigger,%d,%d,%d,%d,%d,%d", &value1, &value2, &value3, &value4,
                             &value5, &value6);
    ...
    if (shouldFileBeCached(txtFileName))
    {
        ...
        if (objectText != NULL)
        {
            ...
            if (numLines >= 14)
            {
                ...
                int contRead = 0;
                sscanf(lines[next], "containable=%d", &(contRead));
                ...
                sscanf(lines[next], "containSize=%f,vertSlotRot=%lf", &(r->containSize),
                ...
                sscanf(lines[next], "permanent=%d,minPickupAge=%d,%d", &(permRead), &(r->minPickupAge),
                       &(r->maxPickupAge));
                ...
                if (strstr(lines[next], "noFlip=") != NULL)
                {
                    ...
                    sscanf(lines[next], "noFlip=%d", &(noFlipRead));
                ...
                if (strstr(lines[next], "sideAccess=") != NULL)
                {
                    ...
                    sscanf(lines[next], "sideAccess=%d", &(sideAccessRead));
                ...
                sscanf(lines[next], "heldInHand=%d", &(heldInHandRead));
                ...
                sscanf(lines[next],
                       ...
                       "drawBehindPlayer=%d",
                       &(blocksWalkingRead), &(r->leftBlockingRadius), &(r->rightBlockingRadius),
                ...
                int numRead = sscanf(lines[next], "mapChance=%f#biomes_%199s", &(r->mapChance), biomeString);
                ...
                r->heatValue = 0;
                sscanf(lines[next], "heatValue=%d", &(r->heatValue));

                next++;

                r->rValue = 0;
                sscanf(lines[next], "rValue=%f", &(r->rValue));

                next++;

                int personRead = 0;
                int noSpawnRead = 0;
                sscanf(lines[next], "person=%d,noSpawn=%d", &personRead, &noSpawnRead);
                ...
                int maleRead = 0;
                sscanf(lines[next], "male=%d", &(maleRead));
                ...
                sscanf(lines[next], "deathMarker=%d", &(deathMarkerRead));
                ...
                if (strstr(lines[next], "tapoutTrigger=") != NULL)
                {
                    ...
                    int numRead = sscanf(lines[next], "tapoutTrigger=%d#%d,%d,%d,%d,%d,%d", &(tapoutTriggerRead),
                                         &value1, &value2, &value3, &value4, &value5, &value6);

                    if (tapoutTriggerRead == 1 && numRead >= 3 && numRead <= 7)
                    {
                        // valid tapout trigger
                        TapoutRecord tr;
                ...
                sscanf(lines[next], "foodValue=%d,%d", &(r->foodValue), &(r->bonusValue));
                ...
                sscanf(lines[next], "speedMult=%f", &(r->speedMult));
                ...
                {
                    sscanf(lines[next], "containOffset=%d,%d", &(r->containOffsetX), &(r->containOffsetY));
                ...
                sscanf(lines[next], "heldOffset=%lf,%lf", &(r->heldOffset.x), &(r->heldOffset.y));

                next++;

                r->clothing = 'n';

                sscanf(lines[next], "clothing=%c", &(r->clothing));
                ...
                sscanf(lines[next], "clothingOffset=%lf,%lf", &(r->clothingOffset.x), &(r->clothingOffset.y));

                next++;

                r->deadlyDistance = 0;
                sscanf(lines[next], "deadlyDistance=%d", &(r->deadlyDistance));

                next++;

                r->useDistance = 1;

                if (strstr(lines[next], "useDistance=") != NULL)
                {
                    // use distance present

                    sscanf(lines[next], "useDistance=%d", &(r->useDistance));
                ...
                if (strstr(lines[next], "creationSoundInitialOnly=") != NULL)
                {
                    // flag present

                    int flagRead = 0;
                    sscanf(lines[next], "creationSoundInitialOnly=%d", &(flagRead));
                ...
                if (strstr(lines[next], "creationSoundForce=") != NULL)
                {
                    // flag present

                    int flagRead = 0;
                    sscanf(lines[next], "creationSoundForce=%d", &(flagRead));
                ...
                {
                    sscanf(lines[next], "numSlots=%d#timeStretch=%f", &(r->numSlots), &(r->slotTimeStretch));
                }
                else
                {
                ...
                r->slotSize = 1;
                sscanf(lines[next], "slotSize=%f", &(r->slotSize));
                ...
                if (strstr(lines[next], "slotStyle=") != NULL)
                {
                    ...
                    int flagRead = 0;
                    sscanf(lines[next], "slotStyle=%d", &(flagRead));
                ...
                r->slotsLocked = 0;
                if (strstr(lines[next], "slotsLocked=") != NULL)
                {
                    // flag present

                    int flagRead = 0;
                    sscanf(lines[next], "slotsLocked=%d", &(flagRead));
                ...
                if (strstr(lines[next], "slotsNoSwap=") != NULL)
                {
                    ...
                    int flagRead = 0;
                    sscanf(lines[next], "slotsNoSwap=%d", &(flagRead));
                ...
                for (int i = 0; i < r->numSlots; i++)
                {
                    ...
                    int vertRead = 0;
                    sscanf(lines[next], "slotPos=%lf,%lf,vert=%d,parent=%d", &(r->slotPos[i].x), &(r->slotPos[i].y),
                           &vertRead, &(r->slotParent[i]));
                ...
                if (r->description[0] == 'T' && r->description[1] == 'h' &&
                ...
                if (strstr(r->description, "monument") != NULL)
                {
                    ...
                    else if (strstr(r->description, "monumentDone") != NULL)
                    {
                        r->monumentDone = true;
                    }
                    else if (strstr(r->description, "monumentCall") != NULL)
                    {
                        r->monumentCall = true;
                ...
                for (int i = 0; i < r->numSprites; i++)
                {
                    sscanf(lines[next], "spriteID=%d", &(r->sprites[i]));

                    next++;

                    sscanf(lines[next], "pos=%lf,%lf", &(r->spritePos[i].x), &(r->spritePos[i].y));

                    next++;

                    sscanf(lines[next], "rot=%lf", &(r->spriteRot[i]));
                    ...
                    sscanf(lines[next], "color=%f,%f,%f", &(r->spriteColor[i].r), &(r->spriteColor[i].g),
                           &(r->spriteColor[i].b));

                    next++;

                    sscanf(lines[next], "ageRange=%lf,%lf", &(r->spriteAgeStart[i]), &(r->spriteAgeEnd[i]));

                    next++;

                    sscanf(lines[next], "parent=%d", &(r->spriteParent[i]));
                    ...
                    sscanf(lines[next], "invisHolding=%d,invisWorn=%d,behindSlots=%d", &invisRead, &invisWornRead,
                           &behindSlotsRead);
                ...
                if (strstr(lines[next], "spritesDrawnBehind=") != NULL)
                {
                    ...
                    sparseCommaLineToBoolArray("spritesDrawnBehind", lines[next], r->spriteBehindPlayer, r->numSprites);
                    next++;
                }
                ...
                if (strstr(lines[next], "spritesAdditiveBlend=") != NULL)
                {
                    ...
                    sparseCommaLineToBoolArray("spritesAdditiveBlend", lines[next], r->spriteAdditiveBlend,
                                               r->numSprites);
                    next++;
                }

                sparseCommaLineToBoolArray("headIndex", lines[next], r->spriteIsHead, r->numSprites);
                next++;

                sparseCommaLineToBoolArray("bodyIndex", lines[next], r->spriteIsBody, r->numSprites);
                next++;

                sparseCommaLineToBoolArray("backFootIndex", lines[next], r->spriteIsBackFoot, r->numSprites);
                next++;

                sparseCommaLineToBoolArray("frontFootIndex", lines[next], r->spriteIsFrontFoot, r->numSprites);
                next++;
                ...
                if (next < numLines)
                {
                    ...
                    sscanf(lines[next], "numUses=%d,%f", &(r->numUses), &(r->useChance));

                    next++;

                    if (next < numLines)
                    {
                        sparseCommaLineToBoolArray("useVanishIndex", lines[next], r->spriteUseVanish, r->numSprites);
                        next++;

                        if (next < numLines)
                        {
                            sparseCommaLineToBoolArray("useAppearIndex", lines[next], r->spriteUseAppear,
                                                       r->numSprites);
                            next++;
                        }
                    }
                ...
                if (next < numLines)
                {
                    sscanf(lines[next], "pixHeight=%d", &(r->cachedHeight));
                    next++;
                }
...
void initObjectBankFinish()
{
    ...
    if (false)
        for (int i = 0; i < femalePersonObjectIDs.size(); i++)
        {
            ...
            {
                printf("%d:  %d\n", personObjectIDs.getElementDirect(j), hitCounts[j]);
            }
            printf("\n\n");
        }
            ...
            if (idMap[i] != NULL)
            {
                ...
                if (o->numUses > 1)
                {
                    ...
                    for (int d = 1; d <= numDummyObj; d++)
                    {
                        ...
                        char *desc = autoSprintf("%s# use %d", o->description, d);
                        ...
                        {
                            AnimationRecord *a = getAnimation(mainID, (AnimType)t);
            ...
            if (idMap[i] != NULL)
            {
                ...
                if (dollarPos != NULL)
                {
                    ...
                    for (int d = 1; d <= numVar; d++)
                    {
                        ...
                        if (commentPos != NULL && targetPos != NULL && commentPos < targetPos)
                        {
                        ...
                        char found;
                        char *desc = replaceOnce(o->description, target, sub, &found);
                        delete[] sub;
                    ...
                    // call it "- ?" instead
                    char found;
                    char *desc = replaceOnce(o->description, target, "- ?", &found);
                ...
                if (triggerName != NULL)
                {
                    ...
                    for (int j = 0; j < mapSize; j++)
                    {
                        if (i != j && idMap[j] != NULL)
                        {
                            ...
                            if (!oJ->isGlobalTriggerOn && !oJ->isGlobalTriggerOff && !oJ->isGlobalReceiver &&
                                strstr(oJ->description, triggerName) == oJ->description)
                            {
                    ...
                    {
                        GlobalTrigger *gRecOld = globalTriggers.getElement(r);
            ...
            if (o->isGlobalReceiver)
            {
                ...
                if (triggerName != NULL)
                {
                    ...
                    for (int j = 0; j < mapSize; j++)
                    {
                        ...
                        {
                            ObjectRecord *oJ = idMap[j];
                            if (oJ->isGlobalTriggerOn && strstr(oJ->description, triggerName) != NULL)
                            {
                                // found this trigger sender
                                // they share the same global trigger index
                                o->globalTriggerIndex = oJ->globalTriggerIndex;
                            }
                        }
                    }
                    delete[] triggerName;
...
void setupSpriteUseVis(ObjectRecord *inObject, int inUsesRemaining, char *inSpriteSkipDrawing)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                // hide all appearing sprites as basis
                inSpriteSkipDrawing[appearingIndices.getElementDirect(i)] = true;
            }
        }
        ...
        if (numVanishingSprites > 0)
        {
            ...
            int numSpritesLeft = (d * (numVanishingSprites)) / numUses;

            int numInLastDummy = numVanishingSprites / numUses;

            int numInFirstDummy = ((numUses - 1) * numVanishingSprites) / numUses;
            ...
            for (int v = numSpritesLeft; v < numVanishingSprites; v++)
            {
                ...
                inSpriteSkipDrawing[vanishingIndices.getElementDirect(v)] = true;
            }
        }
        ...
        if (numAppearingSprites > 0)
        {
            ...
            int numInvisSpritesLeft = lrint((d * (numAppearingSprites)) / (double)numUses);
            ...
            */
            ...
            for (int v = 0; v < numAppearingSprites - numInvisSpritesLeft; v++)
            {
                ...
                inSpriteSkipDrawing[appearingIndices.getElementDirect(v)] = false;
            }
        }
    }
}
...
int reAddObject(ObjectRecord *inObject, char *inNewDescription, char inNoWriteToFile, int inReplaceID)
{
    ...
    int id = addObject(
        desc, inObject->containable, inObject->containSize, inObject->vertContainRotationOffset, inObject->permanent,
        inObject->noFlip, inObject->sideAccess, inObject->minPickupAge, inObject->maxPickupAge, inObject->heldInHand,
        inObject->rideable, inObject->ridingAnimationIndex, inObject->blocksWalking, inObject->leftBlockingRadius,
        inObject->rightBlockingRadius, inObject->drawBehindPlayer, inObject->spriteBehindPlayer,
        inObject->spriteAdditiveBlend, biomeString, inObject->mapChance, inObject->heatValue, inObject->rValue,
        inObject->person, inObject->personNoSpawn, inObject->male, inObject->race, inObject->deathMarker,
        inObject->homeMarker, inObject->isTapOutTrigger, tapoutTriggerParameters, inObject->floor, inObject->noCover,
        inObject->floorHugging, inObject->wallLayer, inObject->frontWall, inObject->foodValue, inObject->bonusValue,
        inObject->speedMult, inObject->containOffsetX, inObject->containOffsetY, inObject->heldOffset,
        inObject->clothing, inObject->clothingOffset, inObject->deadlyDistance, inObject->useDistance,
        inObject->creationSound, inObject->usingSound, inObject->eatingSound, inObject->decaySound,
        inObject->creationSoundInitialOnly, inObject->creationSoundForce, inObject->numSlots, inObject->slotSize,
        inObject->slotPos, inObject->slotStyle, inObject->slotVert, inObject->slotParent, inObject->slotTimeStretch,
        inObject->slotsLocked, inObject->slotsNoSwap, inObject->numSprites, inObject->sprites, inObject->spritePos,
        inObject->spriteRot, inObject->spriteHFlip, inObject->spriteColor, inObject->spriteAgeStart,
        inObject->spriteAgeEnd, inObject->spriteParent, inObject->spriteInvisibleWhenHolding,
        inObject->spriteInvisibleWhenWorn, inObject->spriteBehindSlots, inObject->spriteInvisibleWhenContained,
        inObject->spriteIgnoredWhenCalculatingCenterOffset, inObject->spriteIsHead, inObject->spriteIsBody,
        inObject->spriteIsBackFoot, inObject->spriteIsFrontFoot, inObject->numUses, inObject->useChance,
        inObject->spriteUseVanish, inObject->spriteUseAppear, inNoWriteToFile, inReplaceID, inObject->cachedHeight);
...
ObjectRecord **searchObjects(const char *inSearch, int inNumToSkip, int inNumToGet, int *outNumResults,
                             int *outNumRemaining)
{
        ...
        if (idMap[id] != NULL)
        {
            ...
            if (parent->numUses > 1 && parent->useDummyIDs != NULL)
            {
                for (int i = 0; i < parent->numUses - 1; i++)
                {
                    ...
                    if (dummy != NULL && results.size() < inNumToGet && i + 1 >= inNumToSkip)
    ...
    *outNumResults = tree.getMatches(lowerSearch, inNumToSkip, numToGet, (void **)results);
...
int addObject(const char *inDescription, char inContainable, float inContainSize, double inVertContainRotationOffset,
              char inPermanent, char inNoFlip, char inSideAccess, int inMinPickupAge, int inMaxPickupAge,
              char inHeldInHand, char inRideable, int inRidingAnimationIndex, char inBlocksWalking,
              int inLeftBlockingRadius, int inRightBlockingRadius, char inDrawBehindPlayer, char *inSpriteBehindPlayer,
              char *inSpriteAdditiveBlend, char *inBiomes, float inMapChance, int inHeatValue, float inRValue,
              char inPerson, char inPersonNoSpawn, char inMale, int inRace, char inDeathMarker, char inHomeMarker,
              char inTapoutTrigger, char *inTapoutTriggerParameters, char inFloor, char inPartialFloor,
              char inFloorHugging, char inWallLayer, char inFrontWall, int inFoodValue, int inBonusValue,
              float inSpeedMult, int inContainOffsetX, int inContainOffsetY, doublePair inHeldOffset, char inClothing,
              doublePair inClothingOffset, int inDeadlyDistance, int inUseDistance, SoundUsage inCreationSound,
              SoundUsage inUsingSound, SoundUsage inEatingSound, SoundUsage inDecaySound,
              char inCreationSoundInitialOnly, char inCreationSoundForce, int inNumSlots, float inSlotSize,
              doublePair *inSlotPos, int inSlotStyle, char *inSlotVert, int *inSlotParent, float inSlotTimeStretch,
              char inSlotsLocked, char inSlotsNoSwap, int inNumSprites, int *inSprites, doublePair *inSpritePos,
              double *inSpriteRot, char *inSpriteHFlip, FloatRGB *inSpriteColor, double *inSpriteAgeStart,
              double *inSpriteAgeEnd, int *inSpriteParent, char *inSpriteInvisibleWhenHolding,
              int *inSpriteInvisibleWhenWorn, char *inSpriteBehindSlots, char *inSpriteInvisibleWhenContained,
              char *inSpriteIgnoredWhenCalculatingCenterOffset, char *inSpriteIsHead, char *inSpriteIsBody,
              char *inSpriteIsBackFoot, char *inSpriteIsFrontFoot, int inNumUses, float inUseChance,
              char *inSpriteUseVanish, char *inSpriteUseAppear, char inNoWriteToFile, int inReplaceID,
              int inExistingObjectHeight, char inConsiderIDOffset)
{
    ...
    if (newHeight == -1)
    {
        newHeight = recomputeObjectHeight(inNumSprites, inSprites, inSpritePos);
    }
    ...
    if (objectsDir.exists() && objectsDir.isDirectory())
    {
        ...
        File *nextNumberFile = objectsDir.getChildFile("nextObjectNumber.txt");
    ...
    if (!inNoWriteToFile && objectsDir.exists() && objectsDir.isDirectory())
    {
        ...
        lines.push_back(autoSprintf("containSize=%f,vertSlotRot=%f", inContainSize, inVertContainRotationOffset));
        if (inMaxPickupAge != 9999999)
        {
            lines.push_back(
                autoSprintf("permanent=%d,minPickupAge=%d,%d", (int)inPermanent, inMinPickupAge, inMaxPickupAge));
        }
        else
        {
            lines.push_back(autoSprintf("permanent=%d,minPickupAge=%d", (int)inPermanent, inMinPickupAge));
        }
        ...
        if (inRidingAnimationIndex > -1)
            lines.push_back(autoSprintf("ridingAnimationIndex=%d", inRidingAnimationIndex));

        lines.push_back(autoSprintf("blocksWalking=%d,"
                                    "leftBlockingRadius=%d,"
                                    "rightBlockingRadius=%d,"
                                    "drawBehindPlayer=%d",
                                    (int)inBlocksWalking, inLeftBlockingRadius, inRightBlockingRadius,
                                    (int)inDrawBehindPlayer));

        lines.push_back(autoSprintf("mapChance=%f#biomes_%s", inMapChance, inBiomes));
        ...
        lines.push_back(autoSprintf("person=%d,noSpawn=%d", personNumber, (int)inPersonNoSpawn));
        ...
        if (inTapoutTrigger)
            lines.push_back(autoSprintf("tapoutTrigger=1#%s", inTapoutTriggerParameters));

        lines.push_back(autoSprintf("floor=%d", (int)inFloor));
        if (inPartialFloor)
            lines.push_back(autoSprintf("partialFloor=%d", (int)inPartialFloor));
        lines.push_back(autoSprintf("floorHugging=%d", (int)inFloorHugging));
        if (inWallLayer)
            lines.push_back(autoSprintf("wallLayer=%d", (int)inWallLayer));
        if (inFrontWall)
            lines.push_back(autoSprintf("frontWall=%d", (int)inFrontWall));
        ...
        if (inContainOffsetX != 0 || inContainOffsetY != 0)
            lines.push_back(autoSprintf("containOffset=%d,%d", inContainOffsetX, inContainOffsetY));

        lines.push_back(autoSprintf("heldOffset=%f,%f", inHeldOffset.x, inHeldOffset.y));

        lines.push_back(autoSprintf("clothing=%c", inClothing));

        lines.push_back(autoSprintf("clothingOffset=%f,%f", inClothingOffset.x, inClothingOffset.y));

        lines.push_back(autoSprintf("deadlyDistance=%d", inDeadlyDistance));

        lines.push_back(autoSprintf("useDistance=%d", inUseDistance));

        char *usageStrings[4] = {
            stringDuplicate(printSoundUsage(inCreationSound)), stringDuplicate(printSoundUsage(inUsingSound)),
            stringDuplicate(printSoundUsage(inEatingSound)), stringDuplicate(printSoundUsage(inDecaySound))};

        lines.push_back(
            autoSprintf("sounds=%s,%s,%s,%s", usageStrings[0], usageStrings[1], usageStrings[2], usageStrings[3]));
        for (int i = 0; i < 4; i++)
        {
            delete[] usageStrings[i];
        }

        lines.push_back(autoSprintf("creationSoundInitialOnly=%d", (int)inCreationSoundInitialOnly));
        lines.push_back(autoSprintf("creationSoundForce=%d", (int)inCreationSoundForce));

        lines.push_back(autoSprintf("numSlots=%d#timeStretch=%f", inNumSlots, inSlotTimeStretch));
        ...
        {
            lines.push_back(autoSprintf("slotPos=%f,%f,vert=%d,parent=%d", inSlotPos[i].x, inSlotPos[i].y,
                                        (int)(inSlotVert[i]), inSlotParent[i]));
        }
        ...
        for (int i = 0; i < inNumSprites; i++)
        {
            ...
            lines.push_back(autoSprintf("pos=%f,%f", inSpritePos[i].x, inSpritePos[i].y));
            lines.push_back(autoSprintf("rot=%f", inSpriteRot[i]));
            lines.push_back(autoSprintf("hFlip=%d", inSpriteHFlip[i]));
            lines.push_back(autoSprintf("color=%f,%f,%f", inSpriteColor[i].r, inSpriteColor[i].g, inSpriteColor[i].b));

            lines.push_back(autoSprintf("ageRange=%f,%f", inSpriteAgeStart[i], inSpriteAgeEnd[i]));

            lines.push_back(autoSprintf("parent=%d", inSpriteParent[i]));

            lines.push_back(autoSprintf("invisHolding=%d,invisWorn=%d,"
                                        "behindSlots=%d",
                                        inSpriteInvisibleWhenHolding[i], inSpriteInvisibleWhenWorn[i],
                                        inSpriteBehindSlots[i]));

            lines.push_back(autoSprintf("invisCont=%d", inSpriteInvisibleWhenContained[i]));

            if (inSpriteIgnoredWhenCalculatingCenterOffset[i] != 0)
                lines.push_back(autoSprintf("ignoredCont=%d", inSpriteIgnoredWhenCalculatingCenterOffset[i]));
        }

        if (drawBehindIndicesList.size() > 0)
        {
            lines.push_back(boolArrayToSparseCommaString("spritesDrawnBehind", inSpriteBehindPlayer, inNumSprites));
        }

        if (additiveBlendIndicesList.size() > 0)
        {
            lines.push_back(boolArrayToSparseCommaString("spritesAdditiveBlend", inSpriteAdditiveBlend, inNumSprites));
        }

        lines.push_back(boolArrayToSparseCommaString("headIndex", inSpriteIsHead, inNumSprites));

        lines.push_back(boolArrayToSparseCommaString("bodyIndex", inSpriteIsBody, inNumSprites));

        lines.push_back(boolArrayToSparseCommaString("backFootIndex", inSpriteIsBackFoot, inNumSprites));

        lines.push_back(boolArrayToSparseCommaString("frontFootIndex", inSpriteIsFrontFoot, inNumSprites));

        lines.push_back(autoSprintf("numUses=%d,%f", inNumUses, inUseChance));

        lines.push_back(boolArrayToSparseCommaString("useVanishIndex", inSpriteUseVanish, inNumSprites));
        lines.push_back(boolArrayToSparseCommaString("useAppearIndex", inSpriteUseAppear, inNumSprites));

        lines.push_back(autoSprintf("pixHeight=%d", newHeight));
    ...
    memcpy(r->spriteAgeStart, inSpriteAgeStart, inNumSprites * sizeof(double));

    memcpy(r->spriteAgeEnd, inSpriteAgeEnd, inNumSprites * sizeof(double));

    memcpy(r->spriteParent, inSpriteParent, inNumSprites * sizeof(int));

    memcpy(r->spriteInvisibleWhenHolding, inSpriteInvisibleWhenHolding, inNumSprites * sizeof(char));

    memcpy(r->spriteInvisibleWhenWorn, inSpriteInvisibleWhenWorn, inNumSprites * sizeof(int));

    memcpy(r->spriteBehindSlots, inSpriteBehindSlots, inNumSprites * sizeof(char));

    memcpy(r->spriteInvisibleWhenContained, inSpriteInvisibleWhenContained, inNumSprites * sizeof(char));

    memcpy(r->spriteIgnoredWhenCalculatingCenterOffset, inSpriteIgnoredWhenCalculatingCenterOffset,
           inNumSprites * sizeof(char));

    memcpy(r->spriteIsHead, inSpriteIsHead, inNumSprites * sizeof(char));

    memcpy(r->spriteIsBody, inSpriteIsBody, inNumSprites * sizeof(char));

    memcpy(r->spriteIsBackFoot, inSpriteIsBackFoot, inNumSprites * sizeof(char));

    memcpy(r->spriteIsFrontFoot, inSpriteIsFrontFoot, inNumSprites * sizeof(char));

    setupEyesAndMouth(r);

    memcpy(r->spriteUseVanish, inSpriteUseVanish, inNumSprites * sizeof(char));
    memcpy(r->spriteUseAppear, inSpriteUseAppear, inNumSprites * sizeof(char));
...
HoldingPos drawObject(ObjectRecord *inObject, int inDrawBehindSlots, doublePair inPos, double inRot, char inWorn,
                      char inFlipH, double inAge, int inHideClosestArm, char inHideAllLimbs, char inHeldNotInPlaceYet,
                      ClothingSet inClothing)
{
    ...
    {
        if (inObject->spriteSkipDrawing != NULL && inObject->spriteSkipDrawing[i])
        {
            continue;
        }
        if (inObject->person && !isSpriteVisibleAtAge(inObject, i, inAge))
        {
            // skip drawing this aging layer entirely
            continue;
        }
        if (drawingContained && inObject->spriteInvisibleWhenContained[i])
        {
            continue;
        }
        if (inObject->clothing != 'n' && inObject->spriteInvisibleWhenWorn[i] != 0)
        {
            ...
            else if (!inWorn && inObject->spriteInvisibleWhenWorn[i] == 2)
            {
        ...
        {
            if (inDrawBehindSlots == 0 && !inObject->spriteBehindSlots[i])
            {
                continue;
            }
            else if (inDrawBehindSlots == 1 && inObject->spriteBehindSlots[i])
            {
                continue;
            }
        }
        ...
        if (!inHeldNotInPlaceYet && inHideClosestArm == 1 && frontArmIndices.getElementIndex(i) != -1)
        {
            skipSprite = true;
        }
        else if (!inHeldNotInPlaceYet && inHideClosestArm == -1 && backArmIndices.getElementIndex(i) != -1)
        {
            skipSprite = true;
        }
        else if (!inHeldNotInPlaceYet && inHideAllLimbs)
        {
            if (frontArmIndices.getElementIndex(i) != -1 || backArmIndices.getElementIndex(i) != -1 ||
        ...
        if (i == backFootIndex && inClothing.backShoe != NULL)
        {

            doublePair cPos = add(spritePos, inClothing.backShoe->clothingOffset);
            if (inFlipH)
            {
                cPos.x *= -1;
            }
        ...
        if (i == bodyIndex)
        {
            if (inClothing.tunic != NULL)
            {
                ...
                doublePair cPos = add(spritePos, inClothing.tunic->clothingOffset);
                if (inFlipH)
                {
                    cPos.x *= -1;
                }
            ...
            if (inClothing.bottom != NULL)
            {
                ...
                doublePair cPos = add(spritePos, inClothing.bottom->clothingOffset);
                if (inFlipH)
                {
                    cPos.x *= -1;
                }
            ...
            if (inClothing.backpack != NULL)
            {
                ...
                doublePair cPos = add(spritePos, inClothing.backpack->clothingOffset);
                if (inFlipH)
                {
                    cPos.x *= -1;
                }
        ...
        else if (i == topBackArmIndex)
        {
            ...
            if (inClothing.bottom != NULL)
            {
                drawObject(inClothing.bottom, 2, bottomPos, bottomRot, true, inFlipH, -1, 0, false, false,
                           emptyClothing);
            }
            if (inClothing.tunic != NULL)
            {
                drawObject(inClothing.tunic, 2, tunicPos, tunicRot, true, inFlipH, -1, 0, false, false, emptyClothing);
            }
            if (inClothing.backpack != NULL)
            {
                drawObject(inClothing.backpack, 2, backpackPos, backpackRot, true, inFlipH, -1, 0, false, false,
                           emptyClothing);
            }
        }

        if (i == frontFootIndex && inClothing.frontShoe != NULL)
        {

            doublePair cPos = add(spritePos, inClothing.frontShoe->clothingOffset);
            if (inFlipH)
            {
                cPos.x *= -1;
            }
        ...
        if (!skipSprite)
        {
            ...
            char multiplicative = getUsesMultiplicativeBlending(inObject->sprites[i]);
            ...
            if (sh != NULL)
            {
                drawSprite(sh, pos, drawObjectScale, rot, logicalXOR(inFlipH, inObject->spriteHFlip[i]));
            }
        ...
        {
            drawObject(inClothing.backShoe, 2, backShoePos, backShoeRot, true, inFlipH, -1, 0, false, false,
                       emptyClothing);
        }
        else if (!skipSprite && inClothing.frontShoe != NULL && i == frontFootIndex)
        {
            drawObject(inClothing.frontShoe, 2, frontShoePos, frontShoeRot, true, inFlipH, -1, 0, false, false,
                       emptyClothing);
        }
    }
    ...
    if (inClothing.hat != NULL)
    {
        ...
        doublePair cPos = add(animHeadPos, inClothing.hat->clothingOffset);
        if (inFlipH)
        {
            cPos.x *= -1;
        }
...
HoldingPos drawObject(ObjectRecord *inObject, doublePair inPos, double inRot, char inWorn, char inFlipH, double inAge,
                      int inHideClosestArm, char inHideAllLimbs, char inHeldNotInPlaceYet, ClothingSet inClothing,
                      int inNumContained, int *inContainedIDs, SimpleVector<int> *inSubContained)
{

    drawObject(inObject, 0, inPos, inRot, inWorn, inFlipH, inAge, inHideClosestArm, inHideAllLimbs, inHeldNotInPlaceYet,
               inClothing);
    ...
    for (int i = 0; i < inNumContained; i++)
    {
        ...
        doublePair slotPos = sub(inObject->slotPos[i], centerOffset);
        ...
        if (inSubContained != NULL && inSubContained[i].size() > 0)
        {
            ...
            drawObject(contained, 0, pos, rot, false, inFlipH, inAge, 0, false, false, emptyClothing);
            ...
            for (int s = 0; s < contained->numSlots; s++)
            {
                if (s < inSubContained[i].size())
                {
                    ...
                    doublePair subCenterOffset = computeContainedCenterOffset(contained, subContained);
                    ...
                    if (contained->slotVert[s])
                    {
                        ...
                        if (inFlipH)
                        {
                            subCenterOffset = rotate(subCenterOffset, -rotOffset * 2 * M_PI);
                            subRot -= rotOffset;
                        }
                        else
                        {
                    ...
                    drawObject(subContained, 2, subPos, subRot, false, inFlipH, inAge, 0, false, false, emptyClothing);
                }
            }

            // in front of sub-contained
            drawObject(contained, 1, pos, rot, false, inFlipH, inAge, 0, false, false, emptyClothing);
        }
        else
        {
            // no sub-contained
            // draw contained all at once
            drawObject(contained, 2, pos, rot, false, inFlipH, inAge, 0, false, false, emptyClothing);
        }
    }

    setDrawnObjectContained(false);

    return drawObject(inObject, 1, inPos, inRot, inWorn, inFlipH, inAge, inHideClosestArm, inHideAllLimbs,
...
char isSoundUsedByObject(int inSoundID)
{
    ...
    for (int i = 0; i < mapSize; i++)
    {
        if (idMap[i] != NULL)
        {
            if (doesUseSound(idMap[i]->creationSound, inSoundID) || doesUseSound(idMap[i]->usingSound, inSoundID) ||
                doesUseSound(idMap[i]->eatingSound, inSoundID) || doesUseSound(idMap[i]->decaySound, inSoundID))
            {
                return true;
            }
        }
...
int getRandomPersonObject()
{
    ...
    return personObjectIDs.getElementDirect(randSource.getRandomBoundedInt(0, personObjectIDs.size() - 1));
}
...
int getRandomFemalePersonObject()
{
    ...
    return femalePersonObjectIDs.getElementDirect(randSource.getRandomBoundedInt(0, femalePersonObjectIDs.size() - 1));
}
...
int getRandomPersonObjectOfRace(int inRace)
{
    ...
    return racePersonObjectIDs[inRace].getElementDirect(
        randSource.getRandomBoundedInt(0, racePersonObjectIDs[inRace].size() - 1));
}

int getRandomFamilyMember(int inRace, int inMotherID, int inFamilySpan, char inForceGirl)
{
    ...
    int motherIndex = racePersonObjectIDs[inRace].getElementIndex(inMotherID);
    ...
    for (int o = 1; o <= inFamilySpan; o++)
    {
        for (int s = -1; s <= 1; s += 2)
        {
            ...
            if (familyIndex != motherIndex)
            {
                ...
                int pID = racePersonObjectIDs[inRace].getElementDirect(familyIndex);
...
int getNextPersonObject(int inCurrentPersonObjectID)
{
    ...
    {
        if (personObjectIDs.getElementDirect(i) == inCurrentPersonObjectID)
        {
...
int getRandomDeathMarker()
{
    ...
    return deathMarkerObjectIDs.getElementDirect(randSource.getRandomBoundedInt(0, deathMarkerObjectIDs.size() - 1));
}
...
static ObjectRecord **clothingPointerByIndex(ClothingSet *inSet, int inIndex)
{
    switch (inIndex)
    {
    case 0:
...
void setClothingByIndex(ClothingSet *inSet, int inIndex, ObjectRecord *inClothing)
{
    ObjectRecord **pointer = clothingPointerByIndex(inSet, inIndex);
...
ObjectRecord *getClothingAdded(ClothingSet *inOldSet, ClothingSet *inNewSet)
{
    ...
    for (int i = 0; i <= 5; i++)
    {
        ObjectRecord *oldC = clothingByIndex(*inOldSet, i);

        ObjectRecord *newC = clothingByIndex(*inNewSet, i);
...
char checkSpriteAncestor(ObjectRecord *inObject, int inChildIndex, int inPossibleAncestorIndex)
{
...
int getObjectHeight(int inObjectID)
{
    ...
    if (o->cachedHeight == -1)
    {
        o->cachedHeight = recomputeObjectHeight(o->numSprites, o->sprites, o->spritePos);
    }

    return o->cachedHeight;
}

int recomputeObjectHeight(int inNumSprites, int *inSprites, doublePair *inSpritePos)
{
                ...
                for (int y = 0; y < h; y++)
                {
                    for (int x = 0; x < w; x++)
                    {
                        ...
                        if (hitMap[p])
                        {
                            ...
                            // pixel
                            rad = (h / 2 + spriteRec->centerAnchorYOffset) - y;
                            break;
                        }
                    }
                    if (hit)
...
double getClosestObjectPart(ObjectRecord *inObject, ClothingSet *inClothing, SimpleVector<int> *inContained,
                            SimpleVector<int> *inClothingContained, char inWorn, double inAge, int inPickedLayer,
                            char inFlip, float inXCenterOffset, float inYCenterOffset, int *outSprite, int *outClothing,
                            int *outSlot, char inConsiderTransparent, char inConsiderEmptySlots)
{
    ...
    *outSlot = -1;
    ...
    {
        frontFootPos = inObject->spritePos[frontFootIndex];
    }
    ...
    {
        backFootPos = inObject->spritePos[backFootIndex];
    }
    ...
    for (int i = inObject->numSprites - 1; i >= 0; i--)
    {
        ...
        if (inClothing != NULL)
        {
            ...
            if (i <= inObject->numSprites - 1 && !hatChecked)
            {
                ...
                cObjIndex[0] = 0;
                cObjBodyPartPos[0] = add(headPos, getAgeHeadOffset(inAge, headPos, bodyPos, frontFootPos));
                if (checkSpriteAncestor(inObject, headIndex, bodyIndex))
                {
                    cObjBodyPartPos[0] = add(cObjBodyPartPos[0], getAgeBodyOffset(inAge, bodyPos));
                }
                hatChecked = true;
            }
            else if (!tunicChecked && i < headIndex)
            {
                ...
                cObjIndex[0] = 5;
                cObjBodyPartPos[0] = add(bodyPos, getAgeBodyOffset(inAge, bodyPos));
                cObj[1] = inClothing->tunic;
                cObjIndex[1] = 1;
                cObjBodyPartPos[1] = add(bodyPos, getAgeBodyOffset(inAge, bodyPos));

                cObj[2] = inClothing->bottom;
                cObjIndex[2] = 4;
                cObjBodyPartPos[2] = add(bodyPos, getAgeBodyOffset(inAge, bodyPos));
                tunicChecked = true;
            }
        ...
        for (int c = 0; c < 3; c++)
        {
            ...
            if (cObj[c] != NULL)
            {
                ...
                doublePair cSpritePos = add(cObjBodyPartPos[c], clothingOffset);
                ...
                double dist = getClosestObjectPart(cObj[c], NULL, clothingCont, NULL,
                                                   // clothing is worn
                                                   // on body currently
                                                   true, -1, -1, inFlip, cOffset.x, cOffset.y, &sp, &cl, &sl,
        ...
        if (inObject->person)
        {
            ...
            if (i == headIndex || checkSpriteAncestor(inObject, i, headIndex))
            {

                thisSpritePos = add(thisSpritePos, getAgeHeadOffset(inAge, headPos, bodyPos, frontFootPos));
            }
            if (i == bodyIndex || checkSpriteAncestor(inObject, i, bodyIndex))
            {

                thisSpritePos = add(thisSpritePos, getAgeBodyOffset(inAge, bodyPos));
            }
        }
        ...
        if (!inConsiderTransparent && sr->multiplicativeBlend)
        {
        ...
        if (a != NULL)
        {
            ...
            if (a->numSprites > i && a->spriteAnim[i].rotPhase != 0)
            {
                ...
                if (inObject->spriteRot[i] != 0)
                {
                    if (inFlip)
                    {
                        rotCenter = rotate(rotCenter, -2 * M_PI * inObject->spriteRot[i]);
                    }
                    else
                    {
                        rotCenter = rotate(rotCenter, 2 * M_PI * inObject->spriteRot[i]);
                ...
                doublePair tempOffset = sub(offset, rotCenter);

                if (inFlip)
                {
                    tempOffset = rotate(tempOffset, -a->spriteAnim[i].rotPhase * 2 * M_PI);
                }
                else
                {
                    tempOffset = rotate(tempOffset, a->spriteAnim[i].rotPhase * 2 * M_PI);
        ...
        if (getSpriteHit(inObject->sprites[i], lrint(offset.x), lrint(offset.y)))
        {
            *outSprite = i;
            break;
        }
    ...
    if ((*outSprite == -1 || closestBehindSlots) && *outClothing == -1 && *outSlot == -1)
    {
        // consider slots
        ...
        for (int i = inObject->numSlots - 1; i >= 0; i--)
        {
            ...
            {
                ObjectRecord *contained = getObject(inContained->getElementDirect(i));
                ...
                if (inObject->slotVert[i])
                {
                    ...
                    if (inFlip)
                    {
                        centOffset = rotate(centOffset, -rotOffset * 2 * M_PI);
                        centOffset.x *= -1;
                    }
                    else
                    {
                ...
                if (inObject->slotVert[i])
                {
                    ...
                    if (inFlip)
                    {
                        slotOffset = rotate(slotOffset, -rotOffset * 2 * M_PI);
                    }
                    else
                    {
                        slotOffset = rotate(slotOffset, rotOffset * 2 * M_PI);
                ...
                getClosestObjectPart(contained, NULL, NULL, NULL, false, -1, -1, inFlip, slotOffset.x, slotOffset.y,
                                     &sp, &cl, &sl, inConsiderTransparent);
                if (sp != -1)
                {
                    *outSlot = i;
                    smallestDist = 0;
...
static void getHandIndices(ObjectRecord *inObject, double inAge, int *outHandOneIndex, int *outHandTwoIndex)
{
    *outHandOneIndex = -1;
    *outHandTwoIndex = -1;
    ...
    for (int i = 0; i < inObject->numSprites; i++)
    {
        if (inObject->spriteInvisibleWhenHolding[i])
        {
            ...
            if (inObject->spriteAgeStart[i] != -1 || inObject->spriteAgeEnd[i] != -1)
            {

                if (inAge < inObject->spriteAgeStart[i] || inAge >= inObject->spriteAgeEnd[i])
                {
...
int getBackHandIndex(ObjectRecord *inObject, double inAge)
{
    ...
    if (handOneIndex != -1)
    {
        ...
        if (handTwoIndex != -1)
        {
            if (inObject->spritePos[handOneIndex].x < inObject->spritePos[handTwoIndex].x)
            {
                return handOneIndex;
            }
            else
...
int getFrontHandIndex(ObjectRecord *inObject, double inAge)
{
...
static void getLimbIndices(ObjectRecord *inObject, double inAge, SimpleVector<int> *outList, int inHandOrFootIndex)
{
    ...
    if (inHandOrFootIndex == 0 &&
        !(inObject->spriteInvisibleWhenHolding[inHandOrFootIndex] || inObject->spriteIsFrontFoot[inHandOrFootIndex] ||
...
void getFrontArmIndices(ObjectRecord *inObject, double inAge, SimpleVector<int> *outList)
{
    getLimbIndices(inObject, inAge, outList, getFrontHandIndex(inObject, inAge));
}

void getBackArmIndices(ObjectRecord *inObject, double inAge, SimpleVector<int> *outList)
{

    getLimbIndices(inObject, inAge, outList, getBackHandIndex(inObject, inAge));
}
...
void getAllLegIndices(ObjectRecord *inObject, double inAge, SimpleVector<int> *outList)
{

    getLimbIndices(inObject, inAge, outList, getBackFootIndex(inObject, inAge));

    getLimbIndices(inObject, inAge, outList, getFrontFootIndex(inObject, inAge));
    ...
    if (outList->size() >= 2)
    {
        ...
        for (int i = 0; i < inObject->numSprites; i++)
        {
            ...
            {
                if (bodyPos.y > inObject->spritePos[i].y && inObject->spriteParent[i] == -1)
                {
...
void getAllNudeIndices(ObjectRecord *inObject, double inAge, SimpleVector<int> *outList)
{
...
char isSpriteVisibleAtAge(ObjectRecord *inObject, int inSpriteIndex, double inAge)
{

    if (inObject->spriteAgeStart[inSpriteIndex] != -1 || inObject->spriteAgeEnd[inSpriteIndex] != -1)
    {

        if (inAge < inObject->spriteAgeStart[inSpriteIndex] || inAge >= inObject->spriteAgeEnd[inSpriteIndex])
        {
...
static int getBodyPartIndex(ObjectRecord *inObject, char *inBodyPartFlagArray, double inAge)
{
    if (!inObject->person)
    {
        return 0;
...
char *getTapoutTriggerString(ObjectRecord *inObject)
{
    ...
    else if (tr->tapoutMode == 2)
    {
        ...
        {
            working = autoSprintf("%d,%d,%d,%d,%d,%d", tr->tapoutMode, tr->radiusN, tr->radiusE, tr->radiusS,
                                  tr->radiusW, tr->tapoutCountLimit);
        }
    }
...
doublePair getObjectCenterOffset(ObjectRecord *inObject)
{
    ...
    for (int i = 0; i < inObject->numSprites; i++)
    {
        ...
        if (widestRecord == NULL ||
            ...
            (w == widestWidth && inObject->spritePos[i].y < widestYPos))
        {
    ...
    doublePair centerOffset = {(double)widestRecord->centerXOffset, (double)widestRecord->centerYOffset};

    centerOffset = rotate(centerOffset, 2 * M_PI * inObject->spriteRot[widestIndex]);

    doublePair spriteCenter = add(inObject->spritePos[widestIndex], centerOffset);
...
doublePair getObjectBottomCenterOffset(ObjectRecord *inObject)
{
    ...
    for (int i = 0; i < inObject->numSprites; i++)
    {
        ...
        doublePair centerOffset = {(double)sprite->centerXOffset, (double)sprite->centerYOffset};

        doublePair centerAnchorOffset = {(double)sprite->centerAnchorXOffset, (double)sprite->centerAnchorYOffset};
        ...
        double y = inObject->spritePos[i].y + centerAnchorOffset.y -
                   centerOffset.y
                   // there is no way to calculate the visible dimensions after rotation
...
char isSpriteSubset(int inSuperObjectID, int inSubObjectID, SimpleVector<SubsetSpriteIndexMap> *outMapping)
{
    ...
    else if (subO->numSprites == 1 && superO->numSprites >= 1)
    {
        // special case:
        ...
        for (int ss = 0; ss < superO->numSprites; ss++)
        {
            if (superO->sprites[ss] == spriteID)
            {
                ...
                if (equal(superO->spriteColor[ss], subO->spriteColor[0]))
                {
                    return true;
                }
            }
    ...
    for (int ss = 0; ss < superO->numSprites; ss++)
    {
        if (superO->sprites[ss] == spriteSubZeroID && superO->spriteRot[ss] == spriteSubZeroRot &&
    ...
    for (int s = 0; s < subO->numSprites; s++)
    {
        ...
        doublePair spritePosRel = sub(subO->spritePos[s], spriteSubZeroPos);
...
{
    return inA.r == inB.r && inA.g == inB.g && inA.b == inB.b;
}

void getArmHoldingParameters(ObjectRecord *inHeldObject, int *outHideClosestArm, char *outHideAllLimbs)
{
    *outHideClosestArm = 0;
...
void computeHeldDrawPos(HoldingPos inHoldingPos, doublePair inPos, ObjectRecord *inHeldObject, char inFlipH,
                        doublePair *outHeldDrawPos, double *outHeldDrawRot)
{
    ...
    if (inHeldObject != NULL)
    {
        ...
        {
            heldOffset = sub(heldOffset, getObjectCenterOffset(inHeldObject));
        }
        ...
        if (inHoldingPos.valid && inHoldingPos.rot != 0 && !inHeldObject->rideable)
        {

            if (inFlipH)
            {
                heldOffset = rotate(heldOffset, 2 * M_PI * inHoldingPos.rot);
            }
            else
            {
                heldOffset = rotate(heldOffset, -2 * M_PI * inHoldingPos.rot);
...
doublePair computeContainedCenterOffset(ObjectRecord *inContainerObject, ObjectRecord *inContainedObject)
{
...
void prepareToSkipSprites(ObjectRecord *inObject, char inDrawBehind, char inSkipAll)
{
    if (skipDrawingWorkingArea != NULL)
    {
    ...
    memcpy(skipDrawingWorkingArea, inObject->spriteSkipDrawing, inObject->numSprites);
...
{
    memcpy(inObject->spriteSkipDrawing, skipDrawingWorkingArea, inObject->numSprites);
}
