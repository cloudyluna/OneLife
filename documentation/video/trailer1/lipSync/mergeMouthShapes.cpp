...
void mergeMouth(Image *inFrame, Image *inMouth, int *inOutLastPosX, int *inOutLastPosY)
{
    ...
    for (int y = searchStartY; y < searchEndY; y++)
    {
        for (int x = searchStartX; x < searchEndX; x++)
        {
            ...
            if (r[i] == 1.0 && g[i] == 0 && b[i] == 1.0)
            {
                ...
                int n[4] = {i - 1, i + 1, i - frameW, i + frameW};

                char allNeighborsMagenta = true;
                for (int j = 0; j < 4; j++)
                {
                    if (g[n[j]] == 0 && r[n[j]] == b[n[j]] && r[n[j]] > 0.5 && b[n[j]] > 0.5)
                    {
    ...
    if (posX != -1 && posY != -1)
    {
        ...
        for (int y = startY; y < endY; y++)
        {
            ...
            for (int x = startX; x < endX; x++)
            {
                ...
                frameR[frameI] = mWeight * mouthR[mouthI] + fWeight * frameR[frameI];
                frameG[frameI] = mWeight * mouthG[mouthI] + fWeight * frameG[frameI];
                frameB[frameI] = mWeight * mouthB[mouthI] + fWeight * frameB[frameI];
...
int main(int inNumArgs, char **inArgs)
{
    ...
    if (!mouthsDir.exists() || !mouthsDir.isDirectory())
    {
        usage();
    }
