...
int main(int inNumArgs, char **inArgs)
{
    ...
    samples = readMono16AIFFData(aiffData, dataLength, &numSamples, &sampleRate);
    ...
    while (i < numFrames)
    {
        ...
        int framesQuiet = 0;
        while (i < numFrames && (!getTalking(i, thresh) || framesQuiet < framesPerMove))
        {
            frameShapes[i] = shapeFiles[0];
        ...
        while (i < numFrames && (getTalking(i, thresh) || framesTalking < framesPerMove))
        {
