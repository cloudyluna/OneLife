...
static void stepLog()
{
    ...
    if (timeStruct->tm_year != currentYear || timeStruct->tm_yday != currentDay)
    {
...
void logBirth(int inPlayerID, char *inPlayerEmail, int inParentID, char *inParentEmail, char inIsMale, int inRace,
              int inMapX, int inMapY, int inTotalPopulation, int inParentChainLength)
{
    ...
    if (logFile != NULL)
    {
        ...
        if (logFile != NULL)
        {
            ...
            {
                parentString = autoSprintf("parent=%d,%s", inParentID, inParentEmail);
            }
            ...
            fprintf(logFile, "B %.f %d %s %c (%d,%d) %s pop=%d chain=%d race=%c\n", Time::timeSec(), inPlayerID,
                    inPlayerEmail, genderChar, inMapX, inMapY, parentString, inTotalPopulation, inParentChainLength,
                    raceChar);
...
void logDeath(int inPlayerID, char *inPlayerEmail, char inEve, double inAge, int inSecPlayed, char inIsMale, int inMapX,
              int inMapY, int inTotalRemainingPopulation, char inDisconnect, int inKillerID, char *inKillerEmail)
{
    ...
    if (logFile != NULL)
    {
        ...
        if (logFile != NULL)
        {
            ...
            if (inKillerEmail == NULL)
            {
                ...
                else
                {
                    ...
                    else if (inKillerID < -1)
                    {
                        ...
                        if (o != NULL)
                        {
                            causeString = stringDuplicate(o->description);
            ...
            {
                causeString = autoSprintf("killer_%d_%s", inKillerID, inKillerEmail);
            }
            ...
            fprintf(logFile, "D %.0f %d %s age=%.2f %c (%d,%d) %s pop=%d\n", Time::timeSec(), inPlayerID, inPlayerEmail,
                    inAge, genderChar, inMapX, inMapY, causeString, inTotalRemainingPopulation);
            fflush(logFile);
