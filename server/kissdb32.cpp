...
int KISSDB_open(KISSDB *db, const char *path, int mode, unsigned long hash_table_size, unsigned long key_size,
                unsigned long value_size)
{
    ...
    db->f =
        fopen(path, ((mode == KISSDB_OPEN_MODE_RWREPLACE)
                         ? "w+b"
                         : (((mode == KISSDB_OPEN_MODE_RDWR) || (mode == KISSDB_OPEN_MODE_RWCREAT)) ? "r+b" : "rb")));
    ...
    if (ftello(db->f) < (int)(KISSDB_HEADER_SIZE))
    {
        ...
        if ((hash_table_size) && (key_size) && (value_size))
        {
            ...
            tmp = hash_table_size;
            if (fwrite(&tmp, sizeof(uint32_t), 1, db->f) != 1)
            {
                fclose(db->f);
                return KISSDB_ERROR_IO;
            }
    ...
    else
    {
        ...
        if (fread(&tmp, sizeof(uint32_t), 1, db->f) != 1)
        {
            fclose(db->f);
            return KISSDB_ERROR_IO;
        }
    ...
    while (fread(httmp, db->hash_table_size_bytes, 1, db->f) == 1)
    {
        ...
        memcpy(((uint8_t *)db->hash_tables) + (db->hash_table_size_bytes * db->num_hash_tables), httmp,
