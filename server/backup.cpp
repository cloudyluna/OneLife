...
void initBackup()
{
    lastBackupTime = SettingsManager::getTimeSetting("lastBackupTimeUTC", lastBackupTime);

    targetHour = SettingsManager::getIntSetting("backupHourUTC", targetHour);
    ...
    AppLog::infoF("Backup system inited.  Backups on = %d, "
                  ...
                  SettingsManager::getIntSetting("saveBackups", 0), targetHour, timeStruct.tm_hour,
                  (Time::timeSec() - lastBackupTime) / 3600.0);
}

// call with "map" or "eve" for example
void backupDBFile(const char *inFileNamePrefix, char *inTimeFileNamePart, File *inBackupFolder)
{
    ...
    if (dbFile.exists())
    {
        ...
        char *backFileName = autoSprintf("%s_%s.db", inFileNamePrefix, inTimeFileNamePart);

        File *backFile = inBackupFolder->getChildFile(backFileName);
...
void checkBackup()
{
    ...
    if (curTime - lastBackupTime > 12 * 3600 || curTime < lastBackupTime)
    {
        ...
        if (timeStruct.tm_hour == targetHour)
        {
            ...
            if (SettingsManager::getIntSetting("saveBackups", 0))
            {
                ...
                AppLog::info("Saving a backup of map.db, mapTime.db, and biome.db "
                ...
                char *timeFileNamePart =
                    autoSprintf("%d_%02d_%02d__%02d_%02d_%02d", timeStruct.tm_year + 1900, timeStruct.tm_mon + 1,
                ...
                if (backupFolder.isDirectory())
                {
                    ...
                    backupDBFile("floorTime", timeFileNamePart, &backupFolder);

                    backupDBFile("playerStats", timeFileNamePart, &backupFolder);

                    AppLog::info("...Done saving backups");
                    backupsSaved = true;

                    int keepDays = SettingsManager::getIntSetting("keepBackupsDays", 14);
                    ...
                    int numChildren;
                    File **childFiles = backupFolder.getChildFiles(&numChildren);

                    int numRemoved = 0;

                    for (int i = 0; i < numChildren; i++)
                    {
                        if (curTime - childFiles[i]->getModificationTime() > keepSeconds)
                        {
                            ...
                            {
                                AppLog::errorF("Failed to remove %s", fileName);
                            }
                            delete[] fileName;
                        }
                        delete childFiles[i];
                    ...
                    AppLog::infoF("Done checking, removed %d stale backups", numRemoved);
                }
                ...
                SettingsManager::setSetting("lastBackupTimeUTC", lastBackupTime);
