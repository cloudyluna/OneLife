...
static Craving getRandomFood(int inLineageMaxFoodDepth, Craving inFoodToAvoid = noCraving)
{
    ...
    if (inLineageMaxFoodDepth > 0)
    {
        ...
        for (int i = 0; i < allFoods->size(); i++)
        {
            ...
            if (id != inFoodToAvoid.foodID)
            {
                ...
                if ((!harderCravingOrNot && d <= inLineageMaxFoodDepth) ||
                    (harderCravingOrNot && d > inLineageMaxFoodDepth &&
                     d <= inLineageMaxFoodDepth + maxCravingDepthGap))
                {
            ...
            while (possibleFoods.size() < 3)
            {
                ...
                for (int i = 0; i < allFoods->size(); i++)
                {
                    ...
                    if (id != inFoodToAvoid.foodID)
                    {
                        ...
                        if (d == nextDepth || nextDepth > maxDepth // We reached the top of tech tree
                        )
                        {
                            possibleFoods.push_back(id);
                        }
    ...
    {
        int pick = randSource.getRandomBoundedInt(0, possibleFoods.size() - 1);
        ...
        // printf( "harderCravingOrNot = %d, %d possible foods, picking #%d, max %d, depth %d, gap %d, scale %.2f, bonus
        // %d, %s\n", harderCravingOrNot, possibleFoods.size(), pick, inLineageMaxFoodDepth, getObjectDepth( pickedFood
        // ), getObjectDepth( pickedFood ) - inLineageMaxFoodDepth, cravingBonusScale, bonus, o->description
        // );

        Craving c = {pickedFood, nextUniqueID, bonus};
        nextUniqueID++;
    ...
    else
    {
        ...
        if (allFoods->size() > 0)
        {
            ...
            Craving c = {pickedFood, nextUniqueID, getObjectDepth(pickedFood)};
            nextUniqueID++;
...
Craving getCravedFood(int inLineageEveID, int inPlayerGenerationNumber, Craving inLastCraved)
{
    ...
    for (int i = 0; i < listSize; i++)
    {
        ...
        if (food.foodID == inLastCraved.foodID && food.uniqueID == inLastCraved.uniqueID && i < listSize - 1)
        {
...
void purgeStaleCravings(int inLowestUniqueID)
{
    ...
    for (int i = 0; i < list.size(); i++)
    {
        ...
        for (int f = start; f >= 0; f--)
        {
            ...
            if (l->cravedFoods.getElementDirect(f).uniqueID < inLowestUniqueID)
            {
