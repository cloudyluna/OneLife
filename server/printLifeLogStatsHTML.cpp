...
void addPlayerGame(char *inEmail, double inGameStartTime, double inGameEndTime)
{
    ...
    if (f != NULL)
    {
        ...
        while (scannedLine)
        {
            ...
            if (event == 'B')
            {
                ...
                fscanf(f,
                       "%lf %d %999s %c (%d,%d) %999s pop=%d chain=%d "
                       "race=%c\n",
                       &time, &id, email, &gender, &locX, &locY, parent, &pop, &parentChain, &race);
            ...
            {
                fscanf(f, "%lf %d %999s age=%lf %c (%d,%d) %999s pop=%d\n", &time, &id, email, &age, &gender, &locX,
                       &locY, deathReason, &pop);
...
void saveNewCheckpoint(File *inCheckpointFile, char *inLastScannedFileName)
{
    ...
    if (f != NULL)
    {
        fprintf(f, "%s %f %d %d %d", inLastScannedFileName, folderTotalAge, folderTotalLives, folderLongestFamilyChain,
                folderOver55Count);
        fclose(f);
    }
    delete[] fileName;
...
int processLifeLogFolder(File *inFolder)
{
    ...
    if (checkpointFile->exists())
    {
        ...
        if (f != NULL)
        {
            fscanf(f, "%199s %lf %d %d %d", lastScannedFileName, &folderTotalAge, &folderTotalLives,
    ...
    for (int i = 0; i < numFiles; i++)
    {
        ...
        if (strcmp(name, checkpointFileName) != 0 && strstr(name, "_names") == NULL)
        {
    ...
    for (int i = 0; i < numFiles; i++)
    {
        ...
        if (!checkpointFound || checkpointReached)
        {
...
int main(int inNumArgs, char **inArgs)
{
    ...
    if (mainDir.exists() && mainDir.isDirectory())
    {
        ...
        for (int i = 0; i < allPlayers.size(); i++)
        {
            ...
            printf("INSERT INTO reviewServer_user_stats SET "
                   ...
                   p.email, p.firstGameTime, p.lastGameTime, p.lastGameSeconds, p.gameCount, p.gameTotalSeconds);
