...
const char *findCloseName(char *inString, char *inNameList, int inListLen, int *outIndex = NULL)
{
    if (inNameList == NULL)
    {
        return defaultName;
    ...
    if (lastDiff != 0 && hitEndCount == 0 && hitStartCount == 0)
    {
        ...
        if (nextDiff != 0)
        {
            ...
            if (step < 0)
            {
                prevOffset = getNameOffsetForward(inNameList, inListLen, prevOffset);
            }
            else
            {
                prevOffset = getNameOffsetBack(inNameList, inListLen, prevOffset);
            }

            int crossSim = sharedPrefixLength(tempString, &(inNameList[crossOffset]));
            int prevSim = sharedPrefixLength(tempString, &(inNameList[prevOffset]));
            ...
            else
            {
                ...
                if (strlen(&(inNameList[prevOffset])) < strlen(&(inNameList[crossOffset])))
                {
                    offset = prevOffset;
                }
                else
