...
static void checkSettings()
{
    ...
    {
        curseDuration = SettingsManager::getDoubleSetting("curseBlockDuration", 48 * 3600.0);
        curseBlockRadius = SettingsManager::getIntSetting("curseBlockRadius", 50);

        curseBlockOfflineFactor = SettingsManager::getDoubleSetting("curseBlockOfflineFactor", 1.0);

        curseBlockOfflineExponent = SettingsManager::getDoubleSetting("curseBlockOfflineExponent", 2.0);
...
static char cullStale()
{
    ...
    int error = LINEARDB3_open(&tempDB, "curses.db.temp", KISSDB_OPEN_MODE_RWCREAT, 10000, 80, 8);
    ...
    error = LINEARDB3_open(&db, "curses.db", KISSDB_OPEN_MODE_RWCREAT, 10000, 80, 8);
...
static char rebuildCountDB()
{
    ...
    int error = LINEARDB3_open(&dbCount, "curseCount.db", KISSDB_OPEN_MODE_RWCREAT, 10000, 40, 12);

    if (error)
    {
        AppLog::errorF("Error %d re-opening fresh curseCount LinearDB3", error);
        return false;
    }
...
void initCurseDB()
{
    int error = LINEARDB3_open(&db, "curses.db", KISSDB_OPEN_MODE_RWCREAT, 10000,
    ...
    error = LINEARDB3_open(&dbCount, "curseCount.db", KISSDB_OPEN_MODE_RWCREAT, 10000,
...
static void getKey(const char *inSenderEmail, const char *inReceiverEmail, unsigned char *outKey)
{
    memset(outKey, ' ', 80);
...
static void getOldKey(const char *inSenderEmail, const char *inReceiverEmail, unsigned char *outKey)
{
    memset(outKey, ' ', 80);
...
static void getCountKey(const char *inReceiverEmail, unsigned char *outKey)
{
    memset(outKey, ' ', 40);
...
static void stepStaleCurseCulling()
{
    ...
    {
        int result = LINEARDB3_Iterator_next(&cullingIterator, key, value);
        ...
        if (result <= 0)
        {
            ...
            {
                AppLog::infoF("Curse stale cull iterated through %d curse db entries,"
                              " marked %d as stale.",
                              numRecordsSeenByIterator, numRecordsMarked);
            }
            numRecordsSeenByIterator = 0;
            numRecordsMarked = 0;
        ...
        if (curseTime != 0 && curTimeSec - curseTime > curseDuration)
        {
...
void setDBCurse(int inSenderID, const char *inSenderEmail, const char *inReceiverEmail)
{
    checkSettings();
...
void clearDBCurse(int inSenderID, const char *inSenderEmail, const char *inReceiverEmail)
{
...
void addPersonToPersonalCurseTest(const char *inEmail, const char *inTargetEmail, GridPos inPos)
{
...
static int getCurseRadius(int inLiveCurseCount, int inTotalCurseCount)
{
    return
        ...
        curseBlockOfflineFactor * pow(inTotalCurseCount, curseBlockOfflineExponent);
}

char isBirthLocationCurseBlocked(const char *inTargetEmail, GridPos inPos)
{

    int radius = getCurseRadius(personalLiveCurseCount, personalTotalCurseCount);

    for (int i = 0; i < blockingRecords.size(); i++)
    {
        PersonRecord *r = blockingRecords.getElement(i);

        if (r->blocking == 1 && distance(inPos, r->pos) <= radius)
        {
...
char isBirthLocationCurseBlockedNoCache(const char *inTargetEmail, GridPos inPos)
{
    ...
    int radius = getCurseRadius(liveCurseCount, getCurseCount(inTargetEmail));
