...
FailureRec *findExistingRect(SimpleVector<FailureRec> *inRecList, int inActorID, int inTargetID)
{
...
void addCount(SimpleVector<FailureRec> *inRecList, int inActorID, int inTargetID, int inCount)
{
...
void processLogFile(File *inFile)
{
    ...
    if (fileMonth == 4 || fileMonth == 6 || fileMonth == 9 || fileMonth == 11)
    {
        numDaysInFileMonth = 30;
    }
    else if (fileMonth == 2)
    {
        char isLeapYear = (fileYear % 4 == 0 && fileYear % 100 != 0) || (fileYear % 400 == 0);
        if (isLeapYear)
        {
    ...
    {
        // jan 1
        if (currentYear - 1 == fileYear && fileMonth == 12 && fileDay == 31)
        {
    ...
    else
    {
        ...
        // today is mid-month
        if (currentDay > 1 && currentYear == fileYear && currentDay - 1 == fileDay)
        {
            isYesterday = true;
        }
        // today is first day of month
        else if (currentDay == 1 && currentYear == fileYear && currentMonth - 1 == fileMonth &&
    ...
    if (f != NULL)
    {
        ...
        while (scannedLine)
        {
            ...
            if (lastScannedHour != -1 && isToday &&
                (lastScannedHour == currentHour || lastScannedHour == currentHour - 1))
            {
            ...
            int numScanned = fscanf(f, "%d + %d  count=%d\n", &actorID, &targetID, &count);
...
void printTable(const char *inName, File *inObjectDir, FILE *inFile, SimpleVector<FailureRec> *inRecList)
{
...
int main(int inNumArgs, char **inArgs)
{
    ...
    if (mainDir.exists() && mainDir.isDirectory() && objDir.exists() && objDir.isDirectory())
    {
        ...
        if (outFile != NULL)
        {
            ...
            printTable("Past Hour", &objDir, outFile, &hourRecords);

            printTable("Today (so far)", &objDir, outFile, &todayRecords);

            printTable("Yesterday", &objDir, outFile, &yesterdayRecords);
            /*
