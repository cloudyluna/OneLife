...
char *nameFromLine(char *inLine)
{
    ...
    if (hasTime && tok->size() == 8 || !hasTime && tok->size() == 7)
    {
...
void processLogFile(File *inFile, FILE *outHTMLFile)
{
    ...
    int numRead = sscanf(name, "%d_%d_done_%lf.txt", &x, &y, &timeStamp);
...
void processMonumentLogsFolder(File *inFolder, File *inHTMLFolder)
{
    ...
    if (someDoneFiles)
    {
        ...
        int numRead = sscanf(name, "monumentLogs_%199s", serverName);
...
int main(int inNumArgs, char **inArgs)
{
    ...
    if (mainDir.exists() && mainDir.isDirectory() && htmlDir.exists() && htmlDir.isDirectory())
    {
        ...
        for (int i = 0; i < numChildFiles; i++)
        {
            ...
            if (childFiles[i]->isDirectory())
            {
                ...
                if (strstr(name, "monumentLogs") == name)
                {
                    ...
                    processMonumentLogsFolder(childFiles[i], &htmlDir);
                }
                delete[] name;
            }
