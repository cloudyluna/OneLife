...
static void recreateMaps(LINEARDB *inDB, unsigned int inOldTableSizeA = 0)
{
    uint8_t *oldExistenceMap = inDB->existenceMap;

    inDB->existenceMap = new uint8_t[getExistenceMapSize(inDB->hashTableSizeA)];

    memset(inDB->existenceMap, 0, getExistenceMapSize(inDB->hashTableSizeA));

    if (oldExistenceMap != NULL)
    {
        if (inOldTableSizeA > 0)
        {
            memcpy(inDB->existenceMap, oldExistenceMap, getExistenceMapSize(inOldTableSizeA) * sizeof(uint8_t));
        }
    ...
    memset(inDB->fingerprintMap, 0, inDB->hashTableSizeA * 2 * sizeof(uint16_t));

    if (oldFingerprintMap != NULL)
    {
        if (inOldTableSizeA > 0)
        {
            memcpy(inDB->fingerprintMap, oldFingerprintMap, inOldTableSizeA * 2 * sizeof(uint16_t));
        }
...
{
    return (inDB->existenceMap[inBinNumber / 8] >> (inBinNumber % 8)) & 0x01;
}
...
int LINEARDB_open(LINEARDB *inDB, const char *inPath, int inMode, unsigned int inHashTableStartSize,
                  unsigned int inKeySize, unsigned int inValueSize)
{
    ...
    if (inPath == NULL || ftello(inDB->file) < LINEARDB_HEADER_SIZE)
    {
        ...
        inDB->tableSizeBytes = (uint64_t)(inDB->recordSizeBytes) * (uint64_t)(inDB->hashTableSizeB);
    ...
    else
    {
        ...
        if (ftello(inDB->file) < (int64_t)(LINEARDB_HEADER_SIZE + inDB->tableSizeBytes))
        {
        ...
        for (unsigned int i = 0; i < inDB->hashTableSizeB; i++)
        {
            ...
            if (fseeko(inDB->file, LINEARDB_HEADER_SIZE + i * inDB->recordSizeBytes, SEEK_SET))
            {
                fclose(inDB->file);
                inDB->file = NULL;
                return 1;
            ...
            if (present)
            {
                ...
                // now read key
                numRead = fread(inDB->recordBuffer, inDB->keySize, 1, inDB->file);
                ...
                inDB->fingerprintMap[i] = shortHash(inDB->recordBuffer, inDB->keySize);
            }
        }
    }
...
static int reinsertCellSegment(LINEARDB *inDB, uint64_t inFirstBinNumber, uint64_t inMinCellCount)
{
    ...
    while (numCellsTouched < inDB->hashTableSizeB && (numCellsTouched < inMinCellCount || exists(inDB, c)))
    {
        ...
        if (exists(inDB, c))
        {
            ...
            int putResult = LINEARDB_put(inDB,
                                         // key
                                         &(inDB->recordBuffer[1]),
                                         // value
...
static int expandTable(LINEARDB *inDB)
{
    ...
    int numExpanded = 0;
    while ((double)(inDB->numRecords) / (double)(inDB->hashTableSizeB) > inDB->maxLoad)
    {
    ...
    {
        int numWritten = fwrite(inDB->recordBuffer, inDB->recordSizeBytes, 1, inDB->file);
...
//              inOutValue.
static int locateValue(LINEARDB *inDB, const void *inKey, void *inOutValue, char inPut = false)
{
    ...
    while (true)
    {
            ...
            if (fingerprint == inDB->fingerprintMap[binNumberB])
            {
                ...
                if (keyComp(inDB->keySize, inKey, inDB->recordBuffer))
                {
                    ...
                    else
                    {
                        ...
                        // read value
                        numRead = fread(inOutValue, inDB->valueSize, 1, inDB->file);
        ...
        else if (inPut)
        {
            ...
            if ((double)(inDB->numRecords) / (double)(inDB->hashTableSizeB) > inDB->maxLoad)
            {
...
int LINEARDB_Iterator_next(LINEARDB_Iterator *inDBi, void *outKey, void *outValue)
{
    ...
    while (true)
    {
        ...
        int numRead = fread(db->recordBuffer, db->recordSizeBytes, 1, db->file);
        if (numRead != 1)
        {
            return -1;
        }
        ...
        if (db->recordBuffer[0])
        {
            ...
            // present
            memcpy(outKey, &(db->recordBuffer[1]), db->keySize);

            memcpy(outValue, &(db->recordBuffer[1 + db->keySize]), db->valueSize);
            return 1;
        }
        else
        {
...
unsigned int LINEARDB_getShrinkSize(LINEARDB *inDB, unsigned int inNewNumRecords)
{
...
uint64_t LINEARDB_getMaxFileSize(unsigned int inTableStartSize, unsigned int inKeySize, unsigned int inValueSize,
                                 uint64_t inNumRecords, double inMaxLoad)
{
    if (inMaxLoad == 0)
    {
...
void LINEARDB_forceFile(LINEARDB *inDB, FILE *inFile)
{
    inDB->file = inFile;
}
