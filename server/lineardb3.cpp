...
uint32_t LINEARDB3_getPerfectTableSize(double inMaxLoad, uint32_t inNumRecords)
{
    ...
    uint32_t minTableBuckets = (uint32_t)ceil((double)minTableRecords / (double)RECORDS_PER_BUCKET);
...
int LINEARDB3_open(LINEARDB3 *inDB, const char *inPath, int inMode, unsigned int inHashTableStartSize,
                   unsigned int inKeySize, unsigned int inValueSize)
{
    ...
    else
    {
        ...
        for (uint64_t i = 0; i < numRecordsInFile; i++)
        {
            ...
            int result = LINEARDB3_getOrPut(inDB, &(inDB->recordBuffer[0]), &(inDB->recordBuffer[inDB->keySize]), true,
                                            // ignore data file
                                            // update ram only
...
//
// Updates iterator
static void insertIntoBucket(LINEARDB3 *inDB, BucketIterator *inBucketIterator, uint32_t inFingerprint,
                             uint32_t inFileIndex)
{
    ...
    if (inBucketIterator->nextRecord == RECORDS_PER_BUCKET)
    {
        ...
        inBucketIterator->nextBucket = getBucket(inDB->overflowBuckets, inBucketIterator->nextBucket->overflowIndex);
    }
...
static int expandTable(LINEARDB3 *inDB)
{
    ...
    while ((double)(inDB->numRecords) / (double)(inDB->hashTableSizeB * RECORDS_PER_BUCKET) > inDB->maxLoad)
    {
...
static int LINEARDB3_considerFingerprintBucket(LINEARDB3 *inDB, const void *inKey, void *inOutValue,
                                               uint32_t inFingerprint, char inPut, char inIgnoreDataFile,
                                               FingerprintBucket *inBucket, int inRecIndex)
{
    int i = inRecIndex;
    ...
    if (binFP == inFingerprint)
    {
        ...
        uint64_t filePosRec = LINEARDB3_HEADER_SIZE + inBucket->fileIndex[i] * inDB->recordSizeBytes;
...
int LINEARDB3_getOrPut(LINEARDB3 *inDB, const void *inKey, void *inOutValue, char inPut, char inIgnoreDataFile)
{
    ...
    if (!skipToOverflow || thisBucket->overflowIndex == 0)
        for (int i = 0; i < RECORDS_PER_BUCKET; i++)
        {
            ...
            int result = LINEARDB3_considerFingerprintBucket(inDB, inKey, inOutValue, fingerprint, inPut,
                                                             inIgnoreDataFile, thisBucket, i);
    ...
    if (inPut && thisBucket->overflowIndex == 0)
    {
        ...
        FingerprintBucket *newBucket = getBucket(inDB->overflowBuckets, thisBucket->overflowIndex);
...
int LINEARDB3_Iterator_next(LINEARDB3_Iterator *inDBi, void *outKey, void *outValue)
{
    ...
    while (true)
    {
        ...
        if (db->lastOp == opWrite || ftello(db->file) != (off_t)fileRecPos)
        {
...
unsigned int LINEARDB3_getShrinkSize(LINEARDB3 *inDB, unsigned int inNewNumRecords)
{
    ...
    uint32_t minTableBuckets = LINEARDB3_getPerfectTableSize(inDB->maxLoad, inNewNumRecords);
