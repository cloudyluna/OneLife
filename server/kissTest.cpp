...
void intQuadToKey(int inX, int inY, int inSlot, int inB, unsigned char *outKey)
{
    for (int i = 0; i < 4; i++)
    {
        int offset = i * 8;
...
unsigned int iterateValues()
{
    ...
    printf("Iterating used %d bytes, took %f sec\n", getMallocDelta(), Time::getCurrentTime() - startTime);
...
int main()
{
    ...
    */
    ...
    printf("Optimal shrink size for %d values is %d\n", numValues, shrinkSize);

    DB db2;

    error = DB_open(&db2, "test2.db", KISSDB_OPEN_MODE_RWCREAT, shrinkSize,
