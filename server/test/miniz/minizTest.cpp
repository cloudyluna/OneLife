    ...
    if (f != NULL)
    {
        ...
        if (numRead == dataLength)
        {
            ...
            for (int t = 0; t < 1; t++)
            {
                ...
                int compressedSize;
                unsigned char *compressedChunkData = zipCompress(data, dataLength, &compressedSize);
                ...
                if (false)
                    printf("%d zero bytes in compressed data out of %d bytes\n", numZeroBytes, compressedSize);
                ...
                if (compressedChunkData != NULL)
                {
                    ...
                    unsigned char *decompData = zipDecompress(compressedChunkData, compressedSize, dataLength);
