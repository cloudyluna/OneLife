...
void primeLineageTest(int inNumLivePlayers)
{
    ...
    minRebirthDistance = SettingsManager::getIntSetting("minRebirthDistance", 200);
    ...
    float maxHours = SettingsManager::getFloatSetting("lineageLimitOtherLineHours", 1.5);
    ...
    float maxHoursOneLine = SettingsManager::getFloatSetting("lineageLimitOneLineHours", 0.5);
...
static SimpleVector<HashTableEntry> *lookupBin(const char *inPlayerEmail)
{
    ...
    for (int i = 0; i < len; i++)
    {
        hash = ((hash << 5) + hash) + (uint32_t)(((const uint8_t *)inPlayerEmail)[i]);
    }
...
static void insert(const char *inPlayerEmail, GridPos inBirthPos, double inLivedYears, double inOtherLineRequiredYears)
{
    ...
    LineageTime tNew = {inBirthPos, curTime, inLivedYears, 0, inOtherLineRequiredYears};

    tList->push_back(tNew);

    HashTableEntry e = {stringDuplicate(inPlayerEmail), curTime, tList};
    ...
    for (int i = 0; i < e->times->size(); i++)
    {
        ...
        else if (distance(t->birthPos, inBirthPos) < minRebirthDistance)
        {
            ...
            printf("Lived %f years in this line (over %f) and only "
                   ...
                   t->totalLivedThisLine, oneLineMaxYears, t->totalLivedOtherLines, t->otherLineRequiredYears);
...
void recordLineage(const char *inPlayerEmail, GridPos inBirthPos, double inLivedYears, char inMurdered,
                   char inCommittedMurderOrSID)
{
    ...
    if (e == NULL)
    {
        insert(inPlayerEmail, inBirthPos, livedInThisLineYears, otherLineRequiredYearsThis);
        return;
    }
    ...
    if (!found)
    {
        ...
        LineageTime t = {inBirthPos, curTime, livedInThisLineYears, 0, otherLineRequiredYearsThis};
        e->times->push_back(t);
