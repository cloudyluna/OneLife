...
static void checkSettings()
{
    ...
    {
        tokenTime = SettingsManager::getFloatSetting("curseTokenTime", 7200.0);
        decrementTime = SettingsManager::getFloatSetting("curseDecrementTime", 3600.0);
        ...
        useCurseServer =
            SettingsManager::getIntSetting("useCurseServer", 0) && SettingsManager::getIntSetting("remoteReport", 0);
        ...
        if (useCurseServer)
        {
            ...
            curseServerURL = SettingsManager::getStringSetting("curseServerURL", "");
        }
...
void initCurses()
{
    ...
    if (f != NULL)
    {
        ...
        while (numRead == 5)
        {
            numRead = fscanf(f, "%99s %d %d %lf %lf", email, &tokens, &score, &livedTimeSinceTokenSpent,
...
void freeCurses()
{
    ...
    if (f != NULL)
    {
        for (int i = 0; i < curseRecords.size(); i++)
        {
            ...
            fprintf(f, "%s %d %d %.f %.f\n", r->email, r->tokens, r->score, r->livedTimeSinceTokenSpent,
...
static void stepCurses()
{
    ...
    for (int i = 0; i < curseRecords.size(); i++)
    {
        ...
        if (!r->alive & (r->deathTime == 0 || curTime - r->deathTime > 10 * 60))
            if ((r->tokens == 1 && r->score == 0) || curTime - r->lastBirthTime > 3600 * 96)
            {
...
void cursesLogDeath(char *inEmail, double inAge, GridPos inDeathPos)
{
    ...
    if (r->alive)
    {
        ...
        double lifeTimeSinceToken = curTime - r->aliveStartTimeSinceTokenSpent;
        double lifeTimeSinceScoreDecrement = curTime - r->aliveStartTimeSinceScoreDecrement;
    ...
    for (int i = 0; i < playerNames.size(); i++)
    {
        ...
        if (strcmp(r->email, inEmail) == 0)
        {
            ...
            // player dies
            r->timeCreated = Time::getCurrentTime() + 60 * 5 - playerNameTimeout;
    ...
    if (useCurseServer)
    {
        ...
        RemoteUpdateRecord r = {stringDuplicate(inEmail), autoSprintf("action=live_time&seconds=%f", sec), -1, NULL};
...
static CurseRecord *findCurseRecordByName(char *inName, int *outLineageEveID)
{
...
char cursePlayer(int inGiverID, int inGiverLineageEveID, char *inGiverEmail, GridPos inGiverPos, double inMaxDistance,
                 char *inReceiverName)
{
    stepCurses();
    ...
    CurseRecord *receiverRecord = findCurseRecordByName(inReceiverName, &receiverLineageEveID);
    ...
    if (!receiverRecord->alive && distance(inGiverPos, receiverRecord->deathPos) > inMaxDistance)
    {
    ...
    if (useCurseServer)
    {
        ...
        RemoteUpdateRecord r = {stringDuplicate(receiverRecord->email), stringDuplicate("action=curse"), -1, NULL};
...
void logPlayerNameForCurses(char *inPlayerEmail, char *inPlayerName, int inLineageEveID)
{
    // structure to track names per player
    PlayerNameRecord r = {stringDuplicate(inPlayerName), stringDuplicate(inPlayerEmail), Time::getCurrentTime(),
                          inLineageEveID};
...
CurseStatus getCurseLevel(char *inPlayerEmail)
{
    ...
    else
    {
        ...
        char *curseServerSharedSecret = SettingsManager::getStringSetting("curseServerSharedSecret", "secret_phrase");
        ...
        char *url = autoSprintf("%s?action=is_cursed"
                                "&email=%s"
                                "&email_hash_value=%s",
                                curseServerURL, encodedEmail, hash);
...
void stepCurseServerRequests()
{
    ...
    for (int i = 0; i < serverPendingUpdates.size(); i++)
    {
        ...
        if (r->sequenceNumber == -1 && r->request == NULL)
        {
            ...
            char *url = autoSprintf("%s?action=get_sequence_number"
                                    "&email=%s",
                                    curseServerURL, encodedEmail);
        ...
        else if (result == 1)
        {
            ...
            if (r->sequenceNumber == -1)
            {
                ...
                if (r->sequenceNumber == -1)
                {
                    AppLog::info("Failed to get seq number from curse server.");
                ...
                char *curseServerSharedSecret =
                    SettingsManager::getStringSetting("curseServerSharedSecret", "secret_phrase");

                char *seqString = autoSprintf("%d", r->sequenceNumber);

                char *hash = hmac_sha1(curseServerSharedSecret, seqString);
                delete[] seqString;
                ...
                char *url = autoSprintf("%s?%s"
                                        "&email=%s"
                                        "&sequence_number=%d"
                                        "&hash_value=%s",
                                        curseServerURL, r->actionString, encodedEmail, r->sequenceNumber, hash);
            ...
            else
            {
                ...
                {
                    AppLog::infoF("Failed to get expected result from curseServer, "
                                  "got:  %s",
                                  webResult);
                }
                delete[] webResult;
