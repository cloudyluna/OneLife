...
char *getNextMessage(Client *inC)
{
    ...
    if (inC->pendingCMData)
    {
        if (inC->buffer.size() >= inC->pendingCMCompressedSize)
        {
            ...
            unsigned char *compressedData = new unsigned char[inC->pendingCMCompressedSize];
            ...
            unsigned char *decompressedMessage =
                zipDecompress(compressedData, inC->pendingCMCompressedSize, inC->pendingCMDecompressedSize);
            ...
            {
                char *textMessage = new char[inC->pendingCMDecompressedSize + 1];
                memcpy(textMessage, decompressedMessage, inC->pendingCMDecompressedSize);
    ...
    if (strstr(message, "CM") == message)
    {
        ...
        sscanf(message, "CM\n%d %d\n", &(inC->pendingCMDecompressedSize), &(inC->pendingCMCompressedSize));
...
int main(int inNumArgs, char **inArgs)
{
    ...
    for (int i = 0; i < numClients; i++)
    {
        ...
        char timeout = false;
        connections[i].sock = SocketClient::connectToServer(&a, 5000, &timeout);
        ...
        if (connections[i].sock != NULL)
        {
            ...
            char *message = autoSprintf("LOGIN %s aaaa aaaa#", email);

            connections[i].sock->send((unsigned char *)message, strlen(message), true, false);
        ...
        for (int i = 0; i < numClients; i++)
        {
            ...
            if (connections[i].id == -1)
            {
                ...
                if (message != NULL)
                {
                    ...
                    if (strstr(message, "MC") == message)
                    {
                        ...
                        sscanf(message, "MC\n%d %d %d %d\n%d %d\n", &sizeX, &sizeY, &x, &y, &binarySize, &compSize);
                    ...
                    else if (strstr(message, "PU") == message)
                    {
                        ...
                        if (numLines > 2)
                        {
                            ...
                            parsePlayerUpdateMessage(&(connections[i]), lines[numLines - 2]);
                        }

                        printf("Client %d got first player update, "
                               "pid = %d, pos = %d,%d\n",
                               i, connections[i].id, connections[i].x, connections[i].y);
            ...
            else
            {
                ...
                if (!connections[i].moving)
                {
                    ...
                    char *message = autoSprintf("MOVE %d %d %d %d#", connections[i].x, connections[i].y, xDelt, yDelt);
                ...
                if (message != NULL)
                {
                    ...
                    else if (strstr(message, "PU") == message)
                    {
                        ...
                        {
                            parsePlayerUpdateMessage(&(connections[i]), lines[p]);
                        }
