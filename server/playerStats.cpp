...
{
    int error = KISSDB_open(&db, "playerStats.db", KISSDB_OPEN_MODE_RWCREAT, 80000,
    ...
    useStatsServer =
        SettingsManager::getIntSetting("useStatsServer", 0) && SettingsManager::getIntSetting("remoteReport", 0);

    statsServerURL =
        SettingsManager::getStringSetting("statsServerURL", "http://localhost/jcr13/reviewServer/server.php");

    statsServerSharedSecret = SettingsManager::getStringSetting("statsServerSharedSecret", "secret_phrase");
}
...
int getPlayerLifeStats(char *inEmail, int *outNumLives, int *outTotalSeconds)
{
    ...
    if (useStatsServer)
    {
        ...
        StatRecord r = {stringDuplicate(inEmail),
                        // lookup
                        -1, request, -1, -1, -1, false, "get_sequence_number"};
        records.push_back(r);
        // in progress
        return 0;
    }
...
void recordPlayerLifeStats(char *inEmail, int inNumSecondsLived)
{
    ...
    if (useStatsServer)
    {
        ...
        StatRecord r = {stringDuplicate(inEmail), inNumSecondsLived, request, -1, -1, -1, false, "get_sequence_number"};
        records.push_back(r);
    }
    else
    {
        ...
        else if (result == 1)
        {
            ...
            if (r->sequenceNumber == -1)
            {
                ...
                else
                {
                    ...
                    char *hash = hmac_sha1(statsServerSharedSecret, seqString);
                    ...
                    {
                        // lookup
                        url = autoSprintf("%s?action=get_stats"
                                          "&email=%s"
                                          "&sequence_number=%d"
                                          "&hash_value=%s",
                                          statsServerURL, encodedEmail, r->sequenceNumber, hash);
                        r->serverAction = "get_stats";
                    }
                    else
                    {
                        url = autoSprintf("%s?action=log_game"
                                          "&email=%s"
                                          "&game_seconds=%d"
                                          "&sequence_number=%d"
                                          "&hash_value=%s",
                                          statsServerURL, encodedEmail, r->numGameSeconds, r->sequenceNumber, hash);
            ...
            else
            {
                ...
                {
                    AppLog::infoF("Server %s request rejected by stats server", r->serverAction);
                    r->error = true;
                }
                else if (r->numGameSeconds == -1 && strstr(webResult, "OK") != NULL)
                {
                    // lookup result
