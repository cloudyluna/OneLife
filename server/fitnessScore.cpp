...
static int stepOpRequest(FitnessOpRequest *inR)
{
    ...
    else if (r->seqW != NULL)
    {
        ...
        else if (result == 1)
        {
            ...
            char *sharedSecret = SettingsManager::getStringSetting("fitnessServerSharedSecret", "secret_phrase");
            ...
            char *serverURL = SettingsManager::getStringSetting("fitnessServerURL", "");
            ...
            char *url = autoSprintf("%s?%s"
                                    ...
                                    "&sequence_number=%d"
                                    "&hash_value=%s",
                                    serverURL, r->extraParams, encodedEmail, serverID, sequenceNumber, hash);
    ...
    else
    {
        ...
        char *url = autoSprintf("%s?action=get_server_sequence_number"
                                "&server_name=%s",
                                serverURL, encodedName);
...
int getFitnessScore(char *inEmail, float *outScore)
{
    ...
    for (int i = 0; i < allRequests.size(); i++)
    {
        ...
        if (!r->isDeathRequest && strcmp(r->email, inEmail) == 0)
        {
            // match
...
void logFitnessDeath(int inNumLivePlayers, char *inEmail, char *inName, int inDisplayID, double inAge,
                     SimpleVector<char *> *inAncestorEmails, SimpleVector<char *> *inAncestorRelNames,
                     SimpleVector<char *> *inAncestorData)
{
    ...
    if (inNumLivePlayers < SettingsManager::getIntSetting("minActivePlayersForFitness", 15))
    {
    ...
    for (int i = 0; i < num; i++)
    {
        workingList.appendElementString(inAncestorEmails->getElementDirect(i));
        workingDataList.appendElementString(inAncestorData->getElementDirect(i));
        ...
        char found;
        char *relNameNoSpace = replaceAll(relName, " ", "_", &found);
    ...
    r.extraParams = autoSprintf("action=report_death&"
                                "name=%s&"
                                "display_id=%d&"
                                "self_rel_name=You&"
                                "age=%f&"
                                "ancestor_list=%s&"
                                "data_list=%s",
                                encodedName, inDisplayID, inAge, encodedList, encodedDataList);
