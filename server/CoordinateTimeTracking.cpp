...
CoordinateTimeTracking::CoordinateTimeTracking() : mNextIndex(0)
{
}

char CoordinateTimeTracking::checkExists(int inX, int inY, timeSec_t inCurTime)
{
    ...
    if (numRecords > 0)
    {
        ...
        if (testR->y > inY || (testR->y == inY && testR->x > inX))
        {
            dir = -1;
        }
        ...
        if (dir == 1)
        {
            for (mNextIndex = mNextIndex; mNextIndex < numRecords; mNextIndex++)
            {
