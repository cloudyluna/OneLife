...
int DB_open_timeShrunk(DB *db, const char *path, int mode, unsigned long hash_table_size, unsigned long key_size,
...
int DB_open_naturalTileShrunk_mapDb(DB *db, const char *path, int mode, unsigned long hash_table_size,
                                    unsigned long key_size, unsigned long value_size)
{
    ...
    if (error)
    {
        AppLog::errorF("Failed to open DB file %s in DB_open_naturalTileShrunk", path);
        delete[] dbTempName;
    ...
    AppLog::infoF("Cleaned %d / %d natural map cells from %s", discardCount, totalCount, path);
...
// Biome settings need to be loaded before this
// as it will call the map generating function
int DB_open_naturalTileShrunk(DB *db, const char *path, int mode, unsigned long hash_table_size, unsigned long key_size,
                              unsigned long value_size)
{
...
int DB_open_modeSwitch(DB *db, const char *path, int mode, unsigned long hash_table_size, unsigned long key_size,
                       unsigned long value_size)
{
    ...
    if (dbShrinkMode == 0)
    {
        error = DB_open(db, path, mode, hash_table_size, key_size, value_size);
    }
    else if (dbShrinkMode == 1)
    {
        error = DB_open_timeShrunk(db, path, mode, hash_table_size, key_size, value_size);
    }
    else if (dbShrinkMode == 2)
    {
        if (strcmp(path, "map.db") == 0)
        {
            error = DB_open_naturalTileShrunk_mapDb(db, path, mode, hash_table_size, key_size, value_size);
        }
        else
        {
            error = DB_open_naturalTileShrunk(db, path, mode, hash_table_size, key_size, value_size);
        }
    }
    return error;
}
