...
void reportArcEnd()
{
    ...
    char sendReport =
        SettingsManager::getIntSetting("useArcServer", 0) && SettingsManager::getIntSetting("remoteReport", 0);
    ...
    char *serverURL = SettingsManager::getStringSetting("arcServerURL", "");
...
void stepArcReport()
{
    ...
    if (sequenceNumber == -1)
    {
        ...
        if (sequenceNumber != -1)
        {
            ...
            char *arcServerURL = SettingsManager::getStringSetting("arcServerURL", "");

            char *sharedSecret = SettingsManager::getStringSetting("arcServerSharedSecret", "");
            ...
            char *serverName = SettingsManager::getStringSetting("serverID", "");

            char *url = autoSprintf("%s?action=report_arc_end"
                                    "&server_name=%s"
                                    "&seconds_per_year=%f"
                                    "&sequence_number=%d"
                                    "&hash_value=%s",
                                    arcServerURL, serverName, secondsPerYear, sequenceNumber, hash);
...
void resetArcName()
{
    ...
    if (wordList.size() > 20)
    {
        ...
        randSource.reseed((unsigned int)fmod(Time::getCurrentTime(), UINT_MAX));

        char *pickedWord[2];

        for (int i = 0; i < 2; i++)
        {
            pickedWord[i] = wordList.getElementDirect(randSource.getRandomBoundedInt(0, wordList.size() - 1));
        }
        char *name = autoSprintf("%s %s", pickedWord[0], pickedWord[1]);
...
int getArcYearsToReport(double inSecondsPerYear, int inMilestoneYears)
{
