...
int main(int inNumArgs, char **inArgs)
{
    ...
    int error = STACKDB_open(&db, oldFileName, 0, oldTableSize, keySize, valueSize);
    if (error)
    {
    ...
    uint64_t maxFileSize = LINEARDB_getMaxFileSize(newTableSize, keySize, valueSize, count);
    ...
    error = LINEARDB_open(&dbNew, NULL, 0, newTableSize, keySize, valueSize);
    if (error)
    {
