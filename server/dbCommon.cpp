...
{
    return inValue[3] << 24 | inValue[2] << 16 | inValue[1] << 8 | inValue[0];
}
...
timeSec_t valueToTime(unsigned char *inValue)
{
    ...
    intTime = (uint64_t)inValue[7] << 56 | (uint64_t)inValue[6] << 48 | (uint64_t)inValue[5] << 40 |
              (uint64_t)inValue[4] << 32 | (uint64_t)inValue[3] << 24 | (uint64_t)inValue[2] << 16 |
