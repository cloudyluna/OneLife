...
// may be partial
static PeaceTreaty *getMatchingTreaty(int inLineageAEveID, int inLineageBEveID)
{
    ...
    for (int i = 0; i < peaceTreaties.size(); i++)
    {
        ...
        if ((p->lineageAEveID == inLineageAEveID && p->lineageBEveID == inLineageBEveID) ||
            (p->lineageAEveID == inLineageBEveID && p->lineageBEveID == inLineageAEveID))
        {
            // they match a treaty.
            return p;
        }
...
static char isPeaceTreaty(int inLineageAEveID, int inLineageBEveID, PeaceTreaty **outPartialTreaty = NULL)
{
...
static void addPeaceTreaty(int inLineageAEveID, int inLineageBEveID)
{
    ...
    else
    {
        ...
        PeaceTreaty p = {inLineageAEveID, inLineageBEveID, true, false, false, false};
...
static void removePeaceTreaty(int inLineageAEveID, int inLineageBEveID)
{
    ...
    if (remove)
    {
        for (int i = 0; i < peaceTreaties.size(); i++)
        {
            ...
            if (otherP->lineageAEveID == p->lineageAEveID && otherP->lineageBEveID == p->lineageBEveID)
            {
...
void removeAllOwnership(LiveObject *inPlayer)
{
    ...
    AppLog::infoF("Removing all ownership (%d owned) for "
                  ...
                  num, inPlayer->id, inPlayer->email, Time::getCurrentTime() - startTime);
}
...
FullMapContained getFullMapContained(int inX, int inY)
{
    ...
    for (int c = 0; c < r.numContained; c++)
    {
        if (r.containedIDs[c] < 0)
        {
            ...
            timeSec_t *subContainedEtaDecays = getContainedEtaDecay(inX, inY, &numSub, c + 1);
...
void transferHeldContainedToMap(LiveObject *inPlayer, int inX, int inY)
{
    if (inPlayer->numContained != 0)
    {
        ...
        for (int c = 0; c < inPlayer->numContained; c++)
        {
            ...
            addContained(inX, inY, inPlayer->containedIDs[c], inPlayer->containedEtaDecays[c]);
            ...
            if (numSub > 0)
            {
                ...
                timeSec_t *subDecays = inPlayer->subContainedEtaDecays[c].getElementArray();
...
static double measurePathLength(int inXS, int inYS, GridPos *inPathPos, int inPathLength)
{
...
void quitCleanup()
{
    ...
    SimpleVector<FreshConnection> *connectionLists[2] = {&newConnections, &waitingForTwinConnections};
...
static const char *getCurseWord(char *inSenderEmail, char *inEmail, int inWordIndex)
{
    ...
    if (curseSecret == NULL)
    {
        curseSecret = SettingsManager::getStringSetting("statsServerSharedSecret", "sdfmlk3490sadfm3ug9324");
    }
    ...
    {
        emailPlusSecret = autoSprintf("%s_%s_%s", inSenderEmail, inEmail, curseSecret);
    }
    else
    {
        emailPlusSecret = autoSprintf("%s_%s", inEmail, curseSecret);
    }

    unsigned int c = crc32((unsigned char *)emailPlusSecret, strlen(emailPlusSecret));
...
char *getNextClientMessage(SimpleVector<char> *inBuffer)
{
    ...
    if (index > 1 && inBuffer->getElementDirect(0) == 'K' && inBuffer->getElementDirect(1) == 'A')
    {
...
ClientMessage parseMessage(LiveObject *inPlayer, char *inMessage)
{
    ...
    if (strcmp(nameBuffer, "MOVE") == 0)
    {
        ...
        if (atPos != NULL)
        {
            ...
            m.sequenceNumber = stringToInt(&(tokens->getElementDirect(3)[1]));
        }
        ...
        for (int e = 0; e < m.numExtraPos; e++)
        {
            ...
            if (abs(m.extraPos[e].x) > pathDeltaMax || abs(m.extraPos[e].y) > pathDeltaMax)
            {
    ...
    else if (strcmp(nameBuffer, "USE") == 0)
    {
        ...
        numRead = sscanf(inMessage, "%99s %d %d %d %d", nameBuffer, &(m.x), &(m.y), &(m.id), &(m.i));
    ...
    else if (strcmp(nameBuffer, "SELF") == 0)
    {
        ...
        numRead = sscanf(inMessage, "%99s %d %d %d", nameBuffer, &(m.x), &(m.y), &(m.i));
    ...
    else if (strcmp(nameBuffer, "UBABY") == 0)
    {
        ...
        // id param optional
        numRead = sscanf(inMessage, "%99s %d %d %d %d", nameBuffer, &(m.x), &(m.y), &(m.i), &(m.id));
    ...
    else if (strcmp(nameBuffer, "BABY") == 0)
    {
        m.type = BABY;
        // read optional id parameter
        numRead = sscanf(inMessage, "%99s %d %d %d", nameBuffer, &(m.x), &(m.y), &(m.id));
    ...
    else if (strcmp(nameBuffer, "SREMV") == 0)
    {
        ...
        numRead = sscanf(inMessage, "%99s %d %d %d %d", nameBuffer, &(m.x), &(m.y), &(m.c), &(m.i));
    ...
    {
        m.type = DROP;
        numRead = sscanf(inMessage, "%99s %d %d %d", nameBuffer, &(m.x), &(m.y), &(m.c));
...
int computePartialMovePathStep(LiveObject *inPlayer)
{
    ...
    double fractionDone = (Time::getCurrentTime() - inPlayer->moveStartTime) / inPlayer->moveTotalSeconds;
    ...
    int c = lrint((inPlayer->pathLength) * fractionDone);
    return c - 1;
}
...
static void setDeathReason(LiveObject *inPlayer, const char *inTag, int inOptionalID = 0)
{
    ...
    if (strcmp(inTag, "killed") == 0 || strcmp(inTag, "succumbed") == 0 || strcmp(inTag, "suicide") == 0)
    {
...
{
    if (inPlayer->curseStatus.curseLevel == 0 && inPlayer->parentChainLength > longestShutdownLine)
    {
...
int computeFoodCapacity(LiveObject *inPlayer)
{
    ...
    if (ageInYears < oldAge)
    {
        ...
        if (returnVal > maxFoodcap)
            returnVal = maxFoodcap;
    }
    else
    {
        ...
        if (lostBars > 0 && inPlayer->fitnessScore > 0)
        {
            ...
            double maxLostBars = (maxFoodcap - minFoodCap) * (1 - pow(inPlayer->fitnessScore / 60.0, 2));
...
double computeMoveSpeed(LiveObject *inPlayer)
{
    ...
    if (age < fullSpeedAge)
    {
        ...
        double speedFactor = babySpeedFactor + (1.0 - babySpeedFactor) * age / fullSpeedAge;
    ...
    for (int c = 0; c < NUM_CLOTHING_PIECES; c++)
    {
        ...
        if (cO != NULL)
        {
            ...
            for (int s = 0; s < inPlayer->clothingContained[c].size(); s++)
            {
                ...
                ObjectRecord *sO = getObject(inPlayer->clothingContained[c].getElementDirect(s));
        ...
        for (int c = 0; c < inPlayer->numContained; c++)
        {
            ...
            if (hasSub)
            {
                ...
                for (int s = 0; s < inPlayer->subContainedIDs[c].size(); s++)
                {
                    ...
                    ObjectRecord *subO = getObject(inPlayer->subContainedIDs[c].getElementDirect(s));
...
static void recomputeHeatMap(LiveObject *inPlayer)
{
    ...
    GridPos playerHeatMapPos = {playerMapIndex % HEAT_MAP_D, playerMapIndex / HEAT_MAP_D};
    ...
    inPlayer->envHeat = radiantHeatWeight * radiantAirSpaceHeatVal + containedHeatWeight * containedAirSpaceHeatVal +
...
MoveRecord getMoveRecord(LiveObject *inPlayer, char inNewMovesOnly, SimpleVector<ChangePosition> *inChangeVector = NULL)
{
    ...
    // start is absolute
    char *startString = autoSprintf("%d %%d %%d %.3f %.3f %d", inPlayer->id, inPlayer->moveTotalSeconds, etaSec,
    ...
    for (int p = 0; p < inPlayer->pathLength; p++)
    {
        ...
        char *stepString =
            autoSprintf(" %d %d", inPlayer->pathToDest[p].x - inPlayer->xs, inPlayer->pathToDest[p].y - inPlayer->ys);
...
char *getMovesMessageFromList(SimpleVector<MoveRecord> *inMoves, GridPos inRelativeToPos)
{
    ...
    for (int i = 0; i < inMoves->size(); i++)
    {
        ...
        char *line = autoSprintf(r.formatString, r.absoluteX - inRelativeToPos.x, r.absoluteY - inRelativeToPos.y);
...
char *getMovesMessage(char inNewMovesOnly, GridPos inRelativeToPos, GridPos inLocalPos,
...
static void setPlayerDisconnected(LiveObject *inPlayer, const char *inReason)
{
    ...
    */
    ...
    AppLog::infoF("Player %d (%s) marked as disconnected (%s).", inPlayer->id, inPlayer->email, inReason);
...
void sendGlobalMessage(char *inMessage, LiveObject *inOnePlayerOnly)
{
    ...
    for (int i = 0; i < players.size(); i++)
    {
        ...
        if (!o->error && !o->isTutorial && o->connected)
        {
            ...
            if (curTime - o->lastGlobalMessageTime > minGlobalMessageSpacingSeconds)
            {

                int numSent = o->sock->send((unsigned char *)fullMessage, len, false, false);
...
void sendPeaceWarMessage(const char *inPeaceOrWar, char inWar, int inLineageAEveID, int inLineageBEveID)
{
    ...
    for (int i = 0; i < warPeaceRecords.size(); i++)
    {
        ...
        if ((r->lineageAEveID == inLineageAEveID && r->lineageBEveID == inLineageBEveID) ||
            (r->lineageAEveID == inLineageBEveID && r->lineageBEveID == inLineageAEveID))
        {
    ...
    for (int j = 0; j < players.size(); j++)
    {
        ...
        if (!o->error && o->lineageEveID == inLineageAEveID && o->familyName != NULL)
        {
            nameA = o->familyName;
            break;
        }
    ...
    for (int j = 0; j < players.size(); j++)
    {
        ...
        if (!o->error && o->lineageEveID == inLineageBEveID && o->familyName != NULL)
        {
            nameB = o->familyName;
            break;
        }
    ...
    char *message = autoSprintf("%s BETWEEN %s**AND %s FAMILIES", inPeaceOrWar, nameA, nameB);
...
void checkCustomGlobalMessage()
{
    ...
    double spacing = SettingsManager::getDoubleSetting("customGlobalMessageSecondsSpacing", 10.0);

    double lastTime = SettingsManager::getDoubleSetting("customGlobalMessageLastSendTime", 0.0);
    ...
    char *message = SettingsManager::getSettingContents("customGlobalMessage", "");
    ...
    if (strcmp(message, "") != 0)
    {
        ...
        int nextLine = SettingsManager::getIntSetting("customGlobalMessageNextLine", 0);
        ...
        if (nextLine < numLines)
        {
            ...
            SettingsManager::setDoubleSetting("customGlobalMessageLastSendTime", curTime);
        }
        else
        {
            // out of lines
...
int sendMapChunkMessage(LiveObject *inO, char inDestOverride = false, int inDestOverrideX = 0, int inDestOverrideY = 0)
{
    ...
    if (!inO->firstMapSent)
    {
        ...
        unsigned char *mapChunkMessage =
            getChunkMessage(fullStartX, fullStartY, chunkDimensionX, chunkDimensionY, inO->birthPos, &messageLength);
    ...
    else
    {
        ...
        if (horBarW > 0 && horBarH > 0)
        {
            int len;
            unsigned char *mapChunkMessage =
                getChunkMessage(horBarStartX, horBarStartY, horBarW, horBarH, inO->birthPos, &len);
            messageLength += len;
        ...
        if (vertBarW > 0 && vertBarH > 0)
        {
            int len;
            unsigned char *mapChunkMessage =
                getChunkMessage(vertBarStartX, vertBarStartY, vertBarW, vertBarH, inO->birthPos, &len);
            messageLength += len;
...
char *getHoldingString(LiveObject *inObject)
{
    ...
    if (inObject->numContained > 0)
    {
        for (int i = 0; i < inObject->numContained; i++)
        {
            ...
            char *idString = autoSprintf(",%d", hideIDForClient(abs(inObject->containedIDs[i])));
            ...
            if (inObject->subContainedIDs[i].size() > 0)
            {
                for (int s = 0; s < inObject->subContainedIDs[i].size(); s++)
                {
                    ...
                    idString = autoSprintf(":%d", hideIDForClient(inObject->subContainedIDs[i].getElementDirect(s)));
...
static void setFreshEtaDecayForHeld(LiveObject *inPlayer)
{
    ...
    if (newDecayT != NULL)
    {
        inPlayer->holdingEtaDecay = Time::getCurrentTime() + newDecayT->autoDecaySeconds;
    }
    else
    {
        // no further decay
...
void handleMapChangeToPaths(int inX, int inY, ObjectRecord *inNewObject,
                            SimpleVector<int> *inPlayerIndicesToSendUpdatesAbout)
{
    ...
    if (inNewObject->blocksWalking)
    {
        ...
        for (int j = 0; j < numLive; j++)
        {
            ...
            if (otherPlayer->xd != otherPlayer->xs || otherPlayer->yd != otherPlayer->ys)
            {
                ...
                if (distance(cPos, dropSpot) <= 2 * pathDeltaMax)
                {
                    ...
                    if (blocked && blockedStep > 0)
                    {
                        ...
                        // update timing
                        double dist = measurePathLength(otherPlayer->xs, otherPlayer->ys, otherPlayer->pathToDest,
                                                        otherPlayer->pathLength);

                        double distAlreadyDone =
                            measurePathLength(otherPlayer->xs, otherPlayer->ys, otherPlayer->pathToDest, c);
...
static LiveObject *getPlayerByEmail(char *inEmail)
{
    ...
    {
        LiveObject *otherPlayer = players.getElement(j);
        if (!otherPlayer->error && otherPlayer->email != NULL && strcmp(otherPlayer->email, inEmail) == 0)
        {
...
static void makePlayerSay(LiveObject *inPlayer, char *inToSay, bool inPrivate = false)
{
    ...
    if (getFemale(inPlayer))
    {
        ...
        if (infertilityDeclaring != NULL || fertilityDeclaring != NULL)
            return;
    }
    ...
    if (strcmp(inToSay, curseBabyPhrase) == 0 && SettingsManager::getIntSetting("allowBabyCursing", 0))
    {
    ...
    if (cursedName == NULL && players.size() >= minActivePlayersForLanguages)
    {
        ...
        for (int i = 0; i < players.size(); i++)
        {
            ...
            if (otherPlayer == inPlayer || otherPlayer->error || otherPlayer->lineageEveID == inPlayer->lineageEveID)
            {
                continue;
            }

            if (distance(speakerPos, getPlayerPos(otherPlayer)) > curseDistance)
            {
            ...
            char *translatedPhrase = mapLanguagePhrase(
                inToSay, inPlayer->lineageEveID, otherPlayer->lineageEveID, inPlayer->id, otherPlayer->id, speakerAge,
                computeAge(otherPlayer), inPlayer->parentID, otherPlayer->parentID);
    ...
    if (cursedName != NULL && strcmp(cursedName, "") != 0)
    {
        ...
        for (int i = 0; i < players.size(); i++)
        {
            ...
            if (otherPlayer->name != NULL && strcmp(otherPlayer->name, cursedName) == 0)
            {
                // matching player
    ...
    if (canCurse && cursedName != NULL && strcmp(cursedName, "") != 0)
    {

        isCurse =
            cursePlayer(inPlayer->id, inPlayer->lineageEveID, inPlayer->email, speakerPos, curseDistance, cursedName);
    ...
    if (canCurse && !isCurse)
    {
        ...
        if (isYouShortcut && youCursePlayer != NULL && spendCurseToken(inPlayer->email))
        {
        ...
        else if (isBabyShortcut && babyCursePlayer != NULL && spendCurseToken(inPlayer->email))
        {
        ...
        else if (isBabyShortcut && babyCursePlayer == NULL && inPlayer->lastBabyEmail != NULL &&
                 spendCurseToken(inPlayer->email))
        {

            isCurse = true;

            setDBCurse(inPlayer->id, inPlayer->email, inPlayer->lastBabyEmail);
    ...
    if (dbCurseTargetEmail != NULL && usePersonalCurses)
    {
        ...
        {
            char *message =
                autoSprintf("CU\n%d 1 %s_%s\n#", targetP->id, getCurseWord(inPlayer->email, targetP->email, 0),
                            getCurseWord(inPlayer->email, targetP->email, 1));
    ...
    if (isCurse)
    {
        if (inPlayer->curseStatus.curseLevel == 0 && hasCurseToken(inPlayer->email))
        {
    ...
    if (inPrivate)
        p.responsiblePlayerID = inPlayer->id;
...
// 2HOL mechanics to read written objects
static void forceObjectToRead(LiveObject *inPlayer, int inObjectID, GridPos inReadPos, bool passToRead)
{
    ...
    if (found)
    {
        ...
        newLocationSpeech.push_back(stringDuplicate(quotedPhrase));
...
static int isGraveSwapDest(int inTargetX, int inTargetY, int inDroppingPlayerID)
{
    ...
    for (int i = 0; i < players.size(); i++)
    {
        ...
        if (o->holdingID > 0 && strstr(getObject(o->holdingID)->description, "origGrave") != NULL)
        {

            if (inTargetX == o->heldGraveOriginX && inTargetY == o->heldGraveOriginY)
            {
                return true;
            }
        }
...
void handleDrop(int inX, int inY, LiveObject *inDroppingPlayer, SimpleVector<int> *inPlayerIndicesToSendUpdatesAbout)
{
    ...
    if (oldHoldingID > 0 && getObject(oldHoldingID)->permanent)
    {
        ...
        if (bareTrans != NULL && bareTrans->newTarget > 0)
        {
            ...
            if (bareTrans->newActor > 0)
            {
                ...
                setFreshEtaDecayForHeld(inDroppingPlayer);
                handleDrop(inX, inY, inDroppingPlayer, inPlayerIndicesToSendUpdatesAbout);
            }
    ...
    else if (oldHoldingID > 0 && !getObject(oldHoldingID)->permanent)
    {
        ...
        if (bareTrans != NULL && bareTrans->newTarget > 0 && bareTrans->newActor == 0)
        {
    ...
    if ((inDroppingPlayer->holdingID < 0 && mapSpotBlocking) || (inDroppingPlayer->holdingID > 0 && mapID != 0))
    {
        ...
        else
        {
            ...
            if (inDroppingPlayer->holdingID < 0)
            {
                ...
                if (babyO != NULL)
                {
                    ...
                    {
                        inPlayerIndicesToSendUpdatesAbout->push_back(getLiveObjectIndex(babyID));
                    }
                }
            }
    ...
    if (inDroppingPlayer->holdingID < 0)
    {
        ...
        if (babyO != NULL)
        {
            ...
            {
                // reset food decrement time
                babyO->foodDecrementETASeconds = Time::getCurrentTime() + computeFoodDecrementTimeSeconds(babyO);
            }
    ...
    if (strstr(o->description, "origGrave") != NULL)
    {
        ...
        GraveMoveInfo g = {
            {inDroppingPlayer->heldGraveOriginX, inDroppingPlayer->heldGraveOriginY}, {targetX, targetY}, swapDest};
    ...
    if (inPlayerIndicesToSendUpdatesAbout != NULL)
    {
        handleMapChangeToPaths(targetX, targetY, droppedObject, inPlayerIndicesToSendUpdatesAbout);
    }
}
...
void handleForcedBabyDrop(LiveObject *inBabyObject, SimpleVector<int> *inPlayerIndicesToSendUpdatesAbout)
{
    ...
    for (int j = 0; j < numLive; j++)
    {
        ...
        if (-adultO->holdingID == inBabyObject->id)
        {
            ...
            handleDrop(dropPos.x, dropPos.y, adultO, inPlayerIndicesToSendUpdatesAbout);
...
static void swapHeldWithGround(LiveObject *inPlayer, int inTargetID, int inMapX, int inMapY,
                               SimpleVector<int> *inPlayerIndicesToSendUpdatesAbout)
{

    if (inTargetID == inPlayer->holdingID && inPlayer->numContained == 0 && getNumContained(inMapX, inMapY) == 0)
    {
    ...
    if (pickupTrans != NULL && pickupTrans->newActor > 0 && pickupTrans->newTarget == 0)
    {
    ...
    if (inPlayer->holdingID > 0 && strstr(getObject(inPlayer->holdingID)->description, "origGrave") != NULL &&
        gravePlayerID > 0)
    {
...
static char *getUpdateLineFromRecord(UpdateRecord *inRecord, GridPos inRelativeToPos, GridPos inObserverPos)
{
    ...
    if (inRecord->posUsed)
    {
        ...
        if (distance(updatePos, inObserverPos) > getMaxChunkDimension() * 2)
        {
            ...
            return autoSprintf(inRecord->formatString, 1977, 1977, 1977, 1977, 1977, 1977);
        }

        return autoSprintf(inRecord->formatString, inRecord->absoluteActionTarget.x - inRelativeToPos.x,
                           ...
                           inRecord->absolutePosX - inRelativeToPos.x, inRecord->absolutePosY - inRelativeToPos.y);
    }
    else
    {
...
static char isYummy(LiveObject *inPlayer, int inObjectID)
{
    ...
    if (inObjectID == inPlayer->cravingFood.foodID && computeAge(inPlayer) >= minAgeForCravings)
    {
        return true;
    }
...
static void updateYum(LiveObject *inPlayer, int inFoodEatenID, char inFedSelf = true)
{
    ...
    if (wasYummy || inPlayer->yummyFoodChain.size() == 0)
    {
        ...
        if (eatenID == inPlayer->cravingFood.foodID && computeAge(inPlayer) >= minAgeForCravings)
        {
            ...
            inPlayer->cravingFood =
                getCravedFood(inPlayer->lineageEveID, inPlayer->parentChainLength, inPlayer->cravingFood);
    ...
    if (wasYummy || strstr(o->description, "modTool"))
    {
    ...
    else
    {
        ...
        if (inPlayer->foodStore > cap)
            inPlayer->foodStore = cap;
    }
}
...
static UpdateRecord getUpdateRecord(LiveObject *inPlayer, char inDelete, char inPartial = false)
{
    ...
    if (inPlayer->xs == inPlayer->xd && inPlayer->ys == inPlayer->yd && !inPlayer->heldByOther)
    {
        // not moving
    ...
    if (inPartial || inPlayer->xs != inPlayer->xd || inPlayer->ys != inPlayer->yd)
    {
    ...
    for (int c = 0; c < NUM_CLOTHING_PIECES; c++)
    {
        ...
        if (cObj != NULL && cObj->numSlots > 0)
        {
            ...
            {
                char *contString =
                    autoSprintf(",%d", hideIDForClient(inPlayer->clothingContained[c].getElementDirect(cc)));
    ...
    r.formatString =
        autoSprintf("%d %d %d %d %%d %%d %s %d %%d %%d %d "
                    "%.2f %s %.2f %.2f %.2f %s %d %d %d %d%s\n",
                    inPlayer->id, inPlayer->displayID, inPlayer->facingOverride, inPlayer->actionAttempt,
                    ...
                    hideIDForClient(inPlayer->heldTransitionSourceID), inPlayer->heat, posString, computeAge(inPlayer),
                    1.0 / getAgeRate(), computeMoveSpeed(inPlayer), clothingList, inPlayer->justAte,
                    hideIDForClient(inPlayer->justAteID), inPlayer->responsiblePlayerID, heldYum, deathReason);
...
static char *getUpdateLine(LiveObject *inPlayer, GridPos inRelativeToPos, GridPos inObserverPos, char inDelete,
...
static LiveObject *getHitPlayer(int inX, int inY, int inTargetID = -1, char inCountMidPath = false, int inMaxAge = -1,
...
static char isEveWindow()
{
    ...
    if (players.size() <= SettingsManager::getIntSetting("minActivePlayersForEveWindow", 15))
    {
        // not enough players
        // always Eve window
    ...
    else
    {
        ...
        if (secSinceStart > SettingsManager::getIntSetting("eveWindowSeconds", 3600))
        {
            return false;
        }
        return true;
    ...
    for (int i = 0; i < numParts; i++)
    {
        ...
        if (strstr(lines[i], inEmail) == lines[i])
        {
            ...
            if (on == 1)
            {
                ...
                int numRead = sscanf(lines[i], "%99s %d %d,%d %lf %19s %19s %d %d %d %d %d %d %d %d", emailBuff, &on,
                                     &outRecordToFill->pos.x, &outRecordToFill->pos.y, &outRecordToFill->age,
                                     outRecordToFill->firstName, outRecordToFill->lastName, &outRecordToFill->displayID,
                                     &outRecordToFill->hatID, &outRecordToFill->tunicID, &outRecordToFill->bottomID,
                                     &outRecordToFill->frontShoeID, &outRecordToFill->backShoeID,
                                     &outRecordToFill->backpackID, &outRecordToFill->holdingID);
...
int processLoggedInPlayer(char inAllowReconnect, Socket *inSock, SimpleVector<char> *inSockBuffer, char *inEmail,
                          // passing the whole thing for the seed and famTarget
                          FreshConnection *connection, int inTutorialNumber, CurseStatus inCurseStatus,
                          PastLifeStats inLifeStats, float inFitnessScore,
                          // set to -2 to force Eve
                          int inForceParentID = -1, int inForceDisplayID = -1, GridPos *inForcePlayerPos = NULL)
{
    ...
    if (usePersonalCurses)
    {
        ...
        for (int p = 0; p < players.size(); p++)
        {
            ...
            if (!o->error && !o->isTutorial && o->curseStatus.curseLevel == 0 && strcmp(o->email, inEmail) != 0)
            {
    ...
    newPlayerFoodEatingBonus = SettingsManager::getIntSetting("newPlayerFoodEatingBonus", 5);
    newPlayerFoodDecrementSecondsBonus = SettingsManager::getFloatSetting("newPlayerFoodDecrementSecondsBonus", 8);
    newPlayerFoodBonusHalfLifeSeconds = SettingsManager::getFloatSetting("newPlayerFoodBonusHalfLifeSeconds", 36000);
    ...
    if (familyDataLogFile != NULL)
    {
        ...
        fprintf(familyDataLogFile,
                ...
                "avAge:%.2f\n",
                Time::getCurrentTime(), newObject.id, cFam, cM, cB, players.size(), eveCount, inCount, averageAge);
    }
    ...
    for (int p = 0; p < 2; p++)
    {
        ...
        for (int i = 0; i < numPlayers; i++)
        {
            ...
            if (player->lastSidsBabyEmail != NULL && strcmp(player->lastSidsBabyEmail, newObject.email) == 0)
            {
            ...
            if (isFertileAge(player))
            {
                ...
                if (checkCooldown && Time::timeSec() < player->birthCoolDown)
                {
                    continue;
                }
                ...
                if (usePersonalCurses && isBirthLocationCurseBlocked(newObject.email, motherPos))
                {
                    // this spot forbidden
                ...
                for (int s = 0; s < tempTwinEmails.size(); s++)
                {
                    if (usePersonalCurses &&
                        ...
                        //  for our email)
                        isBirthLocationCurseBlockedNoCache(tempTwinEmails.getElementDirect(s), motherPos))
                    {
                        twinBanned = true;
                ...
                if ((inCurseStatus.curseLevel <= 0 && player->curseStatus.curseLevel <= 0) ||
                    (inCurseStatus.curseLevel > 0 && player->curseStatus.curseLevel > 0))
                {
        ...
        if (p == 0)
        {
            ...
            AppLog::infoF("Trying to place new baby %s, out of %d fertile moms, "
                          "all are on cooldown, lineage banned, or curse blocked.  "
                          "Trying again ignoring cooldowns.",
    ...
    if (parentChoices.size() > 0)
    {
        ...
        if (newObject.isEve)
        {
            ...
            int parentIndex = randSource.getRandomBoundedInt(0, parentChoices.size() - 1);
        ...
        if (!newObject.isEve)
        {
            ...
            parent->foodDecrementETASeconds += computeFoodDecrementTimeSeconds(parent);
            ...
            if (parentRaceIndex != -1)
            {
                ...
                if (forceDifferentRace || randSource.getRandomDouble() > childSameRaceLikelihood)
                {
                ...
                if (childRace == parentObject->race)
                {
                    ...
                    newObject.displayID =
                        getRandomFamilyMember(parentObject->race, parent->displayID, familySpan, forceGirl);
                }
                else
                {
    ...
    else if (inTutorialNumber > 0)
    {
        ...
        placed = loadTutorialStart(&(newObject.tutorialLoad), mapFileName, startX, startY);
    ...
    if (!placed)
    {
        ...
        for (int i = 0; i < numPlayers; i++)
        {
            ...
            if (player->error || !player->connected || player->isTutorial || player->vogMode)
            {
                continue;
            }
        ...
        int startX, startY;
        getEvePosition(newObject.email, newObject.id, &startX, &startY, &otherPeoplePos, allowEveRespawn);
        ...
        if (useSeedList && connection->hashedSpawnSeed == 0)
        {
            ...
            // parse the seeds
            SimpleVector<char *> *list = SettingsManager::getSetting("defaultSeedList");

            // chose a random seed from the list
            int seedIndex = randSource.getRandomBoundedInt(0, list->size() - 1);
            ...
            tempHashedSpawnSeed = fnv1aHash(seed, fnv1aHash(seedPepper));
        }
        else
        {
    ...
    {
        SimpleVector<int> *objectsPool = SettingsManager::getIntSettingMulti("randomisePlayersObjectPool");
    ...
    if (parent == NULL)
    {
        ...
        if (forceAge > 0)
        {
            newObject.lifeStartTimeSeconds = Time::getCurrentTime() - forceAge * (1.0 / getAgeRate());
        }
    }
    ...
    if (areTriggersEnabled())
    {
        ...
        if (id != -1)
        {
            ...
            newObject.lifeStartTimeSeconds =
                Time::getCurrentTime() - getTriggerPlayerAge(inEmail) * (1.0 / getAgeRate());
    ...
    if (newObject.curseStatus.curseLevel == 0 && hasCurseToken(inEmail))
    {
    ...
    if (parent != NULL && isFertileAge(parent))
    {
        ...
        for (int i = 0; i < parent->lineage->size() && i < maxLineageTracked - 1; i++)
        {
        ...
        if (strstr(newObject.email, "paxkiosk"))
        {
            ...
            // let the mother know
            sendGlobalMessage((char *)"YOUR BABY IS A NEW PLAYER FROM THE PAX EXPO BOOTH.**"
                                      "PLEASE HELP THEM LEARN THE GAME.  THANKS!  -JASON",
                              parent);
        }
        else if (isUsingStatsServer() && !newObject.lifeStats.error &&
                 (newObject.lifeStats.lifeCount < SettingsManager::getIntSetting("newPlayerLifeCount", 5) ||
                  newObject.lifeStats.lifeTotalSeconds <
                      SettingsManager::getIntSetting("newPlayerLifeTotalSeconds", 7200)))
        {
            // a new player (not at a PAX kiosk)
            // let mother know
            char *motherMessage = SettingsManager::getSettingContents("newPlayerMessageForMother", "");
    ...
    if (!newObject.isTutorial && isUsingStatsServer() && !newObject.lifeStats.error)
    {

        int sec = newObject.lifeStats.lifeTotalSeconds;

        double halfLifeFactor = pow(0.5, sec / newPlayerFoodBonusHalfLifeSeconds);

        newObject.personalEatBonus = lrint(halfLifeFactor * newPlayerFoodEatingBonus);

        newObject.personalFoodDecrementSecondsBonus = lrint(halfLifeFactor * newPlayerFoodDecrementSecondsBonus);
    }
    ...
    if (forceSpawn)
    {
        ...
        newObject.lifeStartTimeSeconds = Time::getCurrentTime() - forceSpawnInfo.age * (1.0 / getAgeRate());

        newObject.displayedName = autoSprintf("%s %s", forceSpawnInfo.firstName, forceSpawnInfo.lastName);
        ...
        newObject.clothing.backpack = getObject(forceSpawnInfo.backpackID, true);
    ...
    for (int j = 0; j < players.size(); j++)
    {
        ...
        if (newObject.parentID != otherPlayer->id &&
            (!getFemale(otherPlayer) || countLivingChildren(otherPlayer->id) == 0))
        {
        ...
        else
        {
            ...
            for (int i = 0; i < newObject.lineage->size(); i++)
            {
                ...
                if (newObject.lineage->getElementDirect(i) == otherPlayer->id)
                {
                    ...
                    newObject.ancestorLifeStartTimeSeconds->push_back(otherPlayer->lifeStartTimeSeconds);
                    newObject.ancestorLifeEndTimeSeconds->push_back(-1.0);
    ...
    if (newObject.isTutorial)
    {
        AppLog::infoF("New player %s pending tutorial load (tutorial=%d)", newObject.email, inTutorialNumber);
    ...
    if (!newObject.isTutorial)
        logBirth(newObject.id, newObject.email, newObject.parentID, parentEmail, !getFemale(&newObject),
                 getObject(newObject.displayID)->race, newObject.xd, newObject.yd, players.size(),
                 newObject.parentChainLength);

    AppLog::infoF("New player %s connected as player %d (tutorial=%d) (%d,%d)"
                  " (maxPlacementX=%d)",
                  newObject.email, newObject.id, inTutorialNumber, newObject.xs, newObject.ys, maxPlacementX);
...
{
    AppLog::infoF("Player %s waiting for twin party of %d", inConnection.email, inConnection.twinCount);
    ...
    for (int i = 0; i < waitingForTwinConnections.size(); i++)
    {
        ...
        if (nextConnection->twinCode != NULL && strcmp(inConnection.twinCode, nextConnection->twinCode) == 0 &&
            inConnection.twinCount == nextConnection->twinCount)
        {

            if (nextConnection->curseStatus.curseLevel > anyTwinCurseLevel.curseLevel)
            {
    ...
    if (twinConnections.size() >= inConnection.twinCount)
    {
        ...
        for (int i = 0; i < players.size(); i++)
        {
            ...
            if (!o->error && !o->connected && strcmp(o->email, inConnection.email) == 0)
            {

                // take them out of waiting list too
                for (int i = 0; i < waitingForTwinConnections.size(); i++)
                {
                    if (waitingForTwinConnections.getElement(i)->sock == inConnection.sock)
                    {
                        // found
        ...
        char usePersonalCurses = SettingsManager::getIntSetting("usePersonalCurses", 0);
        ...
        for (int i = 0; i < twinConnections.size(); i++)
        {
            ...
            if (i == 0)
            {
                ...
                newID =
                    processLoggedInPlayer(false, nextConnection->sock, nextConnection->sockBuffer,
                                          nextConnection->email, nextConnection, nextConnection->tutorialNumber,
                                          anyTwinCurseLevel, nextConnection->lifeStats, nextConnection->fitnessScore);
                ...
                else if (newID == -2)
                {
                    ...
                    {
                        FreshConnection *nextConnectionToReject = twinConnections.getElementDirect(j);
                ...
                {
                    newPlayer = tutorialLoadingPlayers.getElement(tutorialLoadingPlayers.size() - 1);
                }
            ...
            else
            {
                ...
                processLoggedInPlayer(false, nextConnection->sock, nextConnection->sockBuffer, nextConnection->email,
                                      nextConnection,
                                      // ignore tutorial number of all but
                                      // first player
                                      0, anyTwinCurseLevel, nextConnection->lifeStats, nextConnection->fitnessScore,
                                      parent, displayID, forcedEvePos);
        ...
        for (int i = 0; i < waitingForTwinConnections.size(); i++)
        {
            ...
            if (nextConnection->twinCode != NULL && nextConnection->twinCount == inConnection.twinCount &&
...
char removeFromContainerToHold(LiveObject *inPlayer, int inContX, int inContY, int inSlotNumber, char inSwap);

// find index of spot on container held item can swap with, or -1 if none found
static int getContainerSwapIndex(LiveObject *inPlayer, int idToAdd, int inStillHeld, int inSearchLimit, int inContX,
                                 int inContY)
{
    ...
    for (int botInd = 0; botInd < inSearchLimit; botInd++)
    {
        ...
        bool hasPickUpTrans = pickUpTrans != NULL && pickUpTrans->newTarget == 0;

        if (bottomItem > 0 &&
            (getObject(bottomItem)->minPickupAge > playerAge || (getObject(bottomItem)->permanent && !hasPickUpTrans)))
        {
            // too young to hold!
...
static char isContainmentWithMatchedTags(int inContainerID, int inContainedID)
{
    ...
    while (contLoc != NULL)
    {
        ...
        if (limitNameLoc[0] != ' ' && limitNameLoc[0] != '\0')
        {
            ...
            if (numRead == 1)
            {
                ...
                char *locInContainerName = strstr(getObject(inContainerID)->description, tag);
...
static void changeContained(int inX, int inY, int inSlotNumber, int inNewObjectID)
{

    int numContained = 0;
    int *contained = getContained(inX, inY, &numContained);

    timeSec_t *containedETA = getContainedEtaDecay(inX, inY, &numContained);

    timeSec_t curTimeSec = Time::getCurrentTime();

    if (contained != NULL && containedETA != NULL && numContained > inSlotNumber)
    {
        ...
        if (oldObjectID > 0)
        {
            ...
            if (oldDecayTrans != NULL && newDecayTrans != NULL &&
                oldDecayTrans->autoDecaySeconds == newDecayTrans->autoDecaySeconds)
            {
...
char containmentPermitted(int inContainerID, int inContainedID)
{
    ...
    if (containerObj->isUseDummy)
        inContainerID = containerObj->useDummyParent;
    if (containedObj->isUseDummy)
        inContainedID = containedObj->useDummyParent;
...
static char addHeldToContainer(LiveObject *inPlayer, int inTargetID, int inContX, int inContY, char inSwap = false)
{
    ...
    else
    {
        ...
        if (inSwap)
        {
            ...
            int swapInd = getContainerSwapIndex(inPlayer, idToAdd, true, numIn, inContX, inContY);
            if (swapInd != -1)
            {
                isRoom = true;
            }
    ...
    if (isRoom && containmentPermitted(inTargetID, inPlayer->holdingID))
    {
        ...
        if (contTrans == NULL)
        {
            ...
            if (contTrans == NULL)
                contTrans = getPTrans(0, target, false, false, 3);
            ...
            int swapInd = getContainerSwapIndex(inPlayer, idToConsiderSwap, true, numIn, inContX, inContY);
        ...
        if (contTrans == NULL)
        {
            ...
            if (contTrans == NULL)
                contTrans = getPTrans(0, target, false, false, 4);
        }
        ...
        if (contTrans != NULL)
        {
            ...
            else
            {
                ...
                int contID = getContained(inContX, inContY, slotNumber);

                if (contID < 0)
                    contID *= -1;

                while (slotNumber >= 0 && containmentPermitted(contTrans->newTarget, contID))
                {
                    ...
                    contID = getContained(inContX, inContY, slotNumber);
        ...
        if (inPlayer->numContained > 0)
        {
            ...
            for (int c = 0; c < inPlayer->numContained; c++)
            {
                ...
                addContained(inContX, inContY, inPlayer->containedIDs[c], inPlayer->containedEtaDecays[c], numIn + 1);
            }
        ...
        if (inSwap && numInNow > 1)
        {
            ...
            {
                // found one to swap
                removeFromContainerToHold(inPlayer, inContX, inContY, swapInd, true);
            }
...
char removeFromContainerToHold(LiveObject *inPlayer, int inContX, int inContY, int inSlotNumber, char inSwap = false)
{
    inPlayer->heldOriginValid = 0;
    inPlayer->heldOriginX = 0;
    inPlayer->heldOriginY = 0;
        ...
        (inContX == inPlayer->xd && inContY == inPlayer->yd))
    {
        ...
        if (target != 0)
        {
                ...
                while (inSlotNumber > 0 &&
                       (getObject(toRemoveID)->minPickupAge > playerAge ||
                        (getObject(toRemoveID)->permanent &&
                         (getPTrans(0, toRemoveID) == NULL || getPTrans(0, toRemoveID)->newTarget != 0))))
                {
            ...
            if (inPlayer->holdingID == 0 && numIn > 0 &&
                ...
                getObject(toRemoveID)->minPickupAge <= computeAge(inPlayer) &&
                // permanent object cannot be removed from container
                (!getObject(toRemoveID)->permanent || hasPickUpTrans))
            {
                // get from container
                ...
                if (numIn == 1)
                {
                    ...
                    if (contTrans == NULL)
                        contTrans = getPTrans(target, -1, false, false, 2);
                }
                else if (targetSlots == numIn)
                {
                    contTrans = getPTrans(target, toRemoveID, false, false, 1);
                    if (contTrans == NULL)
                        contTrans = getPTrans(target, -1, false, false, 1);
                }
                ...
                if (contTrans == NULL && !inSwap)
                {
                    ...
                    if (contTrans == NULL)
                        contTrans = getPTrans(target, -1, false, false, 3);
                }
                ...
                if (contTrans == NULL)
                {
                    ...
                    if (contTrans == NULL)
                        contTrans = getPTrans(target, -1, false, false, 4);
                }
                ...
                if (contTrans != NULL)
                {
                    ...
                    while (slotNumber >= 0 && containmentPermitted(contTrans->newActor, contID))
                    {
                ...
                if (subContain)
                {
                    ...
                    inPlayer->containedIDs =
                        getContained(inContX, inContY, &(inPlayer->numContained), subSlotNumber + 1);
                    inPlayer->containedEtaDecays =
                        getContainedEtaDecay(inContX, inContY, &(inPlayer->numContained), subSlotNumber + 1);
                    ...
                    inPlayer->subContainedIDs = new SimpleVector<int>[inPlayer->numContained];
                    inPlayer->subContainedEtaDecays = new SimpleVector<timeSec_t>[inPlayer->numContained];
                }

                setResponsiblePlayer(-inPlayer->id);

                inPlayer->holdingID = removeContained(inContX, inContY, inSlotNumber, &(inPlayer->holdingEtaDecay));
                ...
                if (contTrans != NULL)
                {
                    if (contTrans->newTarget > 0)
                        handleHoldingChange(inPlayer, contTrans->newTarget);
                }
...
static char addHeldToClothingContainer(LiveObject *inPlayer, int inC,
                                       ...
                                       char inWillSwap = false, char *outCouldHaveGoneIn = NULL,
                                       char skipContainmentCheck = false)
{
    // drop into own clothing
    ObjectRecord *cObj = clothingByIndex(inPlayer->clothing, inC);

    if (skipContainmentCheck || (cObj != NULL && containmentPermitted(cObj->id, inPlayer->holdingID)))
    {

        int oldNum = inPlayer->clothingContained[inC].size();

        if (cObj->numSlots > 0 && outCouldHaveGoneIn != NULL)
        {
        ...
        if (oldNum < cObj->numSlots || (oldNum == cObj->numSlots && inWillSwap))
        {
            ...
            if (oldNum == 0)
            {
                ...
                if (contTrans == NULL)
                    contTrans = getPTrans(0, containerID, false, false, 1);
            }
            else if (numSlots == oldNum)
            {
                contTrans = getPTrans(containedID, containerID, false, false, 2);
                if (contTrans == NULL)
                    contTrans = getPTrans(0, containerID, false, false, 2);
            }
            ...
            if (contTrans == NULL && !inWillSwap)
            {
                ...
                if (contTrans == NULL)
                    contTrans = getPTrans(0, containerID, false, false, 3);
            }
            ...
            if (contTrans == NULL)
            {
                ...
                if (contTrans == NULL)
                    contTrans = getPTrans(0, containerID, false, false, 4);
            }
            ...
            if (contTrans != NULL)
            {
                ...
                if (contTrans->newActor > 0)
                    handleHoldingChange(inPlayer, contTrans->newActor);

                // idToAdd = contTrans->newActor;

                setClothingByIndex(&(inPlayer->clothing), inC, getObject(contTrans->newTarget));
            }
...
static void setHeldGraveOrigin(LiveObject *inPlayer, int inX, int inY, int inNewTarget)
{
...
static void pickupToHold(LiveObject *inPlayer, int inX, int inY, int inTargetID)
{
...
static char removeFromClothingContainerToHold(LiveObject *inPlayer, int inC, int inI = -1, bool inSwap = false)
{
    ...
    if (slotToRemove < 0)
    {
        ...
        while (slotToRemove > 0 &&
               (getObject(inPlayer->clothingContained[inC].getElementDirect(slotToRemove))->minPickupAge > playerAge ||
                (getObject(inPlayer->clothingContained[inC].getElementDirect(slotToRemove))->permanent &&
    ...
    if (oldNumContained > 0 && oldNumContained > slotToRemove && slotToRemove >= 0)
    {

        toRemoveID = inPlayer->clothingContained[inC].getElementDirect(slotToRemove);
    }
        ...
        (!getObject(toRemoveID)->permanent || hasPickUpTrans))
    {
        ...
        inPlayer->holdingID = inPlayer->clothingContained[inC].getElementDirect(slotToRemove);
        holdingSomethingNew(inPlayer);

        inPlayer->holdingEtaDecay = inPlayer->clothingContainedEtaDecays[inC].getElementDirect(slotToRemove);
        ...
        if (oldNumContained == 1)
        {
            ...
            if (contTrans == NULL)
                contTrans = getPTrans(containerID, -1, false, false, 2);
        }
        else if (numSlots == oldNumContained)
        {
            contTrans = getPTrans(containerID, containedID, false, false, 1);
            if (contTrans == NULL)
                contTrans = getPTrans(containerID, -1, false, false, 1);
        }
        ...
        if (contTrans == NULL && !inSwap)
        {
            ...
            if (contTrans == NULL)
                contTrans = getPTrans(containerID, -1, false, false, 3);
        }
        ...
        if (contTrans == NULL)
        {
            ...
            if (contTrans == NULL)
                contTrans = getPTrans(containerID, -1, false, false, 4);
        }
        ...
        if (contTrans != NULL)
        {
            ...
            setClothingByIndex(&(inPlayer->clothing), inC, getObject(contTrans->newActor));
        }
...
static void removeClothingToHold(LiveObject *nextPlayer, LiveObject *targetPlayer, ObjectRecord **clothingSlot,
                                 int clothingSlotIndex)
{
    ...
    *clothingSlot = NULL;
    ...
    nextPlayer->containedIDs = targetPlayer->clothingContained[ind].getElementArray();

    targetPlayer->clothingContained[ind].deleteAll();

    nextPlayer->containedEtaDecays = targetPlayer->clothingContainedEtaDecays[ind].getElementArray();
...
static TransRecord *getBareHandClothingTrans(LiveObject *nextPlayer, ObjectRecord **clothingSlot)
{
    ...
    if (clothingSlot != NULL)
    {
        ...
        if (bareHandClothingTrans != NULL)
        {
            ...
            if (na > 0 && getObject(na)->minPickupAge > computeAge(nextPlayer))
            {
                // too young for trans
...
static void handleHoldingChange(LiveObject *inPlayer, int inNewHeldID)
{
    ...
    if (newHeldSlots > 0 && oldHolding != 0)
    {
        ...
        restretchDecays(newHeldSlots, nextPlayer->containedEtaDecays, oldHolding, nextPlayer->holdingID);
    }
...
static char isWildcardGivingSay(char *inSaidString, SimpleVector<char *> *inPhrases)
{
    if (inSaidString[0] == ':')
    {
        // first : indicates reading a written phrase.
...
LiveObject *getClosestOtherPlayer(LiveObject *inThisPlayer, double inMinAge = 0, char inNameMustBeNULL = false)
{
    ...
    for (int j = 0; j < players.size(); j++)
    {
        ...
        if (otherPlayer != inThisPlayer && !otherPlayer->error && computeAge(otherPlayer) >= inMinAge &&
...
void apocalypseStep()
{
    ...
    if (!apocalypseTriggered)
    {
        ...
        if (apocalypseRequest == NULL && curTime - lastRemoteApocalypseCheckTime > remoteApocalypseCheckInterval)
        {
        ...
        else if (apocalypseRequest != NULL)
        {
            ...
            else if (result == 1)
            {
                ...
                {
                    AppLog::infoF("Apocalypse check:  Bad response from reflector:  %s.", webResult);
                }
                else
                {
                    ...
                    if (newApocalypseNumber > lastApocalypseNumber)
                    {
                        ...
                        AppLog::infoF("Apocalypse check:  New remote apocalypse:  %d.", lastApocalypseNumber);
                        SettingsManager::setSetting("lastApocalypseNumber", lastApocalypseNumber);
                    }
                }
    ...
    if (apocalypseTriggered)
    {
        ...
        if (!apocalypseStarted)
        {
            ...
            if (!apocalypseRemote && SettingsManager::getIntSetting("remoteReport", 0) &&
                SettingsManager::getIntSetting("apocalypseBroadcast", 0) && apocalypseRequest == NULL &&
                reflectorURL != NULL)
            {

                AppLog::info("Apocalypse broadcast set, telling reflector");

                char *reflectorSharedSecret = SettingsManager::getStringSetting("reflectorSharedSecret");

                if (reflectorSharedSecret != NULL)
                {
                    lastApocalypseNumber++;

                    AppLog::infoF("Apocalypse trigger:  New local apocalypse:  %d.", lastApocalypseNumber);
                    ...
                    for (int i = 0; i < players.size(); i++)
                    {
                        ...
                        if (!nextPlayer->error)
                        {
                            ...
                            double dist =
                                abs(nextPlayer->xd - apocalypseLocation.x) + abs(nextPlayer->yd - apocalypseLocation.y);
                            if (dist < closestDist)
                            {
                    ...
                    char *url = autoSprintf("%s?action=trigger_apocalypse"
            ...
            for (int i = 0; i < players.size(); i++)
            {
                ...
                if (!nextPlayer->error && nextPlayer->connected)
                {
                    ...
                    int numSent = nextPlayer->sock->send((unsigned char *)message, messageLength, false, false);
        ...
        if (apocalypseRequest == NULL && Time::getCurrentTime() - apocalypseStartTime >= 8)
        {

            if (!postApocalypseStarted)
            {
                AppLog::infoF("Enough warning time, %d players still alive", players.size());
                ...
                AppLog::infoF("Apocalypse freeMap took %f sec", Time::getCurrentTime() - startTime);
                wipeMapFiles();

                AppLog::infoF("Apocalypse wipeMapFiles took %f sec", Time::getCurrentTime() - startTime);
        ...
        for (int i = 0; i < players.size(); i++)
        {
            ...
            if (!nextPlayer->error && nextPlayer->connected)
            {
                ...
                char *message = autoSprintf("MN\n%d %d %d\n#", monumentCallX - nextPlayer->birthPos.x,
                                            monumentCallY - nextPlayer->birthPos.y, hideIDForClient(monumentCallID));
                int messageLength = strlen(message);
        ...
        *outSuffixAdded = false;
        ...
        if (inIsEve)
        {
            ...
            for (int i = 0; i < players.size(); i++)
            {
                ...
                if (!o->error && o->familyName != NULL && strcmp(o->familyName, lastName) == 0)
                {
        ...
        else if (numNames == 2)
        {
            if (inIsEve)
            {
                ...
                while (dup)
                {
                    ...
                    for (int j = 0; j < players.size(); j++)
                    {
                        ...
                        if (!o->error && o->familyName != NULL && strcmp(o->familyName, tempLastName) == 0)
                        {
            ...
            else
            {
                ...
                while (dup)
                {
                    ...
                    int nextI;
                    tempName = autoSprintf("%s %s", getFirstName(i, &nextI), lastName);
...
void setNoLongerDying(LiveObject *inPlayer, SimpleVector<int> *inPlayerIndicesToSendHealingAbout)
{
    ...
    inPlayerIndicesToSendHealingAbout->push_back(getLiveObjectIndex(inPlayer->id));
}
...
static void checkForFoodEatingEmot(LiveObject *inPlayer, int inEatenID)
{
...
{
    if (!inPlayer->emotFrozen || (inPlayer->emotFrozen && inPlayer->emotFrozenIndex == inEmoteIndex))
    {
...
char addKillState(LiveObject *inKiller, LiveObject *inTarget)
{
    ...
    if (distance(getPlayerPos(inKiller), getPlayerPos(inTarget)) > 8)
    {
        // out of range
        return false;
    }
    ...
    if (!found)
    {
        // add new
        double curTime = Time::getCurrentTime();
        KillState s = {inKiller->id, inKiller->holdingID, inTarget->id, curTime, curTime, 10};
...
static void removeKillState(LiveObject *inKiller, LiveObject *inTarget)
{
    ...
    if (inTarget != NULL && inTarget->emotFrozen && inTarget->emotFrozenIndex == victimEmotionIndex)
    {
...
static void setPerpetratorHoldingAfterKill(LiveObject *nextPlayer, TransRecord *woundHit, TransRecord *rHit,
                                           TransRecord *r)
{
...
void executeKillAction(int inKillerIndex, int inTargetIndex, SimpleVector<int> *playerIndicesToSendUpdatesAbout,
                       SimpleVector<int> *playerIndicesToSendDyingAbout, SimpleVector<int> *newEmotPlayerIDs,
                       SimpleVector<int> *newEmotIndices, SimpleVector<int> *newEmotTTLs)
{
    int i = inKillerIndex;
    ...
    if (nextPlayer->holdingID > 0)
    {
        ...
        if (heldObj->deadlyDistance > 0)
        {
            ...
            if (heldObj->deadlyDistance >= d && !directLineBlocked(playerPos, targetPos))
            {
                ...
                if (hitPlayer != NULL && strstr(heldObj->description, "otherFamilyOnly"))
                {
                    ...
                    if (hitPlayer->lineageEveID == nextPlayer->lineageEveID ||
                        isPeaceTreaty(hitPlayer->lineageEveID, nextPlayer->lineageEveID))
                    {
                ...
                if (hitPlayer != NULL)
                {
                    ...
                    TransRecord *woundHit = getPTrans(nextPlayer->holdingID, 0, true, false);

                    if (woundHit != NULL && woundHit->newTarget > 0 &&
                        strstr(getObject(woundHit->newTarget)->description, "no_replace") != NULL)
                    {

                        TransRecord *rHit = getPTrans(nextPlayer->holdingID, 0, false, true);
                ...
                if (hitPlayer != NULL)
                {
                    ...
                    if (!hitPlayer->dying)
                    {
                        ...
                        playerIndicesToSendDyingAbout->push_back(getLiveObjectIndex(hitPlayer->id));
                ...
                if (someoneHit)
                {
                    ...
                    if (hitPlayer->holdingID > 0 &&
                        strstr(getObject(hitPlayer->holdingID)->description, "sick") != NULL)
                    {
                        wasSick = true;
                    }
                    ...
                    if (woundHit != NULL && woundHit->newTarget > 0)
                    {
                        ...
                        {
                            handleDrop(targetPos.x, targetPos.y, hitPlayer, playerIndicesToSendUpdatesAbout);
                        }
                        ...
                        if (woundChange)
                        {
                            ...
                            playerIndicesToSendUpdatesAbout->push_back(getLiveObjectIndex(hitPlayer->id));
                        }
                    }
                }
                ...
                if (r != NULL)
                {
                    ...
                    else if (hitPlayer == NULL && isMapSpotEmpty(targetPos.x, targetPos.y))
                    {
...
void nameBaby(LiveObject *inNamer, LiveObject *inBaby, char *inName, SimpleVector<int> *playerIndicesToSendNamesAbout)
{
    ...
    const char *close = findCloseFirstName(name);
    ...
    babyO->name = getUniqueCursableName(babyO->name, &(babyO->nameHasSuffix), false);
...
void logFitnessDeath(LiveObject *nextPlayer)
{
    ...
    SimpleVector<double> *ancestorLifeEndTimeSeconds = nextPlayer->ancestorLifeEndTimeSeconds;
    ...
    logFitnessDeath(players.size(), nextPlayer->email, nextPlayer->name, nextPlayer->displayID, computeAge(nextPlayer),
                    ancestorEmails, ancestorRelNames, &ancestorData);
}

// access blocked b/c of access direction or ownership?
static char isAccessBlocked(LiveObject *inPlayer, int inTargetX, int inTargetY, int inTargetID)
{
    int target = inTargetID;
    ...
    if (target > 0)
    {
        ...
        if (isGridAdjacent(x, y, inPlayer->xd, inPlayer->yd))
        {
            ...
            if (targetObj->useDistance == 0 && (inTargetY != inPlayer->yd || inTargetX != inPlayer->xd))
            {
...
// returns NULL if not found
static LiveObject *getPlayerByName(char *inName, LiveObject *inPlayerSayingName)
{
    for (int j = 0; j < players.size(); j++)
    {
        LiveObject *otherPlayer = players.getElement(j);
        if (!otherPlayer->error && otherPlayer != inPlayerSayingName && otherPlayer->name != NULL &&
...
static void sendCraving(LiveObject *inPlayer)
{
    ...
    if (isReallyYummy(inPlayer, inPlayer->cravingFood.foodID))
        totalBonus = totalBonus + 1;

    char *message = autoSprintf("CR\n%d %d\n#", inPlayer->cravingFood.foodID, totalBonus);
    sendMessageToPlayer(inPlayer, message, strlen(message));
    delete[] message;
...
static void handleHeldDecay(LiveObject *nextPlayer, int i, SimpleVector<int> *playerIndicesToSendUpdatesAbout,
                            SimpleVector<int> *playerIndicesToSendHealingAbout)
{
    ...
    if (t != NULL)
    {
        ...
        if (newObj != NULL && newObj->permanent && oldObj != NULL && !oldObj->permanent && !nextPlayer->holdingWound)
        {
            // object decayed into a permanent
            // force drop
            GridPos dropPos = getPlayerPos(nextPlayer);

            handleDrop(dropPos.x, dropPos.y, nextPlayer, playerIndicesToSendUpdatesAbout);
        }
...
static int checkTargetInstantDecay(int inTarget, int inX, int inY)
{
    ...
    if (targetDecay != NULL && targetDecay->autoDecaySeconds == 1 && targetDecay->newTarget > 0 &&
        targetDecay->move == 0)
    {
...
static void setRefuseFoodEmote(LiveObject *hitPlayer)
{
    ...
    int newEmotIndex = SettingsManager::getIntSetting("refuseFoodEmotionIndex", -1);
    if (newEmotIndex != -1)
    {
        newEmotPlayerIDs.push_back(hitPlayer->id);

        newEmotIndices.push_back(newEmotIndex);
...
int main()
{
    ...
    babyInheritMonument = SettingsManager::getIntSetting("babyInheritMonument", 1);
    ...
    infertilitySuffix = SettingsManager::getStringSetting("infertilitySuffix", "+INFERTILE+");
    fertilitySuffix = SettingsManager::getStringSetting("fertilitySuffix", "+FERTILE+");
    ...
    if (false)
    {
        ...
        printf("Average count %d (%s) = %f,  %d (%s) = %f  over %d runs\n", idA, getObject(idA)->description,
               totalCountA / (double)numRuns, idB, getObject(idB)->description, totalCountB / (double)numRuns, numRuns);
    ...
    */
    ...
    while (!quit)
    {
        ...
        if (curStepTime - lastPastPlayerFlushTime > 3600)
        {
            ...
            // default one week
            int pastPlayerFlushTime = SettingsManager::getIntSetting("pastPlayerFlushTime", 604000);
        ...
        if (periodicStepThisStep)
        {
            ...
            forceShutdownMode = SettingsManager::getIntSetting("forceShutdownMode", 0);
        ...
        if (forceShutdownMode)
        {
            ...
            for (int i = 0; i < players.size(); i++)
            {
                ...
                {
                    nextPlayer->sock->send((unsigned char *)shutdownMessage, messageLength, false, false);
        ...
        if (periodicStepThisStep)
        {
            ...
            int enableArcReport = SettingsManager::getIntSetting("enableArcReport", 1);
            ...
            if (arcMilestone != -1 && enableArcReport)
            {
                ...
                if (familyLimitAfterEveWindow > 0 && !isEveWindow())
                {
                ...
                char *message = autoSprintf(":%s: ARC HAS LASTED %d YEARS**"
                                            "%d %s %s ALIVE",
                                            getArcName(), arcMilestone, numFams, familyLine, familyWord);
                delete[] familyLine;
            ...
            for (int i = 0; i < players.size(); i++)
            {
                ...
                if (nextPlayer->cravingFood.uniqueID > -1 && nextPlayer->cravingFood.uniqueID < lowestCravingID)
                {
                ...
                if (nextPlayer->globalMessageQueue.size() > 0 &&
                    curStepTime - nextPlayer->lastGlobalMessageTime > minGlobalMessageSpacingSeconds)
                {

                    // send next one
                    char *message = nextPlayer->globalMessageQueue.getElementDirect(0);
        ...
        for (int i = 0; i < numLive; i++)
        {
            ...
            if (nextPlayer->xd != nextPlayer->xs || nextPlayer->yd != nextPlayer->ys)
            {
                ...
                double moveTimeLeft = nextPlayer->moveTotalSeconds - (curTime - nextPlayer->moveStartTime);
            ...
            for (int c = 0; c < NUM_CLOTHING_PIECES; c++)
            {
                ...
                {
                    timeSec_t decay = nextPlayer->clothingContainedEtaDecays[c].getElementDirect(cc);
        ...
        if (readySock != NULL && !readySock->isSocket)
        {
            ...
            if (sock != NULL)
            {
                ...
                char *secretString =
                    SettingsManager::getStringSetting("statsServerSharedSecret", "sdfmlk3490sadfm3ug9324");
                ...
                else
                {
                    message =
                        autoSprintf("SN\n"
                                    ...
                                    "%s\n"
                                    "%lu\n#",
                                    currentPlayers, maxPlayers, newConnection.sequenceNumberString, versionNumber);
                ...
                int numSent = sock->send((unsigned char *)message, messageLength, false, false);
        ...
        for (int i = 0; i < newConnections.size(); i++)
        {
            ...
            if (nextConnection->email != NULL && nextConnection->curseStatus.curseLevel == -1)
            {
                ...
                if (nextConnection->curseStatus.curseLevel != -1)
                {
                    AppLog::infoF("Got curse level for %s from curse server: "
                                  "%d (excess %d)",
                                  nextConnection->email, nextConnection->curseStatus.curseLevel,
                                  nextConnection->curseStatus.excessPoints);
                }
            }
            else if (nextConnection->email != NULL && nextConnection->lifeStats.lifeCount == -1)
            {
                // keep checking if life stats have arrived from
                // stats server
                int statsResult = getPlayerLifeStats(nextConnection->email, &(nextConnection->lifeStats.lifeCount),
            ...
            else if (nextConnection->ticketServerRequest != NULL && !nextConnection->ticketServerAccepted)
            {
                     ...
                     !nextConnection->lifeTokenSpent)
            {
                ...
                else
                {
                    ...
                    if (spendResult == -1)
                    {
                        ...
                        nextConnection->sock->send((unsigned char *)message, strlen(message), false, false);
                     ...
                     nextConnection->lifeTokenSpent)
            {
                ...
                else
                {
                    ...
                    if (nextConnection->twinCode != NULL && nextConnection->twinCount > 0)
                    {
                    ...
                    else
                    {
                        ...
                        int newID = processLoggedInPlayer(true, nextConnection->sock, nextConnection->sockBuffer,
                                                          nextConnection->email, nextConnection,
                                                          nextConnection->tutorialNumber, nextConnection->curseStatus,
                                                          nextConnection->lifeStats, nextConnection->fitnessScore);

                        if (newID == -2)
                        {
                            nextConnection->error = true;
                            nextConnection->errorCauseString = "Target family is not found or does not have fertiles";
            ...
            else if (nextConnection->ticketServerRequest == NULL)
            {
                ...
                double timeDelta = Time::getCurrentTime() - nextConnection->connectionStartTimeSeconds;

                char result = readSocketFull(nextConnection->sock, nextConnection->sockBuffer);
                ...
                if (message != NULL)
                {
                    ...
                    if (!nextConnection->playerListSent)
                    {
                        ...
                        else if (playerListSecret != NULL)
                        {
                            ...
                            if (0 == constant_time_strcmp(message, requestWithSecret))
                            { // TODO: using plain password is not good, it would be better to expect the client to hash
                              // his password and we check it with by hashing our copy of the password. if the client
                              // can use a hashing library i will leave it to the reviewer to decide.
                                passedSecret = true;
                            }
                    ...
                    if (passedSecret || nextConnection->playerListSent)
                    {
                        if (!nextConnection->playerListSent)
                        {
                            ...
                            int buffSize = 32 * 1024; // NOTE: this can fit 800 players in the worst case, message will
                                                      // be truncated if more than that(and # wont be sent).
                            ...
                            for (int i = 0; i < players.size(); i++)
                            {
                                ...
                                if (player->name == NULL)
                                {
                                    // on linux NULL is printed as "(null)" but i belive on windows it is treated as
                                    // NULL character (empty), here we standaradize it to empty string.
                                    name = (char *)"";
                                }
                                else
                                {
                                ...
                                playerLine =
                                    autoSprintf("%d,%d,%d,%c,%.1f,%d,%d,%s,%s\n", player->id, player->lineageEveID,
                                                player->parentID, gender, age, player->declaredInfertile,
                                                player->isTutorial, name, familyName);
                        ...
                        if (tokens->size() == 4 || tokens->size() == 5 || tokens->size() == 7)
                        {
                            ...
                            if (seedDelimPos != std::string::npos)
                            {
                                ...
                                if (seedLen > minSeedLen)
                                {
                                    ...
                                    nextConnection->hashedSpawnSeed = fnv1aHash(seed, fnv1aHash(seedPepper));
                            ...
                            else
                            {
                                ...
                                if (famTargetDelimPos != std::string::npos)
                                {
                                    ...
                                    nextConnection->famTarget = stringDuplicate(famTarget.c_str());
                            ...
                            if (tokens->size() >= 5)
                            {
                                sscanf(tokens->getElementDirect(4), "%d", &(nextConnection->tutorialNumber));
                            }

                            if (tokens->size() == 7)
                            {
                                nextConnection->twinCode = stringDuplicate(tokens->getElementDirect(5));

                                sscanf(tokens->getElementDirect(6), "%d", &(nextConnection->twinCount));

                                int maxCount = SettingsManager::getIntSetting("maxTwinPartySize", 4);
                            ...
                            // we'll catch that case later above
                            int statsResult =
                                getPlayerLifeStats(nextConnection->email, &(nextConnection->lifeStats.lifeCount),
                                                   &(nextConnection->lifeStats.lifeTotalSeconds));
                            ...
                            if (requireClientPassword && !nextConnection->error)
                            {
                                ...
                                char *trueHash = hmac_sha1(clientPassword, nextConnection->sequenceNumberString);
                            ...
                            else if (!requireTicketServerCheck && !nextConnection->error)
                            {
                                ...
                                int numSent =
                                    nextConnection->sock->send((unsigned char *)message, messageLength, false, false);
        ...
        for (int i = 0; i < tutorialLoadingPlayers.size(); i++)
        {
            ...
            while (twinPlayer != NULL && twinPlayer->tutorialLoad.uniqueLoadID == uniqueID)
            {

                AppLog::infoF("Twin %s tutorial loaded too (loadID = %u )", twinPlayer->email, uniqueID);
        ...
        for (int i = 0; i < numLive; i++)
        {
            ...
            if (nextPlayer->fitnessScore == -1)
            {
                ...
                int fitResult = getFitnessScore(nextPlayer->email, &nextPlayer->fitnessScore);
            ...
            if (curCrossTime < nextPlayer->playerCrossingCheckTime + playerCrossingCheckStepTime)
            {
            ...
            if (checkCrossing)
            {
                ...
                if (nextPlayer->holdingID > 0 && getObject(nextPlayer->holdingID)->rideable)
                {
                    riding = true;
                }
                ...
                if (!riding)
                {
                    ...
                    if (curOverObj == NULL || !curOverObj->permanent || curOverObj->deadlyDistance == 0)
                    {

                        int movingDestX, movingDestY;

                        int curMovingID = getDeadlyMovingMapObject(curPos.x, curPos.y, &movingDestX, &movingDestY);
                        ...
                        if (curMovingID != 0)
                        {
                            ...
                            //  bear)
                            curMovingID = getMapObject(movingDestX, movingDestY);

                            ObjectRecord *movingObj = getObject(curMovingID);
                            if (movingObj->permanent && movingObj->deadlyDistance > 0)
                            {
                ...
                if (!nextPlayer->heldByOther && !nextPlayer->vogMode && curOverID != 0 &&
                    !isMapObjectInTransit(curPos.x, curPos.y) && !wasRecentlyDeadly(curPos))
                {

                    ObjectRecord *curOverObj = getObject(curOverID);

                    if (!riding && curOverObj->permanent && curOverObj->deadlyDistance > 0)
                    {

                        char wasSick = false;

                        if (nextPlayer->holdingID > 0 &&
                            strstr(getObject(nextPlayer->holdingID)->description, "sick") != NULL)
                        {
                            wasSick = true;
                        }
                        ...
                        if (!nextPlayer->dying || wasSick)
                        {
                            ...
                            playerIndicesToSendDyingAbout.push_back(getLiveObjectIndex(nextPlayer->id));
                        }
                        else
                        {
                        ...
                        if (r != NULL)
                        {
                            setMapObject(deadlyDestPos.x, deadlyDestPos.y, r->newActor);
                            ...
                            if (r->newTarget > 0 && (!nextPlayer->holdingWound || wasSick))
                            {
                                ...
                                {
                                    handleDrop(curPos.x, curPos.y, nextPlayer, &playerIndicesToSendUpdatesAbout);
                                }
                                ...
                                playerIndicesToSendUpdatesAbout.push_back(getLiveObjectIndex(nextPlayer->id));
                            }
                        }
                    }
                    else if (riding && curOverObj->permanent && curOverObj->deadlyDistance > 0)
                    {
            ...
            {
                lookAtRegion(nextPlayer->xd - 8, nextPlayer->yd - 7, nextPlayer->xd + 8, nextPlayer->yd + 7);
            ...
            if (message != NULL)
            {
                ...
                AppLog::infoF("Got client message from %d: %s", nextPlayer->id, message);
            ...
            RESTART_MESSAGE_ACTION:
                ...
                else if (m.type == BUG)
                {
                    ...
                    if (allow)
                    {
                        char *bugName = autoSprintf("bug_%d_%d_%f", m.bug, nextPlayer->id, Time::getCurrentTime());
                ...
                else if (m.type == MAP)
                {
                    ...
                    if (allow)
                    {
                        ...
                        SimpleVector<char *> *list = SettingsManager::getSetting("mapRequestAllowAccounts");
                    ...
                    if (allow && nextPlayer->connected)
                    {
                        ...
                        unsigned char *mapChunkMessage =
                            getChunkMessage(m.x - chunkDimensionX / 2, m.y - chunkDimensionY / 2, chunkDimensionX,
                                            chunkDimensionY, centerPos, &length);

                        int numSent = nextPlayer->sock->send(mapChunkMessage, length, false, false);
                    ...
                    {
                        AppLog::infoF("Map pull request rejected for %s", nextPlayer->email);
                    }
                }
                ...
                else if (m.type == VOGS)
                {
                    ...
                    if (allow)
                    {
                        ...
                        SimpleVector<char *> *list = SettingsManager::getSetting("vogAllowAccounts");
                        ...
                        for (int i = 0; i < list->size(); i++)
                        {
                            ...
                            if (strcmp(lowerEmail, lowerAllowed) == 0)
                            {
                            ...
                            else if (strcmp("*", list->getElementDirect(i)) == 0)
                            {
                ...
                else if (m.type == VOGN)
                {
                    if (nextPlayer->vogMode && players.size() > 1)
                    {
                        ...
                        LiveObject *otherPlayer = players.getElement(nextPlayer->vogJumpIndex);
                ...
                else if (m.type == DIE)
                {
                    if (computeAge(nextPlayer) < 2)
                    {
                        ...
                        if (parentO != NULL && parentO->lastSidsBabyEmail != NULL)
                        {
                        ...
                        for (int p = 0; p < players.size(); p++)
                        {
                            ...
                            if (parent->lastSidsBabyEmail != NULL &&
                                strcmp(parent->lastSidsBabyEmail, nextPlayer->email) == 0)
                            {
                        ...
                        if (parentO != NULL)
                        {
                            parentO->lastSidsBabyEmail = stringDuplicate(nextPlayer->email);
                        }
                        ...
                        if (adult != NULL)
                        {
                            ...
                            if (babyBonesID != -1)
                            {
                                ...
                                if (babyBonesO != NULL)
                                {
                                    ...
                                    int stepCount = 0;
                                    while (getGravePlayerID(gravePos.x, gravePos.y) > 0 && stepCount < 100)
                                    {
                                        gravePos.x++;
                                        stepCount++;
                                    }

                                    GraveInfo graveInfo = {gravePos, nextPlayer->id, nextPlayer->lineageEveID};
                                    newGraves.push_back(graveInfo);

                                    setGravePlayerID(gravePos.x, gravePos.y, nextPlayer->id);

                                    playerIndicesToSendUpdatesAbout.push_back(getLiveObjectIndex(holdingAdultID));
                                    ...
                                    for (int c = 0; c < NUM_CLOTHING_PIECES; c++)
                                    {
                                        ...
                                        if (cObj != NULL)
                                        {
                                            ...
                                            if (nextPlayer->clothingContained[c].size() > 0)
                                            {
                                                ...
                                                adult->numContained = nextPlayer->clothingContained[c].size();

                                                adult->containedIDs =
                                                    nextPlayer->clothingContained[c].getElementArray();
                                                adult->containedEtaDecays =
                                                    nextPlayer->clothingContainedEtaDecays[c].getElementArray();

                                                adult->subContainedIDs = new SimpleVector<int>[adult->numContained];
                        ...
                        else
                        {
                            ...
                            int babyBonesGroundID = SettingsManager::getIntSetting("babyBonesGround", -1);
                    ...
                    else
                    {
                        ...
                        if (holdingID < 0)
                            holdingID = 0; // negative ID means a baby
                ...
                else if (m.type == GRAVE)
                {
                    ...
                    DeadObject defaultO = {0, 0, stringDuplicate("~"), defaultLineage, 0, 0};
                    ...
                    if (o != NULL)
                    {
                        ...
                        char *message = autoSprintf("GO\n%d %d %d %d %lf %s%s eve=%d\n#", m.x - nextPlayer->birthPos.x,
                                                    m.y - nextPlayer->birthPos.y, o->id, o->displayID, age,
                                                    formattedName, linString, o->lineageEveID);
                ...
                else if (m.type == OWNER)
                {
                    ...
                    char *leadString = autoSprintf("%d %d", m.x - nextPlayer->birthPos.x, m.y - nextPlayer->birthPos.y);
                ...
                else if (m.type == PHOTO)
                {
                    ...
                    char *photoServerSharedSecret =
                        SettingsManager::getStringSetting("photoServerSharedSecret", "secret_phrase");
                ...
                else if (m.type == FLIP)
                {
                    ...
                    if (currentTime - nextPlayer->lastFlipTime > 1.75)
                    {
                        ...
                        if (oldFacingLeft != nextPlayer->facingLeft)
                        {
                            ...
                            newFlipFacingLeft.push_back(nextPlayer->facingLeft);
                            newFlipPositions.push_back(p);
                        }
                    }
                }
                else if (m.type != SAY && m.type != EMOT && nextPlayer->waitingForForceResponse)
                {
                    ...
                    AppLog::infoF("Ignoring client message because we're "
                                  ...
                                  "relative=(%d, %d)",
                                  nextPlayer->xd, nextPlayer->yd, nextPlayer->xd - nextPlayer->birthPos.x,
                                  nextPlayer->yd - nextPlayer->birthPos.y);
                }
                // if player is still moving (or held by an adult),
                // ignore all actions
                // except for move interrupts
                else if ((nextPlayer->xs == nextPlayer->xd && nextPlayer->ys == nextPlayer->yd &&
                          !nextPlayer->heldByOther) ||
                         m.type == MOVE || m.type == JUMP || m.type == SAY || m.type == EMOT)
                {
                    ...
                    if ((m.type == MOVE || m.type == JUMP) && nextPlayer->heldByOther)
                    {
                        ...
                        if (m.type == JUMP)
                        {
                            ...
                            if (computeAge(nextPlayer) >= 1 || nextPlayer->heldByOtherID != nextPlayer->parentID)
                            {
                    ...
                    else if (m.type == MOVE)
                    {
                                ...
                                */
                                ...
                                if (pathStep != 0)
                                {
                                    ...
                                    if (c == -1)
                                    {
                                        ...
                                        if (firstPos.x == nextPlayer->xs && firstPos.y == nextPlayer->ys)
                                        {
                                            c = 0;
                                        }
                                    }
                        ...
                        */
                        ...
                        if (distance(nextPlayer->lastPlayerUpdateAbsolutePos, m.extraPos[m.numExtraPos - 1]) >
                        ...
                        else
                        {
                            ...
                            */
                            ...
                            if (!startFound && !isGridAdjacentDiag(unfilteredPath.getElementDirect(startIndex).x,
                            ...
                            else
                            {
                                ...
                                if (isMapSpotBlocking(lastValidPathStep.x, lastValidPathStep.y))
                                {
                                    currentBlocked = true;
                                }
                            ...
                            else
                            {
                                ...
                                double dist = measurePathLength(nextPlayer->xs, nextPlayer->ys, nextPlayer->pathToDest,
                                                                nextPlayer->pathLength);

                                double distAlreadyDone = measurePathLength(nextPlayer->xs, nextPlayer->ys,
                                                                           nextPlayer->pathToDest, startIndex);

                                double moveSpeed = computeMoveSpeed(nextPlayer) *
                                                   getPathSpeedModifier(nextPlayer->pathToDest, nextPlayer->pathLength);
                             ...
                             Time::getCurrentTime() - nextPlayer->lastSayTimeSeconds > minSayGapInSeconds)
                    {
                        ...
                        // client)
                        SimpleVector<char *> *tokens = tokenizeString(m.saidText);

                        char *cleanedString;
                        if (tokens->size() > 0)
                        {

                            char **tokensArray = tokens->getElementArray();

                            // join words with single spaces
                            cleanedString = join(tokensArray, tokens->size(), " ");
                        ...
                        if (nextPlayer->ownedPositions.size() > 0)
                        {
                            ...
                            if (namedOwner != NULL)
                            {
                                ...
                                {
                                    LiveObject *otherPlayer = players.getElement(j);
                                    if (!otherPlayer->error && otherPlayer != nextPlayer && otherPlayer->name != NULL &&
                                        strcmp(otherPlayer->name, namedOwner) == 0)
                                    {
                            ...
                            else if (isYouGivingSay(m.saidText))
                            {
                                ...
                                newOwnerPlayer = getClosestOtherPlayer(nextPlayer);
                            }
                            ...
                            if (newOwnerPlayer != NULL)
                            {
                                ...
                                {
                                    GridPos nextPos = nextPlayer->ownedPositions.getElementDirect(j);
                        ...
                        if (nextPlayer->isEve && nextPlayer->name == NULL)
                        {
                            ...
                            if (name != NULL && strcmp(name, "") != 0)
                            {
                                ...
                                nextPlayer->name =
                                    getUniqueCursableName(nextPlayer->name, &(nextPlayer->nameHasSuffix), true);
                                ...
                                if (nextPlayer->declaredInfertile)
                                {
                                    nextPlayer->displayedName =
                                        autoSprintf("%s %s", nextPlayer->name, infertilitySuffix);
                                }
                                else
                                {
                        ...
                        {
                            otherToForgive = getClosestOtherPlayer(nextPlayer);
                        }
                        else
                        {
                            ...
                            if (forgiveName != NULL)
                            {
                                otherToForgive = getPlayerByName(forgiveName, nextPlayer);
                            }
                        }

                        if (otherToForgive != NULL)
                        {
                            clearDBCurse(nextPlayer->id, nextPlayer->email, otherToForgive->email);

                            char *message = autoSprintf("CU\n%d 0 %s_%s\n#", otherToForgive->id,
                                                        getCurseWord(nextPlayer->email, otherToForgive->email, 0),
                                                        getCurseWord(nextPlayer->email, otherToForgive->email, 1));
                            sendMessageToPlayer(nextPlayer, message, strlen(message));
                            delete[] message;
                        }
                        ...
                        if (nextPlayer->holdingID < 0)
                        {
                            ...
                            if (babyO != NULL && babyO->name == NULL)
                            {
                                ...
                                if (name != NULL && strcmp(name, "") != 0)
                                {
                                    nameBaby(nextPlayer, babyO, name, &playerIndicesToSendNamesAbout);
                        ...
                        else
                        {
                            ...
                            if (name != NULL && strcmp(name, "") != 0)
                            {
                                ...
                                LiveObject *closestOther = getClosestOtherPlayer(nextPlayer, babyAge, true);

                                if (closestOther != NULL)
                                {
                                    nameBaby(nextPlayer, closestOther, name, &playerIndicesToSendNamesAbout);

                                    if (closestOther->displayedName != NULL)
                                        delete[] closestOther->displayedName;
                                    if (closestOther->declaredInfertile)
                                    {
                                        closestOther->displayedName =
                                            autoSprintf("%s %s", closestOther->name, infertilitySuffix);
                                    }
                                    else
                                    {
                                ...
                                !getMetadata(nextPlayer->holdingID, metaData))
                            {
                                ...
                                if (strstr(getObject(nextPlayer->holdingID)->description, "+map") != NULL)
                                {
                                    // holding a potential map
                                    // add coordinates to where we're standing
                                    GridPos p = getPlayerPos(nextPlayer);

                                    char *mapStuff = autoSprintf(" *map %d %d %.f", p.x, p.y, Time::timeSec());

                                    int mapStuffLen = strlen(mapStuff);

                                    char *saidText = stringDuplicate(m.saidText);

                                    int saidLen = strlen(saidText);

                                    int extra = saidLen + mapStuffLen - (MAP_METADATA_LENGTH - 1);
                                    ...
                                    textToAdd = autoSprintf("%s%s", saidText, mapStuff);
                                ...
                                nextPlayer->holdingID = addMetadata(nextPlayer->holdingID, metaData);

                                TransRecord *writingHappenTrans = getMetaTrans(0, nextPlayer->holdingID);

                                if (writingHappenTrans != NULL && writingHappenTrans->newTarget > 0 &&
                                    getObject(writingHappenTrans->newTarget)->written)
                                {
                                    ...
                                    // hands as we write
                                    handleHoldingChange(nextPlayer, writingHappenTrans->newTarget);
                                    playerIndicesToSendUpdatesAbout.push_back(i);
                                }
                            }
                        }
                    ...
                    {
                        playerIndicesToSendUpdatesAbout.push_back(i);
                        if (m.id > 0 && nextPlayer->holdingID > 0)
                        {
                                ...
                                if (targetPlayer != NULL)
                                {
                                    ...
                                    if (strstr(heldObj->description, "otherFamilyOnly"))
                                    {
                                        ...
                                        if (targetPlayer->lineageEveID == nextPlayer->lineageEveID ||
                                            isPeaceTreaty(targetPlayer->lineageEveID, nextPlayer->lineageEveID))
                                        {
                    ...
                    else if (m.type == USE)
                    {
                        ...
                        // Log for moderation
                        AppLog::infoF("modLog id:%d %d %d USE x:%d y:%d h:%d t:%d", nextPlayer->id,
                                      nextPlayer->birthPos.x, nextPlayer->birthPos.y, m.x, m.y, nextPlayer->holdingID,
                                      target);
                        ...
                        GridPos newGroundObjectOrigin = {nextPlayer->heldGraveOriginX, nextPlayer->heldGraveOriginY};
                        ...
                        if (nextPlayer->holdingID > 0)
                        {
                            ...
                            else if (heldObj->useDistance > 1)
                            {
                                ...
                                if (heldObj->useDistance >= d && !directLineBlocked(playerPos, targetPos))
                                {
                        ...
                        if (distanceUseAllowed ||
                            (isGridAdjacent(m.x, m.y, nextPlayer->xd, nextPlayer->yd) && !requireExactTileUsage) ||
                            (m.x == nextPlayer->xd && m.y == nextPlayer->yd))
                        {
                            ...
                            else if (nextPlayer->dying)
                            {
                                ...
                                TransRecord *healTrans = getMetaTrans(nextPlayer->holdingID, target);
                                ...
                                if (healTrans != NULL)
                                {
                                    ...
                                    setMapObject(m.x, m.y, healTrans->newTarget);
                                ...
                                else
                                {
                                    ...
                                    if (targetObj != NULL)
                                        if (m.i != -1 && targetObj->permanent && targetObj->numSlots > m.i &&
                                            getNumContained(m.x, m.y) > m.i &&
                                            strstr(targetObj->description, "+useOnContained") != NULL)
                                        {
                                            // a valid slot specified to use
                                            // held object on.
                                            // AND container allows this

                                            int contTarget = getContained(m.x, m.y, m.i);
                                            ...
                                            ObjectRecord *contTargetObj = getObject(contTarget);

                                            TransRecord *contTrans = getPTrans(nextPlayer->holdingID, contTarget);

                                            ObjectRecord *newTarget = NULL;

                                            if (!isSubCont && contTrans != NULL &&
                                                (contTrans->newActor == nextPlayer->holdingID ||
                                                 contTrans->newActor == 0 ||
                                                 canPickup(contTrans->newActor, computeAge(nextPlayer))))
                                            {
                                            ...
                                            if ((newTarget != NULL &&
                                                 containmentPermitted(targetObj->id, newTarget->id)) ||
                                                (contTrans != NULL && contTrans->newTarget == 0))
                                            {

                                                int oldHeld = nextPlayer->holdingID;

                                                handleHoldingChange(nextPlayer, contTrans->newActor);
                                                ...
                                                if (contTrans->newActor > 0 && contTrans->newActor != oldHeld)
                                                {
                                                ...
                                                changeContained(m.x, m.y, m.i, contTrans->newTarget);
                                ...
                                if (healed)
                                {
                                    ...
                                    nextPlayer->heldOriginX = 0;
                                    nextPlayer->heldOriginY = 0;
                                    nextPlayer->heldTransitionSourceID = healTarget;
                            ...
                            else if (target != 0)
                            {
                                ...
                                // 2HOL mechanics to read written objects
                                if (targetObj->written && targetObj->clickToRead)
                                {
                                ...
                                if (nextPlayer->holdingID >= 0 && heldCanBeUsed)
                                {
                                    ...
                                    {
                                        r = getPTrans(nextPlayer->holdingID, target, false, false, 1);
                                        if (r != NULL)
                                    ...
                                    if (r == NULL)
                                    {
                                        ...
                                        TransRecord *heldDecay = getPTrans(-1, nextPlayer->holdingID);
                                        if (heldDecay != NULL && heldDecay->autoDecaySeconds == 1 &&
                                            heldDecay->newTarget > 0)
                                        {
                                            // force decay NOW and try again
                                            handleHeldDecay(nextPlayer, i, &playerIndicesToSendUpdatesAbout,
                                    ...
                                    if (r == NULL)
                                    {
                                        ...
                                        int newTarget = checkTargetInstantDecay(target, m.x, m.y);
                                ...
                                if (r != NULL && r->newActor > 0 && getObject(r->newActor)->floor)
                                {
                                    // special case:
                                ...
                                if (r != NULL && targetObj->numSlots > 0)
                                {
                                    ...
                                    if (numContained > 0 && numSlotsInNew > 0 && numContained <= numSlotsInNew &&
                                        r->newTarget > 0 && getObject(r->newTarget)->slotSize < targetObj->slotSize)
                                    {
                                        ...
                                        {
                                            int contained = getContained(m.x, m.y, i);
                                            if (contained < 0)
                                                contained = -contained;
                                    ...
                                    if (r != NULL)
                                        if (numContained > numSlotsInNew && numSlotsInNew == 0)
                                        {
                                            ...
                                            if (r->newActor > 0 && nextPlayer->numContained == 0 &&
                                                (oldSlots > 0 && newSlots == 0 && r->actor == 0 && r->newActor > 0 &&
                                ...
                                if (r == NULL && (nextPlayer->holdingID != 0 || targetObj->permanent) &&
                                     ...
                                     (m.x == nextPlayer->xd && m.y == nextPlayer->yd)))
                                {
                                    ...
                                    else if (nextPlayer->holdingID <= 0 || targetObj->numSlots == 0)
                                    {
                                ...
                                if (r != NULL && r->newTarget > 0 && r->newTarget != target)
                                {
                                    ...
                                    if (getMapFloor(m.x, m.y) != 0)
                                    {
                                        ...
                                        // to exist on floor
                                        if (strstr(getObject(r->newTarget)->description, "groundOnly") != NULL)
                                        {
                                            r = NULL;
                                        }
                                    }
                                ...
                                if (r == NULL && nextPlayer->holdingID > 0)
                                {
                                ...
                                {
                                    char *des = getObject(r->newTarget)->description;
                                    ...
                                    if (desPos != NULL)
                                    {
                                        ...
                                        sscanf(desPos, "+hungryWork%d", &hungryWorkCost);

                                        if (nextPlayer->foodStore < hungryWorkCost)
                                        {
                                            // block transition,
                                            // not enough food
                                            r = NULL;
                                ...
                                if (r != NULL && oldHolding > 0 && r->newTarget > 0 &&
                                          ...
                                          isMapSpotEmptyOfPlayers(m.x, m.y)))
                                {
                                    ...
                                    if (oldSlots > 0 && newSlots == 0 && r->actor == 0 && r->newActor > 0 &&
                                        getNumContainerSlots(r->newActor) == oldSlots &&
                                        getObject(r->newActor)->slotSize >= targetObj->slotSize)
                                    {
                                        ...
                                        restretchDecays(nextPlayer->numContained, nextPlayer->containedEtaDecays,
                                                        target, r->newActor);
                                    }
                                    else
                                    {
                                        ...
                                        if (newSlots > 0)
                                        {
                                            restretchMapContainedDecays(m.x, m.y, target, r->newTarget);
                                        }
                                    }

                                    // Check for containment transitions - changing container by USE

                                    if (oldSlots > 0 && newSlots > 0 &&
                                        ...
                                        oldSlots == newSlots)
                                    {
                                        ...
                                        if (numContained > 0)
                                        {
                                            ...
                                            for (int i = 0; i < numContained; i++)
                                            {
                                                ...
                                                // The other flags don't make sense here, we're changing the container
                                                // itself not interacting with the contained items
                                                ...
                                                {
                                                    containmentTrans =
                                                        getPTrans(containedID, newContainerID, false, false, 4);
                                                    if (containmentTrans == NULL)
                                                        containmentTrans =
                                                            getPTrans(0, newContainerID, false, false, 4);
                                                    if (containmentTrans != NULL)
                                                        newContainedID = containmentTrans->newActor;
                                                }
                                                if (containmentTrans == NULL)
                                                {
                                                    containmentTrans =
                                                        getPTrans(oldContainedID, containedID, false, false, 4);
                                                    if (containmentTrans == NULL)
                                                        containmentTrans = getPTrans(0, containedID, false, false, 4);
                                                    if (containmentTrans != NULL)
                                                        newContainedID = containmentTrans->newTarget;
                                                }
                                    ...
                                    if (containmentTransition)
                                    {
                                        ...
                                        addContained(m.x, m.y, idToAdd, nextPlayer->holdingEtaDecay);

                                        handleHoldingChange(nextPlayer, 0);
                                    }
                                    ...
                                    if (target > 0 && r->newTarget > 0 && target != r->newTarget &&
                                        !getObject(target)->isOwned && getObject(r->newTarget)->isOwned)
                                    {
                                    ...
                                    if (oldHolding > 0 && r->newTarget > 0 && getObject(oldHolding)->passwordAssigner &&
                                        getObject(r->newTarget)->passwordProtectable)
                                    {
                                    ...
                                    if (r->actor == 0 && target > 0 && r->newTarget > 0 && target != r->newTarget)
                                    {
                                        ...
                                        if (oldDecayTrans != NULL && newDecayTrans != NULL &&
                                            oldDecayTrans->epochAutoDecay == newDecayTrans->epochAutoDecay &&
                                            oldDecayTrans->autoDecaySeconds == newDecayTrans->autoDecaySeconds &&
                                ...
                                else if (nextPlayer->holdingID == 0 && !targetObj->permanent &&
                                ...
                                else if (nextPlayer->holdingID >= 0)
                                {
                                        ...
                                        strstr(targetObj->description, "+useOnContained") != NULL)
                                    {
                                        ...
                                        if (!isSubCont && contTrans != NULL &&
                                            (contTrans->newActor == nextPlayer->holdingID || contTrans->newActor == 0 ||
                                             canPickup(contTrans->newActor, computeAge(nextPlayer))))
                                        {
                                            ...
                                            if (numContained == 1)
                                            {
                                                ...
                                                if (containmentTrans == NULL)
                                                    containmentTrans = getPTrans(0, containerID, false, false, 1);
                                            }
                                            else if (targetObj->numSlots == numContained)
                                            {
                                                containmentTrans = getPTrans(containedID, containerID, false, false, 2);
                                                if (containmentTrans == NULL)
                                                    containmentTrans = getPTrans(0, containerID, false, false, 2);
                                            }
                                            ...
                                            if (containmentTrans == NULL)
                                            {
                                                ...
                                                if (containmentTrans == NULL)
                                                    containmentTrans = getPTrans(0, containerID, false, false, 3);
                                            }
                                            ...
                                            if (containmentTrans == NULL)
                                            {
                                                ...
                                                if (containmentTrans == NULL)
                                                    containmentTrans = getPTrans(0, target, false, false, 4);
                                            }
                                            ...
                                            if (containmentTrans == NULL)
                                            {
                                                ...
                                                {
                                                    containmentTrans =
                                                        getPTrans(containerID, oldContainedID, false, false, 2);
                                                    if (containmentTrans == NULL)
                                                        containmentTrans = getPTrans(containerID, 0, false, false, 2);
                                                }
                                                else if (targetObj->numSlots == numContained)
                                                {
                                                    containmentTrans =
                                                        getPTrans(containerID, oldContainedID, false, false, 1);
                                                    if (containmentTrans == NULL)
                                                        containmentTrans = getPTrans(containerID, 0, false, false, 1);
                                                }
                                            }
                                            ...
                                            {
                                                containmentTrans =
                                                    getPTrans(containerID, oldContainedID, false, false, 3);
                                                if (containmentTrans == NULL)
                                                    containmentTrans = getPTrans(containerID, 0, false, false, 3);
                                            }
                                            ...
                                            if (containmentTrans == NULL)
                                            {
                                                ...
                                                if (containmentTrans == NULL)
                                                    containmentTrans = getPTrans(target, 0, false, false, 4);
                                            }

                                            if (containmentTrans != NULL && noInContTrans)
                                                isOutContTrans = true;
                                            ...
                                            if (containmentTrans != NULL)
                                            {
                                                ...
                                                if (isOutContTrans)
                                                    newContainerID = containmentTrans->newActor;

                                                int newNumSlots = getNumContainerSlots(newContainerID);

                                                if ((isOutContTrans &&
                                                     containmentTrans->target != containmentTrans->newTarget) ||
                                                    (!isOutContTrans &&
                                                     containmentTrans->actor != containmentTrans->newActor))
                                                {
                                                    ...
                                                    // Let the useOnContained transition preceeds the containment
                                                    // transition
                                                ...
                                                else
                                                {
                                                    ...
                                                    newContainedOK =
                                                        containmentPermitted(newContainerID, contTrans->newTarget);

                                                    int slotNumber = numContained - 1;
                                                    if (isOutContTrans && slotNumber == m.i)
                                                        slotNumber--;

                                                    if (slotNumber >= 0)
                                                    {

                                                        int contID = getContained(m.x, m.y, slotNumber);
                                                               ...
                                                               containmentPermitted(newContainerID, contID))
                                                        {
                                                            ...
                                                            contID = getContained(m.x, m.y, slotNumber);
                                                    ...
                                                    if (!otherContainedOK || !newContainedOK)
                                                        containmentTrans = NULL;
                                                    if (!newContainedOK)
                                                    {
                                        ...
                                        if (newTarget != NULL && containmentPermitted(targetObj->id, newTarget->id) &&
                                            !blockedByContainmentTrans)
                                        {
                                            ...
                                            if (contTrans->actor == 0 && contTrans->target > 0 &&
                                                contTrans->newTarget > 0 && contTrans->target != contTrans->newTarget)
                                            {

                                                TransRecord *oldDecayTrans = getTrans(-1, contTrans->target);

                                                TransRecord *newDecayTrans = getTrans(-1, contTrans->newTarget);
                                    ...
                                    if (!handled)
                                    {
                                        if (nextPlayer->holdingID == 0 && targetObj->permanent)
                                        {
                                ...
                                if (targetObj->permanent && (targetObj->foodValue > 0 || targetObj->bonusValue > 0))
                                {
                                    ...
                                    nextPlayer->foodStore += targetObj->foodValue;

                                    updateYum(nextPlayer, targetObj->id);

                                    int bonus = getEatBonus(nextPlayer);

                                    logEating(targetObj->id, targetObj->foodValue + bonus, computeAge(nextPlayer), m.x,
                                              m.y);
                                    ...
                                    if (oldHolding == 0 && nextPlayer->holdingID != 0)
                                    {
                                        ...
                                        if (newHeld->foodValue > 0)
                                        {
                                            ...
                                            if (nextPlayer->lastAteFillMax > nextPlayer->foodStore)
                                            {
                                    ...
                                    nextPlayer->foodDecrementETASeconds =
                                        Time::getCurrentTime() + computeFoodDecrementTimeSeconds(nextPlayer);
                            ...
                            else if (nextPlayer->holdingID > 0)
                            {
                                ...
                                if (floorID > 0)
                                {
                                    ...
                                    if (r != NULL &&
                                        ...
                                        // transition
                                        (r->newActor == 0 ||
                                         getObject(r->newActor)->minPickupAge <= computeAge(nextPlayer)))
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            char canPlace = true;
                                            if (getObject(resultID)->blocksWalking && !isMapSpotEmpty(m.x, m.y))
                                            {
                                                canPlace = false;
                                            }
                                ...
                                if (!usedOnFloor && obj->foodValue == 0)
                                {
                                         ...
                                         getObject(r->newActor)->minPickupAge <= computeAge(nextPlayer)))
                                    {
                                        ...
                                        if (newTargetObj->blocksWalking && !isMapSpotEmpty(m.x, m.y))
                                        {
                                        ...
                                        else if (strstr(newTargetObj->description, "groundOnly") != NULL &&
                                    ...
                                    if (canPlace)
                                    {
                                        nextPlayer->heldTransitionSourceID = nextPlayer->holdingID;

                                        if (nextPlayer->numContained > 0 && r->newActor == 0 && r->newTarget > 0 &&
                            ...
                            else
                            {
                                ...
                                if (floorID > 0)
                                {
                                    ...
                                    TransRecord *r = getPTrans(0, floorID);
                            ...
                            if (target == 0 && newGroundObject > 0)
                            {
                                ...
                                if (strstr(o->description, "origGrave") != NULL)
                                {
                                    ...
                                    GraveMoveInfo g = {
                                        {newGroundObjectOrigin.x, newGroundObjectOrigin.y}, {m.x, m.y}, swapDest};
                                    newGraveMoves.push_back(g);
                                }
                            }
                        }
                    ...
                    else if (m.type == BABY)
                    {
                             ...
                             (m.x == nextPlayer->xd && m.y == nextPlayer->yd)))
                        {
                            ...
                            if (nextPlayer->holdingID == 0)
                            {
                                ...
                                // is anyone there?
                                LiveObject *hitPlayer = getHitPlayer(m.x, m.y, m.id, false, babyAge);

                                if (hitPlayer != NULL && !hitPlayer->heldByOther && computeAge(hitPlayer) < babyAge)
                                {
                                    ...
                                    if (hitPlayer->holdingID != 0)
                                    {
                                        ...
                                        // while held
                                        if (!hitPlayer->holdingWound && hitPlayer->holdingID > 0)
                                        {
                                            handleDrop(m.x, m.y, hitPlayer, &playerIndicesToSendUpdatesAbout);
                                        }
                                    }

                                    if (hitPlayer->xd != hitPlayer->xs || hitPlayer->yd != hitPlayer->ys)
                                    {
                                    ...
                                    if (isFertileAge(nextPlayer))
                                    {
                                        ...
                                        hitPlayer->foodDecrementETASeconds =
                                            Time::getCurrentTime() + computeFoodDecrementTimeSeconds(hitPlayer);

                                        checkForFoodEatingEmot(hitPlayer, 0);
                    ...
                    else if (m.type == SELF || m.type == UBABY)
                    {
                        ...
                        if (nextPlayer->holdingID > 0)
                        {
                            ...
                            if (obj->foodValue > 0 || eatEverythingMode)
                            {
                                ...
                                if (strstr(obj->description, "noFeeding") != NULL)
                                {
                        ...
                        else if (m.type == UBABY)
                        {
                                ...
                                (m.x == nextPlayer->xd && m.y == nextPlayer->yd))
                            {
                                ...
                                // try click on baby
                                int hitIndex;
                                LiveObject *hitPlayer = getHitPlayer(m.x, m.y, m.id, false, babyAge, -1, &hitIndex);
                                ...
                                if (false) // food with noFeeding tag cannot be fed even to elderly
                                    if (hitPlayer == NULL || hitPlayer == nextPlayer)
                                    {
                                        ...
                                        hitPlayer = getHitPlayer(m.x, m.y, m.id, false, -1, 55, &hitIndex);
                                    }

                                if ((hitPlayer == NULL || hitPlayer == nextPlayer) && holdingFood)
                                {

                                    // feeding action
                                    // try click on everyone
                                    hitPlayer = getHitPlayer(m.x, m.y, m.id, false, -1, -1, &hitIndex);
                                }

                                if ((hitPlayer == NULL || hitPlayer == nextPlayer) && !holdingDrugs)
                                {
                        ...
                        if (targetPlayer != NULL)
                        {
                            ...
                            ObjectRecord **clothingSlot = getClothingSlot(targetPlayer, m.i);

                            int targetClothingID = 0;
                            if (clothingSlot != NULL)
                                targetClothingID = (*clothingSlot)->id;

                            // Log for moderation - cases other than this main one is not logged
                            AppLog::infoF("modLog id:%d %d %d SELF x:%d y:%d h:%d t:%d %d", nextPlayer->id,
                                          nextPlayer->birthPos.x, nextPlayer->birthPos.y, m.x, m.y,
                                          nextPlayer->holdingID, targetClothingID, m.i);
                            ...
                            if (targetPlayer != nextPlayer && targetPlayer->dying && !holdingFood)
                            {

                                // try healing wound

                                TransRecord *healTrans = getMetaTrans(nextPlayer->holdingID, targetPlayer->holdingID);
                                ...
                                if (healTrans != NULL)
                                {
                                    ...
                                    if (healerWillHold > 0)
                                    {
                                        if (computeAge(nextPlayer) < getObject(healerWillHold)->minPickupAge)
                                        {
                                            oldEnough = false;
                                        }
                                    }
                                ...
                                if (oldEnough && healTrans != NULL)
                                {
                                    ...
                                    {
                                        // not dying anymore
                                        setNoLongerDying(targetPlayer, &playerIndicesToSendHealingAbout);
                                    }
                                    else
                                    {
                                        // wound changed?

                                        ForcedEffects e = checkForForcedEffects(targetPlayer->holdingID);
                            ...
                            else if (targetPlayer == nextPlayer && nextPlayer->dying && m.i >= 0 &&
                                     m.i < NUM_CLOTHING_PIECES)
                            {

                                ObjectRecord *clickedClothing = clothingByIndex(nextPlayer->clothing, m.i);
                                ...
                                TransRecord *healTrans = getMetaTrans(nextPlayer->holdingID, clickedClothingID);
                                ...
                                if (healTrans != NULL)
                                {
                                    ...
                                    setClothingByIndex(&(nextPlayer->clothing), m.i, getObject(healTrans->newTarget));
                            ...
                            else if (nextPlayer->holdingID > 0)
                            {
                                ...
                                if (m.i >= 0 && m.i < NUM_CLOTHING_PIECES)
                                {
                                    ...
                                    if (clickedClothing != NULL)
                                    {
                                        ...
                                        clickedClothingTrans = getPTrans(nextPlayer->holdingID, clickedClothing->id);

                                        // Check for containment transitions - clickedClothing
                                        if (clickedClothingTrans == NULL)
                                        {
                                            clickedClothingTrans =
                                                getPTrans(nextPlayer->holdingID, clickedClothing->id, false, false, 1);
                                        ...
                                        if (clickedClothingTrans != NULL)
                                        {
                                            ...
                                            if (nt > 0 && getObject(nt)->clothing != clickedClothing->clothing)
                                            {
                                                // don't allow transitions
                                                // that leave a non-wearable
                                                // item on your body
                                ...
                                if (targetPlayer == nextPlayer && m.i >= 0 && m.i < NUM_CLOTHING_PIECES &&
                                    addHeldToClothingContainer(nextPlayer, m.i, false, &couldHaveGoneIn))
                                {
                                    // worked!
                                }
                                // next case:  can what they're holding
                                // be used to transform clothing?
                                else if (m.i >= 0 && m.i < NUM_CLOTHING_PIECES && clickedClothing != NULL &&
                                         clickedClothingTrans != NULL)
                                {
                                    ...
                                    handleHoldingChange(nextPlayer, clickedClothingTrans->newActor);

                                    setClothingByIndex(&(nextPlayer->clothing), m.i,
                                                       getObject(clickedClothingTrans->newTarget));

                                    // Execute containment transitions - clickedClothing
                                    if (isContainmentTransition)
                                    {
                                        addHeldToClothingContainer(nextPlayer, m.i, false, &couldHaveGoneIn, true);
                                    }
                                }
                                         ...
                                         !couldHaveGoneIn)
                                {
                                    ...
                                    logEating(obj->id, obj->foodValue + bonus, computeAge(targetPlayer), m.x, m.y);

                                    targetPlayer->foodStore += bonus;

                                    checkForFoodEatingEmot(targetPlayer, obj->id);
                                    ...
                                    targetPlayer->foodDecrementETASeconds =
                                        Time::getCurrentTime() + computeFoodDecrementTimeSeconds(targetPlayer);
                                ...
                                // we could put on
                                         ...
                                         (targetPlayer == nextPlayer || computeAge(targetPlayer) < babyAge))
                                {
                                    ...
                                    if (clothingSlot != NULL)
                                    {
                                        ...
                                        oldNumContained = targetPlayer->clothingContained[ind].size();

                                        if (oldNumContained > 0)
                                        {
                                            oldContainedIDs = targetPlayer->clothingContained[ind].getElementArray();
                                        ...
                                        *clothingSlot = obj;
                                        ...
                                        if (nextPlayer->numContained > 0)
                                        {
                                            ...
                                            targetPlayer->clothingContained[ind].appendArray(nextPlayer->containedIDs,
                                                                                             nextPlayer->numContained);

                                            targetPlayer->clothingContainedEtaDecays[ind].appendArray(
                                                nextPlayer->containedEtaDecays, nextPlayer->numContained);
                            ...
                            else
                            {
                                ...
                                TransRecord *bareHandClothingTrans = getBareHandClothingTrans(nextPlayer, clothingSlot);

                                if (targetPlayer == nextPlayer && bareHandClothingTrans != NULL)
                                {
                                    ...
                                    handleHoldingChange(nextPlayer, bareHandClothingTrans->newActor);
                                ...
                                else if (clothingSlot != NULL)
                                {
                                    ...
                                    removeClothingToHold(nextPlayer, targetPlayer, clothingSlot, clothingSlotIndex);
                                }
                            }
                        }
                    }
                    else if (m.type == DROP)
                    {
                        ...
                        if (nextPlayer->holdingID > 0 && getObject(nextPlayer->holdingID)->permanent)
                        {
                            canDrop = false;
                        }
                        ...
                        if (nextPlayer->holdingID > 0 && getObject(nextPlayer->holdingID)->useDistance == 0 &&
                            (m.x != nextPlayer->xd || m.y != nextPlayer->yd))
                        {
                        ...
                        // Log for moderation
                        AppLog::infoF("modLog id:%d %d %d DROP x:%d y:%d h:%d t:%d", nextPlayer->id,
                                      nextPlayer->birthPos.x, nextPlayer->birthPos.y, m.x, m.y, nextPlayer->holdingID,
                                      target);
                                 ...
                                 (m.x == nextPlayer->xd && m.y == nextPlayer->yd)))
                            {
                                ...
                                if (nextPlayer->holdingID != 0)
                                {
                                    ...
                                    if (nextPlayer->holdingID < 0)
                                    {
                                        ...
                                        {
                                            handleDrop(m.x, m.y, nextPlayer, &playerIndicesToSendUpdatesAbout);
                                        }
                                    }
                                    ...
                                    else if (canDrop && m.c >= 0 && m.c < NUM_CLOTHING_PIECES &&
                                             m.x == nextPlayer->xd && m.y == nextPlayer->yd &&
                                             nextPlayer->holdingID > 0)
                                    {
                                        ...
                                        if (nextPlayer->holdingID == 0)
                                        {
                                            ...
                                            // up
                                            for (int s = 0; s < nextPlayer->clothingContained[m.c].size() - 1; s++)
                                            {

                                                int otherID = nextPlayer->clothingContained[m.c].getElementDirect(s);

                                                int oldHeldAfterTrans =
                                                    nextPlayer->clothingContained[m.c].getElementDirect(
                                                ...
                                                if (otherID != oldHeldAfterTrans &&
                                                    getObject(otherID)->minPickupAge <= playerAge &&
                                                    (!getObject(otherID)->permanent || hasPickUpTrans))
                                                {

                                                    removeFromClothingContainerToHold(nextPlayer, m.c, s, true);
                                                    break;
                                                }
                                            }
                                            ...
                                            ObjectRecord *cObj = clothingByIndex(nextPlayer->clothing, m.c);
                                            if (nextPlayer->clothingContained[m.c].size() > cObj->numSlots)
                                            {
                                                ...
                                                removeFromClothingContainerToHold(
                                                    nextPlayer, m.c, nextPlayer->clothingContained[m.c].size() - 1,
                                                    true);
                                            }
                                        }
                                    }
                                    else if (nextPlayer->holdingID > 0)
                                    {
                                        ...
                                        ObjectRecord *droppedObj = getObject(nextPlayer->holdingID);
                                        ...
                                        if (target != 0)
                                        {
                                            ...
                                            if (canDrop && containmentPermitted(target, droppedObj->id))
                                            {
                                                canGoIn = true;
                                            }
                                            ...
                                            if (canDrop && canGoIn && targetSlots > 0 &&
                                                nextPlayer->numContained == 0 && getNumContained(m.x, m.y) == 0)
                                            {
                                                ...
                                                TransRecord *t = getPTrans(nextPlayer->holdingID, target);
                                            ...
                                            if (targetObj->permanent)
                                            {
                                                ...
                                                bool targetHasPickupTrans = pickupTrans != NULL &&
                                            ...
                                            // so use swap mode
                                            if (canDrop && canGoIn && !forceUse &&
                                                addHeldToContainer(nextPlayer, target, m.x, m.y, containerAllowSwap))
                                            {
                                                // handled
                                            }
                                            else if (forceUse || (canDrop && !canGoIn && targetIsTrulyPermanent &&
                                                                  nextPlayer->numContained == 0))
                                            {
                                                ...
                                                m.id = -1;
                                                m.c = -1;
                                                playerIndicesToSendUpdatesAbout.deleteElementEqualTo(i);
                                                goto RESTART_MESSAGE_ACTION;
                                            }
                                            else if (canDrop && !canGoIn && !targetIsTrulyPermanent &&
                                                     canPickup(targetObj->id, computeAge(nextPlayer)))
                                            {
                                                    ...
                                                    oldNumContained == nextPlayer->numContained)
                                                {
                                                    ...
                                                    if (oldNumContained == 0 && oldHeld == target)
                                                    {
                                                        ...
                                                        // on target
                                                        TransRecord *sameTrans = getPTrans(oldHeld, target);
                                                        if (sameTrans != NULL && sameTrans->newActor == 0)
                                                        {
                                                            // keep it simple
                                                            // for now
                                                            // this is usually
                                                            // just about
                                                            // stacking
                                                            handleHoldingChange(nextPlayer, sameTrans->newActor);
                                                        ...
                                                        else
                                                        {
                                                            ...
                                                            TransRecord *contTrans =
                                                                getPTrans(oldHeld, target, false, false, 1);
                                                            if (contTrans != NULL)
                                                            {
                                                                handleHoldingChange(nextPlayer, 0);

                                                                setMapObject(m.x, m.y, contTrans->newTarget);

                                                                addContained(m.x, m.y, contTrans->newActor,
                    ...
                    else if (m.type == REMV)
                    {
                        ...
                        // Log for moderation
                        AppLog::infoF("modLog id:%d %d %d REMV x:%d y:%d h:%d t:%d", nextPlayer->id,
                                      nextPlayer->birthPos.x, nextPlayer->birthPos.y, m.x, m.y, nextPlayer->holdingID,
                                      target);
                            ...
                            if (!accessBlocked)
                                if (handEmpty && nextPlayer->holdingID == 0)
                                {
                                    ...
                                    if (target > 0)
                                    {
                                        ...
                                        if (!targetObj->permanent && targetObj->minPickupAge <= computeAge(nextPlayer))
                                        {
                    ...
                    else if (m.type == SREMV)
                    {
                        ...
                        if (m.x == nextPlayer->xd && m.y == nextPlayer->yd && nextPlayer->holdingID == 0)
                        {
                            ...
                            {
                                worked = removeFromClothingContainerToHold(nextPlayer, m.c, m.i);
                            }
                        }

                        if (nextPlayer->holdingID == 0 && m.c >= 0 && m.c < NUM_CLOTHING_PIECES && !worked)
                        {
                    ...
                    else if (m.type == EMOT && !nextPlayer->emotFrozen)
                    {
                        ...
                        if (m.i <= SettingsManager::getIntSetting("allowedEmotRange", 6))
                        {
                            ...
                            SimpleVector<int> *forbidden = SettingsManager::getIntSettingMulti("forbiddenEmots");
        ...
        for (int i = 0; i < activeKillStates.size(); i++)
        {
            ...
            if (getObject(killer->holdingID)->deadlyDistance >= dist && !directLineBlocked(playerPos, targetPos))
            {
                // close enough to kill

                executeKillAction(getLiveObjectIndex(s->killerID), getLiveObjectIndex(s->targetID),
                                  &playerIndicesToSendUpdatesAbout, &playerIndicesToSendDyingAbout, &newEmotPlayerIDs,
                                  &newEmotIndices, &newEmotTTLs);
            }
            else
            {
        ...
        for (int i = 0; i < numLive; i++)
        {
            ...
            if (nextPlayer->tripping)
            {
                ...
                else if (!nextPlayer->emotFrozen && curTime < nextPlayer->trippingEffectETA)
                {
                    nextPlayer->emotFrozen = true;
            ...
            if (nextPlayer->drunkennessEffect)
            {
                ...
                else if (!nextPlayer->emotFrozen && Time::getCurrentTime() < nextPlayer->drunkennessEffectETA)
                {
                    nextPlayer->emotFrozen = true;
            ...
            if (nextPlayer->connected == false ||
                (afkTimeSeconds > 0 && Time::getCurrentTime() - nextPlayer->lastActionTime > afkTimeSeconds))
            {
        ...
        for (int i = 0; i < numLive; i++)
        {
            ...
            if (nextPlayer->emotFrozen && nextPlayer->emotUnfreezeETA != 0 && curTime >= nextPlayer->emotUnfreezeETA)
            {
            ...
            if (!nextPlayer->error && !nextPlayer->cravingKnown && computeAge(nextPlayer) >= minAgeForCravings)
            {

                sendCraving(nextPlayer);
            }

            if (nextPlayer->dying && !nextPlayer->error && curTime >= nextPlayer->dyingETA)
            {
                // finally died
                nextPlayer->error = true;
                ...
                if (!nextPlayer->isTutorial)
                {
                    ...
                    logDeath(nextPlayer->id, nextPlayer->email, nextPlayer->isEve, computeAge(nextPlayer),
                             getSecondsPlayed(nextPlayer), !getFemale(nextPlayer), nextPlayer->xd, nextPlayer->yd,
                             players.size() - 1, false, killerID, nextPlayer->murderPerpEmail);
            ...
            else if (nextPlayer->isNewCursed)
            {
                ...
                for (int p = 0; p < players.size(); p++)
                {
                    ...
                    if (otherPlayer->error || !otherPlayer->connected)
                    {
                        continue;
                    }

                    if (isCursed(otherPlayer->email, nextPlayer->email))
                    {
                        char *message = autoSprintf("CU\n%d 1 %s_%s\n#", nextPlayer->id,
                                                    getCurseWord(otherPlayer->email, nextPlayer->email, 0),
                                                    getCurseWord(otherPlayer->email, nextPlayer->email, 1));

                        sendMessageToPlayer(otherPlayer, message, strlen(message));
                        delete[] message;
                    }
                }
            ...
            else if (nextPlayer->error && !nextPlayer->deleteSent)
            {
                ...
                if (!nextPlayer->isTutorial)
                {
                    recordPlayerLineage(nextPlayer->email, age, nextPlayer->id, nextPlayer->parentID,
                                        nextPlayer->displayID, killerID, nextPlayer->name, nextPlayer->lastSay, male);
                ...
                if (SettingsManager::getIntSetting("babyApocalypsePossible", 1) &&
                    players.size() > SettingsManager::getIntSetting("minActivePlayersForBabyApocalypse", 15))
                {
                    ...
                    if (!nextPlayer->isEve)
                    {
                        ...
                        if (barrierOn && (abs(dropPos.x) > barrierRadius || abs(dropPos.y) > barrierRadius))
                        {

                            insideBarrier = false;
                        }

                        float threshold = SettingsManager::getFloatSetting("babySurvivalYearsBeforeApocalypse", 15.0f);
                        ...
                        else
                        {
                            ...
                            if (lastBabyPassedThresholdTime > 0 &&
                                curTime - lastBabyPassedThresholdTime >
                                    SettingsManager::getIntSetting("babySurvivalWindowSecondsBeforeApocalypse", 3600))
                            {
                ...
                if (!nextPlayer->isTutorial)
                {
                    ...
                    recordLineage(nextPlayer->email, nextPlayer->originalBirthPos, yearsLived,
                ...
                if (!nextPlayer->deathLogged)
                {
                    ...
                    if (!nextPlayer->isTutorial)
                    {
                        logDeath(nextPlayer->id, nextPlayer->email, nextPlayer->isEve, age,
                                 getSecondsPlayed(nextPlayer), male, dropPos.x, dropPos.y, players.size() - 1,
                                 disconnect, killerID, nextPlayer->murderPerpEmail);
                ...
                if (deathID != 0)
                {
                    ...
                    while (oldObject != 0 && n < 4)
                    {
                        ...
                        if (!isGrave(oldObject))
                        {
                            ...
                            if (deathMarkerHasSlots && r->numSlots == 0 && !r->permanent && !r->rideable)
                            {
                                ...
                                oldContainedETADecay.push_back(getEtaDecay(dropPos.x, dropPos.y));
                ...
                if (!isMapSpotEmpty(dropPos.x, dropPos.y, false))
                {
                    ...
                    GridPos newDropPos = findClosestEmptyMapSpot(dropPos.x, dropPos.y, 100, &foundEmpty);
                ...
                if (isMapSpotEmpty(dropPos.x, dropPos.y, false))
                {
                    ...
                    if (deathID > 0)
                    {
                        ...
                        GraveInfo graveInfo = {dropPos, nextPlayer->id, nextPlayer->lineageEveID};
                        ...
                        if ((nextPlayer->name == NULL && nextPlayer->displayedName == NULL) ||
                        ...
                        if (roomLeft >= 5)
                        {
                            ...
                            if (nextPlayer->clothing.tunic != NULL)
                            {
                                ...
                                addContained(dropPos.x, dropPos.y, nextPlayer->clothing.tunic->id,
                            ...
                            if (nextPlayer->clothing.bottom != NULL)
                            {
                                ...
                                addContained(dropPos.x, dropPos.y, nextPlayer->clothing.bottom->id,
                            ...
                            if (nextPlayer->clothing.backpack != NULL)
                            {
                                ...
                                addContained(dropPos.x, dropPos.y, nextPlayer->clothing.backpack->id,
                            ...
                            if (nextPlayer->clothing.backShoe != NULL)
                            {
                                ...
                                addContained(dropPos.x, dropPos.y, nextPlayer->clothing.backShoe->id,
                            ...
                            if (nextPlayer->clothing.frontShoe != NULL)
                            {
                                ...
                                addContained(dropPos.x, dropPos.y, nextPlayer->clothing.frontShoe->id,
                        ...
                        for (int c = 0; c < NUM_CLOTHING_PIECES && roomLeft > 0; c++)
                        {
                            ...
                            for (int cc = 0; cc < nextPlayer->clothingContained[c].size() && roomLeft > 0; cc++)
                            {
                                    ...
                                    oldStretch != newStretch)
                                {
                                    ...
                                    timeSec_t offset =
                                        nextPlayer->clothingContainedEtaDecays[c].getElementDirect(cc) - curTime;

                                    offset = offset * oldStretch;
                                    offset = offset / newStretch;

                                    *(nextPlayer->clothingContainedEtaDecays[c].getElement(cc)) = curTime + offset;
                                }
                        ...
                        {
                            addContained(dropPos.x, dropPos.y, oldContained.getElementDirect(oc),
                ...
                if (nextPlayer->holdingID != 0)
                {
                    ...
                    if (nextPlayer->murderSourceID > 0)
                    {
                        ...
                        TransRecord *woundHit = getPTrans(nextPlayer->murderSourceID, 0, true, false);
            ...
            else if (!nextPlayer->error)
            {
                ...
                if (nextPlayer->holdingEtaDecay != 0 && nextPlayer->holdingEtaDecay < curTime)
                {
                ...
                if (nextPlayer->holdingID > 0 && nextPlayer->numContained > 0)
                {
                    ...
                    for (int c = 0; c < nextPlayer->numContained; c++)
                    {
                        ...
                        SimpleVector<timeSec_t> subContDecay = nextPlayer->subContainedEtaDecays[c];
                        ...
                        if (newDecay != 0 && newDecay < curTime)
                        {
                            ...
                            if (t != NULL)
                            {
                                ...
                                if (newID != 0)
                                {
                                    float stretch = getObject(nextPlayer->holdingID)->slotTimeStretch;

                                    TransRecord *newDecayT = getMetaTrans(-1, newID);

                                    if (newDecayT != NULL)
                                    {
                                        newDecay = Time::getCurrentTime() + newDecayT->autoDecaySeconds / stretch;
                                    }
                                    else
                                    {
                                        // no further decay
                            ...
                            if (newSubDecay != 0 && newSubDecay < curTime)
                            {
                                ...
                                if (t != NULL)
                                {
                                    ...
                                    if (newSubID != 0)
                                    {
                                        ...
                                        {
                                            newSubDecay =
                                                Time::getCurrentTime() + newSubDecayT->autoDecaySeconds / subStretch;
                                        }
                                        else
                                        {
                                            // no further decay
                    ...
                    if (change)
                    {
                        ...
                        else
                        {
                            ...
                            nextPlayer->containedEtaDecays = newContainedETA.getElementArray();

                            nextPlayer->subContainedIDs = newSubContained.getElementArray();
                            nextPlayer->subContainedEtaDecays = newSubContainedETA.getElementArray();
                        }
                    }
                }
                ...
                for (int c = 0; c < NUM_CLOTHING_PIECES; c++)
                {
                        ...
                        if (t != NULL)
                        {
                            ...
                            if (newSlots < oldSlots)
                            {
                                ...
                                for (int s = newSlots; s < oldSlots; s++)
                                {
                                    ...
                                    if (found)
                                    {
                                        setMapObject(spot.x, spot.y,
                                                     nextPlayer->clothingContained[c].getElementDirect(s));

                                        timeSec_t eta = nextPlayer->clothingContainedEtaDecays[c].getElementDirect(s);
                            ...
                            if (oldStretch != newStretch)
                            {
                                ...
                                for (int cc = 0; cc < nextPlayer->clothingContainedEtaDecays[c].size(); cc++)
                                {
                    ...
                    if (cObj != NULL && nextPlayer->clothingContainedEtaDecays[c].size() > 0)
                    {
                            ...
                            if (decay != 0 && decay < curTime)
                            {
                                ...
                                if (t != NULL)
                                {
                                    ...
                                    if (newID != 0)
                                    {
                                        ...
                                        if (newDecayT != NULL)
                                        {
                                            newDecay = Time::getCurrentTime() +
                                                       newDecayT->autoDecaySeconds / cObj->slotTimeStretch;
                                        }
                                        else
                                        {
                                            // no further decay
                ...
                if (nextPlayer->xd != nextPlayer->xs || nextPlayer->yd != nextPlayer->ys)
                {
                    ...
                    if (!nextPlayer->newMove &&
                        Time::getCurrentTime() - nextPlayer->moveStartTime > nextPlayer->moveTotalSeconds)
                    {
                        ...
                        if (nextPlayer->holdingFlightObject && moveSpeed >= minFlightSpeed &&
                            !nextPlayer->pathTruncated && nextPlayer->pathLength >= 2)
                        {

                            // player takes off ?

                            double xDir = nextPlayer->pathToDest[nextPlayer->pathLength - 1].x -
                                          nextPlayer->pathToDest[nextPlayer->pathLength - 2].x;
                            double yDir = nextPlayer->pathToDest[nextPlayer->pathLength - 1].y -
                            ...
                            if (beyondEndFloorID != endFloorID)
                            {
                                ...
                                char foundMap = false;
                                if (Time::getCurrentTime() - nextPlayer->forceFlightDestSetTime < 30)
                                {
                                    // map fresh in memory

                                    destPos = getClosestLandingPos(nextPlayer->forceFlightDest, &foundMap);
                                    ...
                                    AppLog::infoF("Player %d flight taking off from (%d,%d), "
                                                  ...
                                                  nextPlayer->forceFlightDest.x, nextPlayer->forceFlightDest.y,
                                                  foundMap, destPos.x, destPos.y);
                                }
                                if (!foundMap)
                                {
                                    ...
                                    destPos = getNextFlightLandingPos(nextPlayer->xs, nextPlayer->ys, takeOffDir,
                                                                      &flightOutcomeFlag, radiusLimit);
                                    ...
                                    AppLog::infoF("Player %d non-map flight taking off "
                                                  ...
                                                  nextPlayer->id, nextPlayer->xs, nextPlayer->ys, xDir, yDir, destPos.x,
                                                  destPos.y);
                                }

                                sendGlobalMessage((char *)message.c_str(), nextPlayer);
                                ...
                                if (destID > 0 && getObject(destID)->isFlightLanding)
                                {
                                    ...
                                    if (tr != NULL)
                                    {
                                        ...
                                        transferHeldContainedToMap(nextPlayer, destPos.x, destPos.y);
                                ...
                                if (!heldTransHappened)
                                {
                                    ...
                                    // (flight uses fuel)
                                    TransRecord *decayTrans = getPTrans(-1, nextPlayer->holdingID);
                ...
                if (!nextPlayer->vogMode && curTime > nextPlayer->foodDecrementETASeconds)
                {
                    ...
                    nextPlayer->foodDecrementETASeconds = curTime + computeFoodDecrementTimeSeconds(nextPlayer);
                    ...
                    if (decrementedPlayer != NULL && decrementedPlayer->foodStore < 0)
                    {
                        ...
                        if (!decrementedPlayer->deathLogged && !decrementedPlayer->isTutorial)
                        {
                            ...
                            logDeath(decrementedPlayer->id, decrementedPlayer->email, decrementedPlayer->isEve,
                                     computeAge(decrementedPlayer), getSecondsPlayed(decrementedPlayer),
                                     !getFemale(decrementedPlayer), deathPos.x, deathPos.y, players.size() - 1, false,
                                     killerID, nextPlayer->murderPerpEmail);
                        }
                    ...
                    if (decrementedPlayer != NULL)
                    {
                        ...
                        if (computeAge(decrementedPlayer) > defaultActionAge)
                        {

                            double decTime = computeFoodDecrementTimeSeconds(decrementedPlayer);

                            int totalFood = decrementedPlayer->yummyBonusStore + decrementedPlayer->foodStore;
                            ...
                            if (totalTime < 20)
                            {
                                ...
                                {
                                    newEmotPlayerIDs.push_back(decrementedPlayer->id);

                                    newEmotIndices.push_back(starvingEmotionIndex);
        ...
        if (playerIndicesToSendUpdatesAbout.size() > 0)
        {
            ...
            {
                LiveObject *nextPlayer = players.getElement(playerIndicesToSendUpdatesAbout.getElementDirect(i));
            ...
            AppLog::detailF("Need to send updates about these %d players: %s", playerIndicesToSendUpdatesAbout.size(),
                            updateListString);
        ...
        if (currentTimeHeat - lastHeatUpdateTime >= heatUpdateTimeStep)
        {
            ...
            for (r = lastPlayerIndexHeatRecomputed + 1;
                 r < lastPlayerIndexHeatRecomputed + 1 + numPlayersRecomputeHeatPerStep && r < players.size(); r++)
            {
        ...
        for (int i = 0; i < players.size(); i++)
        {
            ...
            if (nextPlayer->error || currentTime - nextPlayer->lastHeatUpdate < heatUpdateSeconds)
            {
                continue;
            }
            ...
            if (abs(targetHeat - envHeatTarget) < abs(targetHeat - nextPlayer->bodyHeat))
            {
            ...
            float heatDeltaScaled = maxDelta - maxDelta / (deltaSlope * heatDeltaAbs + 1);
        ...
        for (int i = 0; i < playerIndicesToSendUpdatesAbout.size(); i++)
        {
            ...
            ChangePosition p = {nextPlayer->xs, nextPlayer->ys, nextPlayer->updateGlobal};
        ...
        if (newUpdates.size() > 0)
        {
            ...
            AppLog::detailF("Sending updates about these %d players: %s", newUpdatePlayerIDs.size(), updateListString);
            delete[] updateListString;
        }
        ...
        if (playerIndicesToSendLineageAbout.size() > 0)
        {
            ...
            {
                LiveObject *nextPlayer = players.getElement(playerIndicesToSendLineageAbout.getElementDirect(i));
            ...
            if (numAdded > 0)
            {
                ...
                else
                {
                    ...
                    lineageMessage =
                        makeCompressedMessage(lineageMessageText, lineageMessageLength, &lineageMessageLength);
        ...
        if (playerIndicesToSendCursesAbout.size() > 0)
        {
            ...
            {
                LiveObject *nextPlayer = players.getElement(playerIndicesToSendCursesAbout.getElementDirect(i));
                ...
                char *line = autoSprintf("%d %d\n", nextPlayer->id, nextPlayer->curseStatus.curseLevel);
            ...
            if (numAdded > 0)
            {
                ...
                {
                    // compress for all players once here
                    cursesMessage = makeCompressedMessage(cursesMessageText, cursesMessageLength, &cursesMessageLength);
        ...
        if (playerIndicesToSendNamesAbout.size() > 0)
        {
            ...
            {
                LiveObject *nextPlayer = players.getElement(playerIndicesToSendNamesAbout.getElementDirect(i));
                ...
                char *line = autoSprintf("%d %s\n", nextPlayer->id, nextPlayer->displayedName);
                numAdded++;
            ...
            if (numAdded > 0)
            {
                ...
                {
                    // compress for all players once here
                    namesMessage = makeCompressedMessage(namesMessageText, namesMessageLength, &namesMessageLength);
        ...
        if (playerIndicesToSendDyingAbout.size() > 0)
        {
            ...
            {
                LiveObject *nextPlayer = players.getElement(playerIndicesToSendDyingAbout.getElementDirect(i));
            ...
            if (numAdded > 0)
            {
                ...
                {
                    // compress for all players once here
                    dyingMessage = makeCompressedMessage(dyingMessageText, dyingMessageLength, &dyingMessageLength);
        ...
        if (playerIndicesToSendHealingAbout.size() > 0)
        {
            ...
            {
                LiveObject *nextPlayer = players.getElement(playerIndicesToSendHealingAbout.getElementDirect(i));
            ...
            if (numAdded > 0)
            {
                ...
                else
                {
                    // compress for all players once here
                    healingMessage =
                        makeCompressedMessage(healingMessageText, healingMessageLength, &healingMessageLength);
        ...
        if (newEmotPlayerIDs.size() > 0)
        {
            ...
            if (numAdded > 0)
            {
                ...
                {
                    // compress for all players once here
                    emotMessage = makeCompressedMessage(emotMessageText, emotMessageLength, &emotMessageLength);
        ...
        {
            newOwnerStrings.push_back(getOwnershipString(newOwnerPos.getElementDirect(u)));
        }
        ...
        for (int i = 0; i < numLive; i++)
        {
            ...
            if (nextPlayer->firstMapSent || nextPlayer->inFlight)
            {
                ...
                if (newFlightDest.size() > 0)
                {
                    ...
                    for (int u = 0; u < newFlightDest.size(); u++)
                    {
                        ...
                        char *flightMessage =
                            autoSprintf("FD\n%d %d %d\n#", f->playerID, f->destPos.x - nextPlayer->birthPos.x,
                                        f->destPos.y - nextPlayer->birthPos.y);

                        sendMessageToPlayer(nextPlayer, flightMessage, strlen(flightMessage));
            ...
            if (!nextPlayer->firstMessageSent)
            {
                ...
                char *valleyMessage = autoSprintf("VS\n"
                                                  "%d %d\n#",
                ...
                for (int i = 0; i < numPlayers; i++)
                {
                    ...
                    if (nextPlayer->inFlight || nextPlayer->vogMode || nextPlayer->postVogMode)
                    {
                    ...
                    char *messageLine = getUpdateLine(o, nextPlayer->birthPos, getPlayerPos(nextPlayer), false, true);
                ...
                if (outOfRangePlayerIDs.size() > 0)
                {
                    ...
                    sendMessageToPlayer(nextPlayer, outOfRangeMessageText, strlen(outOfRangeMessageText));

                    delete[] outOfRangeMessageText;
                }

                char *movesMessage = getMovesMessage(false, nextPlayer->birthPos, getPlayerPos(nextPlayer));
                ...
                for (int i = 0; i < numPlayers; i++)
                {
                    ...
                    if (level == 0)
                    {
                        ...
                        if (usePersonalCurses)
                        {
                            if (isCursed(nextPlayer->email, o->email))
                            {
                                level = 1;
                            }
                        }
                ...
                for (int i = 0; i < numPlayers; i++)
                {
                    ...
                    for (int e = 0; e < o->permanentEmots.size(); e++)
                    {
                        ...
                        char *line = autoSprintf("%d %d -2\n", o->id, o->permanentEmots.getElementDirect(e));
            ...
            else
            {
                ...
                if (nextPlayer->monumentPosSet && !nextPlayer->monumentPosSent && computeAge(nextPlayer) > 0.5)
                {
                    ...
                    char *monMessage =
                        autoSprintf("MN\n%d %d %d\n#", nextPlayer->lastMonumentPos.x - nextPlayer->birthPos.x,
                                    nextPlayer->lastMonumentPos.y - nextPlayer->birthPos.y, nextPlayer->lastMonumentID);
                ...
                if (newGraves.size() > 0)
                {
                    ...
                    for (int u = 0; u < newGraves.size(); u++)
                    {
                            ...
                            g->lineageEveID == nextPlayer->lineageEveID)
                        {
                            ...
                            char *graveMessage = autoSprintf("GV\n%d %d %d\n#", g->pos.x - nextPlayer->birthPos.x,
                                                             g->pos.y - nextPlayer->birthPos.y, g->playerID);

                            sendMessageToPlayer(nextPlayer, graveMessage, strlen(graveMessage));
                            delete[] graveMessage;
                        }
                    }
                }
                ...
                if (newGraveMoves.size() > 0)
                {
                    ...
                    for (int u = 0; u < newGraveMoves.size(); u++)
                    {
                        ...
                        if (distance(g->posStart, playerPos) < maxDist2 || distance(g->posEnd, playerPos) < maxDist2)
                        {

                            char *graveMessage = autoSprintf(
                                "GM\n%d %d %d %d %d\n#", g->posStart.x - nextPlayer->birthPos.x,
                                g->posStart.y - nextPlayer->birthPos.y, g->posEnd.x - nextPlayer->birthPos.x,
                ...
                if (newOwnerPos.size() > 0)
                {
                    ...
                    for (int u = 0; u < newOwnerPos.size(); u++)
                    {
                        ...
                        if (known || distance(p, nextPlayerPos) < maxDist2 || isOwned(nextPlayer, p))
                        {
                            ...
                            char *ownerMessage =
                                autoSprintf("OW\n%d %d%s\n#", p.x - nextPlayer->birthPos.x,
                                            p.y - nextPlayer->birthPos.y, newOwnerStrings.getElementDirect(u));

                            sendMessageToPlayer(nextPlayer, ownerMessage, strlen(ownerMessage));
                            delete[] ownerMessage;
                        }
                    }
                }
                ...
                if (abs(playerXD - nextPlayer->lastSentMapX) > 7 || abs(playerYD - nextPlayer->lastSentMapY) > 8 ||
                    !nextPlayer->firstMapSent)
                {
                    ...
                    for (int j = 0; j < numLive; j++)
                    {
                        ...
                        if (otherPlayer->heldByOther)
                        {
                            ...
                            if (adultO != NULL)
                            {
                                ...
                                if (adultO->id != nextPlayer->id && otherPlayer->id != nextPlayer->id)
                                {
                                    // parent not us
                                    // baby not us

                                    double d = intDist(playerXD, playerYD, adultO->xd, adultO->yd);
                                    ...
                                    if (d <= getMaxChunkDimension() / 2)
                                    {
                                        ...
                                        char *updateLine = getUpdateLine(otherPlayer, nextPlayer->birthPos,
                    ...
                    for (int j = 0; j < numLive; j++)
                    {
                        ...
                        if (!otherPlayer->heldByOther && otherPlayer->id != nextPlayer->id &&
                            otherPlayer->id != ourHolderID)
                        {
                            ...
                            double d = intDist(playerXD, playerYD, otherPlayer->xd, otherPlayer->yd);
                            ...
                            if (d <= getMaxChunkDimension() / 2)
                            {
                                ...
                                char *updateLine =
                                    getUpdateLine(otherPlayer, nextPlayer->birthPos, getPlayerPos(nextPlayer), false);
                ...
                {
                    int numSent = nextPlayer->sock->send(dyingMessage, dyingMessageLength, false, false);
                ...
                {
                    int numSent = nextPlayer->sock->send(healingMessage, healingMessageLength, false, false);
                ...
                {
                    int numSent = nextPlayer->sock->send(emotMessage, emotMessageLength, false, false);
                ...
                if (newUpdates.size() > 0 && nextPlayer->connected)
                {
                    ...
                    for (int u = 0; u < newUpdatesPos.size(); u++)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                middleDistancePlayerIDs.push_back(newUpdatePlayerIDs.getElementDirect(u));
                            }
                        }
                    }
                    ...
                    if (minUpdateDist <= maxDist)
                    {
                        ...
                        for (int u = 0; u < newUpdates.size(); u++)
                        {
                            ...
                            char *line = getUpdateLineFromRecord(newUpdates.getElement(u), nextPlayer->birthPos,
                                                                 getPlayerPos(nextPlayer));
                        ...
                        if (updateChars.size() > 0)
                        {
                            ...
                            {
                                updateMessage =
                                    makeCompressedMessage(updateMessageText, updateMessageLength, &updateMessageLength);
                        ...
                        if (updateMessage != NULL)
                        {
                            ...
                            int numSent = nextPlayer->sock->send(updateMessage, updateMessageLength, false, false);
                ...
                if (moveList.size() > 0 && nextPlayer->connected)
                {
                    ...
                    for (int u = 0; u < movesPos.size(); u++)
                    {
                        ...
                        if (d > maxDist && d <= maxDist2)
                        {
                            middleDistancePlayerIDs.push_back(moveList.getElement(u)->playerID);
                        }
                    }
                    ...
                    if (minUpdateDist <= maxDist)
                    {
                        ...
                        if (closeMoves.size() > 0)
                        {
                            ...
                            char *moveMessageText = getMovesMessageFromList(&closeMoves, nextPlayer->birthPos);
                            ...
                            if (moveMessageText != NULL)
                            {
                                ...
                                {
                                    moveMessage =
                                        makeCompressedMessage(moveMessageText, moveMessageLength, &moveMessageLength);
                                    delete[] moveMessageText;
                                }
                            }

                            int numSent = nextPlayer->sock->send(moveMessage, moveMessageLength, false, false);
                ...
                if (middleDistancePlayerIDs.size() > 0 && nextPlayer->connected)
                {
                    ...
                    if (middleDistancePlayerIDs.size() > 0)
                    {
                        ...
                        {
                            // compress
                            outOfRangeMessage = makeCompressedMessage(outOfRangeMessageText, outOfRangeMessageLength,
                    ...
                    int numSent = nextPlayer->sock->send(outOfRangeMessage, outOfRangeMessageLength, false, false);
                ...
                if (mapChanges.size() > 0 && nextPlayer->connected)
                {
                    ...
                    if (minUpdateDist <= maxDist)
                    {
                        ...
                        for (int u = 0; u < mapChanges.size(); u++)
                        {
                            ...
                            char *lineString =
                                getMapChangeLineString(r, nextPlayer->birthPos.x, nextPlayer->birthPos.y);
                        ...
                        if (mapChangeChars.size() > 0)
                        {
                            ...
                            {
                                mapChangeMessage = makeCompressedMessage(mapChangeMessageText, mapChangeMessageLength,
                        ...
                        if (mapChangeMessage != NULL)
                        {
                            ...
                            int numSent =
                                nextPlayer->sock->send(mapChangeMessage, mapChangeMessageLength, false, false);
                ...
                if (newSpeechPos.size() > 0 && nextPlayer->connected)
                {
                    ...
                    if (minUpdateDist <= maxDist)
                    {
                        ...
                        for (int u = 0; u < newSpeechPos.size(); u++)
                        {
                            ...
                            if (p->responsiblePlayerID != -1 && p->responsiblePlayerID != nextPlayer->id)
                                continue;
                            ...
                            if (d < maxDist)
                            {
                                ...
                                char *trimmedPhrase = stringDuplicate(newSpeechPhrases.getElementDirect(u));
                                ...
                                if (starLoc != NULL)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        int numRead = sscanf(starLoc, " *map %d %d %lf", &mapX, &mapY, &mapT);
                                        if (numRead == 2 || numRead == 3)
                                        {
                                            ...
                                            char *newTrimmed = autoSprintf("%s *map %d %d %.f", trimmedPhrase,
                                                                           mapX - nextPlayer->birthPos.x,
                                                                           mapY - nextPlayer->birthPos.y, age);
                                            ...
                                            if (speakerObj != NULL)
                                            {
                                                ...
                                                speakerObj->forceFlightDestSetTime = Time::getCurrentTime();
                                            }
                                        }
                                    }
                                }
                                    ...
                                    newSpeechPhrases.getElementDirect(u)[0] == '+')
                                {
                                    ...
                                    translatedPhrase = stringDuplicate(trimmedPhrase);
                                }
                                else
                                {
                                    ...
                                    translatedPhrase = mapLanguagePhrase(trimmedPhrase, speakerEveID, listenerEveID,
                                                                         speakerID, listenerID, speakerAge, listenerAge,
                                                                         speakerParentID, listenerParentID);
                                }

                                if (speakerEveID != listenerEveID && speakerAge > 55 && listenerAge > 55)
                                {
                                ...
                                if (translatedPhrase[0] != '+' && translatedPhrase[0] != '[')
                                {
                                    if (speakerObj != NULL && speakerObj->drunkenness > 0)
                                    {
                                        ...
                                        char *slurredPhrase =
                                            slurSpeech(speakerObj->id, translatedPhrase, speakerObj->drunkenness);

                                        delete[] translatedPhrase;
                                        translatedPhrase = slurredPhrase;
                                    }

                                    if (speakerObj != NULL && speakerObj->tripping)
                                    {
                                        // player is high on drugs and yelling

                                        char *processedPhrase = yellingSpeech(speakerObj->id, translatedPhrase);
                                ...
                                char *line = autoSprintf("%d/%d %s\n", speakerID, curseFlag, translatedPhrase);
                                delete[] translatedPhrase;
                                delete[] trimmedPhrase;
                        ...
                        int numSent = nextPlayer->sock->send(message, messageLen, false, false);
                ...
                if (newLocationSpeech.size() > 0 && nextPlayer->connected)
                {
                    ...
                    if (minUpdateDist <= maxDist)
                    {
                        ...
                        for (int u = 0; u < newLocationSpeechPos.size(); u++)
                        {
                            ...
                            char *line =
                                autoSprintf("%d %d %s\n", p->x - nextPlayer->birthPos.x, p->y - nextPlayer->birthPos.y,
                        ...
                        if (len > maxUncompressedSize)
                        {
                            ...
                            unsigned char *compMessage = makeCompressedMessage(message, len, &compLen);
                        ...
                        int numSent = nextPlayer->sock->send((unsigned char *)message, len, false, false);
                ...
                if (nextPlayer->connected)
                {
                    ...
                    for (int u = 0; u < newDeleteUpdates.size(); u++)
                    {
                        ...
                        char *line = getUpdateLineFromRecord(newDeleteUpdates.getElement(u), nextPlayer->birthPos,
                                                             getPlayerPos(nextPlayer));
                    ...
                    if (deleteUpdateChars.size() > 0)
                    {
                        ...
                        else
                        {
                            ...
                            deleteUpdateMessage = makeCompressedMessage(
                                deleteUpdateMessageText, deleteUpdateMessageLength, &deleteUpdateMessageLength);
                    ...
                    {
                        int numSent =
                            nextPlayer->sock->send(deleteUpdateMessage, deleteUpdateMessageLength, false, false);
                ...
                {
                    int numSent = nextPlayer->sock->send(lineageMessage, lineageMessageLength, false, false);
                ...
                {
                    int numSent = nextPlayer->sock->send(cursesMessage, cursesMessageLength, false, false);
                ...
                {
                    int numSent = nextPlayer->sock->send(namesMessage, namesMessageLength, false, false);
                ...
                if (nextPlayer->foodUpdate)
                {
                    ...
                    if (cap > nextPlayer->lastReportedFoodCapacity)
                    {
                        ...
                        while (nextPlayer->yummyBonusStore > 0 && extraCap > 0 && nextPlayer->foodStore < cap)
                        {
                            nextPlayer->foodStore++;
                            extraCap--;
                            nextPlayer->yummyBonusStore--;
                    ...
                    if (nextPlayer->connected)
                    {
                        ...
                        char *foodMessage =
                            autoSprintf("FX\n"
                                        ...
                                        "%d %d\n"
                                        "#",
                                        nextPlayer->foodStore, cap, hideIDForClient(nextPlayer->lastAteID),
                                        nextPlayer->lastAteFillMax, computeMoveSpeed(nextPlayer),
                                        nextPlayer->responsiblePlayerID, nextPlayer->yummyBonusStore, yumMult);

                        int messageLength = strlen(foodMessage);

                        int numSent = nextPlayer->sock->send((unsigned char *)foodMessage, messageLength, false, false);
                ...
                if (nextPlayer->heatUpdate && nextPlayer->connected)
                {
                    ...
                    char *heatMessage = autoSprintf("HX\n"
                                                    "%.2f#",
                                                    nextPlayer->heat);

                    int messageLength = strlen(heatMessage);

                    int numSent = nextPlayer->sock->send((unsigned char *)heatMessage, messageLength, false, false);
                ...
                if (nextPlayer->curseTokenUpdate && nextPlayer->connected)
                {
                    ...
                    int numSent = nextPlayer->sock->send((unsigned char *)tokenMessage, messageLength, false, false);
        ...
        if (newUpdates.size() > 0)
        {
            ...
            {
                char *playerString = autoSprintf("%d, ", playersReceivingPlayerUpdate.getElementDirect(i));
            ...
            AppLog::detailF("%d/%d players were sent part of a %d-line PU: %s", playersReceivingPlayerUpdate.size(),
                            numLive, newUpdates.size(), playerListString);
        ...
        for (int i = 0; i < players.size(); i++)
        {
            ...
            {
                int numSent = nextPlayer->sock->send((unsigned char *)frameMessage, frameMessageLength, false, false);
        ...
        for (int i = 0; i < players.size(); i++)
        {
            ...
            if (nextPlayer->error && nextPlayer->deleteSent && nextPlayer->deleteSentDoneETA < Time::getCurrentTime())
            {
                AppLog::infoF("Closing connection to player %d on error "
                              "(cause: %s)",
                              nextPlayer->id, nextPlayer->errorCauseString);

                AppLog::infoF("%d remaining player(s) alive on server ", players.size() - 1);
