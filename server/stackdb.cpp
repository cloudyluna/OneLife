                 ...
                 unsigned int inValueSize)
{
    ...
    unsigned int tableSizeBytes = (inKeySize + sizeof(uint64_t)) * inHashTableSize;
...
//          0 if found
//          1 if not found
static int findValue(STACKDB *inDB, const void *inKey, char inRecordMiss, void *outValue, char inNewForSure)
{

    uint64_t hash = STACKDB_hash(inKey, inDB->keySize) % (uint64_t)inDB->hashTableSize;

    inDB->lastHashBinLoc = STACKDB_HEADER_SIZE + hash * inDB->hashBinSize;
    ...
    if (keyComp(inDB->keySize, inDB->hashBinBuffer, inKey) && !isKeyZero(inDB->keySize, inKey))
    {
    ...
    if (stackPos != 0)
    {
        ...
        if (lastRecordPointerLoc64 != 0)
        {
            ...
            numWritten = fwrite(&nextRecordStart64, sizeof(uint64_t), 1, inDB->file);
            if (numWritten != 1)
            {
                return -1;
            }
        ...
        numWritten = fwrite(&thisRecordStart64, sizeof(uint64_t), 1, inDB->file);
        if (numWritten != 1)
        {
            return -1;
        }
...
static int STACKDB_put_internal(STACKDB *inDB, const void *inKey, const void *inValue, char inNew)
{
    ...
    else if (result == 1)
    {
        ...
        if (inDB->lastWasQuickMiss)
        {
            ...
            int numWritten = fwrite(inDB->hashBinBuffer, inDB->keySize, 1, inDB->file);
...
int STACKDB_Iterator_next(STACKDB_Iterator *inDBi, void *outKey, void *outValue)
{
    ...
    if (inDBi->nextRecordLoc == 0)
    {
        ...
        while (!foundFullBin && inDBi->hashBin < inDBi->db->hashTableSize)
        {
            ...
            fseeko(f, STACKDB_HEADER_SIZE + inDBi->hashBin * binSize + keySize, SEEK_SET);
