...
static LiveDummySocket newDummyPlayer(const char *inEmail, int inDisplayID, double inStartAge, GridPos inStartPos,
                                      int inHoldingID, ClothingSet inClothing)
{
    PlayerMapping m = {inEmail, inDisplayID, inStartAge, inStartPos, inHoldingID, inClothing};
    ...
    s.sock->send((unsigned char *)message, messageLength, false, false);
...
void sendDummyMove(LiveDummySocket *inDummy, SimpleVector<GridPos> *inOffsets)
{

    char *message = autoSprintf("MOVE %d %d ", inDummy->pos.x, inDummy->pos.y);
    ...
    for (int i = 0; i < inOffsets->size(); i++)
    {
        ...
        char *newPart = autoSprintf("%d %d ", inOffsets->getElementDirect(i).x, inOffsets->getElementDirect(i).y);
    ...
    if (delayTime == 0)
    {
        inDummy->sock->send((unsigned char *)message, strlen(message), false, false);

        delete[] message;
    }
    else
    {
        DelayedMessage m = {Time::getCurrentTime() + delayTime, inDummy->sock, message};
...
void sendDummyAction(LiveDummySocket *inDummy, const char *inAction, GridPos inOffset, char inUseExtraA = false,
                     int inExtraA = -1, char inUseExtraB = false, int inExtraB = -1)
{
    char *message = autoSprintf("%s %d %d", inAction, inDummy->pos.x + inOffset.x, inDummy->pos.y + inOffset.y);
...
void sendDummySay(LiveDummySocket *inDummy, const char *inSay)
{
...
void initTriggers()
{
    triggersEnabled = SettingsManager::getIntSetting("allowTriggerRequests", 0);
...
void stepTriggers()
{
    ...
    for (int i = 0; i < delayedMessages.size(); i++)
    {
        ...
        {
            m->sock->send((unsigned char *)m->message, strlen(m->message), false, false);
