...
void initLineageLog()
{
    ...
    useLineageServer =
        SettingsManager::getIntSetting("useLineageServer", 0) && SettingsManager::getIntSetting("remoteReport", 0);

    lineageServerURL =
        SettingsManager::getStringSetting("lineageServerURL", "http://localhost/jcr13/reviewServer/server.php");
...
void recordPlayerLineage(char *inEmail, double inAge, int inPlayerID, int inParentID, int inDisplayID, int inKillerID,
                         const char *inName, const char *inLastSay, char inMale)
{
    ...
    if (useLineageServer)
    {
        ...
        LineageRecord r = {stringDuplicate(inEmail),
                           inAge,
                           inPlayerID,
                           inParentID,
                           inDisplayID,
    ...
    for (int i = 0; i < records.size(); i++)
    {
        ...
        else if (result == 1)
        {
            ...
            if (r->sequenceNumber == -1)
            {
                ...
                else
                {
                    ...
                    char *lineageServerSharedSecret =
                        SettingsManager::getStringSetting("lineageServerSharedSecret", "secret_phrase");

                    char *hash = hmac_sha1(lineageServerSharedSecret, seqString);
                    ...
                    char *url = autoSprintf("%s?action=log_life"
                                            "&server=%s"
                                            "&email=%s"
                                            "&age=%f"
                                            "&player_id=%d"
                                            ...
                                            "&sequence_number=%d"
                                            "&hash_value=%s",
                                            lineageServerURL, serverID, encodedEmail, r->age, r->playerID, r->parentID,
                                            r->displayID, r->killerID, encodedName, encodedLastSay, maleInt,
                                            r->sequenceNumber, hash);
            ...
            else
            {
                ...
                {
                    AppLog::info("Server log_life request rejected by lineage server");
                }
                recordDone = true;
            }
