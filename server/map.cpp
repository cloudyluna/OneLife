...
// from map
// indexed as x, y, 0, 0
static HashTable<ContRecord> liveDecayRecordLastLookTimeMaxContainedHashTable(1024, defaultContRecord);
...
static int biomeDBGet(int inX, int inY, int *outSecondPlaceBiome = NULL, double *outSecondPlaceGap = NULL)
{
    unsigned char key[8];
...
static void biomePutCached(int inX, int inY, int inBiome, int inSecondPlace, double inSecondPlaceGap)
{
...
static int computeMapBiomeIndex(int inX, int inY, int *outSecondPlaceIndex = NULL, double *outSecondPlaceGap = NULL)
{
    ...
    double randVal2 = (getXYFractal2(inX, inY, roughness, scale));
    ...
    if (abs(randVal2 - boundaryThreshold) < width)
    {
        ...
        randVal2 = (getXYFractal2(inX + xOffset, inY + yOffset, roughness, maskScale));
        ...
        if (randVal2 > maskThreshold)
        {
            ...
            double randVal3 = (getXYFractal2(inX, inY, roughness, scale));
            if (abs(randVal3 - boundaryThreshold) < width / 2)
                pickedBiome = 1; // ShallowRiver
...
static int getMapBiomeIndex(int inX, int inY, int *outSecondPlaceIndex = NULL, double *outSecondPlaceGap = NULL)
{
...
void outputMapImage()
{
    ...
    */
    ...
    const char *biomeNames[] = {"Grass ", "Swamp ", "Yellow",    "Gray  ", "Snow  ",
                                "Desert", "Jungle", "DeepWater", "Flower", "ShallowWater"};
    ...
    for (int i = 0; i < allNaturalMapIDs.size(); i++)
    {
        ...
        printf("%d\t%-30s  (actual=%f, expected=%f\n", objCounts.getElementDirect(i), obj->description,
               count / (double)totalCounts, obj->mapChance / allNaturalMapIDs.size());
    }
...
void writeRecentPlacements()
{
    ...
    if (placeFile != NULL)
    {
        ...
        {
            fprintf(placeFile, "%d,%d %d\n", recentPlacements[i].pos.x, recentPlacements[i].pos.y,
...
static int computeBLCacheHash(int inKeyA, int inKeyB)
{
    ...
    int hashKey = (inKeyA * CACHE_PRIME_A + inKeyB * CACHE_PRIME_B) % DB_CACHE_SIZE;
    if (hashKey < 0)
    {
        hashKey += DB_CACHE_SIZE;
    }
...
static int dbGetCached(int inX, int inY, int inSlot, int inSubCont)
{
    ...
    if (r.x == inX && r.y == inY && r.slot == inSlot && r.subCont == inSubCont && r.value != -2)
    {
        return r.value;
    }
    else
...
static double dbTimeGetCached(int inX, int inY, int inSlot, int inSubCont)
{
    ...
    if (r.x == inX && r.y == inY && r.slot == inSlot && r.subCont == inSubCont && r.timeVal != 1)
    {
        return r.timeVal;
    }
    else
...
static void dbTimePutCached(int inX, int inY, int inSlot, int inSubCont, timeSec_t inValue)
{
...
int cleanMap()
{
    ...
    AppLog::infoF("...%d contained objects present, and %d needed to be cleared.", totalNumContained,
                  numContainedCleared);
...
static char loadIntoMapFromFile(FILE *inFile, int inOffsetX = 0, int inOffsetY = 0, double inTimeLimitSec = 0)
{
    ...
    while (inTimeLimitSec == 0 || Time::getCurrentTime() < startTime + inTimeLimitSec)
    {
        ...
        // read ints first
        int numRead = fscanf(inFile, "%d %d %d %d", &(r.x), &(r.y), &(r.biome), &(r.floor));
...
{
    if (inX <= inPos.x + 1 && inX >= inPos.x - 1 && inY <= inPos.y + 1 && inY >= inPos.y - 1)
    {
        return true;
    }
    return false;
...
static void setupMapChangeLogFile()
{
    ...
    if (logFolder.isDirectory())
    {
        ...
        char *biomeSeedString = autoSprintf("%u_%u", biomeRandSeedA, biomeRandSeedB);
        ...
        if (mapChangeLogFile == NULL)
        {
            ...
            // file does not exist
            char *newFileName = autoSprintf("%.ftime_mapLog.txt", Time::getCurrentTime());
...
void reseedMap(char inForceFresh)
{
    ...
    if (!set)
    {
        ...
        char *secretA = SettingsManager::getStringSetting("statsServerSharedSecret", "secret");
        ...
        unsigned int modTimeSeedA = (unsigned int)fmod(Time::getCurrentTime() + seedBaseA, 4294967295U);

        JenkinsRandomSource tempRandSourceA(modTimeSeedA);

        biomeRandSeedA = tempRandSourceA.getRandomInt();

        unsigned int modTimeSeedB = (unsigned int)fmod(Time::getCurrentTime() + seedBaseB, 4294967295U);
            ...
            if (p > 0)
            {
                ...
                if (randPlacementLoc != NULL)
                {
                    ...
                    for (int p = 0; p < count; p++)
                    {
                        ...
                        while (!placed)
                        {
                            int pickX = placementRandSource.getRandomBoundedInt(-safeR, safeR);
                            int pickY = placementRandSource.getRandomBoundedInt(-safeR, safeR);
    ...
    if (!set && mapChangeLogFile != NULL)
    {
        ...
        char *newFileName = autoSprintf("%.ftime_mapSeed.txt", Time::getCurrentTime());
...
char initMap()
{
    ...
    if (placeFile != NULL)
    {
        for (int i = 0; i < NUM_RECENT_PLACEMENTS; i++)
        {
            fscanf(placeFile, "%d,%d %d", &(recentPlacements[i].pos.x), &(recentPlacements[i].pos.y),
                   &(recentPlacements[i].depth));
        }
    ...
    if (!skipLookTimeCleanup)
    {
        ...
        int error = DB_open(&lookTimeDB_old, lookTimeDBName, KISSDB_OPEN_MODE_RWCREAT, 80000,
        ...
        if (lookTimeDBExists && staleSec > 0)
        {
            ...
            error = DB_open(&lookTimeDB_temp, lookTimeDBName_temp, KISSDB_OPEN_MODE_RWCREAT, newSize,
                            8, // two 32-bit ints, xy
                            8  // one 64-bit double, representing an ETA time
                               // in whatever binary format and byte order
                               // "double" on the server platform uses
    ...
    int error = DB_open(&lookTimeDB, lookTimeDBName, KISSDB_OPEN_MODE_RWCREAT, 80000,
                        8, // two 32-bit ints, xy
                        8  // one 64-bit double, representing an ETA time
                           // in whatever binary format and byte order
                           // "double" on the server platform uses
    ...
    for (int i = 0; i < numObjects; i++)
    {
        ...
        if (p > 0)
        {
            ...
            if (gridPlacementLoc != NULL)
            {
                ...
                MapGridPlacement gp = {id, spacing, 0,
                                       // phaseRandSource.getRandomBoundedInt( 0,
                                       //                                      spacing - 1 ),
                                       wiggleScale, permittedBiomes};
    ...
    for (int j = 0; j < numBiomes; j++)
    {
        AppLog::infoF("Biome %d:  Found %d natural objects with total weight %f", biomes[j], naturalMapIDs[j].size(),
                      totalChanceWeight[j]);
    }
    ...
    // in mapTime.db
    error = DB_open_modeSwitch(&db, "map.db", KISSDB_OPEN_MODE_RWCREAT, 80000,
    ...
    error = DB_open_modeSwitch(&timeDB, "mapTime.db", KISSDB_OPEN_MODE_RWCREAT, 80000,
                               16, // four 32-bit ints, xysb
                               // s is the slot number
                               // s=0 for base object
                               // s=1 decay ETA seconds (wall clock time)
    ...
    error = DB_open_modeSwitch(&biomeDB, "biome.db", KISSDB_OPEN_MODE_RWCREAT, 80000,
                               8, // two 32-bit ints, xy
                               12 // three ints,
    ...
    error = DB_open_modeSwitch(&floorDB, "floor.db", KISSDB_OPEN_MODE_RWCREAT, 80000,
                               8, // two 32-bit ints, xy
    ...
    error = DB_open_modeSwitch(&floorTimeDB, "floorTime.db", KISSDB_OPEN_MODE_RWCREAT, 80000,
                               8, // two 32-bit ints, xy
                               8  // one 64-bit double, representing an ETA time
                                  // in whatever binary format and byte order
                                  // "double" on the server platform uses
    ...
    if (false)
    { // old way to determine nextMetadataID
    ...
    error = DB_open_modeSwitch(&persistentMapDB, "persistentMap.db", KISSDB_OPEN_MODE_RWCREAT, 80000,
...
static void trackETA(int inX, int inY, int inSlot, timeSec_t inETA, int inSubCont = 0,
...
static int getContainerDecaySlot(int inX, int inY, int inSlot, int inSubCont = 0, int inNumContained = -1)
{
    if (inNumContained == -1)
    {
...
int checkDecayObject(int inX, int inY, int inID)
{
        ...
        if (mapETA <= MAP_TIMESEC)
        {
            ...
            if (t->move != 0 && t->desiredMoveDist > 0)
            {
                ...
                else if (t->move > 3 && t->move < 8)
                {
                    ...
                    if (desiredMoveDist == 1)
                    {
                        ...
                        for (int i = 0; i < numMoving; i++)
                        {
                            ...
                            if (m->sourceX == destPosX && m->sourceY == destPosY)
                            {
                ...
                if (destTrans != NULL)
                {
                    ...
                    for (int i = 0; i < numMoving; i++)
                    {
                        ...
                        if (m->sourceX == newX && m->sourceY == newY)
                        {
                            // found something leaving where we're landing
                            // wait for it to finish
                            setEtaDecay(inX, inY, MAP_TIMESEC + 1, t);
                ...
                if (newX == inX && newY == inY && (t->move <= 3 || t->move == 8))
                {
                ...
                if (newX != inX || newY != inY)
                {
                    ...
                    if (destTrans != NULL)
                    {
                        ...
                        if (leftDecayT != NULL)
                        {
                            ...
                            double tweakedSeconds = randSource.getRandomBoundedDouble(
                                leftDecayT->autoDecaySeconds * 0.9, leftDecayT->autoDecaySeconds);
                    ...
                    MovementRecord moveRec = {newX, newY, inX, inY, newID, deadly, etaTime, moveTime};
                    ...
                    oldSlots == newSlots)
                {
                    ...
                    if (numContained > 0)
                    {
                        ...
                        for (int i = 0; i < numContained; i++)
                        {
                            ...
                            int contained = getContained(inX, inY, i);
                            ...
                            if (containmentTrans == NULL)
                            {
                                ...
                                if (containmentTrans == NULL)
                                    containmentTrans = getPTrans(0, newContainerID, false, false, 4);
                                if (containmentTrans != NULL)
            ...
            if (newDecayT != NULL)
            {
                ...
                double tweakedSeconds =
                    randSource.getRandomBoundedDouble(newDecayT->autoDecaySeconds * 0.9, newDecayT->autoDecaySeconds);
    ...
    else
    {
        ...
        double decayTime = randSource.getRandomBoundedDouble(t->autoDecaySeconds / 2, t->autoDecaySeconds);
            ...
            if (mapETA <= MAP_TIMESEC)
            {
                ...
                if (newID != 0)
                {
                    ...
                    if (newDecayT != NULL)
                    {
                        ...
                        // add some random variation to avoid lock-step
                        // especially after a server restart
                        double tweakedSeconds = randSource.getRandomBoundedDouble(newDecayT->autoDecaySeconds * 0.9,
                ...
                if (change)
                {
                    ...
                    if (i == 0)
                    {
                        ...
                        if (contTrans == NULL)
                            contTrans = getPTrans(newContainer, -1, false, false, 2);
                    }
                    else if (i == numSlots - 1)
                    {
                        contTrans = getPTrans(newContainer, oldID, false, false, 1);
                        if (contTrans == NULL)
                            contTrans = getPTrans(newContainer, -1, false, false, 1);
                    }
                    ...
                    if (contTrans == NULL)
                    {
                        ...
                        if (contTrans == NULL)
                            contTrans = getPTrans(newContainer, -1, false, false, 3);
                    }
                    ...
                    if (contTrans == NULL)
                    {
                        ...
                        if (contTrans == NULL)
                            contTrans = getPTrans(newContainer, -1, false, false, 4);
                    }
                    ...
                    if (contTrans != NULL)
                    {
                        ...
                        int contID = getContained(inX, inY, slotNumber);
                        ...
                        while (slotNumber >= 0 && containmentPermitted(contTrans->newActor, contID))
                        {
                            ...
                            contID = getContained(inX, inY, slotNumber);
                    ...
                    if (contTrans != NULL)
                    {
                        ...
                        if (instantDecay != NULL && instantDecay->autoDecaySeconds == 1)
                            newContainer = instantDecay->newTarget;

                        if (newContainer != containerID)
                        {
                            ObjectRecord *newC = getObject(newContainer);
                            if (newC != NULL)
                                numSlots = newC->numSlots;
                        }
                    }
                    ...
                    if (i == 0)
                    {
                        ...
                        if (contTrans == NULL)
                            contTrans = getPTrans(0, newContainer, false, false, 1);
                    }
                    else if (i == numSlots - 1)
                    {
                        contTrans = getPTrans(newID, newContainer, false, false, 2);
                        if (contTrans == NULL)
                            contTrans = getPTrans(0, newContainer, false, false, 2);
                    }
                    ...
                    if (contTrans == NULL)
                    {
                        ...
                        if (contTrans == NULL)
                            contTrans = getPTrans(0, newContainer, false, false, 3);
                    }
                    ...
                    if (contTrans == NULL)
                    {
                        ...
                        if (contTrans == NULL)
                            contTrans = getPTrans(0, newContainer, false, false, 4);
                    }
...
// around x,y
unsigned char *getChunkMessage(int inStartX, int inStartY, int inWidth, int inHeight, GridPos inRelativeToPos,
                               int *outMessageLength)
{
    ...
    char *header = autoSprintf("MC\n%d %d %d %d\n%d %d\n#", inWidth, inHeight, inStartX - inRelativeToPos.x,
                               inStartY - inRelativeToPos.y, chunkDataBuffer.size(), compressedSize);
...
static void runTapoutOperation(int inX, int inY, TapoutRecord *inR, int inTriggerID)
{
    ...
    if (inR->tapoutMode == 0 && inR->tapoutCountLimit != -1)
    {
        ...
        for (int y = inY - inRadiusS; y <= inY + inRadiusN; y++)
        {

            for (int x = inX - inRadiusW; x <= inX + inRadiusE; x++)
            {
                ...
                TransRecord *t = getPTrans(inTriggerID, id);
                if (t != NULL && id == t->target)
                    totalGridCells++;
                TransRecord *t_floor = getPTrans(inTriggerID, id_floor);
                if (t_floor != NULL && id_floor == t_floor->target)
                    totalGridCells++;
            }
        }
    }
                ...
                for (int p = 0; p < 2; p++)
                {
                    ...
                    if (t != NULL)
                    {
                        ...
                        if (inR->tapoutCountLimit != -1)
                        {
                            ...
                            double P =
                                (double)(inR->tapoutCountLimit - tapoutCount) / (totalGridCells - currentGridCellIndex);
    ...
    else if (inR->tapoutMode == 2)
    {
        ...
        int i = 1; // skip center
        int maxRadius = 0;
        if (maxRadius < inR->radiusN)
            maxRadius = inR->radiusN;
        if (maxRadius < inR->radiusE)
            maxRadius = inR->radiusE;
        if (maxRadius < inR->radiusS)
            maxRadius = inR->radiusS;
        if (maxRadius < inR->radiusW)
            maxRadius = inR->radiusW;
...
void setMapObjectRaw(int inX, int inY, int inID)
{
    ...
    else if (o->isTapOutTrigger)
    {
        ...
        if (r != NULL)
        {
            ...
            runTapoutOperation(inX, inY, r, inID);
        }
...
static void logMapChange(int inX, int inY, int inID)
{
    ...
    if (mapChangeLogFile != NULL)
    {
        ...
        {
            fprintf(mapChangeLogFile, "%.2f %d %d %s%du%d %d\n", timeDelta, inX, inY, extraFlag, o->useDummyParent,
                    o->thisUseDummyIndex, respPlayer);
        }
        else if (o != NULL && o->isVariableDummy)
        {
            fprintf(mapChangeLogFile, "%.2f %d %d %s%dv%d %d\n", timeDelta, inX, inY, extraFlag, o->variableDummyParent,
                    o->thisVariableDummyIndex, respPlayer);
        }
        else
        {
            fprintf(mapChangeLogFile, "%.2f %d %d %s%d %d\n", timeDelta, inX, inY, extraFlag, inID, respPlayer);
        }
    }
}
...
void setContainedEtaDecay(int inX, int inY, int inNumContained, timeSec_t *inContainedEtaDecay, int inSubCont)
{
    ...
    for (int i = 0; i < inNumContained; i++)
    {
        dbTimePut(inX, inY, getContainerDecaySlot(inX, inY, i, inSubCont, inNumContained), inContainedEtaDecay[i],
                  inSubCont);
...
char *getMapChangeLineString(MapChangeRecord *inRecord, int inRelativeToX, int inRelativeToY)
{
    ...
    {
        lineString = autoSprintf(inRecord->formatString, inRecord->absoluteX - inRelativeToX,
                                 inRecord->absoluteY - inRelativeToY, inRecord->absoluteOldX - inRelativeToX,
...
void setFloorEtaDecay(int inX, int inY, timeSec_t inAbsoluteTimeInSeconds, TransRecord *inApplicableTrans)
{
...
void getEvePosition(const char *inEmail, int inID, int *outX, int *outY, SimpleVector<GridPos> *inOtherPeoplePos,
                    char inAllowRespawn)
{
...
GridPos getNextFlightLandingPos(int inCurrentX, int inCurrentY, doublePair inDir, int *outFlightOutcomeFlag,
                                int inRadiusLimit)
{
    int closestIndex = -1;
    GridPos closestPos;
...
void stepMapLongTermCulling(int inNumCurrentPlayers)
{
    ...
    if (!longTermCullEnabled || minActivePlayersForLongTermCulling > inNumCurrentPlayers)
    {
        return;
    }
...
int getDeadlyMovingMapObject(int inPosX, int inPosY, int *outMovingDestX, int *outMovingDestY)
{
    ...
    for (int i = 0; i < numMoving; i++)
    {
        ...
        double progress = (m->totalTime - (m->etaTime - curTime)) / m->totalTime;
        ...
        if (curPosX != inPosX || curPosY != inPosY)
        {
            continue;
        }
