...
const int allClusterSizes[NUM_CLUSTER_SETS] = {NUM_STARTING_CONSONANT_CLUSTERS, NUM_ENDING_CONSONANT_CLUSTERS,
                                               NUM_STARTING_VOWEL_CLUSTERS, NUM_VOWEL_CLUSTERS,
                                               NUM_MIDDLE_CONSONANT_CLUSTERS};

const char **allClusters[NUM_CLUSTER_SETS] = {startingConsonantClusters, endingConsonantClusters, startingVowelClusters,
                                              vowelClusters, middleConsonantClusters};

const int *allClustersFreq[NUM_CLUSTER_SETS] = {startingConsonantClustersFreq, endingConsonantClustersFreq,
                                                startingVowelClustersFreq, vowelClustersFreq,
...
static void closeFreqMirrorShuffle(SimpleVector<int> *inIndexList, const int *inFreq, int inCloseWindow)
{
    ...
    for (int p = 0; p < numPairs; p++)
    {
        ...
        while (closeBLoc.size() < thisWindSize)
        {
            ...
            for (int m = numSpotsLeft - 1; m >= 0; m--)
            {
                ...
                if (dist < minDist && (dist > lastClosestSeenDist || (dist == lastClosestSeenDist &&
        ...
        if (closeBInd.size() > 0)
        {
            ...
            int closePick = randSource.getRandomBoundedInt(0, closeBInd.size() - 1);
            ...
            *(inIndexList->getElement(indA)) = inIndexList->getElementDirect(indB);
...
static char *findInitialCluster(char *inWord, int inNumSourceClusters, const char *inSourceClusters[],
                                int *outClustIndex)
{
...
static const char *clusterMap(const char **inSourceClusters[NUM_CLUSTER_SETS],
                              const char **inDestClusters[NUM_CLUSTER_SETS], int inSetIndex, int inClusterIndex,
                              LanguageLearningMap *inLearnA, LanguageLearningMap *inLearnB, char inCanLearnB)
{

    if (inLearnA->allMappings[inSetIndex][inClusterIndex] || inLearnB->allMappings[inSetIndex][inClusterIndex])
    {
    ...
    {
        // map it
        const char *returnVal = inDestClusters[inSetIndex][inClusterIndex];

        if (inCanLearnB && randSource.getRandomDouble() > 0.5)
        {
            // coin flipped heads
            ...
            {
                if (strcmp(inSourceClusters[inSetIndex][c], returnVal) == 0)
                {
...
// used
static char *remapWordNew(char *inWord, const char **inSourceClusters[NUM_CLUSTER_SETS],
                          const char **inDestClusters[NUM_CLUSTER_SETS], LanguageLearningMap *inLearnA,
                          LanguageLearningMap *inLearnB, char inCanLearnB)
{
    ...
    {
        wordWorking =
            findInitialCluster(wordWorking, allClusterSizes[START_I], inSourceClusters[START_I], &startClusterIndex);
    }
    ...
    if (startClusterIndex == -1)
    {
        ...
        // reverse mapping)
        wordWorking = findInitialCluster(wordWorking, allClusterSizes[START_VOWEL_I], inSourceClusters[START_VOWEL_I],
    ...
    for (int c = 0; c < allClusterSizes[END_I]; c++)
    {
        ...
        if (loc != NULL && strlen(clust) == (unsigned int)((remainLen - (loc - wordWorking))))
        {
    ...
    while (strlen(wordWorking) > 0)
    {
        ...
        wordWorking = findInitialCluster(wordWorking, allClusterSizes[setIndex], inSourceClusters[setIndex], &midIndex);
    ...
    if (startClusterIndex != -1)
    {
        mappedWord.appendElementString(
            clusterMap(inSourceClusters, inDestClusters, START_I, startClusterIndex, inLearnA, inLearnB, inCanLearnB));
        // mappedWord.appendElementString(
        //     inDestClusters[ START_I ][ startClusterIndex ] );
    }
    if (startVowelClusterIndex != -1)
    {
        mappedWord.appendElementString(clusterMap(inSourceClusters, inDestClusters, START_VOWEL_I,
                                                  startVowelClusterIndex, inLearnA, inLearnB, inCanLearnB));
        // mappedWord.appendElementString(
    ...
    for (int m = 0; m < middleClusterIndices.size(); m++)
    {
        ...
        mappedWord.appendElementString(
            clusterMap(inSourceClusters, inDestClusters, ci.setIndex, ci.index, inLearnA, inLearnB, inCanLearnB));
        // mappedWord.appendElementString(
    ...
    {
        mappedWord.appendElementString(
            clusterMap(inSourceClusters, inDestClusters, END_I, endClusterIndex, inLearnA, inLearnB, inCanLearnB));
        // mappedWord.appendElementString(
...
static void initMapping(BidirectionalLanguageMap *inMap, int inEveIDA, int inEveIDB, int inRandSeed)
{
    ...
    for (int s = 0; s < NUM_CLUSTER_SETS; s++)
    {
        ...
        closeFreqMirrorShuffle(&(shuffles[s]), allClustersFreq[s], closeSetSize);

        for (int c = 0; c < allClusterSizes[s]; c++)
        {
            inMap->allMappings[s][shuffles[s].getElementDirect(c)] = allClusters[s][c];
        }
    }
}
...
static char checkForWordRepeat(LanguageLearningMap *inMap, char *inWord)
{
...
static void initMapping(LanguageLearningMap *inMap, int inEveIDA, int inEveIDB, int inPlayerID)
{
    inMap->eveIDA = inEveIDA;
    inMap->eveIDB = inEveIDB;
...
static LanguageLearningMap *getPlayerLearningMap(int inEveIDA, int inEveIDB, int inPlayerID, double inPlayerAge,
                                                 int inParentID = -1)
{
    ...
    {
        LanguageLearningMap *checkMap = playerRec->learningMaps.getElementDirect(m);

        if (checkMap->eveIDA == inEveIDA && checkMap->eveIDB == inEveIDB)
        {
            map = checkMap;
            break;
        }
    ...
    if (map == NULL)
    {
        ...
        if (inParentID != -1)
        {
            ...
            parentMap = getPlayerLearningMap(inEveIDA, inEveIDB, inParentID,
                                             // dummy parent age
        ...
        if (parentMap != NULL)
        {
            ...
            for (int s = 0; s < NUM_CLUSTER_SETS; s++)
            {
                memcpy(map->allMappings[s], parentMap->allMappings[s], allClusterSizes[s]);
            }
        }
...
void initLanguage()
{
    ...
    maxLanguageLearningAge = SettingsManager::getFloatSetting("maxLanguageLearningAge", 3.0);
    languageLearningRate = SettingsManager::getFloatSetting("languageLearningRate", 0.5);
    languageLearningRepetitionLimit = SettingsManager::getIntSetting("languageLearningRepetitionLimit", 20);
}
...
static void freePlayerLearningRecord(PlayerLearningRecord *inR)
{
    ...
    {
        inR->learningMaps.getElementDirect(m)->recentWords.deallocateStringElements();
    ...
    if (r != NULL)
    {
        ...
        if (r->langCount <= 0)
        {
            ...
            for (int e = 0; e < langRecords.size(); e++)
            {
                ...
                {
                    BidirectionalLanguageMap *map = rOther->languageMaps.getElementDirect(m);

                    if (map->eveIDA == inEveID || map->eveIDB == inEveID)
                    {
                        delete map;
...
void stepLanguage()
{
    ...
    for (int e = 0; e < langRecords.size(); e++)
    {
        ...
        if (numMappingsThisHas < numMappingsThisShouldHave)
        {
            ...
            int otherEveID = langRecords.getElementDirect(numMappingsThisHas)->eveID;

            int seed = r->eveID + otherEveID + time(NULL);

            initMapping(map, r->eveID, otherEveID, seed);
...
char *mapLanguagePhrase(char *inPhrase, int inEveIDA, int inEveIDB, int inPlayerIDA, int inPlayerIDB, double inAgeA,
                        double inAgeB, int inParentIDA, int inParentIDB)
{
    ...
    // these can be NULL
    LanguageLearningMap *learnA = getPlayerLearningMap(lookupEveID, otherEveID, inPlayerIDA, inAgeA, inParentIDA);
    LanguageLearningMap *learnB = getPlayerLearningMap(lookupEveID, otherEveID, inPlayerIDB, inAgeB, inParentIDB);
    ...
    for (int e = 0; e < langRecords.size(); e++)
    {
        ...
        if (r->eveID == lookupEveID)
        {
            for (int m = 0; m < r->languageMaps.size(); m++)
            {
                ...
                BidirectionalLanguageMap *map = r->languageMaps.getElementDirect(m);
                ...
                if (map->eveIDB == otherEveID)
                {
                    ...
                    while (nextChar < len)
                    {
                        ...
                        while (nextChar < len && lcPhrase[nextChar] >= 'a' && lcPhrase[nextChar] <= 'z')
                        {
                    ...
                    for (int w = 0; w < words.size(); w++)
                    {
                        ...
                        if (wordIsAlpha.getElementDirect(w))
                        {
                            ...
                            if (canLearnThisWord)
                            {
                                ...
                                canLearnThisWord = !checkForWordRepeat(learnB, word);
                            }

                            char *newWord =
                                remapWordNew(word, allClusters, map->allMappings, learnA, learnB, canLearnThisWord);
