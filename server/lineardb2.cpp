...
static void recreateMaps(LINEARDB2 *inDB, unsigned int inOldTableSizeA = 0)
{
    ...
    memset(inDB->fingerprintMap, 0, inDB->hashTableSizeA * 2 * sizeof(FingerprintBucket));

    if (oldFingerprintMap != NULL)
    {
        if (inOldTableSizeA > 0)
        {
            memcpy(inDB->fingerprintMap, oldFingerprintMap, inOldTableSizeA * 2 * sizeof(FingerprintBucket));
        }
...
static int readFingerprintRecord(LINEARDB2 *inDB, FingerprintBucket *inDestBucket)
{
    char foundEmpty = false;
    for (int r = 0; r < LDB2_RECORDS_PER_BUCKET; r++)
    {

        uint8_t *rec = &(inDB->bucketBuffer[r * inDB->recordSizeBytes]);

        if (rec[0])
        {
            // present
            inDB->numRecords++;
            inDestBucket->fingerprints[r] = shortHash(&(rec[1]), inDB->keySize);
        }
        else
        {
            // empty record slot
    ...
    {
        // consider overflow
        uint8_t *overflowP = &(inDB->bucketBuffer[LDB2_RECORDS_PER_BUCKET * inDB->recordSizeBytes]);
        // check overflow flag
        if (overflowP[0])
        {
            inDestBucket->overflow = true;

            inDestBucket->overflowIndex = *((uint32_t *)(&(overflowP[1])));

            // recurse into overflow chain

            if (fseeko(inDB->overflowFile, inDestBucket->overflowIndex * inDB->bucketSizeBytes, SEEK_SET))
            {
                return 1;
            }

            int numRead = fread(inDB->bucketBuffer, inDB->bucketSizeBytes, 1, inDB->overflowFile);
            ...
            FingerprintBucket *overflowBucket = &(inDB->overflowFingerprintBuckets[inDestBucket->overflowIndex]);
...
int LINEARDB2_open(LINEARDB2 *inDB, const char *inPath, int inMode, unsigned int inHashTableStartSize,
                   unsigned int inKeySize, unsigned int inValueSize)
{
    ...
    if (inPath == NULL || ftello(inDB->file) < LINEARDB2_HEADER_SIZE)
    {
        ...
        inDB->tableSizeBytes = (uint64_t)(inDB->bucketSizeBytes) * (uint64_t)(inDB->hashTableSizeB);
        ...
        {
            // last partial buffer
            int numWritten = fwrite(buff, inDB->tableSizeBytes - i, 1, inDB->file);
            if (numWritten != 1)
            {
                return 1;
            }
        ...
        inDB->overflowFingerprintBuckets = new FingerprintBucket[inDB->overflowAreaSize];

        memset(inDB->overflowFingerprintBuckets, 0, inDB->overflowAreaSize * sizeof(FingerprintBucket));
        ...
        {
            int numWritten = fwrite(inDB->bucketBuffer, inDB->bucketSizeBytes, 1, inDB->overflowFile);
            if (numWritten != 1)
            {
                return 1;
            }
    ...
    else
    {
        ...
        if (ftello(inDB->file) < (int64_t)(LINEARDB2_HEADER_SIZE + inDB->tableSizeBytes))
        {
        ...
        for (unsigned int i = 0; i < inDB->hashTableSizeB; i++)
        {
            ...
            if (fseeko(inDB->file, LINEARDB2_HEADER_SIZE + i * inDB->bucketSizeBytes, SEEK_SET))
            {
                return 1;
            }
            ...
            if (readFingerprintRecord(inDB, &(inDB->fingerprintMap[i])) == 1)
            {
...
static int makeNewOverflowBucket(LINEARDB2 *inDB, uint16_t inFingerprint, const void *inKey, const void *inValue,
                                 uint32_t *outOverflowIndex)
{
    ...
    if (!found)
    {
        ...
        memcpy(inDB->overflowFingerprintBuckets, oldOverflow, oldSize * sizeof(FingerprintBucket));
    ...
    // fingerprint
    inDB->overflowFingerprintBuckets[foundIndex].fingerprints[0] = inFingerprint;
...
int LINEARDB2_getOrPut(LINEARDB2 *inDB, const void *inKey, void *inOutValue, char inPut)
{
    ...
    for (int i = 0; i < LDB2_RECORDS_PER_BUCKET; i++)
    {
        ...
        if (binFP == 0)
        {
            ...
            if (inPut)
            {
                ...
                binFP = fingerprint;
                inDB->fingerprintMap[binNumber].fingerprints[i] = fingerprint;
        ...
        if (binFP == fingerprint)
        {
            ...
            uint64_t filePosRec = LINEARDB2_HEADER_SIZE + binNumber * inDB->bucketSizeBytes + i * inDB->recordSizeBytes;
            ...
            if (!emptyRec)
            {
                ...
                int numRead = fread(inDB->bucketBuffer, inDB->keySize, 1, inDB->file);
    ...
    while (!foundEmpty && thisBucket->overflow)
    {
        ...
        thisBucket = &(inDB->overflowFingerprintBuckets[thisBucketIndex]);
        ...
        for (int i = 0; i < LDB2_RECORDS_PER_BUCKET; i++)
        {
            ...
            if (binFP == fingerprint)
            {
                ...
                uint64_t filePosRec = thisBucketIndex * inDB->bucketSizeBytes + i * inDB->recordSizeBytes;

                if (!emptyRec)
                {
                    // read key to make sure it actually matches
                    if (fseeko(inDB->overflowFile, filePosRec + 1, SEEK_SET))
                    {
                        return -1;
                    }
                    int numRead = fread(inDB->bucketBuffer, inDB->keySize, 1, inDB->overflowFile);
                ...
                if (inPut)
                {
                    ...
                    uint8_t present = 1;
                    int numWritten = fwrite(&present, 1, 1, inDB->overflowFile);

                    numWritten += fwrite(inKey, inDB->keySize, 1, inDB->overflowFile);

                    numWritten += fwrite(inOutValue, inDB->valueSize, 1, inDB->overflowFile);
                    if (numWritten != 3)
                    {
                        return -1;
                    }
                ...
                else
                {
                    ...
                    int numRead = fread(inOutValue, inDB->valueSize, 1, inDB->overflowFile);
    ...
    if (!foundEmpty && inPut && !thisBucket->overflow)
    {
        ...
        int result = makeNewOverflowBucket(inDB, fingerprint, inKey, inOutValue, &overflowIndex);
        if (result == -1)
        {
            return -1;
        }
        ...
        if (thisBucketIsOverflow)
        {
            ...
            if (fseeko(inDB->overflowFile,
                       thisBucketIndex * inDB->bucketSizeBytes + LDB2_RECORDS_PER_BUCKET * inDB->recordSizeBytes,
                       SEEK_SET))
            {
                return -1;
            }
            // write it
            int numWritten = fwrite(&(thisBucket->overflowIndex), sizeof(uint32_t), 1, inDB->overflowFile);
            if (numWritten != 1)
            {
                return -1;
            }
        ...
        else
        {
            ...
            if (fseeko(inDB->file,
                       LINEARDB2_HEADER_SIZE + binNumber * inDB->bucketSizeBytes +
            ...
            // write it
            int numWritten = fwrite(&(thisBucket->overflowIndex), sizeof(uint32_t), 1, inDB->file);
            if (numWritten != 1)
            {
                return -1;
            }
...
int LINEARDB2_Iterator_next(LINEARDB2_Iterator *inDBi, void *outKey, void *outValue)
{
    ...
    while (true)
    {
        ...
        if (inDBi->nextRecordFile == db->file && inDBi->nextBucketIndex >= db->hashTableSizeB)
        {
        ...
        if (inDBi->nextRecordFile == db->overflowFile && inDBi->nextBucketIndex >= db->overflowAreaSize)
        {
            return 0;
        }
        ...
        nextRecordLoc = inDBi->nextBucketIndex * db->bucketSizeBytes + inDBi->nextRecordIndex * db->recordSizeBytes;
        ...
        int numRead = fread(&present, 1, 1, inDBi->nextRecordFile);
        if (numRead != 1)
        {
            return -1;
        }
        ...
        if (present)
        {
            numRead = fread(outKey, db->keySize, 1, inDBi->nextRecordFile);
            if (numRead != 1)
            {
                return -1;
            }

            numRead = fread(outValue, db->valueSize, 1, inDBi->nextRecordFile);
            if (numRead != 1)
            {
                return -1;
            }
...
unsigned int LINEARDB2_getShrinkSize(LINEARDB2 *inDB, unsigned int inNewNumRecords)
{
...
uint64_t LINEARDB2_getMaxFileSize(unsigned int inTableStartSize, unsigned int inKeySize, unsigned int inValueSize,
                                  uint32_t inNumRecords, double inMaxLoad)
{
    if (inMaxLoad == 0)
    {
    ...
    uint32_t numBuckets = (uint32_t)ceil(inNumRecords / LDB2_RECORDS_PER_BUCKET);

    double load = (double)inNumRecords / (double)(inTableStartSize * LDB2_RECORDS_PER_BUCKET);
    ...
    if (load > inMaxLoad)
    {
        ...
        uint64_t tableSizeRec = (uint64_t)(ceil((inNumRecords / inMaxLoad)));
...
void LINEARDB2_forceFile(LINEARDB2 *inDB, FILE *inFile, FILE *inOverflowFile)
{
    inDB->file = inFile;
